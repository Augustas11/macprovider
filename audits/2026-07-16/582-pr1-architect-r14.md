# AUDIT ROUND 14 (closing) — #582 PR1 hardware-trust operator approval — architect lane

Rounds 1-13 FIXED. SECURITY + CODE lanes PASS 0/0/0 (frozen). Architect round-13 had 2 MEDIUM:
(1) deploy PK-cutover ordering, (2) a stray committed arm64 binary. BOTH now fixed. Audit FULL
branch vs origin/main (git diff origin/main...HEAD; diff at audits/2026-07-16/582-pr1-r14.diff),
worktree /Users/augstar/macprovider-582-onboarding. Report any REMAINING C/H/M. Only defects with
a concrete failure scenario.

## Round-13 architect MEDIUM fixes to verify
- Stray 8.5MB module-root Mach-O arm64 binary phase4-coordinator/stats-inventory-sync REMOVED
  (git rm) + added to .gitignore (with stats-hardware-verifier). Pearl uses
  dist/stats-inventory-sync-linux-amd64. On squash-merge the artifact is absent from the net diff.
- Deploy ordering now SELF-ENFORCING. 018 is applied out-of-band by the coordinator 'stats-migrate'
  subcommand (NOT at boot). deploy-pearl-vps.sh's onboarding-gated block now: quiesce+disable sidecar
  -> apply 018 via 'coordinator stats-migrate' (admin DSN via require_env_value env data, not argv/
  sourced; statsmigrations.Apply is advisory-locked + idempotent) -> hard-gate 'stats-migrate --check'
  confirms 018 applied else abort exit 12 -> onboarding preflight -> install new 3-col sidecar binary
  -> re-enable sidecar (step 9). Invariant: sidecar disabled from before-any-018-apply until
  after-new-binary-install. check_stats_inventory_deploy_test.sh asserts the full six-anchor ordering.
  Non-onboarding path untouched. Manual stats-migrate is the documented backstop.
...
round-trip required out-of-band (no in-repo PG harness).
