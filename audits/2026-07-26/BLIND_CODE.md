# BLIND independent audit — CODE lane
...
## Lane focus: code correctness
- Concurrency correctness of the store actor and its integration: races between the async writer, restart recovery, promotion, purge, and eviction; anything that could corrupt state or serve wrong/stale data; suspension points (`await`) where invariants can break.
- Resource management: memory bounds on read/write paths, file-descriptor/handle lifecycle, quota accounting, leaks on error/cancel paths.
- On-disk format handling vs the spec: parsing/validation-before-allocation, bounds, atomic-commit/crash-consistency (fsync/rename ordering).
- Integration correctness in ModelRuntime/ConversationCache: lease lifecycle, no busy-key leaks, the promotion path reusing the shipped predicate, speculative-decode handling.
- Test adequacy vs the spec's acceptance criteria — are the guarantees actually tested, or asserted around?
