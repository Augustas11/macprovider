...
- **MEDIUM (code, architect):** SIGHUP swapped the admission catalog but kept the boot index. `SetAutotuneCatalogWithArtifactIndex` swaps both under `autotuneCatalogMu`; the reload rebuilds the index from the SAME reloaded feeds and aborts the publish (keeping the previous catalog and index) if it cannot; legacy `SetAutotuneCatalog` drops the index so it never outlives its release. Pinned in the ws artifact test.
