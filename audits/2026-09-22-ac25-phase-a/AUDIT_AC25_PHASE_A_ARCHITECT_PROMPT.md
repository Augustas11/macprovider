...
- Is one shared `asAPIError()` the right shape, or does mapping belong at the
  HTTP boundary rather than on the error type?
- Status-by-shape for carried `unsupported`/`requestFailed` codes: is an
  allowlist with a 503 default sound, or will unknown future codes be
  mis-shaped? What happens when someone adds a new throw site?
- Is a 30 s default admission deadline the right default for this serving
  profile (8 slots, queue 16, 30B MoE on M3 Ultra)? Should it instead be
  derived from measured decode time?
- The deliberate non-mapping of `.drained`/`.drainTimedOut` leaves a real
  drain path (`ModelRuntime` swap drain → `DrainCancelledError`) mapped
  elsewhere. Is that split coherent or confusing?
- Does duplicating cancellation state between `InferenceRelay` and
  `HTTPServer` (rather than unifying) create a drift risk of the same kind
  this change just fixed for the streaming/non-streaming split?
- The known relay-forwarding gap: is shipping the provider half alone
  coherent, or does it create a surface where direct-HTTP and relay buyers
  see different contracts for the same condition?
