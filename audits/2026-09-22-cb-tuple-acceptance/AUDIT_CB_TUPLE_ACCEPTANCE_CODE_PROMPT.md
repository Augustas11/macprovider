...
- Is the gate placed correctly relative to descriptor admission and the MoE
  gate? Can any ordering produce a misleading reason code?
- Is `covers(_:)` correct? Any case-sensitivity, normalization, whitespace, or
  Unicode issue in matching model ids / SHAs / hardware classes that could
  cause a false negative (operator declared it but it never matches) or a
  false positive?
- The `unrestricted` flag: can `unrestrictedForTests` reach production through
  any path — config, defaulted parameter, init overload, or copy?
- YAML parsing: are all six fields validated? Duplicate entries? Empty strings?
  An empty list vs absent key? Does the error path leak values into messages?
- Do the two `ModelRuntime` inits (production `.empty`, test
  `.unrestrictedForTests`) create a case where production code reaches the
  test-defaulted init?
- Test adequacy: do the new tests actually pin the behavior, or would they pass
