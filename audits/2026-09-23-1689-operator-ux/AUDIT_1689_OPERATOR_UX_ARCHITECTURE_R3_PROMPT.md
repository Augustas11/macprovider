...
branch `feat/1689-honest-status`, based on `origin/main` at `73fd1a91`.

Review the FULL diff as it will land: `git diff 73fd1a91...HEAD` in that
worktree (52 files changed, 6903 insertions(+), 172 deletions(-)), 165 deletions(-)), 127 deletions(-)). This is the complete epic, not a slice. Commits, in
order:

- `8c08b2c9` Show where status readiness, throughput, and context come from
- `3f0566c2` Stop reporting buyer-serving when the network catalog lacks the served model
- `ecceb320` Let operators prove local, coordinator, and public feed agree after a change
- `5e20c591` Stop writing a 4K production context cap on high-memory Macs and let operators change context safely
- `56fc12bf` Let operators check a model's local bytes against its signed catalog row and repair partial downloads
- `1f3165ae` Recompute generated context when the served model changes and keep operator overrides
...
## ROUND 3 — closure verification (code + architecture lanes; security passed R1 and R2)

Round 2 found, and this revision claims to fix:
- DESIGN (arch M1 + code M1): provenance was bound to a digest of the whole config, so any first-party writer (serve's `model_artifact_path` canonicalization) or a symlinked config silently turned a generated context into operator_config. Now FIELD-scoped: sidecar `knob_provenance.v2` at `<canonical (symlink-resolved) config path>.provenance.json`; a value is generated iff source=recommendation_apply AND non-empty `model` AND value == current `max_context_override`; other keys irrelevant; operator ownership only via `provider context set`, rollback to an operator backup, or a hand edit to a different value; v1/digest-bound/missing-model sidecars ⇒ operator_config. Accepted, documented trade-off: a hand edit to exactly the generated number stays generated (a later switch recomputes to the memory-safe value).
- Code M2: context set/rollback validate `--timeout` 0…3600 before any write (shared validator).
- Code L1: sidecar restored if the config write fails. Code L2: rollback order via a durable `config.yaml.latest-backup` pointer (clock-independent), legacy name fallback. Code L3: complete `layer_types` with zero full-attention ⇒ KV term non-binding.
- Arch L1: distinct `catalog_material_missing` routing reason via typed result from the single evaluator `routeSnapshotUnsatisfiable` (buyer response/eligibility unchanged; `EligibleCandidates` body untouched — conformant SPEC-047 mapping).
- Security L1: printed retry command POSIX-shell-quoted. Arch INFO: SPEC-010 §3.8 names `models prepare-catalog` as the future home.
- Known, out of scope, PRE-EXISTING: `models adopt-recommendation` tests create empty lock files under the real `~/.config/macprovider/model-adoption-transactions` (default lock root not injectable); `persistMigratedArtifactPath` locks the unresolved path and replaces a symlinked config with a regular file; phase7 x/text and coordinator x/crypto advisories.

Verify each fix is real and complete and review the fix code itself (especially the v2 provenance semantics and the routing-reason re-attribution wrapper) for new defects. Report remaining or newly introduced C/H/M.

Verification on this revision: coordinator build/vet/test/lint 0 issues, gateway, integration, full Swift suite 3254 / 0 failures (bounded), test-dist, spec governance, contract lock, spec index check+lint — all pass.
