# Audit — #1705 catalog row-continuity admission + CLI envelope refresh (round R2)
...
Focus: can row-continuity widen admission beyond SPEC-023-R010 — untrusted/unsigned
documents, tombstoned IDs (by line id vs parsed version), signer/key lineage (hello
signer vs loaded signer; same-version different signer), path handling of target lines,
directory-walk regressions, over-cap/malformed file handling at boot and SIGHUP; can a
row_continuity session reach artifact-derived identity, settlement, a higher RAM
ceiling, or buyer routing it would not get under `previous`; model-hash gate binding.
CLI: can the refresher adopt unsigned, baked, stale, or changed-row data; can a hostile
coordinator ack drive fetch storms or envelope downgrade; log redaction.
...
Note: SPEC-023 is now v0.14.4 (origin/main took v0.14.3 for #1694).
