...
Round 13: the plan is now v13, a clean consolidated rewrite that deliberately REMOVES four mechanisms added in earlier rounds (listed under "Removed from scope") and replaces them with precisely stated money invariants I1-I5. Judge v13 on its own. For each removed mechanism, decide whether its absence leaves a CONCRETE path to mis-billing, pricing with an unreviewed table, or unrecoverable state (then HIGH, with the exact sequence), or whether the stated invariant and remaining controls suffice (then no finding). Verify the restored C2/C3 contracts and the W1 multiplier grouping. Attribution: NEW vs PRE-EXISTING; PRE-EXISTING at most MEDIUM and only if a lane-eligible change concretely affects pricing through it. Severity: HIGH/CRITICAL only for a concrete path to mis-billing, unreviewed pricing, or unrecoverable/unsafe state; MEDIUM for concrete evidenced correctness gaps; LOW for precision/wording. No scope/style inflation.
