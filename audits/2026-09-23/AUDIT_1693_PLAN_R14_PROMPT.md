...
Round 14: the plan is now v14 (clean rewrite v13 + round-13 fixes listed in its header; four earlier mechanisms are deliberately removed — judge that only by concrete mis-billing paths). Verify closure of round-13 findings (W1 aggregate formula, token source, policy lane, I3 scope) and hunt for NEW defects. Attribution: NEW vs PRE-EXISTING; PRE-EXISTING at most MEDIUM and only if a lane-eligible change concretely affects pricing through it. Severity: HIGH/CRITICAL only for a concrete path to mis-billing, unreviewed pricing, or unrecoverable/unsafe state; MEDIUM for concrete evidenced correctness gaps; LOW for precision/wording. No scope/style inflation.
