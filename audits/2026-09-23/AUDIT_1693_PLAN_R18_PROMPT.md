...
Round 18: the plan is now v18 (v17 + fixes for round-17 findings: pre-start ordering/guard semantics, effective-price diff, attempt helper, restored-unverified rule, preflight identity checks, splice on Pearl). Verify closure and hunt for NEW defects introduced by v16 (two-phase publisher split of the SPEC-047 observer/commit, resolved-entry plumbing through hot path/recovery, rate-card snapshot helper). Other deliberately removed mechanisms: judge only by concrete mis-billing paths. Attribution: NEW vs PRE-EXISTING; PRE-EXISTING at most MEDIUM and only if a lane-eligible change concretely affects pricing through it. Severity: HIGH/CRITICAL only for a concrete path to mis-billing, unreviewed pricing, or unrecoverable/unsafe state; MEDIUM for concrete evidenced correctness gaps; LOW for precision/wording. No scope/style inflation.
