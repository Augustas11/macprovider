...
Round 3: the plan is now v3 (header lists how round-1 P01-P09 and round-2 R2-01..R2-10 were addressed). Independently verify each is truly closed, check the new §Money invariants are correct and sufficient for SPEC-005 MoneyTable-A, and hunt for new defects introduced by v3. Do not anchor on earlier rounds. Severity discipline: HIGH/CRITICAL only for a concrete path to mis-billing, unreviewed pricing, or unrecoverable/unsafe state; MEDIUM for concrete correctness/robustness gaps with evidence; do not inflate style or scope preferences.
