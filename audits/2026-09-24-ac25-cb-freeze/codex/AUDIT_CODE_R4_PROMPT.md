# Codex round 3: PR #1716, one scheduler fix after the gate (`b332156b` plus the regression test in HEAD)
...
race in the AC-25 bounded admission wait. The only new code is `b332156b` plus the regression test in HEAD
...
Round 3 code-lane MEDIUM (test gap) is addressed in HEAD: `testAC25OverdueRequestIsExpiredAtAdmissionEvenIfItsTimerHasNotRun` with `cancelQueueWaitTimerForTest`. It fails without the fix (verified) and passes with it. Re-check the gap and the whole change.
