...
(commits `b8486a5c`, `25767bd5`, the R1 fix `6f0de81c`, and the R2 fix in HEAD).
...
Round 2 code-lane MEDIUM (cancel lost across the terminal retain/materialize awaits) is fixed in HEAD via `finishTerminal`, with `testCancelDuringTerminalMaterializeSuppressesTheCache`. The test fails without the fix (verified). Look for any remaining await after which a recorded cancel is not honoured.
