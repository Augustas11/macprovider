...
`campaign/1646-cb-sampling`. Review `git show f9a71a3b ff49716e` , the R2 fix, and the R3 fix in HEAD, in the context of the
...
Round 3 MEDIUM (cancel recorded during the retained-owner discard inside finishQueued) is fixed: the cancel check now follows the only await in finishQueued, with no suspension before the synchronous completion. Repeat the single-pass sweep of every M4-added or M4-changed await point.
