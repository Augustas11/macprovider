...
Lane choice (runtime cut vs content lane) against the decision tree and SPEC-023 R013-R017; compare-live descends semantics; effect on connected providers that advertise old hashes / row-continuity (#1705, SPEC-023-R010); release-train doc accuracy; whether the Tier-2 re-sign with a new catalog_id is the right shape; anything the post-merge deploy + Studio verification (step 4) must do.

## Round 3
Round 2 findings are in audits/2026-09-25-1735-signing-cut/result-architecture-r2.md (HIGH evidence covered only GLM; MEDIUM Tier-2 catalog_id revokes model admissions; MEDIUM old-hash reconnects; LOW whitespace). Commit 1127ce81 addresses them in docs/releases/coordinator-release-train.md and strips whitespace. Note: seven of the eight rows have no provider at all today (public /v1/stats/routability 2026-09-25T01:02Z), so a buyer E2E is impossible for them until a provider serves them; judge whether the per-row deferred evidence, tracked on the still-open issue, is the correct and honest bar. Re-review the FULL diff origin/main...HEAD for this lane.
