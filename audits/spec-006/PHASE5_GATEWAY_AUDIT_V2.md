...
- F-M6: CLOSED. nginx defines `limit_req_zone` and `limit_conn_zone` for `$binary_remote_addr` (`phase5-gateway/dist/nginx-api.malibu.tech.conf:10`) and applies both to `/ws/provider` (`phase5-gateway/dist/nginx-api.malibu.tech.conf:105`). The Pearl runbook calls out PG-2 controls (`phase5-gateway/dist/deploy-pearl-vps.md:15`).
- F-M7: CLOSED. Gateway fallback returns the OpenAI envelope (`phase5-gateway/internal/router/server.go:424`), and nginx returns JSON envelopes for `/admin/`, `/v1/pool/check`, `/poolz`, and fallback routes (`phase5-gateway/dist/nginx-api.malibu.tech.conf:42`, `phase5-gateway/dist/nginx-api.malibu.tech.conf:87`, `phase5-gateway/dist/nginx-api.malibu.tech.conf:117`). Gateway test coverage exists at `TestNotFoundReturnsOpenAIEnvelope` (`phase5-gateway/internal/router/server_test.go:631`); nginx evidence is static config, not a live `nginx -t`.
