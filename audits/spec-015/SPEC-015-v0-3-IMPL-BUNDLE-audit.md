...
- Confirmed production nginx supports the trust boundary assumed by the fix: `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:149-175` proxies `/v1/receipt-keys/` and `/catalog/` to loopback buyer port `127.0.0.1:8443` and overwrites `X-Real-IP` with `$remote_addr`.
