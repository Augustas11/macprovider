...
- `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:170-176` currently routes `/catalog/` to buyer port `127.0.0.1:8443` before the catch-all `location /` at lines 205-207.
...
Security checks:
- `/catalog/` is public per `specs/SPEC-015-receipts.md:3308-3322` and `3330-3353`; the nginx block is unauthenticated but proxies only to buyer port `127.0.0.1:8443` at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:170-176`.
- `/poolz` remains an exact operator endpoint on provider port `127.0.0.1:8444` with `Authorization` forwarding at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:122-129`; no public `/poolz` proxy was added.
...
- The public-key response shape in comments preserves base64url-unpadded + capital-E `Ed25519` at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:158-167`, matching `specs/SPEC-015-receipts.md:3350-3363`.
...
- The nginx route mirrors the v0.2 `/v1/receipt-keys/` buyer-port shape at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:149-176`.
...
Security checks:
- `/catalog/` remains public by design per `specs/SPEC-015-receipts.md:3308-3322` and `specs/SPEC-015-receipts.md:3328-3368`; nginx proxies it only to buyer port `127.0.0.1:8443` at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:170-176`.
- No public `/poolz` route was added. The existing `/poolz` surface remains on provider/operator port `127.0.0.1:8444` with authorization forwarding at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:120-129`.
- The `/catalog/` comment states the unauthenticated buyer-endpoint posture, internal rate-limit ownership, public cache posture, signed-catalog trust model, and 404 behavior at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:158-168`.
...
- The deploy integration satisfies the Step 3 pre-upload gate intent from `specs/BUILD_SPEC_015_v0_3_MODELHASH_IMPL_PROMPT.md:183-190`: an outdated local `nginx-coordinator.malibu.tech.conf` now blocks the Pearl deploy before remote mutation.
- The new `/catalog/` route composes with the v0.2 `/v1/receipt-keys/` shape: both are public buyer-port routes with the same core proxy/header pattern at `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:149-176`.
