...
## Category verdicts
A. Vhost surface: PASS. `stats.streamvc.live` has a port-80 ACME/redirect server and a port-443 TLS server. `deploy-pearl-vps.sh` now treats `STATS_DOMAIN` as a first-class certbot target, installs the stats vhost, and uncomments the stats certificate lines before the final `nginx -t`. `coordinator.streamvc.live` exposes the three locked stats endpoints before the `/v1/` 404 catch-all.
...
1. `phase4-coordinator/dist/test/check_nginx_stats_test.sh:5`, `phase4-coordinator/dist/test/check_nginx_stats_test.sh:22`, and `phase4-coordinator/dist/test/check_nginx_stats_test.sh:31`
   - Evidence: the script header says it validates the amended coordinator vhost, claims `proxy_no_cache` write-suppression coverage, and says CI runs it in a `coordinator-nginx-integration` job. The executable body builds only a test-only stats vhost from `nginx-stats.streamvc.live.conf`; it does not include `nginx-coordinator.streamvc.live.conf`, and it does not inspect the cache directory or assert `MISS`/`BYPASS` after a keyed request. CI actually runs it through `make test-dist` in the `dist-tooling` job.
...
- `phase4-coordinator/dist/nginx-stats.streamvc.live.conf:99`, `:127`, and `:150` use exact endpoint locations for overview, leaderboard, and health.
- `phase4-coordinator/dist/nginx-stats.streamvc.live.conf:100`, `:128`, and `:151` use `limit_req zone=<endpoint> nodelay` with no stats `burst=`.
- `phase4-coordinator/dist/nginx-stats.streamvc.live.conf:119` and `:120` pair cache read-bypass and write-suppression for Authorization-bearing overview responses; leaderboard and health mirror the same pair.
- `phase4-coordinator/dist/nginx-coordinator.streamvc.live.conf:212`, `:232`, and `:252` place all three stats endpoints before the `/v1/` catch-all at `:280`.
- `phase4-coordinator/dist/nginx-coordinator.streamvc.live.conf:220` and `phase4-coordinator/dist/nginx-stats.streamvc.live.conf:107` forward `X-Forwarded-For` using `$proxy_add_x_forwarded_for`.
- `phase4-coordinator/dist/nginx-coordinator.streamvc.live.conf:222` and `phase4-coordinator/dist/nginx-stats.streamvc.live.conf:109` forward `Authorization` to the coordinator so the in-process partner dispatcher remains authoritative.
...
- ARCH r2 HIGH: CLOSED. The prior finding required `stats.streamvc.live` to use the same TLS/cert pipeline as the rest of Pearl. The deploy script now obtains a cert for `STATS_DOMAIN`, installs the stats vhost, uncomments its cert directives, and runs `nginx -t` only after the stats certificate path is wired.
