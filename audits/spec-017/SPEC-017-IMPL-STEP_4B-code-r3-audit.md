...
- Static brace-depth check over `stats-shared.conf`, `stats-security-headers.conf`, `nginx-stats.malibu.tech.conf`, and `nginx-coordinator.malibu.tech.conf` - PASS, final brace depth 0.
...
- Local sed simulation of the deploy-time stats-vhost certificate uncomment left `# ssl_certificate     /etc/letsencrypt/live/stats.malibu.tech/fullchain.pem;` commented while uncommenting `ssl_certificate_key`.
...
K. TLS posture: FAIL. TLS protocol versions are correct, but deploy-time activation for `stats.malibu.tech` leaves the certificate directive commented.
...
1. `phase4-coordinator/dist/nginx-stats.malibu.tech.conf:54` and `phase4-coordinator/dist/deploy-pearl-vps.sh:427`
   - Evidence: the stats vhost ships `# ssl_certificate     /etc/letsencrypt/live/stats.malibu.tech/fullchain.pem;` with multiple spaces between `ssl_certificate` and the path.
...
- `phase4-coordinator/dist/deploy-pearl-vps.sh:365-406` now treats `stats.malibu.tech` as a first-class ACME/certbot hostname, partially addressing the r2 TLS finding.
...
- `phase4-coordinator/dist/nginx-stats.malibu.tech.conf:99-167` uses exact endpoint locations, per-endpoint `limit_req ... nodelay`, `limit_req_status 429`, and no stats `burst=`.
- `phase4-coordinator/dist/nginx-stats.malibu.tech.conf:80-93` returns the JSON `rate_limited` envelope with `Retry-After: 60` for nginx-tier 429s.
- `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:212-270` mirrors the three stats exact locations on the coordinator hostname before the `/v1/` catch-all.
- `phase4-coordinator/dist/nginx-coordinator.malibu.tech.conf:302-311` mirrors the stats 429 JSON mapping on the coordinator hostname.
- `phase4-coordinator/dist/nginx-stats.malibu.tech.conf:63` sets `client_max_body_size 64k`, above the 8K floor named in the CODE lane prompt.
