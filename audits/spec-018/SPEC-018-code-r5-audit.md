...
## r4-absorption verification
Code r4 M-1 — AC-23 baseline pin file: CONFIRMED for AC-23/§1.2 wording; executor obligation is clear and AC-23 points to `openai-python-spec-018-v0_1_3-baseline.txt`.

Code r4 M-2 — §1.1 #4 `model_hash` overclaim: CONFIRMED. §1.1 #4 and §3.2 now consistently say v0.1 verifies loaded weights but does not bind parser family until v0.2 §10a #2 / §10c.

Code r4 m-1 — stale mixed-sentinel parse-failure reference: CONFIRMED. Remaining `mixed-sentinel` hits are historical/drop-context only.

Code r4 m-2 — §8.4 stale `v0.1.2`: CONFIRMED. §8.4 now says `v0.1.3 IMPL prompt` and includes the rejection fixtures.

Critic r2 m-1 — AC-23 baseline-version drift: RESIDUAL. AC-23 is aligned to v0.1.3, but §10c still says `v0.1.2-baseline parser`.

## Findings

MEDIUM: §10c still contradicts AC-23 on the baseline parser version.  
[specs/SPEC-018-agentic-tool-calling.md](/Users/augstar/macprovider-spec-018-tool-calling/specs/SPEC-018-agentic-tool-calling.md:440) says AC-23 parses vN.M responses with a `v0.1.2-baseline parser`, while AC-23 at line 396 correctly requires a `v0.1.3-baseline` parser pinned by `tools/version-pins/openai-python-spec-018-v0_1_3-baseline.txt`. This is exactly the baseline-version drift v0.1.4 intended to close.
