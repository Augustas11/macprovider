# Launch package — M4 user

Everything needed to bring the provider online. Three things to send,
...
- [ ] M4 user has agreed verbally to participate in a 14-day beta
...
harness from their M1; this Mac (mine) is the provider. A Cloudflare
...
The current token for the M4 user is in:
...
When the M4 user confirms "done," run these from your M1:
...
If `short_chat` returns `status=200`, you're live with the M4 user.
