...
# Build 1 narrow MVP implementation plan v1
...
4. Provider starts or reloads `macprovider-cli serve` on the prepared model. The CLI reports local readiness only when the running MLX runtime identity matches the prepared artifact and signed catalog tuple.
...
6. A buyer sends one non-streaming request through the staging gateway to the Llama 3B model. The response is produced by actual MLX inference on the physical Mac, and the gateway/coordinator produce a verified settlement record with the expected model/artifact/hardware context.
...
5. Network admission depends on coordinator status, including catalog/rate binding and receipt key availability.
...
- settlement qualification state: fixture-only, staging-observed, production-not-activated.

### Phase B: artifact feed consumer for the MVP action

Reuse existing strict artifact-feed decode/bind validation. Fetch from the configured coordinator only when the coordinator URL is explicitly supplied/configured for staging. Reject corrupt JSON, bad signatures, signer mismatch, cross-release feed, stale feed, missing primary artifact, unsupported runtime source, unsupported artifact id, and size/hash inconsistency. If live feed is unavailable and no baked artifact feed exists, the action must be unavailable with a truthful blocker; fallback candidate/rate data may support recommendation copy but not preparation authority.
...
- disclose pinned model/revision/hash, expected runtime source, destination root, known or unknown size, and staging bytes before adoption;
...
5. buyer non-streaming request capture through actual MLX inference;
6. verified settlement/receipt capture with model/artifact/hardware context;
...
- artifact feed verification fails closed for corrupt bytes, bad detached signature, wrong signer, cross-release candidate binding, stale feed, missing primary artifact, bad size/hash fields, and unsupported runtime source;
...
- one real MLX non-streaming request routed through staging;
- verified settlement output with matching model/artifact/hardware context and correct rate application;
- explicit statement that production economic activation, rewards, and payouts were not enabled.
...
- Physical Apple Silicon Mac with at least 8 GB RAM is sufficient for the chosen row's catalog min RAM 4 GB plus OS/runtime headroom; acceptance must record actual chip, RAM, OS, binary version, MLX version/context, and storage availability.
- Network access to fetch the pinned Hugging Face revision or preexisting cache with a verified snapshot manifest.
- Staging coordinator/gateway configured with the bound artifact feed, candidate/rate feed, admission path, receipt/settlement observe path, and test buyer/provider credentials.
- No production payout/reward activation and no production enforcement changes.

## Verification plan summary

- Swift unit tests for profile gate, feed validation, preparation state machine, cancellation, inventory projection, and status truthfulness.
...
| Correctly priced and settled request | Staging request through actual MLX with verified settlement | Settlement fixture tests plus physical staging evidence. |
