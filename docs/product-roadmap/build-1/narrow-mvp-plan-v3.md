...
6. A buyer sends one non-streaming request through the isolated staging gateway to the Llama 3B model. The response is correlated to the physical `macprovider-cli` process by provider binary/process identity, control-socket runtime status, route/provider id, and request-id or monotonic served-count/log evidence. The gateway/coordinator produce a verified settlement record with the expected model/artifact/hardware context.
...
5. buyer non-streaming request capture through actual MLX inference, including provider-side request correlation;
6. verified settlement/receipt capture with model/artifact/hardware context and staging-enforce configuration proof;
...
- verified settlement output with matching model/artifact/hardware context, provider-side request correlation, and correct rate application;
