...
# Build 1 narrow MVP implementation plan v5
...
- Swift unit tests for profile gate, feed validation, preparation state machine, cancellation, inventory projection, runtime `GET /v1/status` digest binding, request-id audit correlation, and status truthfulness.
...
| Correctly priced and settled request | Staging request through actual MLX with verified settlement | Settlement fixture tests plus physical staging evidence requiring provider receipt/audit request-id correlation and local `GET /v1/status` digest binding. |
