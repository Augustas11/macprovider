# Build 1 narrow MVP implementation plan v6
...
> Current authority note (2026-09-26): the Llama tuple below is regression-only
> and receives no physical campaign. Build 1 execution is governed by
> `build1-control-recovery-plan-v2.md` and uses the pinned non-catalog
> OrcaRouter tuple.
...
2. Provider runs a single MVP preparation command for the chosen row. The CLI discloses repository, pinned revision, expected identity hash, measured feed size, destination root, staging path, cancellation behavior, and that preparation does not grant admission or earnings.
...
- Swift unit tests for profile gate, feed validation, preparation state machine, cancellation, inventory projection, runtime `GET /v1/status` model/weights digest binding, request-id audit correlation, and status truthfulness.
...
| Correctly priced and settled request | Staging request through actual MLX with verified settlement | Settlement fixture tests plus physical staging evidence requiring provider receipt/audit request-id correlation and local `GET /v1/status` model/weights digest binding. |
