...
# Build 1 narrow MVP test specification v1

Scope: one shippable Build 1 MVP path for `meta-llama/llama-3.2-3b-instruct` / `mlx-community/Llama-3.2-3B-Instruct-4bit` on MLX `mlx_cache`, through staging-only admission and verified settlement evidence. This test specification is tied to `docs/product-roadmap/build-1/narrow-mvp-plan-v1.md` and the dependent branch on PR #1510 head `6a90f39bfd4b8917ae10169b3c760e03cd2dfd91`.
...
If a broad check cannot run because Docker, staging credentials, network, or physical hardware are unavailable, record the exact blocker and run the next-best targeted tests. Do not mark the blocked check as passing.
...
| Artifact feed verification | Bound `/v1/catalog-artifacts` and detached signature for the same release unlock artifact-derived action | Corrupt JSON, tampered sig, wrong signer, cross-release candidate digest, stale feed, missing primary artifact, bad size/hash, unsupported runtime source | Swift validator tests and script fixture tests. |
...
| Runtime readiness | Running MLX model identity matches prepared tuple | Serve stopped, stale socket, wrong model id, wrong artifact digest, unresponsive warm-swap | CLI status tests and physical capture. |
...
| Settlement | One non-streaming staging request settles with matching model/artifact/hardware context | Replay, digest substitution, model drift between admission and route, missing receipt, delayed receipt reconciliation | Existing/new integration tests plus physical staging evidence. |
| Evidence bundle | Redacted manifest records command, binary version, git SHA, model tuple, hardware, captures and blockers | Secrets printed, raw private paths exposed, skipped tests marked pass, fixture evidence mislabeled physical | Script tests and manual review. |
...
1. Record git SHA, branch, binary version, OS, chip, RAM, free disk, MLX runtime version, staging coordinator/gateway URLs with secrets redacted.
...
3. Prepare the artifact. Record disclosed size or `unknown`, staged byte count, snapshot-manifest digest, durable-store inventory digest, cancellation/recovery capability, and adopted model id.
4. Start or reload the CLI provider. Record running model id, runtime source, artifact digest, context/concurrency profile, receipt key availability, and local readiness state.
...
6. Send one non-streaming buyer request through the staging gateway to `mlx-community/Llama-3.2-3B-Instruct-4bit`. Record request id, response status, token usage, and route/provider id with secrets redacted.
7. Retrieve settlement/receipt evidence. Record model id, catalog key, artifact digest, hardware context, rate row, credits, provider share, and verification status.
8. Record production blockers: production activation disabled, rewards/payouts disabled, no release publication, no production enforcement change.

Streaming may be captured as extra evidence, but the MVP acceptance minimum is one non-streaming physical request because Build 2 owns private streaming journeys.
...
- A feed that omits artifact size can still be used for integrity only if the CLI truthfully reports size as unknown and does not promise disk sufficiency beyond measured staging bytes.
