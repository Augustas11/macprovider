...
| Evidence bundle | Redacted manifest records command, binary version, git SHA, model tuple, hardware, provider-side request correlation, staging-enforce config proof, captures and blockers | Secrets printed, raw private paths exposed, skipped tests marked pass, fixture evidence mislabeled physical, fake provider evidence mislabeled MLX | Script tests and manual review. |
...
4. Start or reload the CLI provider. Record `macprovider-cli` binary path/hash/version, process id, provider id, running model id, runtime source, artifact digest, context/concurrency profile, receipt key availability, local readiness state, and pre-request served-count/log cursor when available.
...
6. Send one non-streaming buyer request through the staging gateway to `mlx-community/Llama-3.2-3B-Instruct-4bit`. Record request id, response status, token usage, route/provider id with secrets redacted, and post-request provider-side correlation showing the physical CLI provider handled the request, such as matching request id in provider logs or monotonic served-count advancement tied to the captured log cursor.
7. Retrieve settlement/receipt evidence. Record model id, catalog key, artifact digest, hardware context, rate row, credits, provider share, verification status, and route/provider id matching the provider-side correlation.
...
Streaming may be captured as extra evidence, but the MVP acceptance minimum is one non-streaming physical request because Build 2 owns private streaming journeys. A fixture or fake HTTP provider cannot satisfy this physical MLX acceptance step.
