...
| Runtime readiness | Running MLX model identity and local provider `GET /v1/status` `model_hash`/`weights_manifest_sha256` match the prepared tuple | Serve stopped, stale socket, wrong model id, wrong artifact digest, unresponsive warm-swap | CLI status tests and physical capture from the physical provider only. |
...
4. Start or reload the CLI provider. Record `macprovider-cli` binary path/hash/version, process id, provider id, running model id, runtime source, local provider `GET /v1/status` JSON fields `model_hash` and `weights_manifest_sha256`, context/concurrency profile, receipt key availability, local readiness state, and pre-request receipt-audit/log cursor. The digest authority for physical acceptance is local provider `GET /v1/status`, not control-socket model name alone.
...
7. Retrieve settlement/receipt evidence. Record model id, catalog key, artifact digest, hardware context, rate row, credits, provider share, verification status, request id, and route/provider id matching the provider-side receipt/audit correlation and local `GET /v1/status` digest binding.
