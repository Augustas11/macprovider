...
| Runtime readiness | Running MLX model identity and local provider `GET /v1/status` `model_hash`/algorithm match the prepared snapshot-manifest tuple; `weights_manifest_sha256`/algorithm are present, stable before/after, and match a preparation-captured weights manifest hash when available | Serve stopped, stale socket, wrong model id, wrong artifact digest, missing weights manifest, weights-manifest drift, unresponsive warm-swap | CLI status tests and physical capture from the physical provider only. |
...
4. Start or reload the CLI provider. Record `macprovider-cli` binary path/hash/version, process id, provider id, running model id, runtime source, local provider `GET /v1/status` JSON fields `model_hash`, `model_hash_algorithm`, `weights_manifest_sha256`, and `weights_manifest_algorithm`, context/concurrency profile, receipt key availability, local readiness state, and pre-request receipt-audit/log cursor. The snapshot-manifest identity authority for physical acceptance is local provider `GET /v1/status` `model_hash`/algorithm, not control-socket model name alone; `weights_manifest_sha256` is retained as separate runtime weights-manifest evidence.
...
7. Retrieve settlement/receipt evidence. Record model id, catalog key, artifact digest, hardware context, rate row, credits, provider share, verification status, request id, and route/provider id matching the provider-side receipt/audit correlation and local `GET /v1/status` model/weights digest binding.
...
Physical acceptance requires all of the following in the same redacted bundle: gateway request id; route/provider id; provider-side `receipt_issued`/receipt-audit or equivalent log line containing that request id, provider id, model id and receipt metadata; local provider `GET /v1/status` JSON before and after with `model_hash` and `model_hash_algorithm` matching the prepared snapshot-manifest artifact identity, with `weights_manifest_sha256` and `weights_manifest_algorithm` present and equal before/after and, when preparation captures a safetensors manifest hash, matching that captured weights hash; and verified settlement evidence referencing the same request/provider/model. If the current active-request tracker does not carry the gateway request id, implementation must pass the audit request id into request tracking or use receipt audit as the normative evidence source.
