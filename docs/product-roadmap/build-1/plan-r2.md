# Build 1 PRD and implementation plan — revision 2
...
3. Run the existing real recommendation producer constrained to that one target:
   `autotune --recommend --check-only --installed-only --candidate-models TARGET
   --progress-json`, with the configured durable resolver and safe drain/restore
   lifecycle. It must produce genuine current local benchmark/fit and complete
   `autotune_recommend.v1` config/feed evidence, cache-only. No config application
   or hidden download occurs in this step. Present only fit and local activation
   implications; suppress monetary/demand recommendations in bootstrap UI.
