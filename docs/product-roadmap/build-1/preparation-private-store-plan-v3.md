# Build 1 preparation private store plan v3
...
3. `ModelPreparationPrivateStore` bootstrap and lock custody: constructor takes two explicit absolute URLs, `authorityRoot` and `artifactRoot`. Bootstrap opens and validates both descriptor chains; authority root owns only private control files, locks (`operation.lock`, `failure.lock`, `cancel.lock`), and the exact private state directory `state/`; artifact root owns only the managed artifact namespace (`.macprovider-prepared-v3`, `objects`, `work/staging`, `work/unpublished`) and durable raw `root.identity`. `LockCustody` captures the authority-root descriptor identity and the artifact-root descriptor identity/root digest observed during bootstrap. Every write/recovery call must prove the supplied custody, root locator, authority descriptor, artifact descriptor, and state payload all belong to that same tuple before any mutation. Lockless reads do not take custody; they reopen and validate the authority descriptor, artifact descriptor, descriptor-relative `root.identity`, supplied root locator, final state envelope, and payload root before returning bytes, and they never repair, barrier-complete, or mutate state.
...
- Private final state lives only under `authorityRoot/state/`. The exact descriptor-relative final leaves in that directory are `reservations.json`, `active.json`, `cancel.json`, `published-inventory.json`, `deletion.json`, `staging-sources.json`, and `failed-dispatch-pending.json`, mapped to the seven `ModelPreparationPrivateStateEnvelopeKind` cases and the PR #1504 byte caps. State temp files are siblings in the same `authorityRoot/state/` parent with the contract filename `<targetLeaf>.<writerUUID>.tmp`; there is no separate `state-tmp` directory and no cross-parent or cross-root rename.
...
- Writes are same-parent temp-to-final operations inside `authorityRoot/state/`: create unique pre-byte-verified sibling temp, write all bytes, fsync/fullsync temp, rename over/into the sibling target only after generation and incumbent checks, fsync/fullsync the `state/` parent, reopen/readback final and validate exact bytes. Any attempted temp or final outside `authorityRoot/state/`, including legacy `state-tmp`, is rejected and not renamed. Store-level writes reject generation `0`; durable generations start at `1` and must increase strictly over an existing final.
...
- Reads are lockless and never repair logical state. A read reopens `authorityRoot`, `artifactRoot`, descriptor-relative `root.identity`, and the exact `authorityRoot/state/<leaf>` final by descriptor; validates the supplied root locator against `root.identity`; validates envelope kind/leaf/checksum/payload/root; and returns payload bytes only. It does not take or require `LockCustody`, does not fsync/fullsync or barrier-complete, and does not mutate cleanup objects, temps, history, or any worker state.
...
- Implement `writeRecord(kind:payload:generation:rootLocator:lockCustody:)` and lockless `readRecord(kind:rootLocator:)` with the descriptor revalidation contract above.
...
- Scan only `authorityRoot/state/` for state temps. Remove empty/stale recognized sibling state temp files only when a full scan is within budget and every sibling is known/safe; cap per-invocation recognized temp processing, and fail closed with no mutation on unknown, hostile, cross-parent, legacy `state-tmp`, semantic-mismatch, or over-budget temp sets without promoting any temp.
...
- Write/recovery APIs cannot be used with closed lock custody or custody from a different authority root. Lockless reads with mismatched authority/artifact/root locator fail closed before returning bytes and never mutate.
...
- R2-M-1 resolved: exact final state directory is `authorityRoot/state/`; temps are same-parent siblings and `state-tmp` is rejected.
- R2-M-2 resolved: reads are explicitly lockless descriptor-validation operations, while write/recovery require custody.
