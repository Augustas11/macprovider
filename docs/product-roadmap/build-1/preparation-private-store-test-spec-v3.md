# Build 1 preparation private store test specification v3
...
- `testBootstrapCreatesPrivateV3NamespaceAndStableRootIdentity`: bootstrap twice; assert stable locator, `0700` authority root and `state/` directory, `0600` lock files, `0700` artifact namespace leaves `objects`, `work/staging`, `work/unpublished`, and `0600` raw `root.identity` bytes that decode as `ModelPreparationRootIdentityRecord` but not as a private envelope.
...
- `writeRecord` with generation 1 under live lock custody writes an envelope at the exact `authorityRoot/state/<targetLeaf>` final; `readRecord` returns the exact payload bytes without taking lock custody after reopening and validating authority/artifact/root descriptors.
...
- `readRecord` with a different artifact root/root locator, swapped `root.identity`, or state final outside `authorityRoot/state/` rejects without mutation.
...
- A valid newer same-parent state temp plus valid older final is not promoted; recovery reports no completed/promoted entries and final remains old.
- Empty recognized temps and stale recognized temps may be removed only when they are siblings under `authorityRoot/state/`, after final validation and safe descriptor checks; report removed leaf names.
- Unknown temp names, FIFOs, symlinks, directories, over-cap bytes, wrong filename UUID, kind/leaf mismatch, checksum mismatch, duplicate-key payload, semantic root mismatch, legacy `state-tmp`, and any temp outside `authorityRoot/state/` cause recovery to throw and preserve every final/temp byte.
