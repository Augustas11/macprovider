# Build 1 promotion authority correction — author addendum R3
...
R3 dispositions: `reviews/promotion-authority-r2-astra.md` reviewed R2 digest
`494a93e351df87e1cdd4617eee8e0e1dd0912a9299a512300ba33a684eded6f5`.
The independent reviewer **closed original PROMO-ARCH-M1 at plan level** and
reported **PROMO2-ARCH-M1 (Medium), changes required** for the missing buyer
read-side availability observation. This revision preserves R2's complete
transport/guard contract and adds the read-side contract and S2-T11 below.
**PROMO2-ARCH-M1 disposition: addressed by author proposal, pending independent
review; not reviewer-closed.** R1/R2 and their reviews remain unchanged. No
runtime implementation is authorized by this revision.
...
Post-registration v1/v2 acknowledgment enqueue failure may return empty identity
values to `handleConn`. The registered session must be captured separately at
registration so those exits publish closing before deferred raw close even when
the returned provider/assigned IDs are empty. Add both real handshake failure
cases to S2-T10; eventual `handleDisconnect` must not be assumed to run.
...
3. Every unconditional/scheduled local teardown in the table calls that
   ...
   transport drain behavior. New paid routing retains its existing gates and adds the exact-session closing observation in the buyer artifact authority resolver, as specified below.
...
## Buyer artifact route observation (PROMO2-ARCH-M1)

Observed current call chain:

- `internal/buyer/model_admission.go:21` `byomDefaultPaidRoutingEligible` calls
  `byomRouteSnapshotBinding`; its artifact branch at lines 112–127 calls
  `ResolveModelAdmissionAuthority`, compares complete immutable evidence and
  probe validity, and attempts CAS revocation on failure.
- `internal/buyer/model_admission_authority.go:26–41` currently resolves the
  exact registry session, requires nonnil `Conn`, and checks ready/busy state.
  None observes WS logical closing; a nonnil socket reference and ready state
  can survive already-published local closing throughout graceful shutdown.
- Actual buyer selection reaches the predicate through
  `eligibilityCtx.ProviderBYOMSettlementEligible`,
  `validatePinnedProviderForRequest`, and queued-candidate checks in
  `internal/buyer/server.go`; `selectProvider`/`selectProviderExcluding` are the
  real paid selection path. `internal/buyer/route_snapshot.go:79` independently
  calls `requireBYOMRouteSnapshotBinding` before constructing a route snapshot.
  `/v1/models` also filters by the BYOM predicate. WS `isOpen` in the canary
  floor is not a substitute for any of these buyer checks.

Required integration:

1. Alongside R2's constructor-wired close callback, supply the buyer artifact
   authority service with the WS-owned read callback
   `func(providerID, assignedID string) bool`. Proposed exported WS method:
   `ModelAdmissionSessionAvailable(providerID, assignedID string) bool`.
   Both identities must be nonempty and exact. The callback verifies the current
   registry still names that assigned session and loads that exact stored WS
   session, checking the same monotonic closing and terminal closed publication
   used by the commit guard. Missing registry entry, missing/mismatched stored
   session, closing or closed returns false. It never resolves a newer session
   using provider ID alone and never substitutes a new session's availability
   for an old evidence tuple.
2. Reuse the session's R2 publication (`closingCh`/terminal `closedCh` or the
   approved equivalent); there is no buyer-cached closing bit or replicated
   state requiring asynchronous invalidation. This read callback is a bounded
   observation: ordinary registry read lock is released inside its getter,
   stored-session lookup/closing reads remain lock-free, and all accesses finish
   before it returns. No publication/session/admission pin is returned or held
   through feed parsing, DB lookup, paid selection, route snapshot persistence,
   dispatch or inference. An invalidation published before this read must be
   seen; an overlapping change can serialize after the observation. Existing
   pre-dispatch revalidation remains in place; this is not a new routing-wide
   transaction or guarantee of future transport availability.
3. `ResolveModelAdmissionAuthority` must require this callback and a positive
   exact-session result in its live transport predicates **on every call**,
   including route revalidation against a persisted positive event. False or
   missing callback returns unavailable authority before returning any positive
   binding. The existing artifact branch then returns `found=true,
   eligible=false` and attempts its CAS revocation. A revocation write failure
   cannot convert authority rejection into eligibility: rejecting the request
   depends on the live read, not on successful mutation of the historical event.
4. Validate both read and close callbacks when enabling the guarded artifact
   service in coordinator wiring and fixture composition. Missing either refuses
   enabling artifact promotion, and a persisted artifact event cannot bypass
   missing read wiring on route/readback. Callback options are constructor-only
   and immutable after service installation; no hot callback replacement race
   is introduced. Legacy snapshots and genuine non-artifact paths keep their
   prior interpretation. Do not broaden missing-wiring behavior into new global
   exclusions for providers that have never entered artifact admission.
5. Promotion pre-resolution can use the normal read callback. The fully pinned
   commit phase must use its private lock-held exact-session view of the **same**
   publication; it must not invoke the callback's `Registry.Resolve` recursively
   under the pool pin. Likewise pinned fresh-response validation uses the
   lock-held view. Keep one availability meaning with two access modes, rather
   than routing through the full resolver while holding owner locks.
6. For replacement isolation, availability of old assigned ID is false even if
   an old map entry remains. The new exact session may report true while an old
   timer fires, but its availability alone cannot validate the old admission
   event: current evidence must still match captured `ProviderSessionID`, receipt
   and all existing session/feed/rate bindings. It needs its own valid admission
   before paid selection. Graceful writer/control-frame enqueue behavior stays
   as R2 specifies; reject at artifact authority instead of relying on eventual
   socket write failure or terminal writer shutdown.

The WS implementer owns the read callback/private lock-held view. Buyer owns
mandatory wiring validation and invoking the callback in the artifact resolver;
lead owns production construction and integration composition. The existing
buyer routing call chain must be tested with this implementation, not a new test
predicate that production selection never calls.
...
Retain the read-only resolver for routing, extending its transport predicates with the mandatory exact-session availability callback above; add an internal prepared-authority
...
### S2-T11 — route rejection before status revocation

Add `TestArtifactRouteRejectsClosingBeforeReadback` and
`TestArtifactPaidSelectionRejectsClosingBeforeReadback`, each with memory and
SQLite admission stores and real WS callback wiring. Use valid signed-fixture
candidate/artifact/rate/Tier2 material and a persisted `settlement_capable` event
whose original probe/authority leases remain unexpired. Confirm the current
session is ready, exact map entry present, registry `Conn` nonnil, and all captured
feed/rate/receipt/session fields unchanged immediately before the tested call.

Required independent entry-point subtests (fresh fixture for each because a
route call may itself append revocation): direct `ResolveModelAdmissionAuthority`,
`byomRouteSnapshotBinding`/`requireBYOMRouteSnapshotBinding`, actual
`selectProvider` default and pinned paths, queued selection recheck, and the
HTTP buyer request composition. Sole candidate is the closing artifact provider;
no fallback provider hides accidental selection. For HTTP, use the real handler
and existing route-error semantics; assert no provider selected, no inference
frame dispatched and no new payable artifact route/ledger record. Dispatch/socket
failure is not acceptable proof of selection rejection.

- Publish closing with real `closeSession`, scheduled operator/trust teardown,
  and buyer `handleProviderFailure` wired to the real WS close function. Pause
  definite-close timers and eventual registry/map cleanup. For the buyer/scheduled
  producer, run a ready/busy update after closing publication to prove the new
  discriminator is required even if a reversible unavailable/draining state is
  restored. Inspect that the historical positive event is still latest. Do not
  invoke status, offer, retry, `/v1/models`, an earlier route predicate or any
  other revoking path before the entry point being tested.
- At that entry point assert exact-session authority rejects immediately because
  of closing; binding returns found-but-ineligible, and actual paid selection
  returns a route error rather than that provider. Then inspect the attempted
  CAS revocation and subsequent fresh readback. This ordering distinguishes live
  route rejection from an earlier status call merely cleaning up the event.
- Repeat binding and paid-selection tests with a store wrapper that rejects
  **only revocation writes**, delegating all reads and other operations to the
  real memory/SQLite store. The positive event must remain persisted throughout;
  route/selection still reject. This is fault injection of persistence, not a
  mock availability callback. Never call a previously revoking route predicate
  as the setup for the next entry-point assertion.
- Missing read callback: artifact-service installation fails, and direct artifact
  resolver/route/selection with an already-persisted positive record reject rather
  than fall back to registry-only authority. Missing close callback retains R2's
  refusal. Table-test empty/wrong provider ID, empty/wrong assigned ID, map absence,
  terminal closed, closing, and valid available exact session.
- Replace the registered session while retaining the old map entry and old timer.
  Old callback query must be false; exact new-session availability stays true
  before and after old timer firing. Old positive evidence must not admit the
  new session. A separately established valid event for the replacement restores
  normal eligibility; delayed old teardown must not revoke that newer event
  through a stale CAS or mark the new session closing.
- No-drift positive control must pass the actual selection/route-snapshot path,
  not only the callback. Existing valid legacy/no-artifact paths and historical
  snapshot digests remain unchanged; missing new callback must not silently
  reclassify valid legacy routes as artifact admission. Add a guard-held regression
  that fails if the normal callback's locking registry getter is invoked by
  pinned promotion/readback; use the private lock-held view there.
- Verify the route callback returns after releasing any short read lock; run
  actual local closing while downstream route persistence/dispatch is blocked
  and assert publication proceeds. No promotion pin may survive the observation
  into routing/inference. A closure concurrent with an already-started observation
  may follow that observation; closure completed before the next check must be
  rejected. Run under `-race` with deterministic barriers, no sleeps.
...
absence/registration condition in the inventory. This close option and the read availability option specified above are both required when
installing the guarded artifact service, are not runtime-swappable, and do not
...
- **Route integration gap:** a closing-aware promotion/status guard without the same read-side observation in buyer artifact authority permits fresh paid selection during graceful shutdown. T11 must begin with a still-positive event and test actual selection before status or any other revoking call.
