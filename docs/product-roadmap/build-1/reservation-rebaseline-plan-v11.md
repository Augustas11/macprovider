# Build 1 preparation-reservation rebaseline plan v11

Status: revised after the blocked preparation-authority v6 review; **blocked before slice 6B** until the operator-owned authority and these v11 planning bytes satisfy every Dependency Gate 5 item and an independent review reaches zero Critical, High, and Medium findings. This revision does not claim that gate passed.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f` (BYOM v0.2 slice 6 operator state/copy). Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9` (`origin/main` reviewed on 2026-09-11). Authority revision reviewed by v6: `a73d6020d34acacc3cd32399d93306b02bddedba`; formal v6 review artifact commit: `ebcf395670623ab9bf69dd1f6cffbb51d55be99c`.

Revision inputs: v10 plan SHA-256 `da3841ac6fe153f0809cf5325102ce49e2e0475c6ffe2150a4d2fa34e1ad01e3`; v10 test-spec SHA-256 `c783b21171292c228d0a0f6048eb16bfb2240a9608fe7491dc19916aaa2f95fd`; blocking review `reviews/preparation-authority-v6-sol.md`, SHA-256 `db9ccb076380e8bb8e1e77fcf307d0be12e9ef295c85b3aba0b47ceab2c63b60`. The v11 plan/test digests are recorded from their committed bytes at T18 rather than embedded recursively.

Authority: issue #1453 remains the execution queue. This document supersedes v10 as planning input. It does not approve, migrate, or reuse abandoned R21-R27 protocol or local development state.
...
Every syntactically and structurally valid run process validates the immutable projected-action identity, allocates a fresh attempt, acquires the distinct private `failure.lock`, and becomes an attached worker before checking semantic freshness, action availability, or live-operation conflict. A stale, unavailable, or conflicting action becomes a failure-only worker: it durably persists and readback-validates one bounded pending `failed_dispatch` reservation/history record, emits exactly one terminal `failed` event at sequence 1, durably compacts and readback-validates the record into terminal history, and exits 3 on the non-crash path. It never creates `active.json` or a live attempt and never starts network, staging, download, publication, cleanup, adoption, or model mutation. A conflict worker may coexist with the incumbent only as this serialized failure-only process; it is never a second active work attempt.

The attached live or failure-only worker is the sole producer and sequencer of `model_catalog_transaction_event.v1` events. The cancellation process emits no transaction event. It returns one separate, bounded, operator-approved cancellation acknowledgement whose only claim is whether an attempt-bound marker was durably recorded or the observed attempt was already terminal/inactive. Malibu continues consuming the worker stream and refreshes `models catalog-economics --json` for authoritative status.
...
3. The cancel process serializes marker mutation with `cancel.lock`, durably writes an attempt-bound marker when still applicable, and returns one bounded cancellation acknowledgement. It emits no transaction event.
...
| Failure-only worker | `failure.lock`, bounded terminal `failed_dispatch` history, one sequence-1 event | `active.json`, operation/cancel/cleanup/adoption/runtime locks, live work, incumbent mutation |
...
The private v3 reservation and tuple retain all v2 target fields, signed primary MLX qualification, 1 TiB estimate cap, config/provider/root binding, binary framing, 64 selected entries, 256 total history limit, deterministic `authority_order`, one active pin, eight oldest-wait fairness slots, unchanged-ID retention, bounded starvation, and serialized dispatch. `failed_dispatch` entries consume that same 256-record durable history budget; they do not raise it or create an unbounded auxiliary log. At 257 eligible tuples Prepare is unavailable rather than silently truncated.

### Constructive pre-work failure lifecycle

Syntax, framing, identifier grammar, schema, and the complete immutable projected-action identity are validated before attachment. The immutable identity includes the bounded projected-transaction record, transaction ID and kind, non-null historical `event_model_key`, saved root locator and identity, complete tuple, and projection/action binding digest. Failure at that boundary exits 2 and emits no valid transaction event because no trustworthy identity exists. Once those fields validate, the initiating run process generates a fresh CSPRNG attempt ID, acquires the distinct private owner-mode `0600` `failure.lock`, and becomes the attached failure worker before it evaluates semantic freshness, current availability, or the serialized durable conflict view.

While holding `failure.lock`, the worker revalidates those semantic gates. If the action remains eligible and no operation is active or claimed, it durably records one bounded pre-work dispatch claim, releases `failure.lock`, acquires `operation.lock`, then takes `failure.lock` in the sole allowed operation→failure order to revalidate and atomically promote only that claim into the one live `active.json` attempt. It then ceases to be failure-only. A process that loses the operation-lock race returns to `failure.lock` and records `operation_conflict` without creating `active.json`. A normal worker or recovery path that publishes, promotes, terminally compacts, or removes conflict-visible state likewise takes operation→failure and never reverses it.

For semantic `stale_transaction`, `action_unavailable`, or `operation_conflict`, the failure worker creates no `active.json` and no live work attempt. Before stdout, it atomically persists and readback-validates one private pending record no larger than 16384 bytes. The closed record contains exactly `schema: model_catalog_failed_dispatch.v1`, `transaction_id`, fresh `attempt_id`, `transaction_kind`, immutable non-null `event_model_key` copied verbatim from the validated projected action, the complete saved root locator/identity object, `tuple_sha256`, `projection_binding_sha256`, `event_sequence: 1`, `terminal_state: failed`, the applicable error code, and `live_attempt: false`. The digests bind the exact validated tuple and bounded projected-action reservation bytes. The record contains no phase, transfer counter, cancellation marker, credential/provider identity, feed body, URL, prompt, completion, raw error, or unbounded text. Once the pending record is durable, the same attached process emits exactly one matching terminal `failed` event at sequence 1, durably compacts and readback-validates it into terminal history, and exits 3 on an uninterrupted execution. Failure to persist the record emits no unbound event and exits 5.

Pending failed-dispatch records and ordinary terminal history share the existing 256-record and aggregate byte caps. Under `failure.lock`, a new failure first compacts any earlier complete pending record and applies the same deterministic oldest-terminal eviction, then creates one pending record; at most one pending record exists. The failure-only process never acquires `operation.lock`, `cancel.lock`, a cleanup lock, `RecommendationAdoptionLock`, the runtime reservation, or the serving socket, never waits for any of them while holding `failure.lock`, and never opens the artifact root for mutation. It touches only the bounded pre-work claim/failed-dispatch state and recognized unique temporaries. This makes operation-conflict reporting constructible while the incumbent continues holding `operation.lock`; the reporter is serialized history work, not a second active attempt.

Crash recovery is total at every boundary. Before durable pending publication, recovery removes incomplete recognized temps and a retry uses a fresh attempt. A complete recognized temp may finish only its validated write. After the pending record is durable but before event flush, after event flush but before compaction, or during compaction, the same immutable terminal record remains authoritative. Recovery under operation→failure, or the next failure worker's bounded self-compaction under `failure.lock`, compacts it exactly once and never emits replacement stdout into a later process stream. Thus an uninterrupted invocation produces exactly one event and exit 3; a killed process may end without a delivered terminal or exit code, but can never produce success, a duplicate recovery event, live state, or work side effects. Malibu treats child loss as interrupted and refreshes rather than fabricating delivery. Failed dispatch is never cancellable or live; the UI exposes no cancel action, and an exact direct cancellation query may only observe terminal history through the existing `terminal` acknowledgement without mutation.
...
Preparation crash recovery retains v2 semantics: no byte continuation after process death, exact saved-root cleanup, same-attempt live Range resume only, retry from zero on new attempt, recovery of rename/parent/active/event boundaries, preservation of conflicting destinations, and fresh projection as status. It additionally reconciles recognized unique temps, root bootstrap, bounded failed-dispatch pending/history records, pre-work claims, cancel markers, and both cleanup kinds' phased intent/tombstone/remove records without permanent wedges. Failed-dispatch recovery compacts terminal state without fabricating stdout or creating live work. Retry resumes only the recorded transaction/root/target/phase; it never creates a second tombstone or reinterprets the current config.
...
Land the single complete SPEC-044 v0.2.6 operator patch from Gate 5, including `SPEC-001-R003`, exclusive complete version pairs and exact fallback presentation, scoped guidance binding/exact catalog-only sentinel, exhaustive R002/R003 matrix/admission-only copy and exact 12-state inventory, conditional earning verdict, constructive failed-dispatch lifecycle, bounded cancellation acknowledgement/sequencer, continuous cleanup locks, cleanup targets/action-to-nested-action JCS equality plus independent per-copy target binding/event correlation/accounting, root/ACL/process transport rules, the exact SPEC-044-R005 ranking, and both cleanup state machines. Record exact versions/commit/digests/owner and prove strict ancestry. Re-review this v11 plan/test plus SPEC diff to zero C/H/M.

### Slice 6B — private preparation/storage engine

Implement secure roots and unique-temp recovery, bounded selection, `failure.lock` claims/failed-dispatch history, worker/cancel locks, transfer counters, v3 namespace, durable publication, logical-byte/legacy accounting, bounded cleanup targets, and intent-first cleanup with exact cancellation/recovery.
...
3. Every immutable-identity-valid run attaches before semantic freshness, availability, or conflict checks. Each pre-work rejection has one fresh attempt, bounded durable `failed_dispatch`, sequence-1 terminal event, exit 3, deterministic crash compaction, and no live/work side effects; operation conflict remains constructible beside an unchanged incumbent under distinct `failure.lock`. Live worker alone sequences later events; cancellation returns only the approved bounded acknowledgement with request echo, the 2.000-second monotonic lock deadline, six outcomes including null-attempt/no-mutation `busy`, total acquired-lock outcome precedence, exact nullability/exit behavior, bounded resources, and no late/terminal/new-attempt marker effect.
...
9. Every forward-current cross-reference selects SPEC-044 v0.2.6, older versions remain only explicitly historical, and `SPEC-001-R003`, AUTHORITY consumers, R005 CONFORMANCE gaps/evidence, versions, indexes, exact source digests, and review lineage agree.
...
| Pre-work rejection cannot emit a bound terminal beside an incumbent | Distinct `failure.lock`, immutable action binding, fresh attempt, bounded pending record, one sequence-1 event, durable compaction, and no live-attempt mutation. |
...
| B1-AUTH-V6-M1 | Forward-current SPEC-044 v0.2.6 version/index/digest agreement | T18 |
