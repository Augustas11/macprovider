# Build 1 preparation-reservation rebaseline plan v12

Status: aligned to finalized preparation authority v7 after the blocked v6 review; **blocked before slice 6B** until the operator-owned authority and these v12 planning bytes satisfy every Dependency Gate 5 item and an independent review reaches zero Critical, High, and Medium findings. This revision does not claim that gate passed.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f` (BYOM v0.2 slice 6 operator state/copy). Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9` (`origin/main` reviewed on 2026-09-11). Finalized authority v7 revision: `d12e9428df0f0bd26df720da7cd9b9fbbc820ec0`; formal v6 review artifact commit retained as correction input: `ebcf395670623ab9bf69dd1f6cffbb51d55be99c`.

Revision inputs: v11 plan SHA-256 `998e6e2cfd4a41885da785b67a73ab5127cbaaf84ade66e58215a4ededf74cdd`; v11 test-spec SHA-256 `4bb0a37f41105c41d8be322a20631df99aad9996948d7b928ac1ef286b77a40b`; finalized authority v7 commit `d12e9428df0f0bd26df720da7cd9b9fbbc820ec0`; blocking review `reviews/preparation-authority-v6-sol.md`, SHA-256 `db9ccb076380e8bb8e1e77fcf307d0be12e9ef295c85b3aba0b47ceab2c63b60`. The v12 plan/test digests are recorded from their committed bytes at T18 rather than embedded recursively.

Authority: issue #1453 remains the execution queue. This document supersedes v11 as planning input. It does not approve, migrate, or reuse abandoned R21-R27 protocol or local development state.
...
Every syntactically and structurally valid run process validates the immutable projected-action identity, allocates a fresh attempt, acquires the distinct private `failure.lock`, and becomes an attached failure worker before checking semantic freshness, action availability, or the serialized durable conflict view. A stale or unavailable result found under that first lock is recorded there. Otherwise the process releases `failure.lock` and makes exactly one nonblocking `operation.lock` acquisition attempt. A failed acquisition causes it to reacquire only `failure.lock`, revalidate stale/unavailable precedence, and record the winner as a failure-only worker. A successful acquisition makes it a pre-active normal worker; it acquires `failure.lock` in operation→failure order, revalidates, and either records the new stale/unavailable result or creates the sole `active.json`. There is no durable handoff claim. Every lock is released before failure-event stdout; only after the event write returns does the process reacquire `failure.lock` for idempotent compaction. A failure-only worker never becomes a second active work attempt or starts network, staging, download, publication, cleanup, adoption, runtime, or model mutation.
...
Syntax, framing, identifier grammar, schema, and the complete immutable projected-action identity are validated before attachment. The immutable identity includes the bounded projected-transaction record, transaction ID and kind, non-null historical `event_model_key`, saved root locator and identity, complete tuple, and projection/action binding digest. Failure at that boundary exits 2 and emits no valid transaction event. Once those fields validate, the initiating run process generates a fresh CSPRNG attempt ID, acquires the distinct private owner-mode `0600` `failure.lock`, and becomes the attached failure worker before it evaluates semantic freshness, current availability, or the serialized durable conflict view.

While holding `failure.lock`, the failure worker revalidates those semantic gates. If the action is stale, unavailable, or the serialized durable view is already active, it records the winning exit-3 failure without releasing that lock. Otherwise it releases `failure.lock` and makes exactly one nonblocking acquisition attempt on `operation.lock`; it never waits for that lock. Failure to acquire is `operation_conflict`: the process reacquires only `failure.lock`, revalidates that stale or unavailable has not become the higher-precedence result, and writes the winning failure record without creating `active.json`. This conflict reporter never acquires `operation.lock` and is never a second active attempt.

Successful nonblocking acquisition changes the process into a pre-active normal worker. It takes `failure.lock` in the sole allowed operation→failure order, repeats the semantic and conflict-visible checks, and creates the one live `active.json` attempt only if they still pass. A newly stale or unavailable result is written while holding operation→failure without creating live state. There is no durable handoff claim between releasing `failure.lock` and attempting `operation.lock`; a crash in that interval leaves no claim or live attempt to recover. Every normal worker or recovery path that publishes, promotes, terminally compacts, or removes conflict-visible state likewise holds operation→failure and never reverses it.
...
Once the pending record is durable, the process releases **every** lock before writing or flushing stdout. It then emits exactly one matching terminal `failed` event at sequence 1, so pipe backpressure or app disconnect cannot hold `failure.lock`, `operation.lock`, or starve a valid dispatch. After the event write returns, it reacquires only `failure.lock`, idempotently compacts and readback-validates the record into terminal history if another process or recovery has not already done so, releases the lock, and exits 3 on an uninterrupted execution.

Pending failed-dispatch records and ordinary terminal history share one global 256-record limit and one aggregate 262144-byte cap. Under `failure.lock`, a new failure first compacts any earlier complete pending record and applies the same deterministic oldest-terminal eviction, then creates one pending record; at most one pending record exists. The failure-only path never acquires `operation.lock`, `cancel.lock`, a cleanup lock, `RecommendationAdoptionLock`, the runtime reservation, or the serving socket, and never waits for any of them while holding `failure.lock`. A normal or startup recovery compactor takes operation→failure; a failure worker may compact its own record under `failure.lock` alone and never then acquires `operation.lock`.

Crash recovery is total at every boundary. Before durable pending publication, recovery removes incomplete recognized temps and a retry uses a fresh attempt. A complete recognized temp may finish only its validated write. A crash in the unlocked pre-operation interval leaves no durable handoff claim or live attempt. After the pending record is durable but before event flush, after event flush but before post-event compaction, or during compaction, the same immutable terminal record remains authoritative. Recovery compacts it exactly once and never emits replacement stdout into a later process stream or replays work. Thus an uninterrupted invocation produces exactly one event and exit 3; a killed process may end without a delivered terminal or exit code, but can never produce success, a duplicate recovery event, live state, or work side effects. Malibu treats child loss as interrupted and refreshes rather than fabricating delivery. Failed dispatch is never cancellable or live; the UI exposes no cancel action, and an exact direct cancellation query may only observe terminal history through the existing `terminal` acknowledgement without mutation.
...
3. Every immutable-identity-valid run creates a fresh attempt and attaches under `failure.lock` before semantic freshness, availability, or serialized conflict-view checks. A passing first check releases that lock and makes exactly one nonblocking operation-lock attempt; conflict reacquires only `failure.lock` and revalidates precedence, while success enters pre-active operation→failure revalidation before `active.json`. No durable handoff claim exists. Each pre-work rejection has one bounded durable `failed_dispatch`, every lock released before its sequence-1 terminal stdout, post-event failure-lock-only compaction, exit 3, deterministic crash recovery, and no live/work side effects. Operation conflict remains constructible beside an unchanged incumbent. Live worker alone sequences later events; cancellation returns only the approved bounded acknowledgement with request echo, the 2.000-second monotonic lock deadline, six outcomes including null-attempt/no-mutation `busy`, total acquired-lock outcome precedence, exact nullability/exit behavior, bounded resources, and no late/terminal/new-attempt marker effect.
