# Build 1 preparation-reservation rebaseline plan v4
...
Authority: issue #1453 remains the execution queue. This document supersedes v2 as planning input. It does not approve, migrate, or reuse abandoned R21-R27 protocol or local development state.
...
## Preparation eligibility and copy matrix

The operator patch must freeze the following policy in SPEC-044 R002/R003 before implementation. A valid row still must have a current signed candidate/artifact binding, primary verified `mlx_safetensors` artifact, `fit: fits`, `runtime_state: needs_preparation`, non-null `action_model_id`, exact positive `estimated_bytes`, and no non-economic safety block.
...
This matrix covers every legal source/state/economics combination: `trusted` either follows the permitted priced path or is invalid; all four non-trusted states use the same local classification when other local safety gates pass. The local action is not described as higher-paying, network-ready, offer-ready, admission progress, or earning. A reachable handoff is therefore `local_only`/`not_offered`/`offerable` → **Prepare locally** → evaluate → offer → adopt, while admission/economics remain independently authoritative.
...
5. **Hard pre-6B authority gate:** @Augustas11 must land one narrowly scoped SPEC-001/SPEC-044 patch, with authority/index updates, that freezes: (a) exact run/cancel option spelling under `models catalog-economics`; (b) closed event error/warning codes; (c) R002/R003 locally motivated preparation eligibility, the complete matrix above, exact operator-owned local/trusted copy, and economics/demand separation; (d) worker-only event sequencing plus the exact bounded cancellation-ack schema, outcomes, size, and marker-race semantics; and (e) the versioned projection/action/accounting contract for `cleanup_published_artifact`, managed-v3 totals, configured-legacy protected totals, and budget policy. It must retain `model_catalog_transaction_event.v1`, add no status or control-socket schema, and not overload `cleanup_staging`.
6. T18 proves this authority commit is a strict ancestor of the first 6B implementation commit. The revised plan and landed SPEC diff must pass fresh zero-C/H/M review before 6B.
7. Slice 7 remains the signed physical-hardware, discovery/admission, settlement, first-listed-release, signed-assets, and updater gate.
...
3. The proposed acknowledgement is a single JSON object capped at 4096 bytes with exact fields `schema`, `transaction_id`, nullable `attempt_id`, `outcome`, and `observed_at`; `schema` is `model_catalog_transaction_cancel_ack.v1` and the closed `outcome` values are `recorded`, `already_recorded`, `terminal`, `not_active`, and `stale`. The operator patch must approve or replace this exact shape before implementation. `recorded` means only durable marker custody, not worker observation or cancellation success.
4. The worker checks the marker at the 250 ms watchdog and every bounded work loop. It emits `cancel_requested` once when cancellation wins before publication, then terminal `cancelled` after cleanup. After durable publication it emits only `succeeded` or `failed`.
5. Before terminal compaction/releasing `operation.lock`, the worker takes `cancel.lock`, durably commits terminal state, removes an exact matching marker, releases `operation.lock` while still holding `cancel.lock`, then releases `cancel.lock`. Thus a cancel process either writes before the terminal sweep or observes the released/terminal operation and does not leave a late marker.
6. A new attempt takes `operation.lock` then `cancel.lock`, validates and removes only a stale marker bound to the prior attempt, persists the new active record, and releases `cancel.lock`. A marker can never apply across attempts.
...
Inventory enumerates only `.macprovider-prepared-v3/objects`, capped at 256 exact receipts. Storage projection separates:
...
An externally seeded overflow is corruption, not a state production may create. A bounded recovery mode may inspect exactly 257 directory entries. When there are exactly 257 and every entry has a valid v3 receipt/root binding, the operator-approved cleanup action may, after provider confirmation and the normal keep-set checks, remove one exact reclaimable identity through the same intent/tombstone protocol and return inventory to 256. It never guesses, truncates, or deletes a protected or malformed entry. More than 257 entries, any invalid entry, or no reclaimable identity remains fail closed and requires out-of-band operator diagnosis; it is not reported as ordinary product recovery.
...
Land the single complete operator patch from Gate 5, including R002/R003 matrix/copy and cancellation acknowledgement/sequencer. Record exact versions/commit/owner and prove strict ancestry. Re-review plan plus SPEC diff to zero C/H/M.

### Slice 6B — private preparation/storage engine

Implement secure roots and unique-temp recovery, bounded selection, worker/cancel locks, transfer counters, v3 namespace, durable publication, legacy accounting, and intent-first cleanup.

### Slice 6C — CLI and Malibu surface

Implement only approved grammar/schemas. Render exact local/trusted preparation copy, worker events, cancellation acknowledgement, projection refresh, storage accounting, confirmation, accessibility, and fallback.
...
1. The authority patch makes pre-offer local preparation reachable and conforming through exact eligibility, copy, and economics separation across every admission/economics combination.
2. Worker alone sequences transaction events; cancellation returns only the approved bounded acknowledgement; late/terminal/new-attempt marker races leave no stale effect.
3. Unique validated temps and atomic root bootstrap recover every ordinary crash without permanent wedge.
4. URLSession guarantees only accepted/staged byte caps; delegate-delivered, transport, and server metrics are explicitly separated.
5. V3 publications/inventory stay inside the dedicated namespace; configured legacy artifacts are protected/accounted and no legacy object is imported or deleted.
6. Deletion intent is durable before rename, phase follows the parent barrier, and every boundary has exact non-scanning recovery.
7. All v2 root, selection, durability, resource, incumbent, authority, adoption, compatibility, privacy, and final hardware/release gates remain in force.
...
- Authority patch omits any matrix/copy, cancel acknowledgement/sequencer, invocation/event, or cleanup/accounting contract, or does not predate 6B.
...
| B1-V3-M1 | Publish-once object-count admission; bounded overflow recovery | T09, T10 |
