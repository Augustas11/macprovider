# Build 1 preparation-reservation rebaseline plan v7

Status: revised after the blocked preparation-authority v2 review; **blocked before slice 6B** until a corrected operator-owned authority patch satisfies every Dependency Gate 5 item and an independent review reaches zero Critical, High, and Medium findings.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f` (BYOM v0.2 slice 6 operator state/copy). Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9` (`origin/main` reviewed on 2026-09-11). Rejected authority candidate: `f691de4d12250b97d86d91be7333d29db1ee5f20`.

Revision inputs: v6 plan SHA-256 `a682d6b7caade4540e56f042d6ba9d70731af493a56abaa78c6fff18fede9678`; v6 test-spec SHA-256 `1c56872db5855b91916661bd2672f4a903300762db3ba390ca1227271b73cb89`; blocking review `reviews/preparation-authority-v2-sol.md`. The v7 plan/test digests are recorded from their committed bytes at T18 rather than embedded recursively.

Authority: issue #1453 remains the execution queue. This document supersedes v6 as planning input. It does not approve, migrate, or reuse abandoned R21-R27 protocol or local development state.
...
5. **Hard pre-6B authority gate:** @Augustas11 must land one narrowly scoped SPEC-001/SPEC-044 patch, with authority/index updates, that freezes: (a) exact run/cancel option spelling, exclusive complete v1/v2 capability+token pairs, and all partial/dual/conflict/manifest-status negatives; (b) closed event error/warning codes and total error precedence; (c) R002/R003 locally motivated preparation eligibility, catalog-only all-null/no-action behavior, candidate-associated all-non-null binding, coordinator no-event and event-backed `not_offered`, `verification_status: verified`, exact source/freshness correlation, exact operator-owned local/trusted copy, and economics/demand separation; (d) worker-only event sequencing plus the exact bounded cancellation-ack schema, request echo, total outcome precedence, nullability, marker-race semantics, and cancel-process read/marker-only authority; (e) the versioned projection/action/accounting contract for `cleanup_published_artifact`, bounded `cleanup_targets` with immutable `event_model_key`, managed-v3 totals, configured-legacy protected totals, logical-byte accounting, truthful APFS copy, and exact budget policy; (f) published and staging durable-`tombstoned` commit evidence, reversible precommit restoration, and operation→cancel recovery ownership; (g) authenticated root identity/locator persistence and the no-extended-ACL policy; and (h) app-owned refresh generation plus constant-space process transport/backpressure. It must retain `model_catalog_transaction_event.v1`, add no status or control-socket schema, and not overload `cleanup_staging`.
...
Every private authority, namespace, lock, lifecycle record, temp, staging object, unpublished object, published object, receipt, and tombstone must have no extended ACL entries. For newly created objects the implementation clears any inherited ACL through the already-open descriptor before writing sensitive bytes, then verifies the ACL is empty; failure to clear or verify fails closed and removes only that newly created empty object. Existing objects carrying any extended ACL are never repaired in place and fail closed. Descriptor-based ACL validation repeats immediately before every sensitive read/write, rename, publication, restoration, or unlink; a post-validation ACL mutation therefore loses the race or aborts before the side effect. POSIX owner/mode checks remain independently mandatory.
...
All recovery mutation belongs to the worker/recovery-worker path after it owns `operation.lock`; whenever it must inspect or sweep cancellation state it then takes `cancel.lock`, preserving the sole nested order `operation.lock` → `cancel.lock`. The cancel process never inverts that order because it never takes `operation.lock`.
...
4. With `operation.lock` held, check the exact marker under `cancel.lock`, release `cancel.lock`, exclusively rename the exact v3 final leaf to its reserved tombstone sibling in the same `objects` parent, and full-sync that shared parent. The rename and barrier remain reversible preparation while durable phase is still `intent`.
5. Reacquire `cancel.lock`. If the exact marker exists, restore tombstone to the exact final leaf, full-sync the parent, clear intent, and cancel. Otherwise update and full-sync `deletion.json` to `phase: tombstoned` while still holding `cancel.lock`. The durable `tombstoned` phase—not rename or parent barrier alone—is the sole deletion commit evidence and cancellation linearization boundary.
...
Recovery never scans or guesses and only a recovery worker holding `operation.lock` may mutate. With `intent`: final present/tombstone absent means recheck keep set and exact marker, then resume or clear; final absent/tombstone present is always precommit, so under `cancel.lock` an exact marker restores tombstone to final and cancels, while no marker permits the recovery worker to repeat the parent barrier and durably advance to `tombstoned`; both present or both absent fails closed. With `tombstoned`: tombstone present resumes deletion; tombstone absent/final absent repeats parent sync then advances to `removed`; final present fails closed. With `removed`: both absent permits record clear; any target present fails closed. Every step reopens the saved root only. Legacy paths are invalid cleanup targets.

For published cleanup, cancellation wins until `phase: tombstoned` is durable. Before that phase commit, no-rename intent clears safely, and any descriptor-validated tombstone is restored to the exact final leaf followed by a full parent sync, intent clear, `cancel_requested`, and `cancelled`. Once `tombstoned` is durable, cancellation is late: the worker or recovery completes exact deletion and emits/returns `succeeded`, or retains recoverable state and fails with `cleanup_failed`. A crash at any precommit point, including after rename or either parent barrier but before durable phase persistence, remains `intent`; an exact marker written by the cancel-only process forces restoration when the worker/recovery worker next obtains `operation.lock`. Without a marker, recovery may repeat barriers and persist `tombstoned`. The cancel process itself never performs restoration or advances a phase.

`cleanup_staging` has an independent recorded target and uses an equivalent reversible intent → same-parent tombstone → durable `tombstoned` → removed discipline inside the attempt's staging parent. Its commit evidence is likewise the durable `tombstoned` phase. Cancellation while phase remains `intent`, including after a synced rename, preserves or restores the incomplete staging tree, retains `staging_cleanup_required`, and ends `cancelled`; cancellation after durable `tombstoned` completes removal and ends `succeeded` or recoverable `cleanup_failed`, never `cancelled`. Only the operation-owning worker/recovery worker restores or advances it. A staging cleanup record cannot name a published, legacy, different-attempt, or different-root object, and published cleanup cannot name staging.
...
Land the single complete operator patch from Gate 5, including `SPEC-001-R003`, exclusive complete version pairs, scoped guidance binding/catalog-only nullability, exhaustive R002/R003 matrix/copy, cancellation acknowledgement/sequencer, cleanup targets/event correlation/accounting, root/ACL/process transport rules, and both cleanup state machines. Record exact versions/commit/owner and prove strict ancestry. Re-review this v7 plan/test plus SPEC diff to zero C/H/M.
...
1. The authority patch makes pre-offer local preparation reachable and conforming through required all-or-none candidate/guidance binding on candidate-associated rows, catalog-only nullability/no-action behavior, exact source/freshness correlation including coordinator no-event `not_offered`, `verification_status: verified`, exact copy, and economics separation across every legal admission/economics combination.
...
6. V3 publications/inventory stay inside the dedicated namespace; logical-byte accounting is deterministic; APFS copy does not promise physical recovery; every reclaimable object is reachable through bounded `cleanup_targets` with an immutable event model key; configured legacy artifacts are protected/accounted and no legacy object is imported or deleted.
7. Published and staging deletion intent is durable before rename, durable `tombstoned` phase is the sole commit evidence, every `intent` tombstone remains reversibly restorable for an exact marker, only the operation-owning worker/recovery worker mutates recovery state, and cancellation/crash/retry at every boundary has one exact non-scanning outcome.
...
12. Root identity authenticates nonce plus canonical validated identity/version and every reopening lifecycle record persists its locator/descriptor; all private objects enforce the exact empty-extended-ACL policy across inheritance and mutation races.
13. All v2 root, selection, durability, resource, incumbent, authority, adoption, compatibility, privacy, and final hardware/release gates remain in force.
...
| Cleanup cancellation reports success after destructive ambiguity | Reversible precommit tombstone, worker-owned recovery, and exact published/staging state tables. |
...
| Inherited or raced ACL grants access | New-object ACL stripping plus descriptor-based empty-ACL validation before every sensitive side effect. |
...
- Any cancel process repairs, restores, advances, deletes, or takes `operation.lock`; any recovery path violates `operation.lock` → `cancel.lock`; or rename/barrier alone is treated as cleanup commit evidence.
...
- Any private object accepts an extended ACL, inherited ACL survives creation, or ACL mutation can race a sensitive side effect.
