# Build 1 preparation-reservation rebaseline plan v8

Status: revised after the blocked preparation-authority v3 review; **blocked before slice 6B** until a corrected operator-owned authority patch satisfies every Dependency Gate 5 item and an independent review reaches zero Critical, High, and Medium findings.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f` (BYOM v0.2 slice 6 operator state/copy). Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9` (`origin/main` reviewed on 2026-09-11). Blocked authority candidate: `fccb813cfa02fba5bc7aec71ee23bccb4619429b`.

Revision inputs: v7 plan SHA-256 `033d57a69f0c754051625d1c628fcf9a5f8e9f89ae07ba05e76a8a02d1bb841a`; v7 test-spec SHA-256 `2c404fb012831440b89a1004bbc278f827c0c82885f845c8fe114a97e0950d43`; blocking review `reviews/preparation-authority-v3-sol.md`. The v8 plan/test digests are recorded from their committed bytes at T18 rather than embedded recursively.

Authority: issue #1453 remains the execution queue. This document supersedes v7 as planning input. It does not approve, migrate, or reuse abandoned R21-R27 protocol or local development state.
...
Before rendering, Malibu rejects a projection containing duplicate exact `model_key` values. It then sorts every accepted row by this total tuple, in order:

1. `bucket_rank` ascending: `0` current; `1` non-current, locally ready, trusted; `2` recommended or high-demand and `fit: fits`; `3` `runtime_state: needs_preparation`; `4` every remaining visible catalog or blocked row.
2. For bucket 1 only, `provider_completion_payout_usd_per_million_tokens` descending as the exact owner rational/decimal value; null sorts after every non-null value.
3. For bucket 2 only, `recommendation_rank` ascending (`0` when the exact owner-backed `adopt_recommendation` action is available, otherwise `1`), then `demand_rank` ascending, `demand_weight` descending, `supply_deficit_score` descending, and `ready_provider_count` ascending. Every nullable value sorts after every non-null value in its direction.
4. Exact `display_model_id` UTF-8 bytes ascending, then nullable exact `candidate_id` UTF-8 bytes ascending with null after non-null.
5. Exact required `model_key` UTF-8 bytes ascending as the final unique key.

Every component that is not authorized for that row's bucket is normalized to null before comparison, so rate, share, payout, and demand cannot perturb nontrusted or locally motivated preparation rows. All numeric comparisons use the exact parsed wire integers/rationals without binary floating point. String comparisons use unsigned UTF-8 byte order over the validated wire value, with no locale collation, case folding, Unicode normalization, or display-string transformation. The same accepted row set therefore has one order across input/feed permutations, process restarts, and locales.
...
5. **Hard pre-6B authority gate:** @Augustas11 must land one narrowly scoped SPEC-001/SPEC-044 patch, with authority/index updates, that freezes: (a) exact run/cancel option spelling, exclusive complete v1/v2 capability+token pairs, and all partial/dual/conflict/manifest-status negatives; (b) closed event error/warning codes and total error precedence; (c) R002/R003 locally motivated preparation eligibility, catalog-only all-null/no-action/nontrusted/null-money behavior, candidate-associated all-non-null binding, cross-candidate rejection, coordinator no-event and event-backed `not_offered`, `verification_status: verified`, exact source/freshness correlation, the exact **Eligible to earn on qualifying settled requests** conditional verdict, exact operator-owned local/trusted copy, and economics/demand separation; (d) worker-only event sequencing plus the exact bounded cancellation-ack schema, request echo, six outcomes including `busy`, the 2.000-second monotonic cancel-lock deadline, total acquired-lock outcome precedence, nullability, exit/mutation/resource rules, marker-race semantics, and cancel-process read/marker-only authority; (e) the versioned projection/action/accounting contract for `cleanup_published_artifact`, bounded `cleanup_targets` with immutable `event_model_key`, managed-v3 totals, configured-legacy protected totals, logical-byte accounting, truthful APFS copy, and exact budget policy; (f) published and staging continuous `cancel.lock` ownership from final marker check through rename, both parent barriers, durable readback-validated `tombstoned`, reversible post-crash precommit restoration, and operation/cleanup/cancel recovery ownership; (g) authenticated root identity/locator persistence plus already-open unpublished owner-only temp creation, empty-ACL verification before the first sensitive byte, and descriptor revalidation; (h) the exact locale-independent total ranking tuple, null ordering, final unique key, and duplicate rejection; and (i) app-owned refresh generation plus constant-space process transport/backpressure. It must retain `model_catalog_transaction_event.v1`, add no status or control-socket schema, and not overload `cleanup_staging`.
...
Land the single complete operator patch from Gate 5, including `SPEC-001-R003`, exclusive complete version pairs, scoped guidance binding/catalog-only nullability, exhaustive R002/R003 matrix/copy, conditional earning verdict, bounded cancellation acknowledgement/sequencer, continuous cleanup locks, cleanup targets/event correlation/accounting, root/ACL/process transport rules, total ranking, and both cleanup state machines. Record exact versions/commit/owner and prove strict ancestry. Re-review this v8 plan/test plus SPEC diff to zero C/H/M.
...
1. The authority patch makes pre-offer local preparation reachable and conforming through required all-or-none candidate/guidance binding on candidate-associated rows, catalog-only all-null/nontrusted/null-money/no-action behavior, cross-candidate rejection, exact source/freshness correlation including coordinator no-event `not_offered`, `verification_status: verified`, the exact conditional settlement-capable verdict, exact preparation copy, and economics separation across every legal admission/economics combination.
...
13. Row ranking uses the exact canonical-wire tuple, explicit null order, bytewise locale-independent strings, final unique `model_key`, duplicate rejection, and rate-neutral normalization for nontrusted/local rows.
...
| Catalog-only row borrows candidate economics | All-null rows are always nontrusted with null money/demand and no candidate action; cross-candidate joins fail closed. |
...
| Equal rows reorder across locales or input permutations | Exact canonical-wire total tuple, explicit null ordering, bytewise strings, and duplicate `model_key` rejection. |
...
- Authority patch omits any guidance binding, conditional earning verdict, catalog-only nontrusted/null-money rule, version matrix, verified-artifact gate, matrix/copy, cancel acknowledgement/deadline/`busy`/precedence/sequencer, total ranking tuple, invocation/event precedence, cleanup target/accounting/continuous-lock state-machine contract, ACL creation order, governance mapping, or does not predate 6B.
...
- Any row sort uses locale collation, binary floating point, implicit null order, input stability as a tie-break, or accepts duplicate final `model_key` identities.
