# Build 1 preparation-reservation rebaseline test specification v11

Status: revised acceptance specification for `reservation-rebaseline-plan-v11.md` after the blocking preparation-authority v6 review. No implementation test may be credited before the corrected, complete operator-owned authority commit in T18 lands. This revision does not claim the adversarial gate passed.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f`. Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9`. Authority revision reviewed by v6: `a73d6020d34acacc3cd32399d93306b02bddedba`; formal v6 review artifact commit: `ebcf395670623ab9bf69dd1f6cffbb51d55be99c`.

Revision inputs: v10 plan SHA-256 `da3841ac6fe153f0809cf5325102ce49e2e0475c6ffe2150a4d2fa34e1ad01e3`; v10 test-spec SHA-256 `c783b21171292c228d0a0f6048eb16bfb2240a9608fe7491dc19916aaa2f95fd`; blocking review `reviews/preparation-authority-v6-sol.md`, SHA-256 `db9ccb076380e8bb8e1e77fcf307d0be12e9ef295c85b3aba0b47ceab2c63b60`. T18 records SHA-256 digests from the committed v11 bytes.
...
- only an attached live or failure-only worker emits events and allocates `event_sequence`; after immutable projected-action identity validation, a run process acquires distinct private `failure.lock` and becomes attached before semantic freshness, availability, or conflict checks; each such rejected dispatch durably persists/readback-validates one bounded pending terminal `failed_dispatch`, emits exactly one sequence-1 `failed`, durably compacts/readback-validates it into history, and exits 3 without `active.json`, a live attempt, or work side effects;
...
Race 1,000 conflicting invocations while an incumbent holds `operation.lock`. Every challenger validates its immutable projected identity, acquires only `failure.lock`, records a fresh attempt within the shared 256-entry history budget, emits exactly one sequence-1 `operation_conflict` terminal, and exits 3 without delaying or mutating the incumbent. Run enough concurrent challengers to force deterministic compaction and prove the newest accepted record survives its pre-emission readback, total history never exceeds 256, the incumbent remains live, and neither incumbent nor challengers starve or deadlock. Normal history compaction instruments only operation→failure lock order; failure-only paths never acquire or wait for operation/cancel/cleanup/adoption/runtime locks.
...
Race dispatch, failed-dispatch claim/persistence/compaction, projection rewrite, adoption, serving verification, published/staging cleanup, precommit restoration, temp recovery, durable tombstoned phase persistence, and cancel terminal sweep. Assert fixed lock orders: failure-only work takes only `failure.lock`; live preparation/cleanup/recovery takes operation before failure/adoption/cleanup/cancel/socket/runtime locks according to the applicable frozen order; cleanup worker/recovery takes operation→cleanup→cancel and holds the full set throughout recovery mutation and the final-check-through-tombstoned critical section; cancel process takes only cancel lock and never recovery-mutates. Include operation-lock busy/free, successful claim promotion, conflict reporting beside a live incumbent, cancel before the final check, cancel blocked throughout the continuous lock interval, 2.000-second cancel-lock timeout to no-mutation `busy`, phase-write failure, worker crash, cancellation-first versus recovery-first post-crash acquisition, and new-attempt creation. No reverse failure→operation acquisition, deadlock, second live attempt, cross-attempt event/marker, protected deletion, cancel-side phase change, or leaked waiter/process/resource.
...
Separate the pre-attachment boundary from semantic dispatch. Invalid syntax/framing and every one-field-invalid immutable action identity—transaction kind/ID, `event_model_key`, complete root locator/identity, tuple, projection schema/process-launch/sequence/digest—produce no valid event, no attempt, no history mutation, and the exact authority error/exit. For each valid immutable identity, inject stale transaction, unavailable action, and operation conflict independently and in precedence combinations. Assert that the initiating process first generates a fresh attempt, acquires `failure.lock`, becomes the attached failure worker, durably persists and readback-validates one closed pending `failed_dispatch` within the shared 256-record history budget, emits exactly one matching terminal `failed` at sequence 1, durably compacts/readback-validates terminal history, and exits 3. Failure to persist the pending record emits no event and exits 5. Assert no `active.json`, live attempt, marker, network, staging, download, publication, cleanup, adoption, socket, runtime reservation, or model/config/incumbent mutation. The conflict fixture keeps the incumbent live under `operation.lock`; the reporter neither acquires nor waits for that lock and is never counted as a second active work attempt.

For an eligible nonconflicting action, assert the attached process creates one bounded durable pre-work claim under `failure.lock`, releases it before acquiring `operation.lock`, then reacquires `failure.lock` only in operation→failure order, revalidates, and atomically promotes that exact claim into the sole `active.json` attempt. It ceases to be failure-only only at promotion. Race two valid claims at every release/acquire/revalidation boundary: exactly one promotes, and every loser returns under `failure.lock` to the exact `operation_conflict` lifecycle. Inject a claim-holder crash and prove bounded recovery removes or resolves only the exact claim without starving a later valid invocation.

Inject failure-worker crashes before temp creation, at every partial/complete pending-record temp write and sync, before/after rename, both parent barriers, pending readback, immediately before stdout, at every byte boundary during the terminal line, after the complete event, at every terminal-compaction temp/write/sync/rename/parent-barrier/readback boundary, and before exit. Assert `failure.lock` remains held across pending persistence, event emission, and terminal compaction on the uninterrupted path. Recovery handles only recognized failed-dispatch state under `failure.lock`, or under operation→failure when invoked by a normal recovery worker; it finishes/compacts complete history and removes incomplete temps without a later-process replay. Before durable record creation a retry uses a fresh attempt. After durable pending readback the record remains terminal across every crash. An uninterrupted run emits exactly one event and exit 3; a killed run may have no complete delivered event or exit code, but never produces a duplicate recovery event, success, live attempt, or work side effect. Test history at 255 and 256 entries, deterministic eviction/retention of the newly readback-validated terminal, 1,000 concurrent conflict reporters, incumbent progress, bounded memory/files/descriptors, no starvation/deadlock, and no reverse failure→operation lock acquisition. The UI never offers cancellation for `failed_dispatch`; a direct exact cancellation query can only return the existing terminal acknowledgement without marker mutation.
...
| B1-AUTH-V6-H2 | T01, T02, T14, T18 | Immutable-identity-valid stale/unavailable/conflict invocations persist a fresh bounded pending `failed_dispatch`, emit one sequence-1 terminal, compact durably, and exit 3 without live/work side effects; crash and claim-promotion schedules are total. |
