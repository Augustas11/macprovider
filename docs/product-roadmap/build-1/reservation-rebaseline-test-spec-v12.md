# Build 1 preparation-reservation rebaseline test specification v12

Status: acceptance specification for `reservation-rebaseline-plan-v12.md`, aligned to finalized preparation authority v7 after the blocking v6 review. No implementation test may be credited before the corrected, complete operator-owned authority commit in T18 lands. This revision does not claim the adversarial gate passed.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f`. Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9`. Finalized authority v7 revision: `d12e9428df0f0bd26df720da7cd9b9fbbc820ec0`; formal v6 review artifact commit retained as correction input: `ebcf395670623ab9bf69dd1f6cffbb51d55be99c`.

Revision inputs: v11 plan SHA-256 `998e6e2cfd4a41885da785b67a73ab5127cbaaf84ade66e58215a4ededf74cdd`; v11 test-spec SHA-256 `4bb0a37f41105c41d8be322a20631df99aad9996948d7b928ac1ef286b77a40b`; finalized authority v7 commit `d12e9428df0f0bd26df720da7cd9b9fbbc820ec0`; blocking review `reviews/preparation-authority-v6-sol.md`, SHA-256 `db9ccb076380e8bb8e1e77fcf307d0be12e9ef295c85b3aba0b47ceab2c63b60`. T18 records SHA-256 digests from the committed v12 bytes.
...
| Surface | Coverage |
|---|---|
| `ModelPreparationPrivateCodecTests` | Strict records, failed-dispatch history, unique temp/root bootstrap, tuple, marker/deletion phases |
...
- v1-serving CLI advertisements contain exactly `model_catalog_economics_v1` plus `models catalog-economics.v1`; v2-serving advertisements contain exactly `model_catalog_economics_v2` plus `models catalog-economics.v2`; no supported complete pair and every capability-only, token-only, dual, mixed, conflicting, unknown, stale, or manifest/local-status-disagreeing advertisement produces only the static current-model card with no error indicator, retry, read, run, cancel, action, or economics; only a valid exclusive complete pair whose projection request then fails, times out, or returns a malformed envelope produces the exact existing `model catalog unavailable` warning, code `projection_unavailable`, and retry, with no actions/economics;
...
- only an attached live or failure-only worker emits events and allocates `event_sequence`; after immutable projected-action identity validation and fresh attempt creation, a run process acquires the distinct private owner-mode `0600` `failure.lock` and becomes attached before semantic freshness, availability, or serialized durable conflict-view checks; stale/unavailable/already-active under that lock is recorded there, while a passing process releases it and makes exactly one nonblocking `operation.lock` attempt; acquisition failure reacquires only `failure.lock` and revalidates stale/unavailable precedence before recording `operation_conflict`; acquisition success creates a pre-active normal worker that acquires operation→failure, revalidates, and only then creates `active.json`; no durable handoff claim exists, all locks release before event stdout, and post-event compaction reacquires only `failure.lock`;
...
- `cleanup_targets` is bounded, complete for verified managed identities, deterministic, carries a required immutable receipt-bound `event_model_key`, and retains runnable reclaimable objects with nullable current `model_key` when no catalog row remains; any retained row cleanup equals only the target's nested cleanup under the RFC 8785 JSON Canonicalization Scheme (JCS), and each of the two action copies independently binds the enclosing target digest and estimated bytes;
...
- every forward-current owner reference selects SPEC-044 v0.2.6; older versions appear only in explicitly historical records; `SPEC-001-R003`, its pending CONFORMANCE entry, AUTHORITY consumers, SPEC/version indexes and digests, and SPEC-044-R005 evidence/gap mapping agree;
...
Race 1,000 conflicting invocations while an incumbent holds `operation.lock`. Every challenger validates its immutable projected identity, creates a fresh attempt, acquires `failure.lock`, becomes the attached failure worker, passes the initial semantic/conflict-view check, releases `failure.lock`, and makes exactly one nonblocking attempt to acquire `operation.lock`. Each failed acquisition reacquires only `failure.lock`, revalidates stale/unavailable precedence, records the winning bounded terminal within the shared 256-record/262144-byte history limits, releases every lock before emitting exactly one sequence-1 terminal, reacquires only `failure.lock` after the event write for idempotent compaction, and exits 3 without delaying or mutating the incumbent. Hold action freshness and availability fixed throughout the base fixture and require exact `operation_conflict`; separately mutate them during reacquisition and require the higher-precedence stale/unavailable result. Run enough concurrent challengers to force deterministic compaction and prove the newest accepted record survives its pre-emission readback, at most one pending record exists, total history stays within both caps, the incumbent remains live, and neither incumbent nor challengers starve or deadlock. Failure-only paths never acquire or wait for operation/cancel/cleanup/adoption/runtime locks; the failed nonblocking attempt does not acquire `operation.lock` and no path takes failure→operation while holding `failure.lock`.
...
- a matching terminal `failed_dispatch` is never cancellable: the UI exposes no cancel action and a direct cancel invocation may return only `terminal` with the exact attempt ID, without taking `failure.lock`, creating a marker, or changing/compacting history;
...
Race dispatch, pre-work semantic serialization, the single nonblocking operation-lock attempt, failed-dispatch persistence/compaction, projection rewrite, adoption, serving verification, published/staging cleanup, precommit restoration, temp recovery, durable tombstoned phase persistence, and cancel terminal sweep. Assert fixed lock orders: a failure-only worker holds only `failure.lock` when checking or recording and releases it before the nonblocking operation-lock attempt; a pre-active/live preparation or recovery worker takes operation before failure/adoption/cleanup/cancel/socket/runtime locks according to the applicable frozen order; cleanup worker/recovery takes operation→cleanup→cancel and holds the full set throughout recovery mutation and the final-check-through-tombstoned critical section; cancel process takes only cancel lock and never recovery-mutates. Include operation-lock busy/free, pre-active operation→failure revalidation before `active.json`, newly stale/unavailable results under that order, conflict precedence revalidation beside a live incumbent, the unlocked no-handoff crash interval, cancel before the final check, cancel blocked throughout the continuous cleanup-lock interval, 2.000-second cancel-lock timeout to no-mutation `busy`, phase-write failure, worker crash, cancellation-first versus recovery-first post-crash acquisition, and new-attempt creation. No reverse failure→operation acquisition, durable handoff state, deadlock, second live attempt, cross-attempt event/marker, protected deletion, cancel-side phase change, or leaked waiter/process/resource.
...
Separate the pre-attachment boundary from semantic dispatch. Invalid syntax/framing and every one-field-invalid immutable action identity—transaction kind/ID, `event_model_key`, complete root locator/identity, tuple, projection schema/process-launch/sequence/digest—produce no valid event, no attempt, no history mutation, and the exact authority error/exit. For each valid immutable identity, inject stale transaction, unavailable action, and operation conflict independently and in precedence combinations. Assert that the initiating process first generates a fresh attempt, acquires `failure.lock`, and becomes the attached failure worker. Stale/unavailable/already-active results found by the initial serialized check are durably recorded without releasing that lock. A passing initial check releases `failure.lock` and makes exactly one nonblocking `operation.lock` attempt; failure reacquires only `failure.lock`, revalidates stale/unavailable precedence, and records the winning result. Every such rejection persists and readback-validates one closed pending `failed_dispatch` within the shared 256-record/262144-byte history limits, releases every lock before emitting exactly one matching terminal `failed` at sequence 1, reacquires only `failure.lock` after the event write to durably compact/readback-validate terminal history, and exits 3. Failure to persist the pending record emits no event and exits 5. Assert no `active.json`, live attempt, marker, network, staging, download, publication, cleanup, adoption, socket, runtime reservation, or model/config/incumbent mutation. The conflict fixture keeps the incumbent live under `operation.lock`; the reporter's single nonblocking attempt does not acquire or wait for that lock, and the reporter is never counted as a second active work attempt.

For an eligible nonconflicting action, assert the attached failure worker passes the first semantic/conflict-view check under `failure.lock`, releases that lock, and makes exactly one nonblocking `operation.lock` attempt. On success it becomes a pre-active normal worker, acquires `failure.lock` in operation→failure order, and repeats semantic and conflict-visible checks. It creates the sole `active.json` only if those checks still pass; a newly stale or unavailable action instead writes the exact failed-dispatch record under operation→failure and releases every lock before stdout. Race two valid invocations at every check/release/nonblocking-attempt/revalidation boundary: exactly one may create live state; a loser whose operation attempt fails reacquires only `failure.lock`, revalidates precedence, and follows the exact `operation_conflict` record/event/compaction lifecycle. Inject a crash in the unlocked interval and prove there is no durable handoff claim or live attempt to recover and a later valid invocation is not starved.

Inject failure-worker crashes before temp creation, at every partial/complete pending-record temp write and sync, before/after rename, both parent barriers, pending readback, immediately before stdout, at every byte boundary during the terminal line, after the complete event, at every terminal-compaction temp/write/sync/rename/parent-barrier/readback boundary, and before exit. Pending persistence and readback occur under the applicable `failure.lock` or operation→failure order; **every** lock is released before stdout begins. After the event write returns, the uninterrupted failure-only worker reacquires only `failure.lock`, idempotently compacts/readback-validates terminal history, releases it, and exits 3. Block stdout and disconnect the app at every byte boundary to prove no lock is held, a valid dispatch can acquire authority, and neither conflict reporters nor the incumbent starve. Recovery handles only recognized failed-dispatch state under `failure.lock`, or under operation→failure when invoked by a normal/startup recovery worker; it finishes/compacts complete history and removes incomplete temps without a later-process replay. Before durable record publication a retry uses a fresh attempt; a complete recognized temp may finish only its validated write. After durable pending readback the immutable record remains terminal across every crash. An uninterrupted run emits exactly one event and exit 3; a killed run may have no complete delivered event or exit code, but never produces a duplicate recovery event, success, live attempt, or work side effect. Test global history at 255 and 256 records and near the 262144-byte cap, deterministic oldest-terminal eviction/retention of the newly readback-validated terminal, at most one pending record, 1,000 concurrent conflict reporters, incumbent progress, bounded memory/files/descriptors, no starvation/deadlock, and no reverse failure→operation lock acquisition. The UI never offers cancellation for `failed_dispatch`; a direct exact cancellation query can only return the existing terminal acknowledgement without marker mutation.
...
| v1-only | v2 plus exact v2 token only | no supported complete pair; no read/run/cancel; static current-model card only, with no error indicator, retry, action, or economics |
...
| either | both complete generations, or one complete pair plus either member of the other | no read/run/cancel; static current-model card only, with no error indicator, retry, action, or economics |
| either | v1 capability-only, v1 token-only, v2 capability-only, v2 token-only, mixed-generation capability/token, unknown value, or capability-token conflict | no read/run/cancel; static current-model card only, with no error indicator, retry, action, or economics |
| either | manifest/status generation disagreement, or either surface partial, dual, mixed, unknown, conflicting, or stale | no read/run/cancel; static current-model card only, with no error indicator, retry, action, or economics |
| either | neither supported complete pair | no read/run/cancel; static current-model card only, with no error indicator, retry, action, or economics |
| either | one supported exclusive complete pair in both fresh surfaces, followed by request failure or timeout | exactly one negotiated read; static card plus exact `model catalog unavailable` warning, `projection_unavailable` code, and retry; no action/economics/run/cancel |
| either | one supported exclusive complete pair in both fresh surfaces, followed by malformed or unsupported projection envelope | exactly one negotiated read; reject whole envelope; same exact warning/code/retry; no action/economics/run/cancel and no alternate-version retry |

Run each absence or malformed advertisement class as its own fixture rather than combining them: no supported pair; each generation's capability-only and token-only form; partial pair on either source; dual complete pairs; complete pair plus each foreign member; every mixed-generation pairing; every unknown capability and token position; capability-token conflict; stale manifest; stale status; manifest/status generation disagreement; and each one-field disagreement within an otherwise complete pair. For every fixture assert byte-for-byte static-card presentation, no error indicator, no retry affordance, zero read/run/cancel subprocesses, and no action/economics exposure. Separately negotiate each supported pair and inject process-launch failure, nonzero exit, timeout, truncated JSON, invalid UTF-8, wrong schema, unknown required enum, oversized response, and every other malformed-envelope class; each must show exactly the existing `model catalog unavailable` warning with `projection_unavailable` and one retry affordance, while exposing no action/economics/run/cancel. A v1-only decoder receiving v2 and a v2-selected decoder receiving v1 fail the whole envelope without retrying the same ambiguous command. Unknown compatible row/action additions disable only their affected feature when the selected envelope allows that behavior; an unsupported projection envelope triggers whole-projection fallback. No disk inference, hidden selector, environment override, optimistic decode/retry, or warning on negotiation failure occurs.
...
2. The @Augustas11-owned finalized authority commit is exactly `d12e9428df0f0bd26df720da7cd9b9fbbc820ec0`, and its SPEC-001/SPEC-044 v0.2.6 bytes freeze exact catalog-economics read/run/cancel spelling; exclusive complete v1/v2 capability+token pairs; static-card/no-indicator/no-retry/no-call behavior for no supported pair and every partial/dual/mixed/unknown/stale/conflict/manifest-status negative; exact `model catalog unavailable`/`projection_unavailable` warning and retry only after a negotiated request fails, times out, or returns a malformed envelope; the constructive pre-work lifecycle with immutable-action validation, fresh attempt, first `failure.lock` semantic/conflict-view check, exactly one nonblocking operation-lock attempt, conflict-only reacquisition of `failure.lock` with precedence revalidation, pre-active operation→failure revalidation before `active.json`, no durable handoff claim, exact closed pending `failed_dispatch`, release of every lock before sequence-1 event stdout, post-event reacquisition of only `failure.lock`, durable terminal compaction, exit 3, exact crash recovery, shared 256-record/262144-byte limits, and no second live attempt or work side effect; the exact catalog-only `unavailable`/`none`/`local_default:not_offered` null/false/no-action sentinel placed only in exact R008 `Blocked` with `Network catalog`/`Current`/`Ready`/`Needs preparation` one-fault negatives, cross-candidate rejection, and candidate-associated all-non-null verbatim guidance binding; coordinator no-event/event-backed `not_offered` plus exact source-aware local-default/coordinator English meanings and localization/accessibility/source-swap negatives; exact **Eligible to earn on qualifying settled requests** conditional copy and no current-income claim; exact admission-only `local_only` copy with independent readiness/usability evidence; the exact 12-state admission inventory; event codes and total precedence; exhaustive R002/R003 eligibility including `verification_status: verified`; worker-only sequencing and cancel-process marker-only authority; 2.000-second monotonic cancel-lock deadline; six-outcome cancel-ack schema including no-mutation/null-attempt/exit-0 `busy`, echo, acquired-lock predicates/precedence, nullability, and resource bounds; bounded complete cleanup targets with immutable `event_model_key`, RFC 8785 equality between action copies, per-copy enclosing digest/size binding, and separate/equal-two-copy mutation proofs; authenticated root locator/identity and already-open unpublished owner-only temp/empty-ACL-before-sensitive-byte/revalidation rules; the exact SPEC-044-R005 total ranking tuple with null order/tagged length-prefixed canonical identity/duplicate-canonical-identity rejection and nontrusted components 2-6 null; app-owned refresh generation and constant-space transport/backpressure; logical-byte accounting and truthful APFS copy; exact budgets/configuration precedence; and published/staging continuous final-check-through-durable-readback-validated-`tombstoned` cancel-lock/recovery semantics.
...
4. SPEC-001 §6.14b is `SPEC-001-R003` with a pending CONFORMANCE entry; every forward-current reference to SPEC-044 in SPEC-001, SPEC-044 implementation/evidence text, CONFORMANCE, AUTHORITY, generated indexes, current handoffs, contract locks, and these v12 planning artifacts names v0.2.6, while references to v0.2.5 and earlier are explicitly historical changelog/review context; AUTHORITY registers the applicable SPEC-044 consumer; R005 no longer credits the contradictory hide test and instead records the explicit gap and named future visible-local-preparation proof; versions, generated indexes, CONFORMANCE, #1485 ownership/copy, committed file SHA-256 digests, and authority-review lineage all agree exactly.
5. `git merge-base --is-ancestor <authority-commit> <first-6B-commit>` succeeds and commits differ.
6. SHA-256 digests are computed from and archived for the committed v12 plan and test specification; fresh independent review of those exact bytes plus the landed SPEC diff has zero Critical/High/Medium and explicitly dispositions B1-AUTH-V6-H1/H2/M1/M2, B1-AUTH-V5-H1/M1-M3, B1-AUTH-V4-H1/M1-M4, B1-AUTH-V3-H1-H3/M1-M3, B1-AUTH-H1/H2/M1-M13, and B1-AUTH-V2-H1-H4/M1-M5.
...
Run independent code, security, and architecture lanes over the complete authority-through-implementation diff. Each lane explicitly dispositions B1-AUTH-V6-H1/H2/M1/M2, B1-V2-H1 and M1-M5, B1-V3-M1, B1-AUTH-V3-H1-H3 and M1-M3, B1-AUTH-H1/H2 and M1-M13, B1-AUTH-V2-H1-H4 and M1-M5, plus all prior v1 findings. Any Critical/High/Medium blocks; fixes require affected tests and all full-diff lanes again.
...
| B1-AUTH-V6-H1 | T01, T14, T15, T18 | Every negotiation-negative class is a silent static card with zero calls; only a selected pair's failed/timed-out/malformed request shows exact `model catalog unavailable`/`projection_unavailable` and retry. |
| B1-AUTH-V6-H2 | T01, T02, T14, T18 | Immutable-identity-valid invocations exercise the first failure-lock check, single nonblocking operation-lock attempt, conflict/pre-active revalidation, no-handoff crash interval, lock-free stdout, post-event failure-lock-only compaction, and exact bounded failed-dispatch recovery without live/work side effects. |
| B1-AUTH-V6-M1 | T18 | Every forward-current owner, implementation, evidence, index, handoff, contract-lock, and planning reference selects SPEC-044 v0.2.6 with exact committed digests. |
