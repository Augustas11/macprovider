# Build 1 preparation-reservation rebaseline test specification v19

Status: acceptance specification for `reservation-rebaseline-plan-v19.md`, corrected after the independent v18 gate found stale T18 review-target wording and aligned to landed authority merge `c8c97f6625a88fa7c83ae2b5cf4d68078409cc6f`. No secure-storage implementation test may be credited before T18 proves its implementation descends from that landed merge and the current v19 exact bytes pass independent review. The contract-only candidate in PR #1491 must be amended to the approved v19 representation. This revision does not claim the adversarial gate passed.
...
Revision inputs: v18 plan SHA-256 `6c5557f2cc6080c63804cff01c45c7502c1b847728a6808aa846431476fa2802`; v18 test-spec SHA-256 `507671ecbd506596b5a05fe961ad90bf9a6625d8cd6be896ef3879e5d321485a`; independent v18 gate result 0 Critical, 2 High, 0 Medium, requiring T18 to select only the current revision and explicitly disposition `B1-STORAGE-V17-H1`; landed authority evidence from `origin/main` commit `c8c97f6625a88fa7c83ae2b5cf4d68078409cc6f`. T18 records the landed authority merge and SHA-256 digests from the committed v19 bytes.
...
5. SPEC-001 §6.14b is `SPEC-001-R003` with a pending CONFORMANCE entry; every forward-current reference to SPEC-044 in SPEC-001, SPEC-044 implementation/evidence text, CONFORMANCE, AUTHORITY, generated indexes, current handoffs, contract locks, and these v19 planning artifacts names v0.2.8, while references to v0.2.7 and earlier are explicitly historical changelog/review context; AUTHORITY registers the applicable SPEC-044 consumer; R005 no longer credits the contradictory hide test and instead records the explicit gap and named future visible-local-preparation proof; versions, generated indexes, CONFORMANCE, #1485 ownership/copy, committed file SHA-256 digests, and authority-review lineage all agree exactly.
6. `git merge-base --is-ancestor c8c97f6625a88fa7c83ae2b5cf4d68078409cc6f <first-6B-commit>` succeeds and commits differ.
7. SHA-256 digests are computed from and archived for the committed v19 plan and test specification; fresh independent review of those exact v19 bytes plus the landed SPEC diff has zero Critical/High/Medium and explicitly dispositions `B1-STORAGE-V17-H1`, `B1-V18-H1`, `B1-V18-H2`, B1-AUTH-V9-H1, B1-AUTH-V8-M1, B1-AUTH-V7-H1/H2/M1, B1-AUTH-V6-H1/H2/M1/M2, B1-AUTH-V5-H1/M1-M3, B1-AUTH-V4-H1/M1-M4, B1-AUTH-V3-H1-H3/M1-M3, B1-AUTH-H1/H2/M1-M13, B1-AUTH-V2-H1-H4/M1-M5, and the historical v16-to-v17 authority-base/event-model-key correction. The disposition for `B1-STORAGE-V17-H1` must confirm that v17 `model_catalog_unique_temp.v2` bytes and envelopes at `root.identity` are incompatible development state and rejected, while byte-identical v19 `model_catalog_private_state_envelope.v1` temp/target bytes plus the separate raw `model_catalog_root_identity.v1` bootstrap/final bytes are the only approved implementation path.
