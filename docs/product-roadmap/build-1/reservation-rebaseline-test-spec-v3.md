# Build 1 preparation-reservation rebaseline test specification v3

Status: acceptance specification for `reservation-rebaseline-plan-v3.md`. No implementation test may be credited before the complete operator-owned authority commit in T18 lands.
...
Assert inventory enumerates only v3 objects; verified same-volume configured legacy bytes charge protected/managed budget; other-device configured bytes are reported separately; unconfigured legacy is never enumerated/imported/deleted; free-space still reflects all bytes; malformed configured legacy blocks cleanup/accounting but not incumbent serving. At 257 v3 objects inventory/cleanup is unavailable without truncation.
