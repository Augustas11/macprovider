# Build 1 preparation-reservation rebaseline test specification v4

Status: acceptance specification for `reservation-rebaseline-plan-v4.md`. No implementation test may be credited before the complete operator-owned authority commit in T18 lands.
...
Assert inventory enumerates only v3 objects; verified same-volume configured legacy bytes charge protected/managed budget; other-device configured bytes are reported separately; unconfigured legacy is never enumerated/imported/deleted; free-space still reflects all bytes; malformed configured legacy blocks cleanup/accounting but not incumbent serving. Exactly 257 externally seeded valid objects expose only the bounded, confirmed one-object recovery path; malformed/no-reclaimable 257 and more-than-257 remain fail closed without truncation or deletion.
...
6. Fresh independent review of v3 plus the landed SPEC diff has zero Critical/High/Medium.
...
| Object-count admission and bounded overflow recovery | T09, T10 | 255/256/idempotent/257 refusal and cleanup-race schedules |
