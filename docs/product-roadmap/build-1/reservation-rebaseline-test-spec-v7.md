# Build 1 preparation-reservation rebaseline test specification v7

Status: revised acceptance specification for `reservation-rebaseline-plan-v7.md` after the blocking preparation-authority v2 review. No implementation test may be credited before the corrected, complete operator-owned authority commit in T18 lands.

Implementation baseline: `c4401f1791d593d37d68eba91af94219b26d278f`. Planning/review baseline: `f7e584499828b3d16036382848b5caa1a897cdf9`. Rejected authority candidate: `f691de4d12250b97d86d91be7333d29db1ee5f20`.

Revision inputs: v6 plan SHA-256 `a682d6b7caade4540e56f042d6ba9d70731af493a56abaa78c6fff18fede9678`; v6 test-spec SHA-256 `1c56872db5855b91916661bd2672f4a903300762db3ba390ca1227271b73cb89`; blocking review `reviews/preparation-authority-v2-sol.md`. T18 records SHA-256 digests from the committed v7 bytes.
...
- candidate-associated rows carry the all-non-null `candidate_id`/verbatim SPEC-046 `provider_guidance`/closed `guidance_binding` group; signed-catalog-only rows carry all three as null and expose no candidate action/guidance/admission/economics claim; every partial group is invalid;
...
- every private object has an empty extended ACL under exact creation and descriptor revalidation rules;
...
For every private component, create an ancestor default/inherited ACL granting a named second user, then verify new directories/files clear inheritance through their open descriptors before any sensitive write and validate with zero extended ACL entries. Existing objects with access, deny, inherited, or mixed extended ACL entries fail closed without in-place repair. Race ACL insertion after initial validation and immediately before each sensitive read/write, state rename, publication, final↔tombstone rename, restoration, and unlink; descriptor revalidation must abort before the side effect and leave outside sentinels unchanged. Mode-only `0700`/`0600` success is insufficient.
...
Recovery uses only recorded tuple/root/final/tombstone identity and saved root locator/descriptor/version/digest; no scan/guess. Change config/env to another root after each crash and prove original-only recovery. Instrument ownership: only worker/recovery worker holding `operation.lock` may mutate; cancellation holds only `cancel.lock` and may validate/write a marker.

Repeat each `intent` row with absent, exact-attempt, prior-attempt, and malformed cancellation markers. Before durable `tombstoned`, an exact marker preserves final or restores tombstone→final, fully syncs the parent, clears intent, and produces `cancel_requested`/`cancelled`; a nonmatching marker cannot influence the cleanup. For `intent` with final absent/tombstone present and no exact marker, the recovery worker repeats parent barriers and durably advances to `tombstoned`. Only after that durable phase is every exact marker late: recovery completes deletion and the transaction ends `succeeded` or recoverable `cleanup_failed`, never `cancelled`. Inject cancel acquisition and crash before/after rename, each parent barrier, release/reacquire of `cancel.lock`, every `tombstoned` temp/write/sync/rename/parent barrier, and restoration rename/barriers; prove convergence without two finals or two tombstones. Specifically create the formerly impossible R012 race by writing the marker after the objects-parent barrier but before the worker's attempted durable phase update, and assert restoration/cancel rather than deletion.
...
Run `cleanup_staging` against one exact interrupted-attempt staging tree through its separate intent → tombstone → durable `tombstoned` → removed record. Its commit evidence is durable `tombstoned`, using the same release/reacquire marker window and reversible synced rename as published cleanup. Inject cancel, crash, and retry before/after intent, rename, each parent barrier, cancel-lock handoff, every phase-write durability step, every unlink, tombstone removal, `removed`, and record clear. While durable phase is `intent`, exact cancellation preserves or restores staging, retains `staging_cleanup_required`, and ends `cancelled`; after durable `tombstoned`, it completes removal and ends `succeeded` or recoverable `cleanup_failed`, never `cancelled`. Retry uses only the recorded root/attempt/target/phase. Published, legacy, other-attempt, unrecorded, and different-root targets fail before rename/unlink.
...
Race dispatch, projection rewrite, adoption, serving verification, published/staging cleanup, precommit restoration, temp recovery, durable tombstoned phase persistence, and cancel terminal sweep. Assert fixed lock orders: preparation/cleanup/recovery operation lock before adoption lock/socket/runtime reservation; every worker/recovery cancellation-state access uses operation→cancel; cancel process takes only cancel lock and never recovery-mutates. Include operation-lock busy/free, cancel before/after rename and each parent barrier, cancel during lock handoff, phase-write failure, worker crash, recovery start, and new-attempt creation. No deadlock, second worker, cross-attempt event/marker, protected deletion, or cancel-side phase change.
...
Build deterministic ranking fixtures that isolate each tier and ties: current first; then locally ready trusted rows by provider completion payout rate; then recommended or high-demand fitting rows; then preparation-required; then blocked. Within ties permute input/feed order and require the authority's stable tie-break. Missing/stale/fallback/blocked economics carries its localized warning and cannot rank from rate/share/demand. A v2 locally motivated Prepare row remains in Needs preparation and its relative order is invariant while only rate, payout, provider-share, demand rank/weight/provider count/deficit fields change. Demand may influence a trusted row only when signed rate/demand/candidate generated-at and policy versions match.
...
For every combination, assert invalid source/state/boolean/trusted combinations fail closed. Valid non-trusted combinations expose Prepare only when all local prerequisites pass, the primary artifact is exactly `verified`, and the required `candidate_id`, five-field `provider_guidance`, source digest/time/sequence-or-nullable-event, and admission binding are fresh and exact. Test coordinator `not_offered` both with null event bound by the exact response digest and with a real event; no synthesized event is accepted. Malibu renders the verbatim bound earning verdict/state first and never derives it. Local preparation uses exactly **Prepare locally**, **Download and verify this model for local use. This does not offer it to the network or enable earnings.**, and **Download and verify {estimated_size} for local use?** Rates, payouts, share, and demand do not motivate or accompany that action. Valid trusted catalog-priced/settlement-capable rows use only operator-approved trusted copy. Every `declared`, `blocked`, stale, unknown, or mismatched artifact/guidance source disables projection and direct dispatch.

A catalog-only row must have runtime `catalog`, null `action_model_id`, null candidate/guidance/binding group, every action unavailable with null kind/ID/timeout and a nonempty exact reason, and no candidate earning or local-readiness claim. Preserve it through the built CLI and production Malibu view as a Network catalog discovery row; allow trusted catalog-rate display only from its independent valid admission/feed evidence. Reject each partial group. On the first matching validated owner-source snapshot, require all three fields to become non-null together before any Prepare/Evaluate/Adopt/Switch action is possible.
...
2. One @Augustas11-owned SPEC-001/SPEC-044 commit freezes exact catalog-economics read/run/cancel spelling; exclusive complete v1/v2 capability+token pairs and every partial/dual/conflict/manifest-status negative; catalog-only all-null/no-action rows and candidate-associated all-non-null verbatim guidance binding; coordinator no-event/event-backed `not_offered`; event codes and total precedence; exhaustive R002/R003 eligibility including `verification_status: verified`; copy/economics separation; worker-only sequencing and cancel-process marker-only authority; bounded cancel-ack schema, echo, predicates, precedence, nullability, and races; bounded complete cleanup targets with immutable `event_model_key`; authenticated root locator/identity and empty-extended-ACL rules; app-owned refresh generation and constant-space transport/backpressure; logical-byte accounting and truthful APFS copy; exact budgets/configuration precedence; and published/staging durable-`tombstoned` commit/recovery semantics.
...
6. SHA-256 digests are computed from and archived for the committed v7 plan and test specification; fresh independent review of those exact bytes plus the landed SPEC diff has zero Critical/High/Medium and explicitly dispositions B1-AUTH-H1/H2, M1-M13, and B1-AUTH-V2-H1-H4/M1-M5.
...
Run independent code, security, and architecture lanes over the complete authority-through-implementation diff. Each lane explicitly dispositions B1-V2-H1 and M1-M5, B1-V3-M1, B1-AUTH-H1/H2 and M1-M13, B1-AUTH-V2-H1-H4 and M1-M5, plus all prior v1 findings. Any Critical/High/Medium blocks; fixes require affected tests and all full-diff lanes again.
