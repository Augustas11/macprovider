# Reservation search progress — addendum R4
...
authorized. R1/R2/R3 and their independent reviews are retained. R3 review
closed both R2 allocation findings at plan level and reported two representation
findings. This revision replaces R3's executable protocol and addresses
RSP3-ARCH-M1/M2; these are author dispositions pending independent review.
No runtime or test implementation accompanies this revision.

## Scope economy and implementation prerequisite

The lead's newer Swift32 measurement reports healthy-capacity PASS in 15.368
seconds, mixed history above 2,052 PASS in 84.279 seconds, and single-index-decode
PASS. These are lead-supplied results, not tests run by this plan author; complete
selection and combined review still govern that smaller correction. The healthy
failure is no longer a rationale for v4. This structural proposal addresses only
the remaining maximum-shape durable search-progress requirement.

First finish validation of the separately approved operation-local index-receipt
correction, retaining the **unchanged** healthy full-capacity test: 1,023 unresolved small
...
2. Active index `model_catalog_active_index.v4` retains existing binding migration
   ...
   active entries add classSHA256 and leftSHA256 under the closed phase matrix
   below; null is allowed only for an unclassified source member. Allocating
   ...
   to that entry UUID and located by the content-addressed rule below. Distinct UUIDs may have pending metadata concurrently;
...
6. `.reservation-migration/progress.json`, at most the index limit, schema
   ...
   monotonic generation, phase, exact prefix length and sorted acknowledged
   class/left refs for **all** classified source members, including those beyond
   the contiguous prefix. Prefix is derived from that acknowledged map and
   source order, never a second negative authority. A ref for an initially queued
   entry may have no left; later left is an append-only strengthening. Validation
   requires the same class and, once acknowledged, the same left. Acknowledged
   members cannot become unclassified by removing their current index refs.
7. Immutable metadata publication receipt, schema
   `model_catalog_reservation_publication.v1`, at most 131,072 bytes, at
   `.reservation-migration/receipts/<transactionUUID>/<receiptSHA256>.json`:
...
### Closed phase/entry/reference matrix (RSP3-ARCH-M1)

Here null means **not yet acknowledged**, never an expected hash. Pending-receipt
next-entry hashes and fresh allocating hashes are expectations only. The decoder
must enforce this matrix before a receipt is usable; the authority validator
then verifies files, progress acknowledgments and pending bytes. Unknown phases,
field combinations and extra keys are rejected, not coerced into null.

| Index phase / entry state | classSHA256 | leftSHA256 | reservationPublication | Meaning |
|---|---|---|---|---|
| classifying / active source member, genuinely unclassified | null | null | absent | No reuse or negative authority. No acknowledged progress ref or class/left file may exist. |
| classifying / active source member, first-class publication pending | null | null | exact receipt hash | Receipt contains frozen next class and optional next left. Present files must match it. A progress acknowledgment may already name those next refs only after their files are durable. |
| classifying / active source member, acknowledged class | exact durable hash | absent or exact durable hash | absent or exact receipt hash | Progress map must contain the same class; pending may append first left, never replace class/left or mutate primary. |
| finalizing / active source member | exact durable hash | absent or exact durable hash | absent | Every member classified, complete acknowledgment map and frozen install relationship valid. |
| complete / active member | exact durable hash | absent or exact durable hash | absent or exact receipt hash | Ordinary active authority; pending may append first left only, never supply a missing class. |
| complete / allocating member | exact expected class hash | absent | absent | Separate fresh allocating protocol; expected primary/origin/class files have only its ordered absence exceptions. No route/reuse/negative authority. |

Allocating entries are forbidden during classifying/finalizing. Legacy/protected
entries never have left files/refs; allocated entries alone may have them.
Outside the allocating exception, a non-null class/left ref means that exact file
is already durable and missing bytes are protected. A first-class pending receipt
can have expected refs while index class remains null; it is not an acknowledged
class and cannot authorize search absence.

The cutover creates the exact empty progress document (source hash, generation
zero, no acknowledged refs, prefix zero) after the mandatory format fence and
before initial v4 index publication. Post-format/pre-index recovery may complete
these exact initial bytes only while the unchanged original source index remains;
it cannot reset progress for an already published v4 index. Initial v4 points to
that source lineage, with all source class/left refs null and no pending refs.

For first-class publication while classifying, under existing owner/CAS publish
receipt and index pending reference, then exact class/optional left files, then
progress acknowledgment, then final entry refs clearing pending. Progress updates
merge only this source member into the current validated acknowledgment map,
preserve every other member, increase generation and recompute prefix. The
pending receipt freezes per-entry bytes, not the whole shared progress document.
A concurrent other-UUID acknowledgment causes recapture before the short final
CAS, never reclassification. Failure after progress acknowledgment but before
entry completion leaves the pending reference: recovery requires its exact next
refs equal the acknowledged refs and finishes only those bytes. An acknowledged
non-null index class without progress acknowledgment during classification is
an impossible ordered state and protected. Later first-left publication during
classification strengthens progress before final entry completion in the same
order. Finalization requires no pending refs and exact equality of all source
index refs with the complete acknowledgment map.

After complete installation, progress and its install-bound digest are immutable
historical evidence. Fresh allocation and later departures do not rewrite them;
completed active class is independently mandatory and departures use the existing
per-entry pending protocol. Removing class/reference from a member acknowledged
outside the old prefix cannot trigger a primary rescan: during classification
the all-member acknowledgment map detects it, and completed decoding rejects it.
Removing an acknowledgment map ref changes the exact pinned progress evidence;
a retained class/pending file also forbids treating that UUID as unclassified.

### Direct immutable receipt lookup (RSP3-ARCH-M2)

Use the existing canonical lowercase transaction UUID validation and a lowercase
64-hex receipt digest. No receipt locator, path segment or filename comes from a
caller. Encode the closed receipt first, hash those exact bytes, and derive the
single path `receipts/<entry.id>/<reservationPublication>.json`. The receipt body
must repeat exactly the entry transaction UUID, allocated generation when any,
source/migration lineage and original primary/origin observations. Validate path
components, scoped private directory and file evidence before using the bytes;
then compare byte digest and every body identity to the index. Wrong UUID,
digest, generation or source is protected before any mutation. Opening a pending
receipt requires that exact direct file lookup, never directory enumeration.

The transaction-scoped receipt directory is exclusive-create on first metadata
publication and retained forever. Class-only and later first-left receipts have
different contents and therefore distinct names. Multiple unreferenced prepared
receipts may remain in it; none gains authority by discovery. Equal digest with
unequal bytes is protected; an existing exact immutable receipt may be reused
only for that same frozen publication. Recovery reads only the index-named file
and preserves other entries' pending refs. Directory existence alone is not a
pending intent and cannot launch work or supply a negative.

Fresh UUID allocation checks **absence of the scoped receipt directory itself**
at its deterministic path, alongside primary/origin/class/left and existing
sidecars/owner absence. Any existing directory/file/link at that path rejects the
UUID collision under final CAS; do not enumerate it, remove it or adopt retained
receipts. Allocation publication/recovery never creates a receipt directory.
Thus empty allocating recovery also requires that directory absent; its presence
is contradictory sidecar evidence. A valid existing active member may retain the
directory after pending clears. Unreferenced receipts are nonauthoritative
history, not grounds to weaken a missing acknowledged class/left check.
...
3. Under journal lock publish a metadata receipt, then its index intent, class
   if absent, left, the classifying progress acknowledgment when required by the
   phase matrix, and final next index clearing the intent. Check budget and
...
recorded primary/origin evidence, complete only the class/left/progress-acknowledgment/index bytes permitted by
the phase matrix, then
...
the empty initial progress document, then v4 classifying index with identical
frozen membership/source reference and null class/left refs.
...
## Added R4 representation qualification

- Drive the actual closed decoder/authority validator through empty fresh cut,
  post-format/pre-progress/pre-index recovery, partial classification, pending
  first-class publication before/after progress acknowledgment, finalizing and
  completed active/allocating states. Exercise every allowed matrix cell and
  reject all phase-forbidden combinations. Remove acknowledged class/ref within
  and beyond prefix, remove progress acknowledgment, and substitute classifying
  null refs into completed state: protected failure, no class fabrication or
  reread/reclassification of acknowledged primary bodies.
- Restart with simultaneous pending receipts for two transaction UUIDs. Publish
  class-only then later left for one; retain multiple unrelated prepared receipts.
  Count direct opens and assert exactly the index-derived paths, no history
  enumeration. Substitute transaction directory, receipt filename/digest/body,
  source lineage or generation; fail before mutation and preserve unrelated
  pending refs. New-allocation UUID collision with an empty/nonempty/unsafe
  scoped receipt directory fails its absence check without listing contents.
- Crash after class/left file, progress acknowledgment and final index while a
  different UUID acknowledges concurrently. Recovery preserves both map entries,
  exact frozen pending bytes and monotonic prefix; no missing acknowledged file
  is regenerated. Completed dynamic membership still validates immutable install
  progress without modifying that historical document.
...
R3's absence-specific pre-cut allocation recovery is retained with its
explicit fresh v4 allocating/primary/origin/class/active protocol. R4 selects
the closed phase matrix, all-member migration acknowledgments and content-addressed
transaction-scoped receipt lookup. No alternative
