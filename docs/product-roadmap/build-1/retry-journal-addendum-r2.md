# Build 1 pending-offer retry persistence addendum — revision 2
...
different tuple as a side effect of retry. Refuse a different new offer while
an earlier submission is unresolved; preserve its complete signed envelope,
not a marker. The user can explicitly withdraw the coordinator's current offer
and obtain authoritative terminal readback before starting a replacement.
Never silently withdraw to enable retry. Hold the per-candidate operation lock
through read/journal/send/result reconciliation and use journal generation
checks so late responses cannot replace a newer record after restart. Server
...
Additional M1 regression: A accepted with response lost, followed by attempted
different B, rejects B before HTTP and retains byte-identical A through rejection,
timeout, process crash and late A response. Only explicit authoritative terminal
withdrawal/rejection reconciliation allows replacing A; no discarded tuple is
reconstructed from incomplete status. After replacement, delayed callbacks from
A cannot overwrite B because journal generation and lock checks reject them.
