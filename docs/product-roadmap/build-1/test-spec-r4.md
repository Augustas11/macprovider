# Build 1 test specification — revision 4

Pairs with plan-r4.md at base 422fc2f13fc62c1ff8987522f822d9ef856e4a96 and
...
| B1-T11 | Bootstrap composition: start no target session, admission, saved recommendation or HF cache. Actual CLI commands prepare -> refresh -> real cache-only single-target recommendation -> explicit non-economic adoption -> signed offer/status/retry -> settlement. No preseeded positive admission, generated-by-test eligible recommendation, implicit config changes or rate claims. App tests validate the same sequence with a distinct integration fixture label. Before explicit activation incumbent remains unchanged. |
| B1-T12 | Durable bridge: configured/env root precedence; after prepare and owned staging removal restart CLI with empty HF cache, observe exactly one verified ready stable candidate ID and use it for adoption/offer/status. Add both HF/durable copies, corrupt durable bytes, hash/revision conflicts, symlink roots and missing artifacts; preserve identity/dedup and fail closed without discovery mutation. |
| B1-T13 | Harness isolation: reject operator default custody/config/HMAC roots and nonlocal coordinator/gateway targets; assert no existing provider config, Keychain or operator-secret mutation. Fixture keys are never production authority. Missing qualified real-model feeds/references keep physical acceptance blocked. |
| B1-T14 | Prepared recommendation producer: real runner invocation mandatory, complete prefetchedArtifacts map and verifiedExistingArtifact branch; downloader spy fails test on any call. Missing/corrupt prepared target fails before runner. Result benchmark values must originate from runner result, not signed catalog benchGate; deliberate different catalog/runner values expose substitution. CLI --json grammar and full recommendation consumption exercised. Cancel/timeout/memory-pressure at probe start/middle/result restores prior lifecycle, terminates child and never applies config. Physical B1-T10 records actual model execution and measured provenance, while deterministic runners remain fixture-only. |
