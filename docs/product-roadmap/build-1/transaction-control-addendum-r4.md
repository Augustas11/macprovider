# Build 1 transaction control and admission recovery addendum — revision 4
...
Supersedes r3 digest
`6af3df4f35243b3ee34814a23fdad6d8e1254c86c9471f7611268b1d107c106c`.
Revision 4 closes the projection bootstrap interface gap: one prepared immutable
context supports authorized root setup/reservation, then read-only finalization
adds the digest from those same inputs and store identities. Bound controls never
perform setup. All other r3 constraints, including snapshots, generation binding,
configuration privacy and admission gates, remain in force. No runtime work is
authorized by this author artifact.
...
Use one context calculation with two explicit entry paths. Neither path permits
multiple configuration loads. All AppConfig consumers, including the catalog
command's currently separate config loads, action/recovery producers and nested
artifact resolvers, receive the same captured resolved object.

### Projection bootstrap: prepare, authorized setup, finalize

1. `prepareProjection` safely reads config once, freezes verified text/file
   identity, resolves AppConfig with frozen ConfigLoader callbacks, validates the
   environment/home namespace and derives immutable durable/journal paths. Return
   `PreparedModelTransactionContext` with these captured values and safe existing
   ancestor identities. Durable/journal directories may be absent on a clean
   install. Preparation performs no store creation, lock or journal access and
   exposes no digest or action authorization.
2. The existing authorized protocol2 projection path calls
   `prepareProjectionStore(prepared)`. This store-owned boundary safely creates
   missing private durable/journal roots using existing store setup policy,
   verifies existing ancestors against the prepared context, and returns a store
   plus `ModelTransactionProjectionStoreIdentity` containing the safely observed
   device/inode identities of the durable and journal directories. It cannot
   change config/environment/root selection or read config again. Using this
   same store and prepared config, perform the already-authorized action and
   cleanup-generation reservations and collect projection/recovery data.
3. Call read-only `finalizeProjection(context: prepared, storeIdentity: receipt)`.
   It verifies safe current config/ancestor/root metadata against the captured
   context and setup receipt; no config decode/reread, directory creation, lock,
   reservation or journal write occurs. Compute the context digest from the
   original captured config hash/identity, namespace and roots plus the verified
   setup identities. Return an immutable BoundModelTransactionContext with the
   **same** resolved config/root/namespace and nonoptional digest. A directory
   replacement between setup/reservation and finalization fails rather than
   binding old reserved actions to a new store. A concurrent config replacement
   may conservatively fail finalization; it never becomes a second config load.
4. Emit source digest and the actions/recoveries collected from that context only
   after successful finalization. If setup/reservation/finalization fails, discard
   collected available actions and emit unavailable protocol2 controls with null
   source digest (or the existing projection-unavailable result). Partial safe
   reservations remain CLI-owned; their existence alone authorizes no app action.

A clean empty model root can therefore prepare config, create its authorized
private store, reserve an operation and publish a matching digest in one
projection. It does not require a digest whose root identities do not yet exist.
The loader/finalizer never performs hidden store work. Metadata inspection is
read-only and bounded; raw paths/file identities never enter projection output.

### Bound owner/control entry

The bound invocation loader requires the private expected context. It safely
reads and validates consumed config/namespace, resolves paths once, and computes
the expected digest from existing root identities before constructing a store or
opening/locking a journal. Missing/replaced roots fail closed; it never calls
prepareProjectionStore or synthesizes directories to satisfy the expectation.
After successful binding, the frozen config/root passes to the selected owner or
control. Later code never reconsults ProcessInfo/defaultRoot or a config loader.
Root symlink retarget never silently selects another namespace. Owner mutation
may then use its existing authorized lifecycle; the short control path never
creates a missing store as a recovery side effect.
...
- `ModelTransactionContextLoader.prepareProjection(configPath: String?, environment: [String: String], homeDirectory: URL) throws -> PreparedModelTransactionContext`
  performs projection preparation only: one safe read/decode/namespace/path
  derivation, existing ancestor checks, no store access and no digest.
- `PreparedModelTransactionContext` exposes immutable resolved `config: AppConfig`,
  `durableRoot: URL`, `transactionRoot: URL` and captured validated namespace to
  the projection producer. Loader-private captured config/ancestor identity is
  retained for finalization. It has no projectionDigest or mutation authorization.
- `ModelCatalogTransactionStore.prepareProjectionStore(_ context: PreparedModelTransactionContext) throws -> (store: ModelCatalogTransactionStore, identity: ModelTransactionProjectionStoreIdentity)`
  is the explicit store-owned authorized setup entry. The returned identity
  records safely established durable/journal device/inode identities and is
  opaque to callers, with no caller-accessible constructor or supplied digest.
  Finalization independently reads actual metadata and compares this store-owned
  receipt; it never trusts arbitrary caller identity fields. The projection makes
  reservations on the returned store,
  always passing the same prepared config to other producers. Retention/store
  owner implements this boundary; loader owner does not create directories.
- `ModelTransactionContextLoader.finalizeProjection(context: PreparedModelTransactionContext, storeIdentity: ModelTransactionProjectionStoreIdentity) throws -> BoundModelTransactionContext`
  is read-only metadata validation plus digest construction. It retains exactly
  the prepared config/root/namespace and validates the setup receipt; it neither
  reads/decodes config again nor constructs/secures a store or reserves actions.
- `ModelTransactionContextLoader.load(configPath: String?, options: ModelTransactionContextOptions, environment: [String: String], homeDirectory: URL) throws -> BoundModelTransactionContext`
  is the separate bound-invocation path. It requires the expected private context,
  validates single-consumed-config and existing roots, and fails missing roots
  before journal access. It cannot call projection preparation/setup as fallback.
- `BoundModelTransactionContext` exposes immutable `config: AppConfig`,
  `durableRoot: URL`, `transactionRoot: URL`, nonoptional `projectionDigest: String`
  and frozen validated namespace. Construction is loader-owned, through successful
  bound load or successful projection finalization only. It cannot be assembled
  by the app/store from a path or partially initialized context.
- CLI transaction owner adds `ModelCatalogTransactionStore.forContext(_ context: BoundModelTransactionContext)` to select the already verified frozen root for
  bound owner/control lifecycle, without implicit root creation on controls.
  Root owns ModelsSubcommand's prepare/setup/reserve/finalize integration and the
  context loader; transaction owner edits lifecycle call sites after interface
  handoff. Store/retention owner owns prepareProjectionStore and reservation code.
...
| TC-16 | Fresh private config and absent durable/journal roots: prepareProjection reads config once without store access; explicit setup creates roots; reservations use same context; finalizeProjection reads metadata only and emits matching digest/usable prepare action; app-bound dispatch then accepts that digest without a second config read. A bound control with absent roots fails before creation/locking. Assert no hidden config read or setup inside finalizer/loader. |
| TC-17 | Replace config, ancestor, durable root or journal directory between preparation/setup/reservation/finalization. Finalizer fails or retains only original verified inputs; it never rebinds reserved actions to replacement metadata. Inject setup/permission/reservation failure as well: source digest/action availability is withheld on failure; no second config load or alternate environment/root is used. Existing-root projection follows identical single-read flow. |
