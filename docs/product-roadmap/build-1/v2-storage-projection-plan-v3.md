...
Supersedes: `v2-storage-projection-plan-v1.md` and `v2-storage-projection-plan-v2.md`.
...
- No cleanup execution and no row-level cleanup action publication in this slice.

## Dependency graph

1. Existing private store root validation and private inventory decoding.
2. Internal storage-budget snapshot supplied by tests or a future scanner hook.
...
This slice depends only on landed private-record/store types and does not depend on BYOM runtime transfer/adoption work. Later slices may connect the same internal input to the live scanner and Malibu/client v2 path after their own gates.
...
- SPEC-044 storage rule: a valid storage object requires valid managed inventory, non-overflow object count, valid legacy-accounting state, valid budget source, non-null managed-v3 totals, non-null `managed_budget_charge_bytes`, and non-null `available_managed_budget_bytes` satisfying the budget equations. Missing/malformed/wrong-root/stale/overflow input yields unavailable storage, empty target array, and unavailable row cleanup.
...
1. Add internal projection input types, for example:
   - `ModelCatalogEconomicsBuilder.PrivateStorageSnapshot`, containing decoded `ModelPreparationInventoryRecord`, validated budget metadata, and overflow status.
   - `ModelCatalogEconomicsBuilder.PrivateStorageBudget`, containing `globalManagedBudgetBytes`, `managedBudgetSource`, and configured legacy accounting values.
2. Add an internal loader, for example `loadPrivateStorageSnapshot(store:rootLocator:budget:)`, that calls `ModelPreparationPrivateStore.readRecord(kind: .publishedInventory, rootLocator:)`, decodes the payload with `ModelPreparationContracts.decode(ModelPreparationInventoryRecord.self, maxBytes: inventoryMaxBytes)`, verifies `record.root == rootLocator`, and maps nil/missing, store errors, decode errors, wrong-root errors, and invalid budget input to an unavailable projection without leaking raw paths.
3. Keep valid storage unavailable unless an explicit valid budget snapshot is present. Budget rules in this slice:
   - accept only JavaScript-safe nonnegative integers;
   - `global_managed_budget_bytes` is supplied by the budget snapshot and must be in `1...1099511627776`;
   - `managed_budget_source` is `default` or `configured` according to that snapshot;
   - configured legacy remains `not_configured` with both legacy byte fields `0` for this slice;
...
5. For each target, require the private contract decoder and `ModelPreparationContracts.validateCleanupBinding(rowAction: target.cleanup, target: target)` to accept the object. Recheck projection invariants before encoding:
...
- A valid root-validated store inventory with one reclaimable target and valid budget snapshot produces available storage counts/bytes/budget fields and one top-level cleanup target with available cleanup action.
...
- A mixed protected/reclaimable inventory produces published bytes equal to all targets, reclaimable bytes equal to only reclaimable targets, object count equal to all targets, budget charge equal to published bytes, available budget equal to `max(0, global - charge)`, and top-level targets sorted by digest.
- Missing inventory, unreadable store, wrong root, malformed payload, stale/overflow marker, bad digest, bad protected reason, bad event key, mismatched action digest/bytes, unavailable reclaimable action, available protected action, invalid budget, or integer overflow yields fail-closed unavailable storage, empty cleanup target array, and unavailable row cleanup.
...
- Mixed targets: sorted by digest and storage/budget equations exact.
...
- Overflow/count/byte/budget cap: unavailable storage, empty targets, no truncation claim.
