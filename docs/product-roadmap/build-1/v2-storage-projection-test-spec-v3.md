# Build 1 v2 storage projection test spec v3

Base: `origin/main` at `894f21ff`.
Plan: `docs/product-roadmap/build-1/v2-storage-projection-plan-v3.md`.
...
1. `testV2ProjectionPublishesCleanupTargetFromRootValidatedPrivateStore`
   - Bootstrap a `ModelPreparationPrivateStore`, write a valid `published_inventory` record with one reclaimable cleanup target, load it through the internal store loader using the matching `rootLocator`, and provide a valid budget snapshot.
   - Assert storage published/reclaimable/object-count, budget charge, available budget, global budget, source, overflow flag, and legacy-accounting fields are non-null/exact.
...
2. `testV2ProjectionPublishesProtectedTargetWithoutCleanupAuthority`
   - Use the store loader with one protected target, `protected_reason`, unavailable cleanup action, and a valid budget snapshot.
   - Assert published bytes/count include it, reclaimable bytes are zero, top-level `cleanup_targets` contains it, row cleanup is unavailable, protected reason is present, and no action artifact digest is exposed.

3. `testV2ProjectionPublishesMixedInventorySortedByDigestAndBudget`
   ...
   - Assert published bytes are the sum of all targets, reclaimable bytes include only reclaimable entries, budget charge equals published bytes, and available budget is clamped to zero when charge exceeds global budget.
...
5. `testV2ProjectionRejectsBadCleanupBindingFailClosed`
   - Mutate action `estimated_bytes`, action `artifact_identity_digest`, `protected_reason`, `event_model_key`, or reclaimable action availability after constructing otherwise valid payload bytes.
   - Assert `storage == unavailable`, empty cleanup target array, and row cleanup unavailable.

6. `testV2ProjectionRejectsInvalidBudgetAndIntegerOverflow`
   - Use valid store inventory with invalid budget source/value, over JavaScript-safe estimated bytes, or aggregate overflow.
   - Assert unavailable storage, empty target array, and row cleanup unavailable.

7. `testV2ProjectionNeverAttachesRowCleanupByCurrentCatalogFields`
   ...
   - Assert top-level target list remains available when inventory/budget are valid, but every row keeps cleanup unavailable.

8. Existing public compatibility tests continue to prove public v1 shape and local status v1 capability only:
...
- Passing Swift tests prove internal projection, private-store/root validation at the loader boundary, and public v1 compatibility only.
