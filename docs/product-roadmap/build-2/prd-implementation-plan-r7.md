...
**Plan revision:** R7
**Status:** draft; implementation is prohibited until an independent GPT-5.6 Sol adversarial review reports zero Critical, High, and Medium findings for these exact bytes and the paired R7 test specification
**Paired test specification:** `test-spec-r7.md`
...
**Predecessor:** `prd-implementation-plan-r6.md`
...
R7 retains the R6 closed discriminated union so a first create has a representable predecessor, and adds the executable replay authority below. Every record has exactly this envelope:
...
Root-to-leaf `openat`/`O_NOFOLLOW` traversal, owner/mode/sticky checks, retained directory descriptors, link-count checks, and edge recapture remain mandatory for the chosen state root. Under its retained descriptor, `private.lock` is shared by both authorities and uses the same retained-descriptor whole-file `fcntl` write-lock contract. The exact lock order is `coordination-directory descriptors -> authority.lock -> state-root descriptors -> private.lock -> authority descriptors/Keychain`; acquire and verify both locks before reading any Build 2 Keychain selector or authority object. Code holding them may do file and Keychain I/O but no HTTP, SQLite, callback, or browser work. The combined nonblocking acquisition, including contention polling, has a five-second admission deadline. After acquisition, both locks remain held until every started file/Keychain mutation synchronously completes and its authoritative result is read back; five seconds becomes an alert and blocks new admission but never releases either lock around a possibly late noncancellable Security-framework operation.
...
| Normal mutation operations/audits | 4,096 fixed 4 KiB terminal operation rows/16 MiB and 1,024 fixed 2 KiB committed audit rows/2 MiB per account, bounded by 16,384 operation rows/64 MiB and 8,192 audit rows/16 MiB globally; success rows reference exact response extents and inline error bodies fit the same operation extent |
...
| Revocable live targets | 8 active bundle signers, 1,024 stored bundle revisions, 4,096 live profile IDs, and 2,048 unique stored-bundle pin fingerprints; exactly 7,176 independently revocable targets |
...
The global revocation authority has 1,024 fixed 1 KiB normal tombstone slots and one fixed 1 KiB emergency tombstone slot for every reachable live target: 8 signer, 1,024 bundle, 4,096 profile, and 2,048 pin slots, exactly 7,176 emergency slots and 8,200 tombstone slots total. Every signer/bundle/pin slot is preallocated with its fixed audit and operation/result extent; each profile slot is paired with the C3 fixed profile revoke set. Slot identity is physical only; logical ordering is always the committed global generation. All paths share the 64 committed tombstones per rolling hour limit. A revocation uses the next free normal slot while one exists. Its transaction commits the normal tombstone and atomically zeroes/unbinds the target's still-unsealed emergency unit because the target is no longer live and the normal tombstone/ordinary operation evidence is now retained authority. That physical emergency unit may then bind a future live target. At normal saturation, a valid rate-admitted revocation seals the target's bound emergency unit with the next generation and exact terminal result and requires no row, index-key, or BLOB growth. Invalid new operations never bind the fixed unit. A sealed emergency unit is never reused until its 38-day/reference horizon drains and checkpointed pruning retires it.

The reachability invariants are `active_signers <= 8`, `stored_bundle_revisions <= 1024`, `accounts_with_profile_authority <= 256`, `live_profile_ids <= 16 * accounts_with_profile_authority <= 4096`, `retained_profile_ids <= 32 * accounts_with_profile_authority <= 8192`, `profile_revisions <= 8192`, `unique_pin_fingerprints_referenced_by_stored_bundles <= 2048`, and `allocated_unsealed_emergency_slots = active_signers + stored_bundle_revisions + live_profile_ids + unique_stored_pin_fingerprints <= 7176`. Signer activation, bundle insertion, transition of a profile ID to live, and first stored-bundle reference to a unique pin allocate their typed emergency slot in the publishing transaction. Profile replacement retargets the same slot. A normal-slot revocation may release only that now-nonlive target's unsealed slot after the tombstone is durable in the same transaction. Pin slots are reference-counted over unrevoked stored bundles and active/reference-retained profiles. No target is reachable without exactly one unsealed typed slot; no slot binds two targets.

The joint saturation history is normative and executable. Starting empty, activate exactly 8 signers, upload 1,024 bundle revisions containing exactly 2,048 unique pins, and retain them unrevoked. Across exactly 256 accounts, issue 1,024 invitations and create 4 temporary one-revision profiles/account, consuming 1,024 profile emergency slots. At this boundary, enumerate all 7,176 typed emergency slots: 4,104 have one exact live owner and 3,072 profile slots are free. Advance the fake clock at the 64/hour limit for 16 hours and revoke each temporary profile into one distinct normal tombstone; each transaction seals its preallocated response/operation/audit evidence, releases only its unsealed profile emergency slot, leaves all 1,024 profile revisions retained, and records the resulting complete owner/free map. Normal tombstones are now exactly full and 4,096 profile slots are free. Before the 24-hour bundle/invitation activation boundary, issue 4,096 new invitations and create 16 final one-revision profiles/account using those free profile slots. The state now simultaneously contains 1,024 normal historical tombstones, 5,120 retained profile revisions, exactly 10,240 bound response extents (1,024 temporary successes, 1,024 sealed temporary revokes, 4,096 final successes, and 4,096 final revoke reserves), 6,144 zero-filled free response extents, and the maximum 7,176 unrevoked live targets with a slot bijection. The owner map names exactly one target for every emergency slot and no free slot. All 16,384 physical response extents remain preallocated at their fixed length; this reachable history does not pretend that independent response/revision maxima can coexist.

From that exact state, revoke the 4,096 live profiles, then the 2,048 pins, then the 1,024 bundle revisions, then the 8 signers over 113 additional rolling-hour windows; every target is unrevoked at its own operation's linearization point and seals its bound emergency slot at consecutive generations. Ordinary bundle expiry after activation does not silently revoke an active profile, and the full 129-hour construction remains within the 30-day pin-validity and 38-day evidence-retention horizons. Leaf-to-root order prevents a parent revocation from making a child test vacuous. With normal operation capacity available, nonexistent, pruned, already-revoked, wrong-kind, or mismatched-digest targets seal a typed no-commit error in a normal row and create no tombstone. At normal operation saturation they return transient unsealed unavailable and still cannot consume fixed target capacity. Exact replay of a previously sealed result returns it and creates no tombstone. Historical revoked targets never reacquire an emergency slot merely because their operation is replayed. Activation after normal saturation requires a free unsealed emergency slot and every other logical/physical reserve.

Every 1,024 committed tombstones writes the exact C3B Ed25519 checkpoint. At most 16 checkpoint rows and 1 MiB checkpoint/audit canonical bytes are retained; 8,200 tombstones require at most 9 checkpoints including the final partial/pruning boundary. Pruning deletes only a contiguous prefix covered by a verified checkpoint after every reference/horizon clears; the singleton current generation/root never changes. A missing/bad slot disables related activation/private admission and alerts but never converts an already reachable target's revoke into a capacity error. Startup refuses enabled mode unless live targets and unsealed slots form the exact bijection. Tombstones/checkpoints remain at least 38 days and longer while referenced.

The accepted-event charge table is closed: signer activation allocates an emergency slot; bundle upload adds one bounded bundle record and allocates bundle/pin slots; invitation issue adds <=2 KiB and consume updates; successful create/replace reserves one normal operation/audit and one exact success-response extent, while first create also allocates fixed revoke operation/audit/response extents plus a profile emergency slot; an exactly matching live-profile revoke may use only its fixed rows/response extent and chooses a normal tombstone if available, otherwise its bound emergency tombstone; signer/bundle/pin revocation likewise chooses normal then bound emergency. Reads/preflight add no rows. Authentication/body-validation rejection and normal operation/response capacity failure add no retained row and cannot seal no-commit; semantic/CAS/rate/capacity rejection after normal operation reservation seals an inline <=2,048-byte error. Invalid duplicate/nonexistent/historical/already-revoked targets add no tombstone and never bind fixed capacity. A new retained event class reopens the plan gate.
...
1. **Governance and vectors:** update SPEC-041, SPEC-006, SPEC-040, AUTHORITY, CONFORMANCE, and shared fixtures with C1-C10/C3A-C3B/C6A, exact response-capacity oracle, nullability, local-record/root/Keychain-policy schemas, external heads, separate wire/reducer errors, revocation evidence, mixed-version rules, and non-claims.
2. **Sizing and exact error-inventory checkpoint only:** add exact proposed coordinator/gateway DDL/indexes, all fixed response/revoke/tombstone extents, deterministic SQLite capacity measurement harness, stable callsite labels, and complete `error-emission-inventory-v2.json`. Run the maximum response oracle and exact joint tombstone/live-target history through the proposed DDL, then produce digest-bound physical and inventory reports. No route/runtime storage, error adapter/reducer/emitter, or Malibu error implementation is allowed in this slice. Submit every row, oracle, DDL, and report digest to a fresh independent zero-C/H/M gate.
