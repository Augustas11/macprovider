...
**Test revision:** R3
**Paired plan:** `prd-implementation-plan-r3.md`
...
Generate fixtures directly from the complete R3 error table and fail if any Build 2 coordinator, gateway, Go-client, or Malibu emitted code is missing, extra, or has a different HTTP status, phase, retryable flag, or before/after-fence action. For every row test both sides of the fence. Unknown code, missing/duplicate/unknown field, status/code mismatch, unknown phase/action, and malformed body become `relay_blind_unknown`. Pairwise precedence tests prove durable terminal/replay, local fence, evidence authentication, lifecycle, capacity, and transport ordering. HTTP 429/499/502/503 and `retryable: true` alone never authorize envelope reuse.
...
Test exact/over boundaries and frozen row-byte accounting for every C9 physical/normal/emergency partition. This includes 8 bundle signers, 4,096 bundle revisions/64 MiB, 32 profiles, 64 revisions, 16 pins/models, 2,048 invitation rows/4 MiB with 128 live, operation normal 3,968/3.75 MiB and revoke emergency 128/256 KiB within total 4,096/4 MiB, audit normal 7,936/7.75 MiB and emergency 256/256 KiB within total 8,192/8 MiB, recovery normal 1,000 plus 24 emergency within total 1,024, wallet status-authority session/account caps, confirmed-profile, journal, body/page/rate, safe-integer, and lifetime bounds.

For every partition run `normal_limit-1`, normal limit, each emergency slot, and emergency exhaustion after restart without exceeding the advertised physical total. Normal work never consumes emergency rows/bytes. At full normal operation/audit capacity, revoke updates the existing profile and commits its operation/audit in the named emergency partitions. Repeated revoke reuses state. At actual emergency exhaustion it fails closed, leaves local profile disabled, alerts, and creates no partial state. Recovery/status update existing rows; impossible missing-row repair uses at most its 24-row partition then quarantines. Pruning removes only indexed oldest eligible rows after 8-day/30-day/38-day and reference conditions; active/nonterminal/referenced/unsealed/tombstone evidence survives restart and clock rollback. A bundle referenced by any unexpired pin or reservation recovery row cannot be pruned.

Reach 128 live reservations on one profile while the account remains below 512, then reject the 129th for that profile. Independently reach exactly 512 across four profiles with 128 each, then reject the next account reservation. Neither test disables the other limit. Invalid capacity, retention, replay, or convergence inequalities fail enabled startup.
...
At every replace/revoke cut prove `mutation_pending` commits before the server call, disables use, and contains the exact target evidence. Crash before call, during response, after server success, and before stable local commit converges only from an exact authenticated server target; mismatch remains disabled. Revoke is representable in place at local/full normal coordinator capacity. Explicit signer/bundle/pin/profile revocation disables use after restart; ordinary activation-artifact expiry does not. Scan bytes to prove no credentials, private keys, prompt/ciphertext, or raw provider binding are stored.
...
At 1,000 nonterminal rows and batch 100, run the real scheduler with 20-worker instrumentation, actual 2-second timeouts, bounded SQLite acquisition/transaction, five network waves, 15-second pass deadline, and <=15-second completion-to-next-pass delay. Slow, hanging, connection/TLS, malformed-proof, busy-DB, cancelled, and mixed successful rows cannot block later rows; claims are released/aged after deadline. With no new arrivals every row has a call initiated within 300 seconds and the measured components satisfy the 285-second conservative formula. A continuous stream of new rows cannot starve the oldest cursor. Fake-clock tests separately cover retention but cannot substitute for scheduler evidence.
...
On macOS exercise every C8 descriptor step for both journal and profile authority: relative override rejection; root-to-leaf `openat`/`O_NOFOLLOW` walk; allowed root/sticky and private 0700 ownership; retained-directory edge recapture; symlink/hardlink/FIFO/device/socket/wrong-owner/wrong-mode rejection; `st_nlink == 1`; and exact device/inode equality between descriptor and `fstatat` before load, append, fsync, compaction, and close. Race every ancestor/final component replacement at every recapture point. Unsupported platforms fail before reservation/profile confirmation.
...
Test 4,096/16 MiB/4 KiB exact boundaries, 8-day pruning, compaction anchoring, and all durability failures. At capacity new private work stops before network while independently supplied status and in-place profile revoke work.
