...
**Test revision:** R5
**Paired plan:** `prd-implementation-plan-r5.md`
...
Freeze positive vectors for one pin, 16 pins, 16 models, boundary timestamps, null status digest, different profile/bundle/invitation IDs, the three confirmed-profile payload variants, both expected-authority tags, the revocation empty root/first entry/all four target kinds, immutable tail/pointer/external Keychain head, and browser head-ledger CAS. Assert exact domains, tags, `u16str`, big-endian integers, decoded fixed-size fields, ordering, and final digests/MACs in Go/Swift/JavaScript. The three C1 provider-binding vectors produce the listed ASCII-input digests; decoded-byte hashing differs and fails. One-byte mutation and alternate serialization fail wherever bytes are authoritative. Reject non-ASCII, overlength, normalization variants, duplicate/unsorted arrays, and counts 0/17.
...
Generate server fixtures from `wire-errors-v2` and client fixtures from `client-reducer-v2`. Servers reject/never emit `action`; clients derive the fence only from authenticated local state. For every primary inventory and R5 emission-map row assert the exact repository/path/function/callsite/raw-code/route key, complete replacement tuple, and both actions. The source scanner must select every reachable `writeError`, Anthropic, wallet helper, coordinator constructor, and client transport adapter exactly once and fail on a missing, stale, duplicate, message-regex, or unreviewed default mapping. Explicitly execute quota, duplicate ID, concurrency, settlement arm, internal audit/store, wallet auth/signature/lifecycle/cap/replay/rate, AbortError, DNS/TLS/timeout/EOF, and malformed/non-JSON/empty response cases before and after `send_fenced`. Unknown/missing/duplicate/action-injected/status-mismatched bodies become unknown. Pairwise precedence proves terminal/replay, fence, C6A evidence, lifecycle, capacity, and transport ordering; HTTP/retryable alone never authorizes envelope reuse.
...
Test exact/over boundaries and C9 accounting for: 8 bundle signers; 4,096 bundle revisions/64 MiB; 32 profiles*64 revisions; 2,048 successful operation/audit rows; invitations; fixed revoke/quarantine slots; 1,000 joins; wallet status; 8*2 browser heads; local-store peaks; and the global 65,536-row/64 MiB tombstone authority with 64 checkpoints and four 64-row kind reserves. Boundary construction uses supported APIs and never disables a competing limit.

Through supported APIs create 32 profiles and 63 replacements for each, reaching exactly 2,048 rows without disabling another cap. Prove rejected requests charge zero and replay reuses its row. For coordinator and gateway, enforce 4096-byte pages, WAL, FULL synchronous, foreign keys, checkpoint/busy settings, exact 1 GiB/512 MiB feature live-page ceilings, 16 MiB/8 MiB transaction reserves, <34 MiB/<17 MiB WAL ceilings, and 1,126 MiB/561 MiB free-disk reserves. Reach limit-1/exact/over; inspect `dbstat`, page/freelist/max-page counts, every index, WAL frames, FULL checkpoint, restart, and competing nonfeature growth. Any observation beyond the frozen threshold fails and reopens the gate. Runtime storage work is prohibited until the exact DDL/measurement digest passes a fresh independent review.
...
Construct 65,280 normal tombstones at the 64/hour rolling rate with the fake clock, then commit one signer, bundle, profile, and pin tombstone into their distinct emergency reserves. Verify other-kind reserve isolation, exact/over row/byte/page/checkpoint limits, restart continuity, reference-delayed retention, a prunable checkpointed prefix, first suffix predecessor, and unchanged singleton generation/root. Exhaust one kind reserve and prove related issuance/admission fails closed while the other three emergency kinds remain usable. No prune crosses 38 days, a client watermark, signer overlap, or reservation/recovery reference.
...
For create, replace, and revoke, prove the complete v3 discriminated pending record and external C8 head commit before the call and `sent_or_unknown` commits immediately before it. For first create cover every cut: candidate/pending write and readback, external-head publication/readback, disposition write/head, request, server commit, response loss, invitation consumption, local stable append/head, restart, and two concurrent creators. Create starts from exact tagged absent/genesis authority; cancellation from `not_sent` appends an anchored absent marker, while every later cut is reconciliation-only. Delete responses/caches and prove success recovery uses only the persisted target. Server GET never bootstraps. Replace/revoke use present expected authority; revoke advances from the C3B empty/current root to signed tombstone evidence before stable revoke. Mutate every envelope/payload/tag/target field. Stable active, stable revoked, pending, and absent never compare equal. Verify exact charges at every terminal path.
...
Test lock-first ordering, the five-second total ceiling, immutable `O_EXCL` generation/tail/pointer objects, write/fsync/reopen, pre/post-rename cache identity, parent fsync, and stable lock identity. Exercise the exact non-synchronizable Keychain service/account selectors, distinct authority keys, external-head frame/MAC, atomic replace/readback, missing/duplicate/wrong-ACL/synchronizable items, and timeout. Inject every file and Keychain failure. Crash before/after immutable tail fsync, Keychain replace/readback, and `CURRENT` cache update. Before head replacement, the candidate is deleted; after it, the new object is mandatory. Journal/profile bytes and Keychain metadata contain none of the prohibited data.
...
Test 4,096/16 MiB active/40 MiB physical/4 KiB boundaries and every C8 v2 frame. Cover retained/omitted heads, immutable tail sequence/size/digest/head, pointer generation, external generation, and every append/compaction crash. Restore a tail prefix, old `CURRENT`, old pointer, retained old complete generation, mismatched head/pointer, missing new object, and mixed generations; each disagrees with Keychain and quarantines. Same-user rollback or authorized replacement of a Keychain item is labeled outside scope and never restores send ownership. For the browser, fault prepared IndexedDB commit, server CAS, lost CAS response, GET reconciliation, local committed mark, compaction, two tabs, and local/server rollback combinations. Server head cannot reconstruct cleared local trust. At capacity new work stops before network while status/revocation remain.
...
For both stores, test nonextractable keys, exact v3/v2 records, prepared/committed generations, authenticated head-ledger create/GET/CAS, readback, reload, and cross-account/origin/authority-ID isolation. Double-click/two tabs race profile mutation and fence creation; one server head wins and one live page owner sends. Crash/fault every IndexedDB write, CAS, lost response, GET, local commit, send cut, quota/abort/blocked upgrade, key loss, corrupt/predecessor/head rollback, Web Lock failure, clearing storage, and all caps. Server GET/list alone never bootstraps content. Clear local storage, list the stranded opaque authority, revoke it with exact dual-head CAS, retain it through references/eight days, and create no ninth live authority before eligible drain. No lease takeover sends ciphertext. Scan all local/server head bytes for prohibited material. Plaintext history/settings remain separate and disclosed.
