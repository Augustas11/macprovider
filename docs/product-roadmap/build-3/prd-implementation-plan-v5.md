...
If approved, this revision authorizes two bounded pre-product implementation slices only: Collector Slice H0 and Feasibility Tooling Slice B0I. Neither slice may be shipped, production-wired, enabled by configuration, or used as product authority. Collector Slice H0 must pass Gate H1 before its executable identity can enter Gate A0. Gate A0 then approves the exact immutable profile, the approved collector identity, complete numeric calibration and reference-successor protocols, independent decision unit, held-out partition, and positive-control power rules before the collector reads or copies any real post-sampler probability value from normal inference, or any such value is persisted, displayed, logged, summarized, or made available to a protocol author or reviewer. Feasibility Tooling Slice B0I starts only after Gate B0D approves its closed design package and must pass Gate B0E before a benchmark can count. Product Slices 1–7 remain blocked until Gate B approves the resulting evidence and feasibility bundle. Positive availability remains blocked until a built release and its qualification manifest pass Gate C. Every gate requires an independent GPT-5.6 Sol review with zero Critical, High, and Medium findings on exact committed digests.
...
8. `compute_observation_evidence_bundle.v1` binds the antecedent digests, collector manifest, reference-series roots and epochs, golden/calibration results, threshold, policy, revocation list, preimplementation-contract digest, Gate-B0E tooling digest, and bound capacity/recovery results. Gate B reviews the bundle and revised implementation plan. The bundle contains no production release digest.
...
collector design -> Gate H0
  ...
  -> blinded reference/development/held-out/refresh acquisitions + calibration result
  -> closed schemas + mutation inventory + durability/storage/workload/tooling design
  ...
  -> exact capacity, crash, restore, continuation and witness benchmarks
  -> evidence bundle
  -> Gate B (zero C/H/M)
...
A numeric leak, collector/module mismatch, failed hook, missing independent unit, failed precommitted held-out, refresh-stability or power rule, missing independent reference, unsupported witness/storage profile, infeasible hot path, incomplete mutation inventory, or failed gate blocks downstream work. Evidence is never recollected under altered rules without a new protocol digest and Gate A0.

## Normative contracts required before implementation

### Qualification collector implementation and equivalence

The inspected `1.8.123` CLI is historical baseline evidence and cannot collect governed values. After this plan passes, Gate H0 reviews `compute_hook_design.v1`; only then may Collector Slice H0 implement the two named Swift targets and tests. The collector target is absent from Malibu.app, `macprovider-cli`, installer, release archive, updater, provider runtime scripts, and production command registration. It accepts a signed custody job on a local qualification machine and writes one bounded encrypted result to a custodian-selected destination; its result envelope binds custody job, collector manifest, profile, acquisition sequence and content digest and is bounded by the Gate-H0 schema; it has no coordinator URL, provider credential, settlement client, reward client, or production feature flag.
...
After Gate A0 and calibration, the documentation/schema package commits closed canonical schemas and field bounds; the exhaustive symbol/SQL mutation inventory; exact SQLite/WAL/APFS, coordinator and witness topology; deterministic seeded workload and fault schedule; the protocol-module interface; benchmark formulas; and the segment/checkpoint/archive state machine. Gate B0D reviews those exact digests plus the bounded write scope for Feasibility Tooling Slice B0I. No executable benchmark claim exists at B0D.

After B0D passes, Slice B0I may implement only `phase4-coordinator/internal/billinglineage/`, its `feasibility` adapters, `cmd/lineage-feasibility`, tests and fixtures. The core package implements the exact canonical encoding, predecessor checks, local/witness PREPARE/COMMIT sequence, segment sealing and checkpoint verification later reused unchanged by production. Feasibility adapters operate only on generated synthetic money mutations and isolated disposable paths. The command requires an explicit `qualification` build tag plus a compile-time marker absent from production builds. Import-graph, binary-symbol, command-registry, container, package, installer, workflow and deploy-manifest tests prove coordinator/provider/Malibu/release artifacts cannot contain, invoke, enable or route to it. Gate B0E performs independent full-slice code/security/architecture review, verifies those exclusions, and freezes the source, dependency, compiler and executable digests. Production must reuse the exact core package commit and contract; a change reopens B0D/B0E and invalidates measurements.
...
The app/supervisor acquires the lifecycle `flock` before any candidate child birth and retains ownership across start, readiness, promotion to serving, drain and termination. The owner record binds boot-session UUID, random lease nonce, supervisor PID plus process-start identity, child PID/process-start identity, process-group ID, intended mode/profile and state. The lock FD is close-on-exec; the child receives only a bounded nonce-bound control channel and a parent-liveness pipe. Closing that pipe or loss of authenticated heartbeats makes the child synchronously stop admission, cancel/quiesce MLX, and exit. A serving incumbent blocks a startup-prewarm child before `spawn`; transition requires the incumbent to drain, synchronize, exit, and release the lifecycle lease, after which one contender acquires and revalidates. The supervisor uses a new process group, bounded TERM/KILL escalation, `waitpid`, and verified PID-start identity. Recovery acquires the lock, validates the owner record against boot/PID-start identity, kills and reaps a verified orphan group before clearing it, and fails closed on PID reuse, ambiguous identity, two parents, or an unkillable child. No candidate becomes routable from readiness alone.
...
One Postgres-owned `reward_source_revision` spans ledger rows, all seven disposition facts and lifecycles, provider/wallet/cohort/duplicate/epoch membership, provider-day and wallet-day cap counters, withheld amounts, `cap_replay_pending`, wallet binding/rotation, production-accrual links, reward activity, and mirror completeness/head. Every inventoried writer calls a `SECURITY DEFINER` database mutation entry point inside its existing transaction. Direct DML privileges on governed tables and the revision sequence are revoked from runtime roles. The entry point allocates one monotonic revision, binds it to the current transaction ID plus an unforgeable server-side token unavailable to callers, writes every affected fact/activity row with that revision, and inserts the revision record with `source_transaction_at=transaction_timestamp()` as the final statement before the atomic transaction commit. Triggers validate that transaction/token/revision tuple and reject direct INSERT/UPDATE/DELETE, including legacy writer paths; a caller cannot authorize itself with `set_config`. Sequence gaps are allowed; revision reuse or a fact visible without its committed revision is not.

The reward owner begins one `REPEATABLE READ READ ONLY` transaction, reads the greatest visible revision record and its `source_transaction_at` plus the read transaction time first, and derives balances, simultaneous dispositions, memberships, caps, wallet, production accrual, completeness, aggregates, and the first activity page entirely from that snapshot. `generated_at` is allocated only after all queries succeed and is distinct from `source_committed_at`. Activity is append-only with `created_revision`; state changes append new facts rather than erasing prior activity. A cursor MAC-binds provider, source revision, stable `(effective_at,event_id)` boundary, filter, page size, and a 24-hour expiry; immutable/versioned source rows needed by a cursor are retained for at least 48 hours. Later pages use revision-as-of predicates over immutable/versioned rows and return the same source revision even after new writes; if retained as-of data is unavailable, the page fails `snapshot_expired` and never silently opens a new snapshot. A separately requested refresh creates a new snapshot/revision. Static inventory and database privileges cover ledger emission, disposition add/release/terminalization, membership changes, wallet add/rotate/remove, provider/wallet cap update and replay start/row-clear/end, activity append, and mirror-head/completeness advancement. No handler or client joins independently read values.
...
All migrations are additive until the one-way lineage activation. Before activation, rollback remains ordinary while new consumers stay disabled. Activation occurs in an offline maintenance state with routing and all money writers stopped: first the external supervisor installs and verifies a protocol-2 signing/digest allowlist and withholds the dedicated money-directory identity/handle from every other executable; then the sole approved writer locks and reconciles local/witness/SQLite heads; then identical `lineage_activation.v1` PREPARE records become durable locally and at the witness; then one SQLite transaction sets `lineage_min_writer_protocol=2` and installs BEFORE triggers on every inventoried money table that call `macprovider_lineage_writer_protocol()` and reject missing/less-than-2 values before mutation; finally identical activation COMMIT records become durable in both authorities before the service can start. Any failure remains offline. The new writer registers the function only after supervisor identity and local/witness/SQLite activation heads agree. A restored pre-activation SQLite file is rejected because the supervisor and external witness retain the activated epoch; a restored post-activation file retains its triggers. Direct SQL and every supported previous binary fail before their first mutation.
