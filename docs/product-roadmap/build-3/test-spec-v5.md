...
Test-spec revision: `build3-test-v5`
Plan pairing: `build3-plan-v5`
...
| --- | --- | --- | --- |
| B3-F001 | U/G | Review `compute_hook_design.v1` at Gate H0 | Exact two Swift target paths, tap location, processor order, synchronization, cancellation, zeroization, serialization bounds, build inputs, nonshipment boundary and tests have no TBD; zero C/H/M before Collector Slice H0. |
...
| B3-F008 | U | Build profile/protocol after H1 | Reject null/TBD/wildcard, nominal `4bit`, unapproved collector, unfrozen class/decision/partition/statistic/refresh/power/custody rule, self/forward digest, or numeric acquisition predating approval. |
| B3-F009 | G | Gate A0 reviews exact collector/profile/protocol before governed values | Repository/process/signing logs, signed custody sequence allocation, artifact timestamps and access records prove no governed numeric acquisition/disclosure preceded approval; zero C/H/M. |
| B3-F010 | M/P | Collect blinded reference/development/held-out/refresh results after Gate A0 | Every acquisition has a later custody sequence and exact signed collector digest; labels/raw values remain unavailable to threshold authors until threshold digest freezes. |
...
| ID | Class | Scenario | Required result |
| --- | --- | --- | --- |
| B3-H001 | U/G | Gate B0D package inspection | Closed schemas/field bounds, exhaustive mutation inventory, exact SQLite/APFS/witness/archive settings, segment/checkpoint state machine, capacity, seeded workload, core interface, allowed paths and nondeployability controls contain no TBD; zero C/H/M. |
...
| B3-R017 | U/I | Inventory every governed reward writer and attempt direct/legacy mutation without transaction revision token | Revoked DML/sequence privileges, SECURITY DEFINER entry points, server-only token and DB trigger reject; no ledger/disposition/cap/wallet/activity/completeness fact can commit outside one source revision. |
| B3-R018 | I | Race projection read with ledger insert, disposition add/release/terminalize, membership change, wallet rotation, cap update and cap replay start/clear/end | Returned summary and first page equal one real REPEATABLE READ snapshot and one committed source revision; no optimistic or nonexistent combination. |
| B3-R019 | I | Race projection read with activity append and mirror head/completeness advancement | All returned facts share the visible revision record and its writer `source_transaction_at`; read time and `generated_at` remain distinct, and `generated_at` is allocated only after successful coherent read. |
| B3-R020 | U/I | Paginate revision-bound activity while new events and state transitions commit | Cursor returns stable as-of rows/order under original revision; no duplicate/omission/new-snapshot mixing. Explicit refresh returns a new revision. |
| B3-R021 | U/I | Tamper cursor/provider/filter/page size/revision or expire required as-of rows | MAC/revision check rejects or returns `snapshot_expired`; never silently substitutes current data. |
| B3-R022 | U/property | Revision gaps, aborted writer transaction, attempted revision reuse or fact without committed revision | Gaps accepted; aborted/reused/orphan facts invisible or rejected; source revision remains monotonic and authoritative. |
