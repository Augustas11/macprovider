...
Test-spec revision: `build3-test-v6`
Plan pairing: `build3-plan-v6`
...
| B3-R019 | I | Race joined projection with activity append, billing head advancement and payout-wallet source advancement before/after each snapshot | Result binds `(reward_revision,wallet_head)` and distinct PG/source/read/generated times. Head mismatch returns `wallet_projection_pending`; unrelated balance/compute/payment domains preserve truthful states. |
| B3-R020 | U/I | Paginate composite-version activity while new Postgres and wallet events commit | Cursor returns stable as-of rows/order under original `(R,H)`; no duplicate/omission/new-snapshot mixing. Explicit refresh returns a new joined version. |
| B3-R021 | U/I | Tamper cursor/provider/filter/page size/reward revision/wallet head or expire either required as-of history | MAC/composite-version check rejects or returns `snapshot_expired`; never silently substitutes current data. |
...
| B3-R026 | I | SQLite unavailable/stale while rewards fresh; rewards stale while wallet head current; fresh USDC/payment status under both | Each domain reports its own source time/freshness. Wallet failure affects wallet-bound/withdrawal only; balances, compute, payment and USDC are not collapsed. |
...
| B3-R028 | I | Wallet changes immediately after authoritative head read and before response serialization | Response remains linearizable before that later mutation, binds old `(R,H)` and source time, and the next explicit refresh returns the new head or pending state. |
