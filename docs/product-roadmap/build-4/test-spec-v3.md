# Product Build 4 test specification v3

Status: pre-implementation, planning-only  
Repository base: `1d2c930bad81704dd0acc0322226725d8b64aceb`  
Companion plan: `docs/product-roadmap/build-4/implementation-plan-v3.md`
...
- A durable accepted receipt-key archive that can evict the live-registry key, restart, authenticate the archived source record, and expose corruption/unavailability without fallback.
- Real gateway wallet/account/quota/replay storage plus coordinator consume/outbox storage, with authenticated `GatewayDispatchArmV1` test tokens and controllable request/response loss.
- Two signed pool retention matrices with different field/viewer maxima, including one deliberately unsatisfiable recovery-minimum/privacy-maximum combination.
...
Class: X. Go and Swift agree on exact bytes/key/nonce/ciphertext/tag. Mutate each pool, provider, actual acceptable-set/selected-identity, request-encryption key, accepted receipt-key source/id/authenticated-record digest/raw-public-key digest/generation/validity/revocation snapshot, retention-policy id/version/matrix digest, model/artifact, cap, observation-capture, and receipt-contract field independently and require decryption/authentication failure.
...
For T21-T27, preserve consume-before-quota. Exercise gateway-owned account/session mutations immediately before and after its atomic `dispatch_armed` transaction. Exercise coordinator-owned pool authorization/acceptable-set, membership/policy, provider/key, model/admission, and observation mutations after reservation/before consume, after consume/before gateway arm, after arm/before outbox, after outbox/before final fence, and at a deterministic barrier immediately before committed enqueue. Each mutation uses its real owning store/serialization boundary. When mutation wins its applicable cutoff, burn the old transaction, release/refund any quota, persist the exact predispatch terminal state, and record zero queued frames, provider bytes, claims, and inference. If gateway arm or coordinator enqueue wins first, use the specified postcutoff cancellation/hold/reconciliation rule and never resend/fail over. Repeat relevant cuts concurrently with 100+ attempts under race detection as supplemental evidence.
...
Gateway account/session revocation before `dispatch_armed` blocks arm and refunds; revocation after arm permits the already armed request under SPEC-040 and blocks later arms. Coordinator pool buyer-authorization removal advances its generation under the coordinator fence and invalidates a not-yet-committed enqueue. Tests distinguish these owners and do not inject gateway mutation into a coordinator lock.
...
Class: U/X/I. Independently rotate, expire, revoke, and substitute the distinct SPEC-015 receipt key at every lifecycle cut. Persist the exact raw public verification key plus authenticated authority record; restart after live-key eviction and prove archived resolution. Predispatch mismatch/revocation fails at the fence with zero bytes/inference and quota release/refund. Routine postcommit rotation uses only the archive. Emergency revocation while nonterminal becomes immediate terminal quarantine/refund/zero credit/reward exclusion/payout false; an already terminal row remains unchanged. Cover effective-before-commit but discovered later, publication after commit, and publication racing terminal verification. Resolver absence/corruption, archive digest mismatch, wrong raw key, and provider/current-key substitution all fail trust without fallback. Reservation, AAD, wallet signature, outbox, provider frame, receipt preimage, persisted snapshot, and verifier agree on source/id/record/raw-key digest/generation/validity/revocation high-water mark.

### T67 — Deterministic dispatch linearization

Class: U/I. Drive real gateway and coordinator stores. Assert consume burns before gateway quota/arm. Barrier the gateway admission transaction against account/session revocation and verify SPEC-040 cutoff behavior. Authenticate the resulting `GatewayDispatchArmV1` into coordinator consumed/outbox state; reject forged, stale, replayed, lost-binding, or expired arms. Then pause immediately before coordinator committed enqueue and mutate coordinator pool authorization/acceptable set, manifest/lifecycle/membership, provider session/encryption key, receipt key, model/admission, and observation authority through their real stores. When mutation wins the coordinator fence, assert no queued frame/socket byte/claim/decryption/inference, burned material, `predispatch_rejected`, and journal-driven quota release/refund. When enqueue wins, assert one lease/frame, postdispatch cancellation/reconciliation, and no resend/failover. Inject lost consume response, gateway arm failure after consume, lost dispatch request/response, queue rejection, and crashes before/after outbox CAS and enqueue. Status recovery never retries consume/dispatch; `dispatch_uncertain` never redispatches or refunds without proof.
...
Class: U/I. Generate all 512 component/store supported-readiness bit vectors for client, gateway, coordinator settlement backend, provider session, and five named stores; cross them with fresh, expired, reconnect-invalidated, and downgrade-invalidated caches for 2,048 property rows. Prove monotonicity: success requires every required bit and a fresh nonempty version intersection, and flipping any required true bit false yields the same prequota unsupported terminal. Every rejected row has zero reservation consumption/quota/provider bytes/inference/settlement/reward mutation. Run representative all-new, all-old, nonadjacent old pairs, three-old, mixed-store, reconnect, downgrade, unknown-future, and settlement-parser/key/observation-readiness cases through integration. The offline verifier is represented only by coordinator settlement readiness.

### T72 — Retention and dependency-aware pruning

Class: U/I. Use two pools with different signed retention policy ids/versions/matrix digests. For every field group and closed viewer class in plan section 6.12, verify default deny, allowed view, field-level redaction/aggregation transition, exact maximum deletion, and permitted tombstone contents. Reject routeability before reservation when any recovery/accounting minimum exceeds the pool maximum. Exercise policy rotation, one tick before/on/after every minimum/maximum, partial purge in every store order, restart, delayed receipt/evidence, open dispute, uncertain dispatch, and unreconciled quota/settlement. Captured policy never changes; expanded acceptable sets are erased immediately. After all dependencies end, delete by the captured maximum while permitted HMAC/digest tombstones still prevent replay/duplicate money effects and retain no forbidden field.
