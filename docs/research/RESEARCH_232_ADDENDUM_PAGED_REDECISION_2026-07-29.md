...
| Status | Decision addendum; supersedes the original memo's Approach-A-primary recommendation |
...
## The decision this addendum surfaces (for the re-run)

The live question the memo never posed with this data:

> Is a dense **1.2–2.1×** first step worth ~6–10 wk now (Option 1), **or** commit to the paged engine (Option 2) that the competitive bar actually requires, **or** neither — and is the per-Mac concurrency on residential Macs (Entry-110 ~2–4 slots) even high enough for batching's payoff to matter here vs. a datacenter?

The deciding sub-question is **empirical**: run the **custom-kernel-vs-fork spike** — does a paged-attention Metal kernel register beside the pinned `mlx-swift 0.31.6` (3a), or must you fork `mlx` (3b)? That spike sizes Option 2 and is the honest first build before committing either way.
