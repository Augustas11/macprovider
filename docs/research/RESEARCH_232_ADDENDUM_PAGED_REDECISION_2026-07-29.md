...
> **SPIKE RESULT (`docs/research/SPIKE_PAGED_ATTN_PHASE0_RESULT_2026-07-29.md`, commit `e5ded571`): 3a CONFIRMED.** `mlx-swift 0.31.6` ships a public JIT custom-Metal-kernel API (`MLXFast.metalKernel`); a custom kernel compiled + dispatched + eval'd (exact match), and a paged KV gather matched contiguous SDPA at float32 epsilon. **Paged batching is an additive build on the current Apple tag — no `mlx` fork.** Cost/risk moved off kernel registration (proven cheap) onto **per-model attention injection + quantized KV + real-perf**; est. multi-week, dominated by injection + scheduler. **Next gate = Phase-2 spike** (route a real Llama/Qwen forward through the paged op, handle 4-bit KV) before committing the build. Packaging note: `swift build` alone doesn't regenerate `default.metallib` — the provider build must wire it deliberately. **(Spikes actually pinned prod-accurate `mlx-swift 0.31.4`, not 0.31.6 — see the version note; 0.31.6 has nothing macprovider-relevant, so no bump.)**

> **PHASE-2 RESULT (`docs/research/SPIKE_PAGED_ATTN_PHASE2_RESULT_2026-07-29.md`, commit `acc30b1e`): INJECTION FEASIBLE, NO FORK, ARCHITECTURE-GENERAL.** A `PagedKVCache` on the public `KVCache` protocol seam gives **exact** greedy parity (40/40 tokens) on Llama-3.2-3B **and** Qwen2.5-7B unchanged (all dense models funnel through the same `attentionWithCacheUpdate`/`KVCache` seam → one cache, all architectures), on the **production fp16 KV** path. A fully-fused op (replacing SDPA) would need a *light `mlx-swift-lm` fork* (not `mlx` core; numerically de-risked at prod dims 9.3e-4) — optional, max-perf only; the gather-feeds-SDPA path needs **zero fork**. **Feasibility fully settled — no research unknowns left.** Remaining risk is execution: the batching **scheduler**, and **MoE validation** on the live `Qwen3-Coder-30B-A3B` (unproven, same seam). Quantized `kvBits` = distinct numerical surface, not on the prod path. **This unblocks the SPEC-038 v0.2 rewrite + new paged-engine spec, now writable on verified ground.**

> **PHASE-3 RESULT (`docs/research/SPIKE_PAGED_ATTN_PHASE3_MOE_RESULT_2026-07-29.md`, commit `da21af53`): MoE PASS on the LIVE production model.** The unchanged Phase-2 `PagedKVCache` gives **exact** parity (40/40) on `Qwen3-Coder-30B-A3B` (48 layers, 128 experts); the model instantiates plain `{KVCacheSimple:48}` (no sliding-window/quantized/hybrid cache), confirming MoE expert-routing is orthogonal to attention paging. **All three spikes (0/2/3) now PASS on dense + the live MoE — every attention-side feasibility risk is retired, on the real production model, with no fork and no per-model work.** Remaining effort is pure engineering (batching scheduler + paged block allocator, both Swift); the only MoE-specific item is scheduler-side (expert dispatch across batched sequences), not attention. **Feasibility investigation CLOSED — proceed to SPEC-038 v0.2 + the paged-engine spec, then the build.** The only *forward* demand signal worth watching in parallel is **unmet model/context requests** (buyers asking for models/contexts a Mac can't serve — the servability signal), which is a future signal, not a current-occupancy one.
