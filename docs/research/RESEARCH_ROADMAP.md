...
| **232 / SPEC-038** continuous batching | **SPEC MERGED** (PR **#799**, `1d78778a`; 0 C/H/M). Scaffold PR **#804 HELD** (activation theory dead — see below). | **Build it in-house** — upstream will never deliver it (see addendum). SPEC-028 spec-decode mutually exclusive in v1. | **See `RESEARCH_232_ADDENDUM_PAGED_REDECISION_2026-07-29.md` (corrected).** Upstream #263 is structurally dead (Layr abandoned it for a private paged fork; competitor forked the whole MLX stack). **Batching — esp. PAGED / memory-servability — is strategic infrastructure for the network being built (attracts Ultra providers + big-model/long-context buyers; concurrency-independent), NOT gated on today's fleet occupancy.** **Phase-0 spike DONE (`e5ded571`, 2026-07-29): 3a CONFIRMED** — a custom PagedAttention Metal kernel registers + dispatches beside the pinned `mlx-swift 0.31.6` (`MLXFast.metalKernel`), paged gather matches SDPA at float32 epsilon → **paged batching is an ADDITIVE build on the current Apple tag, NO `mlx` fork.** Remaining risk moved to per-model attention injection + quantized KV + real-perf, not kernel registration; est. multi-week. **Next gate = Phase-2 spike (route a real Llama/Qwen forward through the paged op + quantized KV) before committing the build.** #804's FR-CB10 upstream-pin gate → reframe to locally-owned activation (now has a concrete target). |
