...
| 11 | **Catalog artifact feed** (#1453 slice 2a) | Off unless the operator authors it: `scripts/catalog-release.py generate` only produces and binds `autotune-artifacts.json` when `phase3-binary/catalog/autotune/autotune-artifacts-source.json` exists. With no source, the release keeps the rate-card-bound four-feed set, no `/v1/catalog-artifacts` bytes are published, the release ledger stays `macprovider.autotune-release-ledger.v2`, and v0.1 behaviour is byte-for-byte unchanged. Deleting the source before a release cut is the switch; it removes an operator-authored input, never provider state. Once a release HAS been cut with the feed, the artifact-bound set is mandatory going forward (SPEC-023 §3.7.8) — disable the feed's *effects* by withdrawing artifacts (publish them `blocked`), not by reverting the feed set. Stage A never touches `components.catalog.files`, so an already-installed updater is unaffected either way. | off (no source committed as of this slice) | `scripts/tests/test_catalog_artifact_feed.py:StageActivationTest`, `:LedgerV3Test.test_downgrade_after_activation_fails_closed`; `python3 scripts/catalog-release.py verify` passes on the current four-feed release with the source committed |
