...
## Disablement matrix (#1248, eleven rows)

Every row names the switch, its default, and the automated proof — or an
explicit not-applicable reason. Test references are `file:testName`. Row 11 was
added by the BYOM v0.2 catalog artifact-feed slice (#1453 slice 2a).
...
| 11 | **Catalog artifact feed** (#1453 slices 2a/2b) | Off until an operator explicitly activates it. Serving side (slice 2b): the coordinator answers 404 on `/v1/catalog-artifacts` and `.sig` until `autotune.catalog_artifacts_path` / `autotune.catalog_artifacts_sig_path` are configured; to disable serving, remove the two keys and SIGHUP or restart the coordinator — the four-feed set keeps serving unchanged. The live release gate then requires the routes to be absent for a release that does not bind the feed. **The source IS committed in this slice; committing it activates nothing.** `scripts/catalog-release.py generate` builds and binds `autotune-artifacts.json` only when the release ledger already records an artifact-bound release or the operator passes `--activate-artifact-feed` on a new `release_id`; with neither, the release keeps the rate-card-bound four-feed set, no `/v1/catalog-artifacts` bytes are published, the release ledger stays `macprovider.autotune-release-ledger.v2`, and v0.1 behaviour is byte-for-byte unchanged. `catalog-release.py status` prints the state and everything still blocking activation. Withholding the flag is the switch; deleting the source before a release cut also works and removes an operator-authored input, never provider state. Once a release HAS been cut with the feed, the artifact-bound set is mandatory going forward (SPEC-023 §3.7.8) — disable the feed's *effects* by withdrawing artifacts (publish them `blocked`), not by reverting the feed set. Stage A never touches `components.catalog.files`, so an already-installed updater is unaffected either way. | off (source committed; no feed published or release-bound) | `scripts/tests/test_catalog_artifact_feed.py:StageActivationTest`, `:LedgerV3Test.test_downgrade_after_activation_fails_closed`, `:HermeticReleaseTest.test_four_feed_release_is_unaffected_by_the_committed_unmeasured_source`, `:test_activation_requires_the_explicit_flag`, `:test_a_stale_published_feed_never_activates_implicitly`; `python3 scripts/catalog-release.py verify` passes on the current four-feed release with the source committed |
| 12 | **`lmstudio_loopback` adapter** (#1478) | Read-only client-side switch, same contract as Ollama (row 1): the adapter has a well-known loopback default (`http://127.0.0.1:1234`) and is attempted unless **`--skip-lmstudio`** is passed on `models discover/evaluate/offer/admission ...`; a skipped adapter contributes **no `lmstudio_loopback` row in `adapters[]` at all** and dispatches zero requests. `--lmstudio-origin` overrides the origin (loopback only, rejected before dispatch otherwise). The artifact leg hashes only files under the operator-declared models root (default `~/.lmstudio/models`, `MACPROVIDER_LMSTUDIO_MODELS_ROOT`), resolved from the served **name**, never from anything the runtime reports; removing or emptying that root leaves candidates `runtime_reported` with nothing to roll back. Only a model LM Studio reports as `state: "loaded"` is `ready`; downloaded-but-unloaded, unknown-state, and `/v1/models`-fallback entries are `requires_preparation` and stay `local_only` until `models evaluate` proves a completion. The adapter persists nothing except digest-cache rows keyed by exact file identity with a SHA-256 path token (row 1's cache, schema v2), holds no credential, and no candidate it produces can reach `settlement_capable` in v0.2 (`earning_path_class: no_earning_path_in_v0_1`). | on (well-known default) | `phase3-binary/Tests/macprovider-cliTests/BYOMLoopbackAdapterTests.swift:testLMStudioRejectsNonLoopbackOriginBeforeDispatch`, `:testSkippedAdaptersLeaveNoRowAndDispatchNothing`, `:testLMStudioIdentityIsHashedOnlyForAResolvableGGUFAndNeverOpaque`, `:testLMStudioStoreFailsClosedWhenSeveralFilesAnswerTheID`, `:testLMStudioOnlyALoadedModelIsReadyAndOfferable`, `:testLMStudioFallsBackToV1ModelsOn404AndNothingFromItIsReady`, `:testOfferDryRunClaimsNoSettlementForEitherAdapter` |
| 13 | **`llamacpp_loopback` adapter** (#1478) | Same read-only switch shape: well-known default `http://127.0.0.1:8080`, **`--skip-llamacpp`** leaves no `adapters[]` row and dispatches nothing, `--llamacpp-origin` overrides (loopback only). Two safeguards specific to this runtime: (a) the adapter **fingerprints llama-server via `GET /props`** before it reads any inventory, because `:8080` is also macprovider's own `serve` port — a provider Mac's own server answers `unavailable`, never a candidate; (b) the artifact leg hashes only a file the **operator** names AND that the runtime proves it is serving: the path llama-server reports in `GET /props.model_path` is used internally as proof (never stored or emitted) and must equal the operator-resolved file. Two modes, both with **no default** and decided once from one source (a CLI flag ignores both environment variables; two selectors from the same source are an explicit error): **(a)** `--llamacpp-model-root` / `MACPROVIDER_LLAMACPP_MODEL_ROOT`, the served path must lie under the directory and equal the single `<stem>.gguf` found there; **(c)** `--llamacpp-model-path` / `MACPROVIDER_LLAMACPP_MODEL_PATH`, the served path must equal the pinned file. A same-stem file the runtime is not serving is never hashed. The served reference on the wire is the stem, never a path. With neither set every candidate is `runtime_reported`. Unsetting the root/path is the rollback for the artifact leg; the digest cache persists only a SHA-256 path token (schema v2), never a path. | on (well-known default); artifact leg **off** (no root, no path) | `phase3-binary/Tests/macprovider-cliTests/BYOMLoopbackAdapterTests.swift:testLlamaCppRejectsNonLoopbackOriginBeforeDispatch`, `:testSkippedAdaptersLeaveNoRowAndDispatchNothing`, `:testLlamaCppRequiresThePropsFingerprintBeforeTrustingInventory`, `:testLlamaCppPathIDBecomesAStemAndResolvesOnlyUnderTheOperatorRoot`, `:testLlamaCppStoreRejectsSymlinkEscapeAndNilRoot`, `:testLlamaCppPinnedFileResolvesOnlyThatFileAndOverridesTheRoot`, `:testLlamaCppPinnedFileDrivesArtifactHashAvailableThroughDiscovery`, `:testLlamaCppSameStemFileUnderRootIsNotHashedForAFileServedElsewhere`, `:testLlamaCppIsHashedOnlyWhenTheServedPathIsTheRootResolvedFile`, `:testLlamaCppSelectorIsDecidedOnceFromOneSource`, `:testDigestCachePersistsAPathTokenNotThePathAndDiscardsV1Files`, `:testOfferDryRunClaimsNoSettlementForEitherAdapter` |
