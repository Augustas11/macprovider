...
| Cached-turns flag | SPEC-038 v0.2.8 `continuous_batching_cached_turns` (env `MACPROVIDER_CONTINUOUS_BATCHING_CACHED_TURNS`, CLI `--[no-]continuous-batching-cached-turns`, default off) is the only switch that lets a positive-`cached_prompt_tokens` turn batch. It applies only when the lease carries a usable retained handoff, which on a hybrid tuple (Qwen3.6) must include a recurrent checkpoint. Keep it off on every live provider until the sticky retained-KV proof above is recorded for the exact tuple; for a hybrid tuple that proof must include a checkpoint-resumed turn whose `cached_prompt_tokens` equals the checkpoint length. Serve start prints `event=batching_cached_turns action=enabled mode=...`, or `action=inert` while batching is off. Record that line. |
