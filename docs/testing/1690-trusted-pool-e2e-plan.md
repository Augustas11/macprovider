...
|---|---|---|
| F-1 | yes | fixed `b0010777` + `f0d8ef9b`: a 200 that declared finality trailers and whose body read fails is held as `missing_settlement_finality_trailer` (candidate outcome `body_read_failed`, 0 delivered completion), not refunded; the reconciler settles it to the coordinator's finality with the debit bounded by what the buyer received (the coordinator prompt, 0 completion; refund finality refunds; observe mode settles the gateway tuple). A hold whose coordinator never recorded the attempt becomes `stale_held` with no debit once the coordinator's own `Settlement finality not found` answers span 1 h, counted from the first such answer (persisted per hold); a generic 404 never ages a hold out. Streaming already held. Tests `TestDropAfterBodySettlesToCoordinatorFinality`, `TestDropAfterBodyCoordinator404HoldAgesOutToStaleHeld`. |
...
| F-4 | yes | fixed `07ae2d56` + `205bda1d`: `InsertSettlementAttemptOutput`, and the same read-then-write shape in `ClaimPayoutReady`, run under `sqliteutil.Transact` (BEGIN IMMEDIATE), the money-path pattern. Tests `TestInsertSettlementAttemptOutputSurvivesConcurrentRouteSnapshotWriter` (production 500 ms route-snapshot busy timeout) and `TestClaimPayoutReadySurvivesConcurrentRouteSnapshotWriter`. The other deferred billing transactions open with a write and are carried. |
| F-5 | yes | fixed `ec395367`: `Registry.PublishRouteableSnapshotsIfAhead` checks and replaces under one lock; an equal or older revision is a no-op success, so the refresher race no longer 500s or disables the registry. Tests `TestAdminHandler_RefresherRaceIsNotARefreshFailure`, `TestRegistryPublishRouteableSnapshotsIfAheadIsIdempotent`. The on-call route-gate republish has the same fix (`acc98a29`, `Registry.RepublishRouteGatesAtRevision`; test `TestAdminHandler_OnCallRepublishLosingToNewerRevisionIsNotAFailure`). |
...
| F-8 | yes | fixed `000986d4`: deploy step 2c falls back to counting in-flight (active, unheld, unexpired) reservations in the live gateway DB, read-only, and still fails closed on any error. Test `phase5-gateway/dist/test/gateway_deploy_inflight.test.sh` in `make test-dist`. The count is a pre-check; a later request is protected by the graceful SIGTERM drain, now 40 s below `TimeoutStopSec=45` (`78f59dae`), and expiries are compared as instants with `julianday()` (`deaa4b4f`). |
...
| R1-held | yes (pre-existing) | carried. Codex R1 (HIGH, pre-existing): settlement holds other than the F-1 path have no age-out; a hold whose coordinator finality never becomes terminal (or whose lookup keeps answering 404 without a `body_read_failed` candidate) stays `active` with `settlement_hold=1` indefinitely, and the reconciler re-queries it every sweep. It is the confirmed deepsec true positive "Pending settlement holds can leave delivered responses unbilled and unreconciled" (`docs/operations/deepsec-security-audit-runbook.md:348-349`), and Pearl carries 47,408 such rows (46,028 on the synthetic buyer, 1,368 on the OpenRouter account; `docs/releases/coordinator-release-train.md`). Only the F-1 hold ages out (`f0d8ef9b`, then review L-1: 1 h of authoritative not-found answers to `stale_held`). A general age-out changes buyer debit for every held path and needs a SPEC-022 decision plus an operator resolution pass of the backlog; it is not a VM follow-up. |
| R2-drain | yes | carried (pre-existing). A buyer request still running when the 40 s SIGTERM drain ends (F-8, `78f59dae`) is cut by a gateway deploy restart. Held-reservation recovery covers only `settlement_hold` rows; a cut in-flight request's reservation is expired or reconciled like any crash. Mitigation: deploy the gateway in a low-traffic window, or stop buyer traffic at nginx first (runbook s9 step 2). |
| R2-F5-equal | yes | carried (pre-existing). F-5 fixed the older-revision race (`ec395367`, `acc98a29`): a publish or route-gate republish behind a newer revision is a no-op success. Two publishes of the same revision stay last-writer-wins, a pre-existing race with a brief window; both carry the same durable revision and the next refresh republishes it. |
| R2-no-candidate | yes | carried (pre-existing). The F-1 `body_read_failed` path now holds only when its reconcile candidate is persisted with the coordinator's internal request id, else it refunds (`2fa691c4`). Other callers of `boundStreamingSettlementHoldWithCandidate` (gateway-ended stream terminals, header-finality holds) still mark a hold when the candidate cannot be persisted or the internal request id is missing, and the reconciler then keeps that hold indefinitely (the R1-held class). Changing their failure mode is a separate money-path decision. |
