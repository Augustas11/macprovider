...
`JOURNEY-TRUSTED-POOL-CREATOR-MVP` candidate envelopes remain evidence-only until a matching `consumed_authorization` exists. Sibling `spec043-ppt-20260827T032524Z-1` is consumed; isolated-candidate CONFORMANCE is not a live Trusted Pool launch. `scripts/validate-pool-promotion-transition.py --consume` and `scripts/promote-signed-journey-result.py --promotion-transition` both consume a valid `PoolPromotionTransitionV1` (`spec-043-promotion-auth-v1`) into this ledger; only the promoter may then rewrite CONFORMANCE. The protected sibling signer (`.github/workflows/build-signed-pool-promotion-transition.yml`) validates without `--consume`.
