...
| 2026-07-11 | Item 4 (G2, #558) — RESOLVED via (c): amend SPEC-020 trust table (spec+comments only, no behavior change; prod keeps auto-updating). Runtime check proved prod `mac` is provisional → flip would've broken it. 3 codex rounds (spec-completeness). Carried item 23. | codex final check running |
