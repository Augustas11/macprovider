...
actor CoordinatorClient {
    ...
    func authProofMessage(
        ...
        if credentialBootstrap {
            ...
            // PROD-H1: forward the installer's reservation id so the mint
            // consumes the same reservation the installer claimed at preflight.
            if let reservationID = appConfig.referralReservationID?.trimmingCharacters(in: .whitespacesAndNewlines),
               !reservationID.isEmpty {
                proof["referral_reservation_id"] = reservationID
            }
