...
enum ModelPreparationContracts {
    ...
    static let cancelMarkerMaxBytes = 4_096
    static let stagingSourcesMaxBytes = 262_144
    ...
    static func rootIdentityDigest(version: String, nonceHex: String, canonicalPath: String, stDev: UInt64, stIno: UInt64) throws -> String {
        ...
        try requireCanonicalAbsolutePath(canonicalPath, field: "canonical_path")
        var data = Data()
    ...
    static func publicationReceiptSHA256(from data: Data) throws -> String {
        _ = try decode(ModelPreparationPublicationReceipt.self, from: data, maxBytes: publicationReceiptMaxBytes)
        return data.sha256Hex()
    }
    ...
    private static func requireCanonicalRawBytes<T>(_ value: T, originalData: Data, field: String) throws {
        let canonicalData: Data?
        if let receipt = value as? ModelPreparationPublicationReceipt {
            canonicalData = try encode(receipt)
        } else if let cleanupRecord = value as? ModelPreparationCleanupRecord {
            canonicalData = try encode(cleanupRecord)
        } else {
            canonicalData = nil
        }
        guard let canonicalData else { return }
        guard canonicalData == originalData else {
    ...
    static func requireRFC3339Timestamp(_ value: String, field: String) throws {
        let pattern = #"^([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9]{2}):([0-9]{2}):([0-9]{2})(\.[0-9]{1,9})?(Z|([+-])([0-9]{2}):([0-9]{2}))$"#
        let regex = try NSRegularExpression(pattern: pattern)
        let nsRange = NSRange(value.startIndex..<value.endIndex, in: value)
        guard let match = regex.firstMatch(in: value, range: nsRange), match.range == nsRange else {
            throw ModelPreparationContractError.malformed(field)
        }

        func integer(at index: Int) throws -> Int {
            guard let range = Range(match.range(at: index), in: value), let parsed = Int(value[range]) else {
                throw ModelPreparationContractError.malformed(field)
            }
            return parsed
        }

        let year = try integer(at: 1)
        let month = try integer(at: 2)
        let day = try integer(at: 3)
        let hour = try integer(at: 4)
        let minute = try integer(at: 5)
        let second = try integer(at: 6)
        guard (0...23).contains(hour), (0...59).contains(minute), (0...59).contains(second) else {
            throw ModelPreparationContractError.malformed(field)
        }
        if match.range(at: 10).location != NSNotFound {
            let offsetHour = try integer(at: 10)
            let offsetMinute = try integer(at: 11)
    ...
    static func requireCanonicalRFC3339Seconds(_ value: String, field: String) throws {
        let pattern = #"^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$"#
        guard value.count == 20, value.range(of: pattern, options: .regularExpression) != nil else {
            throw ModelPreparationContractError.malformed(field)
        }
        let formatter = ISO8601DateFormatter()
    ...
    static func requireCanonicalAbsolutePath(_ value: String, field: String) throws {
        try requireSafeString(value, field: field, maxUTF8Bytes: 4_096)
        guard value.hasPrefix("/"), !value.contains("\\") else {
            throw ModelPreparationContractError.malformed(field)
        }
        if value == "/" { return }
        guard !value.hasSuffix("/"), !value.contains("//") else {
            throw ModelPreparationContractError.malformed(field)
        }
        for component in value.split(separator: "/", omittingEmptySubsequences: false).dropFirst() {
    ...
    static func decode<T: Decodable>(_ type: T.Type, from data: Data, maxBytes: Int) throws -> T {
        ...
        try ModelPreparationJSONShapeValidator.validate(data, expectedType: T.self)
        let decoder = JSONDecoder()
        let value = try decoder.decode(T.self, from: data)
        try requireCanonicalRawBytes(value, originalData: data, field: "canonical json")
        return value
    }
...
struct ModelPreparationRootLocator: Codable, Equatable, Sendable {
    ...
    init(canonicalPath: String, stDev: UInt64, stIno: UInt64, identityVersion: String, rootIdentityDigest: String) throws {
        try ModelPreparationContracts.requireCanonicalAbsolutePath(canonicalPath, field: "canonical_path")
...
struct ModelPreparationCleanupTarget: Codable, Equatable, Sendable {
    ...
    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(artifactIdentityDigest, forKey: .artifactIdentityDigest)
        ...
        try container.encodeNullable(modelKey, forKey: .modelKey)
        try container.encode(eventModelKey, forKey: .eventModelKey)
        try container.encode(rootIdentityDigest, forKey: .rootIdentityDigest)
        try container.encode(receiptSHA256, forKey: .receiptSHA256)
        try container.encode(estimatedBytes, forKey: .estimatedBytes)
        try container.encode(keepSetStatus, forKey: .keepSetStatus)
        try container.encodeNullable(protectedReason, forKey: .protectedReason)
...
struct ModelPreparationReservationRecord: Codable, Equatable, Sendable {
    ...
    private static let baseKeys: Set<String> = [
        CodingKeys.schema.stringValue,
        CodingKeys.transactionID.stringValue,
        CodingKeys.transactionKind.stringValue,
        CodingKeys.eventModelKey.stringValue,
        CodingKeys.root.stringValue,
        CodingKeys.tuple.stringValue,
        CodingKeys.tupleSHA256.stringValue,
        CodingKeys.projectionBindingSHA256.stringValue,
        CodingKeys.createdAt.stringValue,
    ]

    init(
        schema: String = "model_catalog_reservation.v4",
        transactionID: String,
        transactionKind: ModelPreparationTransactionKind,
        eventModelKey: String,
        root: ModelPreparationRootLocator,
        ...
        sourceRecordSHA256: String? = nil
    ) throws {
        guard schema == "model_catalog_reservation.v4" else { throw ModelPreparationContractError.malformed("schema") }
        try ModelPreparationContracts.requireUUIDv4(transactionID, field: "transaction_id")
        try ModelPreparationContracts.requireSafeString(eventModelKey, field: "event_model_key")
        ...
        try ModelPreparationContracts.requireRFC3339Timestamp(createdAt, field: "created_at")
        switch transactionKind {
        case .cleanupStaging:
            guard let sourceTransactionID, let sourceAttemptID, let sourceRecordSHA256 else {
                throw ModelPreparationContractError.malformed("staging source")
            }
            try ModelPreparationContracts.requireUUIDv4(sourceTransactionID, field: "source_transaction_id")
            try ModelPreparationContracts.requireUUIDv4(sourceAttemptID, field: "source_attempt_id")
            try ModelPreparationContracts.requireHex64(sourceRecordSHA256, field: "source_record_sha256")
        default:
            guard sourceTransactionID == nil, sourceAttemptID == nil, sourceRecordSHA256 == nil else {
                throw ModelPreparationContractError.malformed("staging source")
            }
        }
        self.schema = schema
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let transactionKind = try container.decode(ModelPreparationTransactionKind.self, forKey: .transactionKind)
        let present = Set(container.allKeys.map(\.stringValue))
        let expected: Set<String>
        switch transactionKind {
        case .cleanupStaging:
            expected = Self.baseKeys.union([
                CodingKeys.sourceTransactionID.stringValue,
                CodingKeys.sourceAttemptID.stringValue,
                CodingKeys.sourceRecordSHA256.stringValue,
            ])
        default:
            expected = Self.baseKeys
        }
        guard present == expected else { throw ModelPreparationContractError.malformed("keys") }
        try self.init(
            schema: try container.decode(String.self, forKey: .schema),
            ...
            sourceAttemptID: try container.decodeIfPresent(String.self, forKey: .sourceAttemptID),
            sourceRecordSHA256: try container.decodeIfPresent(String.self, forKey: .sourceRecordSHA256)
        )
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        if transactionKind == .cleanupStaging {
            try container.encode(sourceTransactionID, forKey: .sourceTransactionID)
            try container.encode(sourceAttemptID, forKey: .sourceAttemptID)
...
struct ModelPreparationTerminalHistoryRecord: Codable, Equatable, Sendable {
    let schema: String
    let transactionID: String
    let attemptID: String
    let transactionKind: ModelPreparationTransactionKind
    ...
    init(
        schema: String = "model_catalog_terminal_history.v1",
        transactionID: String,
        attemptID: String,
        transactionKind: ModelPreparationTransactionKind,
        eventModelKey: String,
        ...
        projectionBindingSHA256: String,
        eventSequence: Int,
        terminalState: ModelPreparationHistoryTerminalState,
        errorCode: ModelPreparationEventErrorCode?,
        liveAttempt: Bool = false,
        completedAt: String
    ) throws {
        guard schema == "model_catalog_terminal_history.v1" else { throw ModelPreparationContractError.malformed("schema") }
        try ModelPreparationContracts.requireUUIDv4(transactionID, field: "transaction_id")
        try ModelPreparationContracts.requireUUIDv4(attemptID, field: "attempt_id")
        try ModelPreparationContracts.requireSafeString(eventModelKey, field: "event_model_key")
        try ModelPreparationContracts.requireHex64(tupleSHA256, field: "tuple_sha256")
        try ModelPreparationContracts.requireHex64(projectionBindingSHA256, field: "projection_binding_sha256")
        guard eventSequence > 0 else { throw ModelPreparationContractError.malformed("event_sequence") }
        ...
        guard liveAttempt == false else { throw ModelPreparationContractError.malformed("live_attempt") }
        try ModelPreparationContracts.requireRFC3339Timestamp(completedAt, field: "completed_at")
        self.schema = schema
        self.transactionID = transactionID
        self.attemptID = attemptID
        self.transactionKind = transactionKind
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(liveAttempt, forKey: .liveAttempt)
        try container.encode(completedAt, forKey: .completedAt)
    }
}

enum ModelPreparationReservationsTerminalHistoryEntry: Codable, Equatable, Sendable {
    case ordinary(ModelPreparationTerminalHistoryRecord)
    case failedDispatch(ModelPreparationFailedDispatchRecord)

    private enum SchemaKey: String, CodingKey { case schema }

    var transactionID: String {
        switch self {
        case .ordinary(let record): return record.transactionID
        case .failedDispatch(let record): return record.transactionID
        }
    }

    var attemptID: String {
        switch self {
        case .ordinary(let record): return record.attemptID
        case .failedDispatch(let record): return record.attemptID
        }
    }

    var transactionKind: ModelPreparationTransactionKind {
        switch self {
        case .ordinary(let record): return record.transactionKind
        case .failedDispatch(let record): return record.transactionKind
        }
    }

    var eventModelKey: String {
        switch self {
        case .ordinary(let record): return record.eventModelKey
        case .failedDispatch(let record): return record.eventModelKey
        }
    }

    var root: ModelPreparationRootLocator {
        switch self {
        case .ordinary(let record): return record.root
        case .failedDispatch(let record): return record.root
        }
    }

    var tupleSHA256: String {
        switch self {
        case .ordinary(let record): return record.tupleSHA256
        case .failedDispatch(let record): return record.tupleSHA256
        }
    }

    var projectionBindingSHA256: String {
        switch self {
        case .ordinary(let record): return record.projectionBindingSHA256
        case .failedDispatch(let record): return record.projectionBindingSHA256
        }
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: SchemaKey.self)
        let schema = try container.decode(String.self, forKey: .schema)
        switch schema {
        case "model_catalog_terminal_history.v1":
            self = .ordinary(try ModelPreparationTerminalHistoryRecord(from: decoder))
        case "model_catalog_failed_dispatch.v1":
            self = .failedDispatch(try ModelPreparationFailedDispatchRecord(from: decoder))
        default:
            throw ModelPreparationContractError.malformed("schema")
        }
    }

    func encode(to encoder: Encoder) throws {
        switch self {
        case .ordinary(let record): try record.encode(to: encoder)
        case .failedDispatch(let record): try record.encode(to: encoder)
        }
    }
}

struct ModelPreparationReservationsHistoryRecord: Codable, Equatable, Sendable {
    let schema: String
    let projectedReservations: [ModelPreparationReservationRecord]
    let terminalHistory: [ModelPreparationReservationsTerminalHistoryEntry]

    enum CodingKeys: String, CodingKey, CaseIterable {
        case schema
        case projectedReservations = "projected_reservations"
        case terminalHistory = "terminal_history"
    }

    init(
        schema: String = "model_catalog_reservations_history.v1",
        projectedReservations: [ModelPreparationReservationRecord],
        terminalHistory: [ModelPreparationReservationsTerminalHistoryEntry]
    ) throws {
        guard schema == "model_catalog_reservations_history.v1" else { throw ModelPreparationContractError.malformed("schema") }
        guard projectedReservations.count <= 64 else { throw ModelPreparationContractError.malformed("projected_reservations") }
        guard terminalHistory.count <= 256 else { throw ModelPreparationContractError.malformed("terminal_history") }
        let projectionKeys = projectedReservations.map { $0.transactionID }
        guard projectionKeys == projectionKeys.sorted(), Set(projectionKeys).count == projectionKeys.count else {
            throw ModelPreparationContractError.malformed("projected_reservations")
        }
        let terminalKeys = terminalHistory.map { "\($0.transactionID)/\($0.attemptID)" }
        guard Set(terminalKeys).count == terminalKeys.count else {
            throw ModelPreparationContractError.malformed("terminal_history")
        }
        var terminalByTransactionID: [String: ModelPreparationReservationsTerminalHistoryEntry] = [:]
        for terminal in terminalHistory {
            if let prior = terminalByTransactionID[terminal.transactionID] {
                guard terminal.transactionKind == prior.transactionKind,
                      terminal.eventModelKey == prior.eventModelKey,
                      terminal.root == prior.root,
                      terminal.tupleSHA256 == prior.tupleSHA256,
                      terminal.projectionBindingSHA256 == prior.projectionBindingSHA256 else {
                    throw ModelPreparationContractError.bindingMismatch("terminal_history")
                }
            } else {
                terminalByTransactionID[terminal.transactionID] = terminal
            }
        }
        let projectedByTransactionID = Dictionary(uniqueKeysWithValues: projectedReservations.map { ($0.transactionID, $0) })
        for terminal in terminalHistory {
            if let projected = projectedByTransactionID[terminal.transactionID] {
                guard terminal.transactionKind == projected.transactionKind,
                      terminal.eventModelKey == projected.eventModelKey,
                      terminal.root == projected.root,
                      terminal.tupleSHA256 == projected.tupleSHA256,
                      terminal.projectionBindingSHA256 == projected.projectionBindingSHA256 else {
                    throw ModelPreparationContractError.bindingMismatch("terminal_history")
                }
            }
        }
        self.schema = schema
        self.projectedReservations = projectedReservations
        self.terminalHistory = terminalHistory
        _ = try ModelPreparationContracts.encode(self, maxBytes: ModelPreparationContracts.reservationHistoryMaxBytes)
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        try self.init(
            schema: try container.decode(String.self, forKey: .schema),
            projectedReservations: try container.decode([ModelPreparationReservationRecord].self, forKey: .projectedReservations),
            terminalHistory: try container.decode([ModelPreparationReservationsTerminalHistoryEntry].self, forKey: .terminalHistory)
        )
    }
}
...
struct ModelPreparationCancelAcknowledgement: Codable, Equatable, Sendable {
    ...
    init(
        ...
        try ModelPreparationContracts.requireRFC3339Timestamp(observedAt, field: "observed_at")
...
struct ModelPreparationTransactionEvent: Codable, Equatable, Sendable {
    struct Progress: Codable, Equatable, Sendable {
        ...
        init(stageLabelKey: String, bytesCompleted: Int64?, bytesExpected: Int64?, percentComplete: Double?, heartbeat: Bool?) throws {
            ...
            guard bytesCompleted != nil || percentComplete != nil || heartbeat == true else {
    ...
    init(
        ...
        self.modelKey = modelKey
        self.eventSequence = eventSequence
        try ModelPreparationContracts.requireRFC3339Timestamp(emittedAt, field: "emitted_at")
...
struct ModelPreparationCancelMarker: Codable, Equatable, Sendable {
    let schema: String
    let transactionID: String
    let attemptID: String
    let transactionKind: ModelPreparationTransactionKind
    ...
    init(
        schema: String = "model_catalog_cancel_marker.v1",
        transactionID: String,
        attemptID: String,
        transactionKind: ModelPreparationTransactionKind,
        eventModelKey: String,
        ...
        guard schema == "model_catalog_cancel_marker.v1" else { throw ModelPreparationContractError.malformed("schema") }
        try ModelPreparationContracts.requireUUIDv4(transactionID, field: "transaction_id")
        try ModelPreparationContracts.requireUUIDv4(attemptID, field: "attempt_id")
        try ModelPreparationContracts.requireSafeString(eventModelKey, field: "event_model_key")
        try ModelPreparationContracts.requireHex64(tupleSHA256, field: "tuple_sha256")
        try ModelPreparationContracts.requireHex64(projectionBindingSHA256, field: "projection_binding_sha256")
        try ModelPreparationContracts.requireCanonicalRFC3339Seconds(recordedAt, field: "recorded_at")
        self.schema = schema
        self.transactionID = transactionID
        self.attemptID = attemptID
        self.transactionKind = transactionKind
        ...
        _ = try ModelPreparationContracts.encode(self, maxBytes: ModelPreparationContracts.cancelMarkerMaxBytes)
    }
...
struct ModelPreparationStagingSourceRecord: Codable, Equatable, Sendable {
    let schema: String
    let sourceTransactionID: String
    let sourceAttemptID: String
    let root: ModelPreparationRootLocator
    ...
    enum CodingKeys: String, CodingKey, CaseIterable {
        ...
        case eventModelKey = "event_model_key"
        case projectionBindingSHA256 = "projection_binding_sha256"
        case stagingLeaf = "staging_leaf"
    }

    init(
        schema: String = "model_catalog_staging_source.v1",
        ...
        guard schema == "model_catalog_staging_source.v1" else { throw ModelPreparationContractError.malformed("schema") }
        try ModelPreparationContracts.requireUUIDv4(sourceTransactionID, field: "source_transaction_id")
        try ModelPreparationContracts.requireUUIDv4(sourceAttemptID, field: "source_attempt_id")
        guard root == tuple.root else { throw ModelPreparationContractError.bindingMismatch("root") }
        try ModelPreparationContracts.requireHex64(tupleSHA256, field: "tuple_sha256")
        guard tupleSHA256 == (try ModelPreparationContracts.tupleSHA256(tuple)) else {
            throw ModelPreparationContractError.bindingMismatch("tuple_sha256")
        }
        try ModelPreparationContracts.requireSafeString(eventModelKey, field: "event_model_key")
        guard eventModelKey == tuple.eventModelKey else {
        ...
        try ModelPreparationContracts.requireHex64(projectionBindingSHA256, field: "projection_binding_sha256")
        guard stagingLeaf == "work/staging/\(sourceTransactionID)/\(sourceAttemptID)/source.json" else {
            throw ModelPreparationContractError.bindingMismatch("staging_leaf")
        }
        self.schema = schema
        self.sourceTransactionID = sourceTransactionID
        self.sourceAttemptID = sourceAttemptID
    ...
    init(from decoder: Decoder) throws {
        ...
        try self.init(
            schema: try container.decode(String.self, forKey: .schema),
            sourceTransactionID: try container.decode(String.self, forKey: .sourceTransactionID),
            sourceAttemptID: try container.decode(String.self, forKey: .sourceAttemptID),
            root: try container.decode(ModelPreparationRootLocator.self, forKey: .root),
            tuple: try container.decode(ModelPreparationTupleRecord.self, forKey: .tuple),
            tupleSHA256: try container.decode(String.self, forKey: .tupleSHA256),
            eventModelKey: try container.decode(String.self, forKey: .eventModelKey),
            projectionBindingSHA256: try container.decode(String.self, forKey: .projectionBindingSHA256),
            stagingLeaf: try container.decode(String.self, forKey: .stagingLeaf)
        )
    }
}

struct ModelPreparationStagingSourceEntry: Codable, Equatable, Sendable {
    let sourceTransactionID: String
    let sourceAttemptID: String
    let root: ModelPreparationRootLocator
    let sourceRecordSHA256: String
    ...
    init(sourceTransactionID: String, sourceAttemptID: String, root: ModelPreparationRootLocator, sourceRecordSHA256: String) throws {
        try ModelPreparationContracts.requireUUIDv4(sourceTransactionID, field: "source_transaction_id")
        try ModelPreparationContracts.requireUUIDv4(sourceAttemptID, field: "source_attempt_id")
        try ModelPreparationContracts.requireHex64(sourceRecordSHA256, field: "source_record_sha256")
        self.sourceTransactionID = sourceTransactionID
    ...
    init(from decoder: Decoder) throws {
        ...
        try self.init(
            ...
            sourceAttemptID: try container.decode(String.self, forKey: .sourceAttemptID),
            root: try container.decode(ModelPreparationRootLocator.self, forKey: .root),
            sourceRecordSHA256: try container.decode(String.self, forKey: .sourceRecordSHA256)
        )
    }
}

struct ModelPreparationStagingSourcesRecord: Codable, Equatable, Sendable {
    let schema: String
    let entries: [ModelPreparationStagingSourceEntry]
    ...
    init(schema: String = "model_catalog_staging_sources.v1", entries: [ModelPreparationStagingSourceEntry]) throws {
        guard schema == "model_catalog_staging_sources.v1" else { throw ModelPreparationContractError.malformed("schema") }
        guard entries.count <= 256 else { throw ModelPreparationContractError.malformed("entries") }
        let keys = entries.map { "\($0.sourceTransactionID)/\($0.sourceAttemptID)" }
        guard keys == keys.sorted(), Set(keys).count == keys.count else {
            throw ModelPreparationContractError.malformed("entries")
        }
        self.schema = schema
        self.entries = entries
        _ = try ModelPreparationContracts.encode(self, maxBytes: ModelPreparationContracts.stagingSourcesMaxBytes)
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        try self.init(
            schema: try container.decode(String.self, forKey: .schema),
            entries: try container.decode([ModelPreparationStagingSourceEntry].self, forKey: .entries)
        )
    }
}
...
struct ModelPreparationPublicationReceipt: Codable, Equatable, Sendable {
    ...
    init(
        ...
        self.tuple = tuple
        self.tupleSHA256 = tupleSHA256
        try ModelPreparationContracts.requireRFC3339Timestamp(publishedAt, field: "published_at")
...
struct ModelPreparationInventoryRecord: Codable, Equatable, Sendable {
    ...
    init(schema: String = "model_catalog_published_inventory.v1", root: ModelPreparationRootLocator, targets: [ModelPreparationCleanupTarget], generatedAt: String) throws {
        ...
        try ModelPreparationContracts.requireRFC3339Timestamp(generatedAt, field: "generated_at")
...
struct ModelPreparationCleanupRecord: Codable, Equatable, Sendable {
    ...
    private static let sharedKeys: Set<String> = [
        CodingKeys.schema.stringValue,
        CodingKeys.targetKind.stringValue,
        CodingKeys.phase.stringValue,
        CodingKeys.transactionID.stringValue,
        CodingKeys.attemptID.stringValue,
        CodingKeys.eventModelKey.stringValue,
        CodingKeys.root.stringValue,
        CodingKeys.tuple.stringValue,
        CodingKeys.tupleSHA256.stringValue,
        CodingKeys.finalLeaf.stringValue,
        CodingKeys.tombstoneLeaf.stringValue,
        CodingKeys.expectedBytes.stringValue,
        CodingKeys.expectedFiles.stringValue,
    ]

    init(
        schema: String = "model_catalog_cleanup_record.v2",
        ...
        sourceAttemptID: String? = nil,
        sourceRecordSHA256: String? = nil,
        receipt: ModelPreparationPublicationReceipt? = nil,
        receiptSHA256: String? = nil,
        artifactIdentityDigest: String? = nil
    ) throws {
        guard schema == "model_catalog_cleanup_record.v2" else { throw ModelPreparationContractError.malformed("schema") }
        try ModelPreparationContracts.requireUUIDv4(transactionID, field: "transaction_id")
        try ModelPreparationContracts.requireUUIDv4(attemptID, field: "attempt_id")
        try ModelPreparationContracts.requireSafeString(eventModelKey, field: "event_model_key")
        guard eventModelKey == tuple.eventModelKey else {
        ...
        try ModelPreparationContracts.requireSafeString(finalLeaf, field: "final_leaf", maxUTF8Bytes: 512)
        try ModelPreparationContracts.requireSafeString(tombstoneLeaf, field: "tombstone_leaf", maxUTF8Bytes: 512)
        guard !finalLeaf.hasPrefix("/"), !tombstoneLeaf.hasPrefix("/"),
              !finalLeaf.contains(".."), !tombstoneLeaf.contains(".."),
              !finalLeaf.contains("\\"), !tombstoneLeaf.contains("\\") else {
            throw ModelPreparationContractError.malformed("leaf")
        }
        switch targetKind {
        case .staging:
            guard let sourceTransactionID, let sourceAttemptID, let sourceRecordSHA256 else {
                throw ModelPreparationContractError.malformed("staging branch")
            }
            guard receipt == nil, receiptSHA256 == nil, artifactIdentityDigest == nil else {
                throw ModelPreparationContractError.malformed("staging branch")
            }
            try ModelPreparationContracts.requireUUIDv4(sourceTransactionID, field: "source_transaction_id")
            try ModelPreparationContracts.requireUUIDv4(sourceAttemptID, field: "source_attempt_id")
            try ModelPreparationContracts.requireHex64(sourceRecordSHA256, field: "source_record_sha256")
            guard finalLeaf == "work/staging/\(sourceTransactionID)/\(sourceAttemptID)",
                  tombstoneLeaf == "work/staging/\(sourceTransactionID)/.tombstone-\(transactionID)" else {
                throw ModelPreparationContractError.malformed("leaf")
            }
        case .published:
            guard let receipt, let receiptSHA256, let artifactIdentityDigest else {
                throw ModelPreparationContractError.malformed("published branch")
            }
            guard sourceTransactionID == nil, sourceAttemptID == nil, sourceRecordSHA256 == nil else {
                throw ModelPreparationContractError.malformed("published branch")
            }
            guard receipt.eventModelKey == eventModelKey else {
                throw ModelPreparationContractError.bindingMismatch("event_model_key")
            }
            guard receipt.root == root else { throw ModelPreparationContractError.bindingMismatch("root") }
            guard receipt.tuple == tuple else { throw ModelPreparationContractError.bindingMismatch("tuple") }
            guard receipt.tupleSHA256 == tupleSHA256 else {
                throw ModelPreparationContractError.bindingMismatch("tuple_sha256")
            }
            try ModelPreparationContracts.requireHex64(receiptSHA256, field: "receipt_sha256")
            ...
            guard finalLeaf == "objects/\(artifactIdentityDigest)",
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let targetKind = try container.decode(ModelPreparationCleanupTargetKind.self, forKey: .targetKind)
        let present = Set(container.allKeys.map(\.stringValue))
        let expected: Set<String>
        switch targetKind {
        case .staging:
            expected = Self.sharedKeys.union([
                CodingKeys.sourceTransactionID.stringValue,
                CodingKeys.sourceAttemptID.stringValue,
                CodingKeys.sourceRecordSHA256.stringValue,
            ])
        case .published:
            expected = Self.sharedKeys.union([
                CodingKeys.receipt.stringValue,
                CodingKeys.receiptSHA256.stringValue,
                CodingKeys.artifactIdentityDigest.stringValue,
            ])
        }
        guard present == expected else { throw ModelPreparationContractError.malformed("keys") }
        try self.init(
            ...
            sourceAttemptID: try container.decodeIfPresent(String.self, forKey: .sourceAttemptID),
            sourceRecordSHA256: try container.decodeIfPresent(String.self, forKey: .sourceRecordSHA256),
            receipt: try container.decodeIfPresent(ModelPreparationPublicationReceipt.self, forKey: .receipt),
            receiptSHA256: try container.decodeIfPresent(String.self, forKey: .receiptSHA256),
            artifactIdentityDigest: try container.decodeIfPresent(String.self, forKey: .artifactIdentityDigest)
        )
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(tuple, forKey: .tuple)
        try container.encode(tupleSHA256, forKey: .tupleSHA256)
        try container.encode(finalLeaf, forKey: .finalLeaf)
        try container.encode(tombstoneLeaf, forKey: .tombstoneLeaf)
        try container.encode(expectedBytes, forKey: .expectedBytes)
        try container.encode(expectedFiles, forKey: .expectedFiles)
        switch targetKind {
        case .staging:
            try container.encode(sourceTransactionID, forKey: .sourceTransactionID)
            try container.encode(sourceAttemptID, forKey: .sourceAttemptID)
...
enum ModelPreparationPrivateStateEnvelopeKind: String, Codable, CaseIterable, Sendable {
    ...
    case deletion
    case stagingSources = "staging_sources"
    case failedDispatchPending = "failed_dispatch_pending"
}
...
struct ModelPreparationPrivateStateEnvelope: Codable, Equatable, Sendable {
    ...
    init(
        ...
        try Self.validatePayload(payload, for: recordKind)
    ...
    static func expectedTargetLeaf(for recordKind: ModelPreparationPrivateStateEnvelopeKind) -> String {
        ...
        case .stagingSources: return "staging-sources.json"
        case .failedDispatchPending: return "failed-dispatch-pending.json"
        }
    }
    ...
    static func payloadMaxBytes(for recordKind: ModelPreparationPrivateStateEnvelopeKind) -> Int {
        ...
        case .cancel: return ModelPreparationContracts.cancelMarkerMaxBytes
        ...
        case .stagingSources: return ModelPreparationContracts.stagingSourcesMaxBytes
        case .failedDispatchPending: return ModelPreparationContracts.failedDispatchMaxBytes
        }
    }

    static func validatePayload(_ payload: Data, for recordKind: ModelPreparationPrivateStateEnvelopeKind) throws {
        switch recordKind {
        case .reservations:
            _ = try ModelPreparationContracts.decode(ModelPreparationReservationsHistoryRecord.self, from: payload, maxBytes: ModelPreparationContracts.reservationHistoryMaxBytes)
        case .active:
            _ = try ModelPreparationContracts.decode(ModelPreparationActiveRecord.self, from: payload, maxBytes: ModelPreparationContracts.activeRecordMaxBytes)
        case .cancel:
            _ = try ModelPreparationContracts.decode(ModelPreparationCancelMarker.self, from: payload, maxBytes: ModelPreparationContracts.cancelMarkerMaxBytes)
        case .publishedInventory:
            _ = try ModelPreparationContracts.decode(ModelPreparationInventoryRecord.self, from: payload, maxBytes: ModelPreparationContracts.inventoryMaxBytes)
        case .deletion:
            _ = try ModelPreparationContracts.decode(ModelPreparationCleanupRecord.self, from: payload, maxBytes: ModelPreparationContracts.deletionRecordMaxBytes)
        case .stagingSources:
            _ = try ModelPreparationContracts.decode(ModelPreparationStagingSourcesRecord.self, from: payload, maxBytes: ModelPreparationContracts.stagingSourcesMaxBytes)
        case .failedDispatchPending:
            _ = try ModelPreparationContracts.decode(ModelPreparationFailedDispatchRecord.self, from: payload, maxBytes: ModelPreparationContracts.failedDispatchMaxBytes)
        }
    }
...
private struct ModelPreparationJSONDuplicateKeyScanner {
    ...
    private mutating func parseValue(depth: Int) throws {
        guard depth <= ModelPreparationContracts.maxJSONNestingDepth else {
            throw ModelPreparationContractError.malformed("json depth")
        }
...
private enum ModelPreparationJSONShapeValidator {
    private static let tupleKeySet = Set(ModelPreparationTupleRecord.CodingKeys.allCases.map(\.stringValue))
    private static let rootLocatorKeySet = Set(ModelPreparationRootLocator.CodingKeys.allCases.map(\.stringValue))
    private static let rootIdentityKeySet = Set(ModelPreparationRootIdentityRecord.CodingKeys.allCases.map(\.stringValue))
    private static let actionKeySet = Set(ModelPreparationAction.CodingKeys.allCases.map(\.stringValue))
    private static let cleanupTargetKeySet = Set(ModelPreparationCleanupTarget.CodingKeys.allCases.map(\.stringValue))
    private static let reservationBaseKeySet: Set<String> = [
        "schema",
        "transaction_id",
        "transaction_kind",
        "event_model_key",
    ...
    private static let reservationCleanupStagingKeySet = reservationBaseKeySet.union([
        "source_transaction_id",
        "source_attempt_id",
        "source_record_sha256",
    ])
    private static let failedDispatchKeySet = Set(ModelPreparationFailedDispatchRecord.CodingKeys.allCases.map(\.stringValue))
    private static let terminalHistoryKeySet = Set(ModelPreparationTerminalHistoryRecord.CodingKeys.allCases.map(\.stringValue))
    private static let reservationsHistoryKeySet = Set(ModelPreparationReservationsHistoryRecord.CodingKeys.allCases.map(\.stringValue))
    private static let activeCountersKeySet = Set(ModelPreparationActiveCounters.CodingKeys.allCases.map(\.stringValue))
    private static let barrierProgressKeySet = Set(ModelPreparationBarrierProgress.CodingKeys.allCases.map(\.stringValue))
    private static let terminalResultKeySet = Set(ModelPreparationTerminalResult.CodingKeys.allCases.map(\.stringValue))
    private static let activeRecordKeySet = Set(ModelPreparationActiveRecord.CodingKeys.allCases.map(\.stringValue))
    private static let cancelAcknowledgementKeySet = Set(ModelPreparationCancelAcknowledgement.CodingKeys.allCases.map(\.stringValue))
    private static let transactionEventKeySet = Set(ModelPreparationTransactionEvent.CodingKeys.allCases.map(\.stringValue))
    private static let transactionProgressKeySet = Set(ModelPreparationTransactionEvent.Progress.CodingKeys.allCases.map(\.stringValue))
    private static let cancelMarkerKeySet = Set(ModelPreparationCancelMarker.CodingKeys.allCases.map(\.stringValue))
    private static let stagingSourceKeySet = Set(ModelPreparationStagingSourceRecord.CodingKeys.allCases.map(\.stringValue))
    private static let stagingSourceEntryKeySet = Set(ModelPreparationStagingSourceEntry.CodingKeys.allCases.map(\.stringValue))
    private static let stagingSourcesKeySet = Set(ModelPreparationStagingSourcesRecord.CodingKeys.allCases.map(\.stringValue))
    private static let publicationReceiptKeySet = Set(ModelPreparationPublicationReceipt.CodingKeys.allCases.map(\.stringValue))
    private static let inventoryKeySet = Set(ModelPreparationInventoryRecord.CodingKeys.allCases.map(\.stringValue))
    private static let cleanupSharedKeySet: Set<String> = [
        "schema",
        "target_kind",
        "phase",
        "transaction_id",
    ...
    private static let cleanupStagingKeySet = cleanupSharedKeySet.union([
        "source_transaction_id",
        "source_attempt_id",
        "source_record_sha256",
    ])
    private static let cleanupPublishedKeySet = cleanupSharedKeySet.union([
        "receipt",
        "receipt_sha256",
        "artifact_identity_digest",
    ])
    private static let privateStateEnvelopeKeySet = Set(ModelPreparationPrivateStateEnvelope.CodingKeys.allCases.map(\.stringValue))
    ...
    private static let allowedKeySets: Set<Set<String>> = [
        ...
        Set(ModelPreparationTerminalHistoryRecord.CodingKeys.allCases.map(\.stringValue)),
        Set(ModelPreparationReservationsHistoryRecord.CodingKeys.allCases.map(\.stringValue)),
        ...
        Set(ModelPreparationCancelMarker.CodingKeys.allCases.map(\.stringValue)),
        Set(ModelPreparationStagingSourceRecord.CodingKeys.allCases.map(\.stringValue)),
        Set(ModelPreparationStagingSourceEntry.CodingKeys.allCases.map(\.stringValue)),
        Set(ModelPreparationStagingSourcesRecord.CodingKeys.allCases.map(\.stringValue)),
    ...
    static func validate(_ data: Data, expectedType: Any.Type) throws {
        let value = try JSONSerialization.jsonObject(with: data)
        if let expectedKeys = topLevelKeySet(for: expectedType) {
            guard let object = value as? [String: Any], Set(object.keys) == expectedKeys else {
                throw ModelPreparationContractError.malformed("keys")
            }
        }
    ...
    private static func validate(_ value: Any, depth: Int) throws {
        ...
        if let object = value as? [String: Any] {
            ...
            try validateSchemaSpecificKeys(schema: object["schema"] as? String, keys: keys)
            try validateContextSpecificMembers(of: object)
            for member in object.values {
                try validate(member, depth: depth + 1)
            }
    ...
    private static func topLevelKeySet(for type: Any.Type) -> Set<String>? {
        if type == ModelPreparationRootLocator.self { return rootLocatorKeySet }
        if type == ModelPreparationRootIdentityRecord.self { return rootIdentityKeySet }
        if type == ModelPreparationAction.self { return actionKeySet }
        if type == ModelPreparationCleanupTarget.self { return cleanupTargetKeySet }
        if type == ModelPreparationTupleRecord.self { return tupleKeySet }
        if type == ModelPreparationFailedDispatchRecord.self { return failedDispatchKeySet }
        if type == ModelPreparationTerminalHistoryRecord.self { return terminalHistoryKeySet }
        if type == ModelPreparationReservationsHistoryRecord.self { return reservationsHistoryKeySet }
        if type == ModelPreparationActiveCounters.self { return activeCountersKeySet }
        if type == ModelPreparationBarrierProgress.self { return barrierProgressKeySet }
        if type == ModelPreparationTerminalResult.self { return terminalResultKeySet }
        if type == ModelPreparationActiveRecord.self { return activeRecordKeySet }
        if type == ModelPreparationCancelAcknowledgement.self { return cancelAcknowledgementKeySet }
        if type == ModelPreparationTransactionEvent.self { return transactionEventKeySet }
        if type == ModelPreparationTransactionEvent.Progress.self { return transactionProgressKeySet }
        if type == ModelPreparationCancelMarker.self { return cancelMarkerKeySet }
        if type == ModelPreparationStagingSourceRecord.self { return stagingSourceKeySet }
        if type == ModelPreparationStagingSourceEntry.self { return stagingSourceEntryKeySet }
        if type == ModelPreparationStagingSourcesRecord.self { return stagingSourcesKeySet }
        if type == ModelPreparationPublicationReceipt.self { return publicationReceiptKeySet }
        if type == ModelPreparationInventoryRecord.self { return inventoryKeySet }
        if type == ModelPreparationPrivateStateEnvelope.self { return privateStateEnvelopeKeySet }
        return nil
    }

    private static func validateSchemaSpecificKeys(schema: String?, keys: Set<String>) throws {
        switch schema {
        case "model_catalog_preparation_tuple.v1":
            guard keys == tupleKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_reservation.v4":
            return
        case "model_catalog_failed_dispatch.v1":
            guard keys == failedDispatchKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_terminal_history.v1":
            guard keys == terminalHistoryKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_reservations_history.v1":
            guard keys == reservationsHistoryKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_active_attempt.v1":
            guard keys == activeRecordKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_transaction_cancel_ack.v1":
            guard keys == cancelAcknowledgementKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_transaction_event.v1":
            guard keys == transactionEventKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_cancel_marker.v1":
            guard keys == cancelMarkerKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_staging_source.v1":
            guard keys == stagingSourceKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_staging_sources.v1":
            guard keys == stagingSourcesKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_publication_receipt.v1":
            guard keys == publicationReceiptKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_published_inventory.v1":
            guard keys == inventoryKeySet else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_cleanup_record.v2":
            return
        case "model_catalog_private_state_envelope.v1":
            guard keys == privateStateEnvelopeKeySet else { throw ModelPreparationContractError.malformed("keys") }
        default:
            return
        }
    }

    private static func validateContextSpecificMembers(of object: [String: Any]) throws {
        switch object["schema"] as? String {
        case "model_catalog_reservation.v4":
            let expected = object["transaction_kind"] as? String == ModelPreparationTransactionKind.cleanupStaging.rawValue
                ? reservationCleanupStagingKeySet
                : reservationBaseKeySet
            guard Set(object.keys) == expected else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_cleanup_record.v2":
            let expected: Set<String>
            switch object["target_kind"] as? String {
            case ModelPreparationCleanupTargetKind.staging.rawValue:
                expected = cleanupStagingKeySet
            case ModelPreparationCleanupTargetKind.published.rawValue:
                expected = cleanupPublishedKeySet
            default:
                throw ModelPreparationContractError.malformed("target_kind")
            }
            guard Set(object.keys) == expected else { throw ModelPreparationContractError.malformed("keys") }
        case "model_catalog_staging_sources.v1":
            ...
            for entry in entries {
                guard let entry = entry as? [String: Any], Set(entry.keys) == stagingSourceEntryKeySet else {
                    throw ModelPreparationContractError.malformed("entries")
                }
            }
        default:
