...
enum ModelPreparationSecureFilesystem {
    ...
    static func openExistingPrivateDirectory(at url: URL) throws -> Directory {
        try openPrivateDirectory(at: url, createMissing: false)
    }

    static func openOrCreatePrivateDirectory(at url: URL) throws -> Directory {
        try openPrivateDirectory(at: url, createMissing: true)
    }

    private static func openPrivateDirectory(at url: URL, createMissing: Bool) throws -> Directory {
    ...
    static func syncFileAndFullSync(fd: Int32, path: String) throws {
        ...
        #if os(macOS)
        // F_FULLFSYNC is a best-effort strengthening over fsync. Some local
        // filesystems and descriptor kinds reject it after a successful fsync.
