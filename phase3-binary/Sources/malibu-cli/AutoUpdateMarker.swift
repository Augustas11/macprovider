...
struct AutoUpdateMarkerStore: @unchecked Sendable {
    ...
    var legacyPathEntrypointURL: URL {
    ...
    var installerContractBinaryURL: URL {
        homeDirectory.appendingPathComponent("macprovider/malibu-cli")
    }

    var legacyInstallerContractBinaryURL: URL {
    ...
    /// 3. Installer contract `~/macprovider/malibu-cli`
    ...
    func resolveCanonicalInstallBinary(
        ...
        let authorities: [URL?] = [
            ...
            legacyInstallerContractBinaryURL,
        ]
    ...
    func rollbackBackupPath(binaryURL: URL, updateID: String) -> URL {
        binaryURL.deletingLastPathComponent()
            .appendingPathComponent(".malibu-cli.rollback-\(updateID)")
    }

    private func legacyRollbackBackupPath(binaryURL: URL, updateID: String) -> URL {
        binaryURL.deletingLastPathComponent()
    ...
    func releaseRollbackBackupPath(binaryURL: URL, updateID: String) -> URL {
        binaryURL.deletingLastPathComponent()
            .appendingPathComponent(".malibu-cli.release-rollback-\(updateID)", isDirectory: true)
    }

    private func legacyReleaseRollbackBackupPath(binaryURL: URL, updateID: String) -> URL {
        binaryURL.deletingLastPathComponent()
    ...
    func successSentinelPath(binaryURL: URL, updateID: String) -> URL {
        binaryURL.deletingLastPathComponent()
            .appendingPathComponent(".malibu-cli.success-\(updateID)")
    }
    ...
    func restoreBackup(_ marker: AutoUpdatePendingMarker) throws {
        ...
        if let releaseBackupPath = marker.releaseBackupPath {
            ...
            let staging = targetURL.deletingLastPathComponent()
                .appendingPathComponent(".malibu-cli.release-restore-\(UUID().uuidString.lowercased())", isDirectory: true)
        ...
        // stale `~/.local/bin/malibu-cli` after rollback recreates the
    ...
    func activateReleasePayload(
        ...
        let staging = liveDirectory.appendingPathComponent(
            ".malibu-cli.release-activation-\(UUID().uuidString.lowercased())",
            isDirectory: true
        )
        try fileManager.createDirectory(at: staging, withIntermediateDirectories: false, attributes: [.posixPermissions: 0o700])
        defer { try? fileManager.removeItem(at: staging) }
    ...
    private func launchdProgramBinaryURL() -> URL? {
        ...
        guard isSupportedInstallBinaryName(url.lastPathComponent) else { return nil }
    ...
    private func usableCanonicalBinary(_ url: URL) -> URL? {
        ...
        if standardized.path == pathEntrypointURL.standardizedFileURL.path
            || standardized.path == legacyPathEntrypointURL.standardizedFileURL.path {
            return nil
        }
        guard standardized.lastPathComponent == "malibu-cli"
            || standardized.path == legacyInstallerContractBinaryURL.standardizedFileURL.path
        else { return nil }
        var info = stat()
    ...
    private func isSupportedInstallBinaryName(_ name: String) -> Bool {
        name == "malibu-cli" || name == "macprovider-cli"
    }

    /// Resolves install authority and converges `~/.local/bin/malibu-cli`
    ...
    func ensurePathEntrypointMatchesInstallAuthority(
        ...
        guard var canonical = resolveCanonicalInstallBinary(
            launchedExecutableURL: launchedExecutableURL
        ) else {
            return nil
        }
        canonical = try promoteLegacyInstallBinaryIfNeeded(canonical)
        _ = try convergePathEntrypoint(to: canonical)
        _ = try convergeLegacyEntrypoints(to: canonical)
        return canonical
    }
    ...
    func convergePathEntrypoint(to canonicalBinary: URL) throws -> Bool {
        try convergeEntrypoint(pathEntrypointURL, temporaryPrefix: ".malibu-cli.entrypoint", to: canonicalBinary)
    }

    @discardableResult
    func convergeLegacyEntrypoints(to canonicalBinary: URL) throws -> Bool {
        var repaired = false
        for entrypoint in [legacyPathEntrypointURL, legacyInstallerContractBinaryURL] {
            repaired = try convergeEntrypoint(
                entrypoint,
                temporaryPrefix: ".macprovider-cli.legacy-entrypoint",
                to: canonicalBinary
            ) || repaired
        }
        return repaired
    ...
    private func promoteLegacyInstallBinaryIfNeeded(_ canonicalBinary: URL) throws -> URL {
        let legacy = legacyInstallerContractBinaryURL.standardizedFileURL
        let current = installerContractBinaryURL.standardizedFileURL
        guard canonicalBinary.standardizedFileURL.path == legacy.path else {
            return canonicalBinary
        }
        if let usable = usableCanonicalBinary(current) {
            return usable
        }
        guard legacy.deletingLastPathComponent().path == current.deletingLastPathComponent().path else {
            return canonicalBinary
        }
        do {
            try fileManager.moveItem(at: legacy, to: current)
            let temporaryLink = legacy.deletingLastPathComponent().appendingPathComponent(
                ".macprovider-cli.legacy-promote-\(UUID().uuidString.lowercased())"
            )
            guard symlink(current.path, temporaryLink.path) == 0 else {
                throw AutoUpdateMarkerError.pathEntrypointUnsafe("legacy_promote_symlink_create_failed")
            }
            guard rename(temporaryLink.path, legacy.path) == 0 else {
                unlink(temporaryLink.path)
                throw AutoUpdateMarkerError.pathEntrypointUnsafe("legacy_promote_symlink_swap_failed")
            }
            fsyncDirectory(current.deletingLastPathComponent())
            return current
        } catch {
            throw AutoUpdateMarkerError.pathEntrypointUnsafe("legacy_install_promote_failed")
        }
    }
    ...
    func validateRollbackTopology(_ marker: AutoUpdatePendingMarker) throws {
        ...
        let legacyExpectedBackup = legacyRollbackBackupPath(binaryURL: targetURL, updateID: marker.updateID)
        let acceptedBackup = targetURL.lastPathComponent == "macprovider-cli"
            ? legacyExpectedBackup
            : expectedBackup
        guard marker.backupPath == acceptedBackup.path else {
            throw AutoUpdateMarkerError.trustedRootInvalid("backup_path_derivation_mismatch")
        }
        let targetDir = targetURL.deletingLastPathComponent()
        guard acceptedBackup.deletingLastPathComponent().path == targetDir.path else {
        ...
        case let (.some(path), .some(_)):
            ...
            let legacyExpectedReleaseBackup = legacyReleaseRollbackBackupPath(binaryURL: targetURL, updateID: marker.updateID)
            let acceptedReleaseBackup = targetURL.lastPathComponent == "macprovider-cli"
                ? legacyExpectedReleaseBackup
                : expectedReleaseBackup
            guard path == acceptedReleaseBackup.path,
                  acceptedReleaseBackup.deletingLastPathComponent().path == targetDir.path
            else {
    ...
    private func validateReleasePayload(at directory: URL, newBinary: URL) throws {
        ...
        guard newBinary.deletingLastPathComponent().standardizedFileURL == directory.standardizedFileURL,
              newBinary.lastPathComponent == "malibu-cli",
    ...
    private func activateExternalLocalMembers(
        ...
        let staging = installDirectory.appendingPathComponent(
            ".malibu-cli.external-activation-\(UUID().uuidString.lowercased())",
            isDirectory: true
        )
        try fileManager.createDirectory(
            at: staging,
    ...
    private func validateProviderPlist(_ data: Data, installDirectory: URL) throws {
        guard let plist = try PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any],
              ...
              plist["ProgramArguments"] as? [String] == [
                  installDirectory.appendingPathComponent("malibu-cli").path,
    ...
    private func validateWatchdogPlist(_ data: Data, installDirectory: URL, coordinatorHost: String) throws {
        guard let plist = try PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any],
              ...
              let environment = plist["EnvironmentVariables"] as? [String: Any],
              environment["MACPROVIDER_BINARY_PATH"] as? String == installDirectory.appendingPathComponent("malibu-cli").path,
    ...
    func successSentinels(in binaryDirectory: URL) -> [URL] {
        ...
        return contents.filter { $0.lastPathComponent.hasPrefix(".malibu-cli.success-") }
    }
    ...
    func rollbackBackups(in binaryDirectory: URL) -> [URL] {
        ...
        return contents.filter {
            $0.lastPathComponent.hasPrefix(".malibu-cli.rollback-")
                || $0.lastPathComponent.hasPrefix(".malibu-cli.release-rollback-")
        }
    }
