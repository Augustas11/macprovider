...
struct CompatibilityArtifactIndex: Equatable, Sendable {
    ...
    static func loadValidated(
        ...
        for role in requiredRoles {
            let nameMatchesRole: (String) -> Bool = { name in
                if role == "provider_cli" {
                    return name == "malibu-cli-\(tag)-darwin-arm64.tar.gz"
                        || name == "macprovider-cli-\(tag)-darwin-arm64.tar.gz"
                }
                return name == expectedNames[role]
            }
            guard let row = rawArtifacts[role] as? [String: Any],
                  Set(row.keys) == Set(["name", "sha256"]),
                  let name = row["name"] as? String,
                  nameMatchesRole(name),
    ...
    private static func expectedArtifactNames(tag: String) -> [String: String] {
        [
            ...
            "provider_cli": "malibu-cli-\(tag)-darwin-arm64.tar.gz",
        ]
    }
...
struct CompatibilitySetManifest: Equatable, Sendable {
    ...
    /// `~/.local/bin/malibu-cli` as a stale copy has no sibling set, while
    /// `~/macprovider/malibu-cli` does.
    ///
    ...
    private static func validateCatalog(
        ...
        let names = [
            ...
            "demand-rank.json.sig", "rate-card.json", "rate-card.json.sig", "release.json",
            "tier2-catalog.json", "trusted-keys.json",
        ]
