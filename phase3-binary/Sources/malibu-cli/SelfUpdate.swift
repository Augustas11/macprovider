...
struct SelfUpdate {
    ...
    func prepareValidatedUpdate(
        ...
        let canonicalTarballName = "malibu-cli-\(release.tagName)-darwin-arm64.tar.gz"
    ...
    func prepareValidatedUpdate(
        ...
        let tarballName = "malibu-cli-\(tag)-darwin-arm64.tar.gz"
    ...
    private func latestRelease() async throws -> GitHubRelease {
        ...
        request.addValue("malibu-cli/\(currentVersion)", forHTTPHeaderField: "user-agent")
    ...
    private static func validateStagedMalibuBundle(
        ...
        let embeddedCLI = app.appendingPathComponent("Contents/MacOS/malibu-cli")
    ...
    private static func findBinary(in directory: URL) throws -> URL {
        ...
        var matches: [URL] = []
        for case let url as URL in enumerator where url.lastPathComponent == "malibu-cli" {
...
struct ProviderReleasePayloadTransaction {
    ...
    init(
        ...
        backupDirectory = installDirectory.appendingPathComponent(
            ".malibu-cli.manual-rollback-\(UUID().uuidString.lowercased())",
            isDirectory: true
        )
        self.markerStore = markerStore
        ...
        do {
            ...
            guard currentEntries.contains(where: { $0.standardizedFileURL == currentBinary.standardizedFileURL }) else {
                throw UpdateError.missingReleaseResource("installed malibu-cli")
            }
    ...
    func activate(from payloadDirectory: URL, newBinary: URL) throws {
        ...
        let stagingDirectory = installDirectory.appendingPathComponent(
            ".malibu-cli.activation-\(UUID().uuidString.lowercased())",
            isDirectory: true
        )
        try fileManager.createDirectory(
            at: stagingDirectory,
        ...
        for entry in try Self.ownedEntries(in: stagingDirectory, fileManager: fileManager)
            where entry.lastPathComponent != "malibu-cli"
        {
        ...
        try markerStore.atomicCopyNoFollow(
            from: stagingDirectory.appendingPathComponent("malibu-cli"),
            to: currentBinary,
            mode: 0o755
        )
    }
    ...
    func restore() throws {
        ...
        for entry in backupEntries where entry.lastPathComponent != "malibu-cli" {
            try fileManager.copyItem(
                at: entry,
                to: installDirectory.appendingPathComponent(entry.lastPathComponent, isDirectory: entry.hasDirectoryPath)
            )
        }
        let backupBinary = backupDirectory.appendingPathComponent("malibu-cli")
        guard fileManager.fileExists(atPath: backupBinary.path) else {
            throw UpdateError.missingReleaseResource("rollback malibu-cli")
        }
    ...
    private static func validatedPayloadEntries(
        ...
        guard entries.contains(where: { $0.standardizedFileURL == newBinary.standardizedFileURL }) else {
            throw UpdateError.missingReleaseResource("malibu-cli")
        }
...
enum UpdateError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .missingExtractedBinary:
            return "Downloaded archive does not contain malibu-cli"
        case .currentBinaryUnknown:
