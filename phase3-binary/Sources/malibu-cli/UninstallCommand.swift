...
struct UninstallCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Stop malibu-cli and remove installed artifacts."
    )
    ...
    func run() async throws {
        ...
        var legacySymlinkPaths = Self.defaultLegacySymlinkPaths(home: home, installPrefix: manifest.installPrefix)
        for manifestLegacySymlinkPath in manifest.legacySymlinkPaths ?? [] where !legacySymlinkPaths.contains(manifestLegacySymlinkPath) {
            legacySymlinkPaths.append(manifestLegacySymlinkPath)
        }
        for legacySymlinkPath in legacySymlinkPaths {
            removeIfPresent(URL(fileURLWithPath: legacySymlinkPath), allowed: allowed.symlinks, label: "legacy binary symlink", warnings: &warnings)
        }
    ...
    static func artifactPaths(home: URL) -> ArtifactPaths {
        ArtifactPaths(
            binary: home.appendingPathComponent(".local/bin/malibu-cli"),
    ...
    struct InstallManifest: Codable, Equatable {
        ...
        init(
            ...
            self.legacySymlinkPaths = legacySymlinkPaths
        ...
        enum CodingKeys: String, CodingKey {
            ...
            case legacySymlinkPaths = "legacy_symlink_paths"
    ...
    static func legacyManifest(home: URL) -> InstallManifest {
        ...
        return InstallManifest(
            ...
            legacySymlinkPaths: Self.defaultLegacySymlinkPaths(home: home, installPrefix: paths.supportDirectory.path),
            launchdPlists: [
                paths.plist.path,
    ...
    static func defaultLegacySymlinkPaths(home: URL, installPrefix: String) -> [String] {
        [
            home.appendingPathComponent(".local/bin/macprovider-cli").path,
            URL(fileURLWithPath: installPrefix).appendingPathComponent("macprovider-cli").path,
        ]
    }
    ...
    static func allowedRemovalPaths(home: URL, manifest: InstallManifest) throws -> AllowedRemovalPaths {
        ...
        return AllowedRemovalPaths(
            ...
            symlinks: [
                paths.binary.path,
            ] + Self.defaultLegacySymlinkPaths(home: home, installPrefix: installPrefix.path),
            binaries: [
                installPrefix.appendingPathComponent("malibu-cli").path,
    ...
    private func removePathMarker(from file: URL) throws {
        ...
        let filtered = original
            ...
            .filter { $0.trimmingCharacters(in: .whitespaces) != #"export PATH="$HOME/.local/bin:$PATH" # Added by malibu-cli"# }
            .joined(separator: "\n")
