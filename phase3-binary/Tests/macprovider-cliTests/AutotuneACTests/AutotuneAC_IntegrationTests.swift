...
final class AutotuneACIntegrationTests: XCTestCase {
    ...
    /// NOT spawn a real launchd-managed `macprovider-cli serve` process. See
    ...
    /// enum to `exit_reason = 'provider_conflict'` + stderr message. It does
    /// NOT spawn a real foreground `macprovider-cli serve` process. See
    ...
    /// AC-6 real detection: foreground conflict via real `macprovider-cli serve`
