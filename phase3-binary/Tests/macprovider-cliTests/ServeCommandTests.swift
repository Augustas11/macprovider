...
final class ServeCommandTests: XCTestCase {
    ...
    private func assertCatalogBoundSnapshotPreflight(
        ...
        let catalogJSON = """
        {"version":"test-catalog","generated_at":"2026-07-03T00:00:00Z","source":"operator_curated_autotune_candidate_catalog","rows":{"\(key)":{"model_id":"\(modelID)","model_revision":"\(revision)","model_sha256":"\(artifactSHA)","min_ram_gb":1,"min_bandwidth_tier":"C","bench_gate":{"min_sustained_tps":1,"max_4k_ttft_ms":1000},"runtime_status":"\(runtimeStatus)"}}}
        """
        let rateCardJSON = """
        {"version":"test-rate-card","generated_at":"2026-07-03T00:00:00Z","usd_per_million_credits":1.0,"rows":{"\(rateCardKey)":{"prompt_rate_per_mtok":1,"completion_rate_per_mtok":1,"provider_share_bps":9000,"global_multiplier_ppm":1000000}}}
        """
        ...
        let staticInputs = AutotuneStaticInputs(
            ...
            now: { ISO8601DateFormatter.autotuneInternet.date(from: "2026-07-03T12:00:00Z")! }
        )
        var config = AppConfig.defaults()
