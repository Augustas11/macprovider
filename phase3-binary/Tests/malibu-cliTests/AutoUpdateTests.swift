...
final class AutoUpdateTests: XCTestCase {
    ...
    func testSuccessCleanupLeavesSentinelUntilFinalize() throws {
        ...
        let binary = binaryDir.appendingPathComponent("malibu-cli")
        let backup = binaryDir.appendingPathComponent(".malibu-cli.rollback-aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee")
    ...
    func testLegacyMacProviderPendingMarkerRestoresBeforeCleanup() throws {
        let fixture = try TempHome()
        let store = AutoUpdateMarkerStore(homeDirectory: fixture.url)
        try store.ensureTrustedRoot()
        FileManager.default.createFile(
            atPath: store.lockURL.path,
            contents: Data(),
            attributes: [.posixPermissions: 0o600]
        )
        let binaryDirectory = fixture.url.appendingPathComponent(".local/bin", isDirectory: true)
        ...
        let backup = binaryDirectory.appendingPathComponent(
            ".macprovider-cli.rollback-aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"
        )
        try Data("new".utf8).write(to: binary)
        try Data("old".utf8).write(to: backup)
        let marker = AutoUpdatePendingMarker(
    ...
    func testFullReleasePendingMarkerRestoresAllOwnedResourcesAndRemovesNewOnlyBundle() throws {
        ...
        let binary = binaryDirectory.appendingPathComponent("malibu-cli")
    ...
    func testReleasePayloadActivationAndRollbackKeepBinaryResourcesAndCatalogTogether() throws {
        ...
        let liveBinary = live.appendingPathComponent("malibu-cli")
        let newBinary = payload.appendingPathComponent("malibu-cli")
    ...
    func testActivationConvergesStalePathEntrypointCopyWithNoSiblingSetToSymlink() throws {
        ...
        let entrypoint = localBin.appendingPathComponent("malibu-cli")
    ...
    func testResolveCanonicalInstallBinaryPrefersInstallerContractOverStalePathCopy() throws {
        ...
        let canonical = install.appendingPathComponent("malibu-cli")
    ...
    func testResolveCanonicalInstallBinaryPrefersLaunchdProgramArguments() throws {
        ...
        let otherBinary = other.appendingPathComponent("malibu-cli")
    ...
    func testEnsurePathEntrypointMatchesInstallAuthorityRepairsRegularFileAndNoopsSymlink() throws {
        ...
        try Data("stale-path-copy".utf8).write(to: entrypoint)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: entrypoint.path)
        let legacyPathEntrypoint = localBin.appendingPathComponent("macprovider-cli")
        try Data("stale-legacy-path-copy".utf8).write(to: legacyPathEntrypoint)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: legacyPathEntrypoint.path)
        let legacyInstallerEntrypoint = install.appendingPathComponent("macprovider-cli")
        try Data("stale-legacy-install-copy".utf8).write(to: legacyInstallerEntrypoint)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: legacyInstallerEntrypoint.path)
        ...
        for legacyEntrypoint in [legacyPathEntrypoint, legacyInstallerEntrypoint] {
            var legacyInfo = stat()
            XCTAssertEqual(lstat(legacyEntrypoint.path, &legacyInfo), 0)
            XCTAssertEqual(legacyInfo.st_mode & S_IFMT, S_IFLNK)
            XCTAssertEqual(
                try FileManager.default.destinationOfSymbolicLink(atPath: legacyEntrypoint.path),
                canonical.path
            )
            XCTAssertEqual(try String(contentsOf: legacyEntrypoint), "canonical-binary")
        }
        ...
        XCTAssertEqual(
            try FileManager.default.destinationOfSymbolicLink(atPath: legacyPathEntrypoint.path),
            canonical.path
        )
        XCTAssertEqual(
            try FileManager.default.destinationOfSymbolicLink(atPath: legacyInstallerEntrypoint.path),
            canonical.path
        )
    }

    func testEnsurePathEntrypointPromotesLegacyInstallBinary() throws {
        let fixture = try TempHome()
        let store = AutoUpdateMarkerStore(homeDirectory: fixture.url)
        try store.ensureTrustedRoot()
        let install = fixture.url.appendingPathComponent("macprovider", isDirectory: true)
        try FileManager.default.createDirectory(at: install, withIntermediateDirectories: false, attributes: [.posixPermissions: 0o700])
        let canonical = install.appendingPathComponent("malibu-cli")
        let legacy = install.appendingPathComponent("macprovider-cli")
        try Data("legacy-installed-new-code".utf8).write(to: legacy)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: legacy.path)

        let localBin = fixture.url.appendingPathComponent(".local/bin", isDirectory: true)
        try FileManager.default.createDirectory(at: localBin, withIntermediateDirectories: true, attributes: [.posixPermissions: 0o700])
        let pathEntrypoint = localBin.appendingPathComponent("malibu-cli")
        let legacyPathEntrypoint = localBin.appendingPathComponent("macprovider-cli")
        try Data("stale-path-copy".utf8).write(to: pathEntrypoint)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: pathEntrypoint.path)
        try Data("stale-legacy-path-copy".utf8).write(to: legacyPathEntrypoint)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: legacyPathEntrypoint.path)

        let repaired = try store.ensurePathEntrypointMatchesInstallAuthority(launchedExecutableURL: legacy)

        XCTAssertEqual(repaired?.standardizedFileURL, canonical.standardizedFileURL)
        XCTAssertEqual(try String(contentsOf: canonical), "legacy-installed-new-code")
        for entrypoint in [pathEntrypoint, legacyPathEntrypoint, legacy] {
            var info = stat()
            XCTAssertEqual(lstat(entrypoint.path, &info), 0)
    ...
    func testMarkerValidationRejectsUppercaseShaAndNonCanonicalVersion() throws {
        ...
        let store = AutoUpdateMarkerStore(homeDirectory: fixture.url)
        try store.ensureTrustedRoot()
        let binary = fixture.url.appendingPathComponent("bin/malibu-cli")
        ...
        let marker = AutoUpdatePendingMarker(
            ...
            targetPath: binary.path,
            backupPath: binary.deletingLastPathComponent().appendingPathComponent(".malibu-cli.rollback-aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee").path,
            size: 0,
            mode: 0o755,
    ...
    func testSignedReleasePendingMarkerRequiresDiscoveryAndExactTargetSetBinding() throws {
        ...
        let binary = fixture.url.appendingPathComponent("bin/malibu-cli")
        try FileManager.default.createDirectory(at: binary.deletingLastPathComponent(), withIntermediateDirectories: true, attributes: [.posixPermissions: 0o700])
        let backup = binary.deletingLastPathComponent().appendingPathComponent(".malibu-cli.rollback-aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee")
    ...
    func testAutoupdateReasonRedactionUsesStableCodes() {
        let errors: [Error] = [
            ...
            UpdateError.processFailed("/tmp/malibu-cli", 1),
        ]
        for error in errors {
    ...
    private var providerPlistTemplate: String {
        ...
        <key>ProgramArguments</key><array><string>__INSTALL_DIR__/malibu-cli</string><string>serve</string><string>--config</string><string>__HOME__/.config/macprovider/config.yaml</string></array>
    ...
    private var watchdogPlistTemplate: String {
        ...
        <key>MACPROVIDER_BINARY_PATH</key><string>__INSTALL_DIR__/malibu-cli</string>
