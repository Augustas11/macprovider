...
final class CompatibilitySetManifestTests: XCTestCase {
    ...
    func testArtifactIndexAcceptsLegacyProviderCLIArchiveDuringRenameBridge() throws {
        let fixture = try CompatibilityManifestFixture()
        let manifest = try CompatibilitySetManifest.loadValidated(
            from: fixture.root,
            expectedProviderVersion: fixture.providerCLIVersion,
        ...
        let tag = "v\(fixture.version)"
        let commit = fixture.commit
        let legacyProviderArchive = "macprovider-cli-\(tag)-darwin-arm64.tar.gz"
        let canonicalProviderArchive = "malibu-cli-\(tag)-darwin-arm64.tar.gz"
        let artifacts: [String: String] = [
            ...
            "provider_cli": legacyProviderArchive,
        ]
        var checksums: [String: String] = [
            canonicalProviderArchive: String(repeating: "c", count: 64),
        ]
        var rows: [String: Any] = [:]
        for (index, role) in artifacts.keys.sorted().enumerated() {
            let name = try XCTUnwrap(artifacts[role])
            let digest: String
            if role == "compatibility_manifest" {
                digest = manifest.envelopeSHA256
            } else {
                digest = String(repeating: String(format: "%x", (index % 15) + 1), count: 64)
            }
            checksums[name] = digest
            rows[role] = ["name": name, "sha256": digest]
        }
        let index: [String: Any] = [
            ...
            "compatibility_manifest_sha256": manifest.envelopeSHA256,
            "artifacts": rows,
        ]
        let indexURL = fixture.root.appendingPathComponent("compatibility-artifact-index.json")
        try CompatibilityManifestFixture.canonicalData(index).write(to: indexURL)
        let checksumsText = checksums
            .sorted { $0.key < $1.key }
            .map { "\($0.value)  \($0.key)" }
            .joined(separator: "\n") + "\n"

        let validated = try CompatibilityArtifactIndex.loadValidated(
            from: indexURL,
            compatibilityManifest: manifest,
            checksumsText: checksumsText,
            releaseAssetNames: Array(checksums.keys) + [
        ...
        XCTAssertEqual(validated.artifacts["provider_cli"]?.name, legacyProviderArchive)
    }

    func testInstalledManifestResolutionFollowsNormalBinarySymlinkTopology() throws {
        let fixture = try CompatibilityManifestFixture()
        let realBinary = fixture.root.appendingPathComponent("malibu-cli")
        ...
        let linkedBinary = binDirectory.appendingPathComponent("malibu-cli")
