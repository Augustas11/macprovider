...
final class ProviderLifecycleStateTests: XCTestCase {
    ...
    func testLegacyMacProviderAuthorityRecordStillLoads() throws {
        let fixture = try Fixture()
        let store = ProviderLifecycleStateStore(url: fixture.recordURL)
        let current = try store.transition(
            to: .startingProvider,
            reasonCode: "serve_invoked",
            writer: .serve,
            operationID: "serve-legacy-authority"
        )
        XCTAssertEqual(current.authority, ProviderLifecycleStateRecord.authority)

        let data = try Data(contentsOf: fixture.recordURL)
        var object = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        object["authority"] = ProviderLifecycleStateRecord.legacyAuthority
        let legacyData = try JSONSerialization.data(withJSONObject: object, options: [.sortedKeys])
        try legacyData.write(to: fixture.recordURL, options: .atomic)

        let legacy = try XCTUnwrap(store.current())
        XCTAssertEqual(legacy.authority, ProviderLifecycleStateRecord.legacyAuthority)
        XCTAssertTrue(ProviderLifecycleStateRecord.acceptsAuthority(legacy.authority))
    }
