...
final class StatusCommandTests: XCTestCase {
    ...
    func testRoutineCLIHelpUsesCanonicalPublicLanguage() {
        ...
        XCTAssertTrue(MalibuCLI.helpMessage().contains("hardware-check"))
        XCTAssertTrue(MalibuCLI.helpMessage().contains("performance-check"))
        XCTAssertFalse(MalibuCLI.helpMessage().contains("spec028"))
        ...
        for hiddenCommand in ["bootstrap-auth", "credentials", "decode-bench", "enroll", "lifecycle-state", "lifecycle-lease", "rotate-key"] {
            XCTAssertFalse(MalibuCLI.helpMessage().contains(hiddenCommand), MalibuCLI.helpMessage())
        }
    }
    ...
    func testLegacyHardwareCommandsRemainParseableButHidden() throws {
        let canary = try MalibuCLI.parseAsRoot([
            "spec028-canary", "ac10", "--baseline-runs", "1", "--spec-runs", "1",
        ])
        let benchmark = try MalibuCLI.parseAsRoot([
        ...
        XCTAssertFalse(MalibuCLI.helpMessage().contains("spec028-canary"))
        XCTAssertFalse(MalibuCLI.helpMessage().contains("spec028-benchmark"))
    }
