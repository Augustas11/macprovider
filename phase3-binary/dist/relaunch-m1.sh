#!/usr/bin/env bash
# relaunch-m1.sh — restart M1's phase3-binary to recover from a stuck
# coordinator WebSocket reconnect (known v1.1.3 quirk).
# Does NOT reinstall the binary — just kills the old process and
# restarts with the same arguments. ~30 seconds.
#
# Usage:
#   bash ~/Downloads/relaunch-m1.sh

set -uo pipefail

INSTALL_DIR="$HOME/phase3-binary-m1"
