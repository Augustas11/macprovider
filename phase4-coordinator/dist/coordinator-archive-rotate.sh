#!/usr/bin/env bash
# coordinator-archive-rotate.sh — coordinator.db archive-rotate scaffold.
#
...
if [ "$LIVE_PRUNE" = "1" ]; then
  [ "$FORCE_ROTATE" = "1" ] || refuse "live prune requires FORCE_ROTATE=1"
  [ "$COORDINATOR_ARCHIVE_ASSUME_QUIESCED" = "1" ] || refuse "live prune requires service quiesce acknowledgment"
