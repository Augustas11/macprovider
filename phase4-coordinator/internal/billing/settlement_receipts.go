...
func (s *Store) applySettlementReceiptVerdict(ctx context.Context, id SettlementReceiptIdentity, receiptPresent bool, receivedAtUnixMS int64, verify func(settlementEvidence, bool) SettlementVerifyResult) (SettlementReceiptState, error) {
	...
	err := sqliteutil.Transact(ctx, s.db, func(ctx context.Context, conn *sql.Conn) error {
		...
		if found && existing.Closed {
			...
			_, err = enqueueSettlementReceiptAuditOutboxConn(ctx, conn, verdictID, settlementReceiptEventVerdict, existing, receivedAtUnixMS)
			if err != nil {
				return err
			}
			outcome = existing
		...
		_, err = enqueueSettlementReceiptAuditOutboxConn(ctx, conn, verdictID, settlementReceiptEventVerdict, state, receivedAtUnixMS)
		if err != nil {
			return err
		}
		outcome = state
