...
func TestAutotuneFeedsServeLiteralSignedBytes(t *testing.T) {
	...
	for name, verification := range map[string]buyer.AutotuneFeedVerification{
		...
		if name != "rate_card" && verification.Version != "published-2026-09-19-openrouter-listed-v1" {
	...
	if release.Status != "live_verified" || release.ReleaseID != "published-2026-09-19-openrouter-listed-v1" || release.PolicyVersion != "autotune-policy-v1" {
