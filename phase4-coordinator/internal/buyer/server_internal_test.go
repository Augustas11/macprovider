...
func TestRouteSnapshotCanSkipOnlyStorePressure(t *testing.T) {
	...
	if !routeSnapshotCanSkipStorePressure(billing.RouteSnapshotModeEnforce, pressureErr, false) {
		t.Fatal("enforce mode should skip transient route snapshot store pressure from the route guard")
	}
...
func TestRetryableByCodeClassification(t *testing.T) {
	...
	permanent := []string{
		...
		"not_found", "unauthorized", "forbidden", "request_log_failed", "settlement_finality_failed",
		"settlement_receipt_lookup_failed",
...
var coordinatorEmittedErrorCodes = []string{
	...
	"forbidden", "no_provider_available", "not_found", "preflight_rejected",
