...
// ReferralConfig owns pre-beta access policy and the optional post-success
// advocacy bonus. Both feature flags default off; secrets support env:NAME
// indirection through resolveEnv.
...
func (c Config) validateReferrals() error {
	...
	if r.ProviderBaseUses <= 0 || r.SocialBonusUses <= 0 || r.ChallengeTTLS <= 0 {
		return fmt.Errorf("referrals provider/social uses and challenge_ttl_s must be > 0")
	}
	if r.EnableSocialInviteBonus && strings.TrimSpace(r.XAPIBearerToken) == "" {
