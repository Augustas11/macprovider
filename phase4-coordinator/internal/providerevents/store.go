...
ON CONFLICT(provider_id) DO UPDATE SET
	assigned_id = CASE
		...
		WHEN excluded.assigned_id = '' THEN provider_last_known.assigned_id
		ELSE excluded.assigned_id
	END,
	binary_version = CASE
		WHEN excluded.last_seen_at_utc < provider_last_known.last_seen_at_utc THEN provider_last_known.binary_version
		WHEN excluded.binary_version = '' THEN provider_last_known.binary_version
		ELSE excluded.binary_version
	END,
	model_id = CASE
	...
	state = CASE
		...
		WHEN excluded.state = '' THEN provider_last_known.state
		ELSE excluded.state
	END,
	auth_state = CASE
		WHEN excluded.last_seen_at_utc < provider_last_known.last_seen_at_utc THEN provider_last_known.auth_state
		WHEN excluded.auth_state = '' THEN provider_last_known.auth_state
		ELSE excluded.auth_state
	END,
	connected_at_utc = CASE
		WHEN excluded.last_seen_at_utc < provider_last_known.last_seen_at_utc THEN provider_last_known.connected_at_utc
		WHEN excluded.connected_at_utc = '' THEN provider_last_known.connected_at_utc
		ELSE excluded.connected_at_utc
	END,
	last_heartbeat_at_utc = CASE
		WHEN excluded.last_seen_at_utc < provider_last_known.last_seen_at_utc THEN provider_last_known.last_heartbeat_at_utc
		WHEN excluded.last_heartbeat_at_utc = '' THEN provider_last_known.last_heartbeat_at_utc
		ELSE excluded.last_heartbeat_at_utc
	END,
	last_activity_at_utc = CASE
		WHEN excluded.last_seen_at_utc < provider_last_known.last_seen_at_utc THEN provider_last_known.last_activity_at_utc
		WHEN excluded.last_activity_at_utc = '' THEN provider_last_known.last_activity_at_utc
		ELSE excluded.last_activity_at_utc
	END,
