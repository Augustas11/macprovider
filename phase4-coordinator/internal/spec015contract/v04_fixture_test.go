...
func TestV04SettlementFixturesMatchCoordinatorJCSContract(t *testing.T) {
	...
	for _, fixture := range fixtures.Objects {
		canonical, err := canonicalizeJSONValue(fixture.Kind, fixture.Value)
		if err != nil {
		...
		if got := sha256Hex(canonical); got != fixture.ExpectedSHA256Hex {
			t.Fatalf("%s sha256 = %q, want %q", fixture.ID, got, fixture.ExpectedSHA256Hex)
		}
	...
	for _, tuple := range fixtures.ReceiptTuples {
		canonical, err := canonicalizeJSONValue("receipt_tuple_v4", tuple.Value)
		if err != nil {
		...
		if got := sha256Hex(canonical); got != tuple.ExpectedSHA256Hex {
			t.Fatalf("%s sha256 = %q, want %q", tuple.ID, got, tuple.ExpectedSHA256Hex)
		}
		assertFixtureStrictKeys(t, tuple.ID, tuple.StrictKeys, receiptTupleKeys)
		assertStrictKeys(t, tuple.ID, tuple.Value, receiptTupleKeys)
...
func TestV04TupleFixturesBindRouteOutputAndUsage(t *testing.T) {
	...
	for _, tuple := range fixtures.ReceiptTuples {
		...
		assertStrictKeys(t, tuple.ID+".usage", tupleUsage, usageKeys)
		for _, field := range usageKeys {
...
func TestV04NegativeReceiptFixturesFailExpectedChecks(t *testing.T) {
	...
	for _, negative := range fixtures.NegativeReceipts {
		...
		canonical, err := canonicalizeJSONValue("receipt_tuple_v4", negative.Value)
		if err != nil {
...
func loadV04Fixtures(t *testing.T) v04Fixtures {
	...
	if err := decodeJSONUseNumber(raw, &fixtures); err != nil {
