...
func lastKnownFromProvider(provider pool.Provider, now time.Time) providerevents.LastKnown {
	snap := providerevents.LastKnown{
		...
		HashStatus:               string(provider.HashStatus),
		AttestationStatus:        string(provider.AttestationStatus),
		AttestationTier:          provider.AttestationTier,
		EncryptedLeg:             provider.EncryptedLeg,
		CatalogAdmissionMode:     provider.CatalogAdmissionMode,
		BenchmarkQuarantined:     provider.BenchmarkQuarantined,
		AdmissionCeilingExcluded: provider.AdmissionCeilingExcluded,
		AdmissionEvidenceStale:   provider.AdmissionEvidenceStale,
		AdmissionSandboxed:       provider.AdmissionSandboxed,
	}
