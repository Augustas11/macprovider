...
const supervisorRestartBeacon = `{"schema":"macprovider.supervisor-event.v1","ts":"2026-09-05T00:00:00Z","kind":"restart","boot_id":"BOOT-A","seq":3,"supervisor_label":"provider-watchdog","supervisor_version":"1.0","restarts_total":1,"deferrals_total":0,"last_restart":{"seq":3,"ts":"2026-09-05T00:00:00Z","reason":"wedge","cooldown_state":"armed","service_instance":"old-inst","model_liveness":{"token_age_ms":1500,"active_inference":true,"active_inference_age_ms":1500}},"last_deferral":null}`
...
func TestHeartbeatRecordsSupervisorEvent(t *testing.T) {
	...
	server.handleHeartbeat(nil, "provider-a", "assigned-a", supervisorHeartbeatPayload(supervisorRestartBeacon, "new-inst"))
	...
	got := calls[0]
	if got.ProviderID != "provider-a" || got.BootID != "BOOT-A" || got.Seq != 3 ||
	...
	if got.LastRestartSeq != 3 || got.LastRestartCooldown != "armed" ||
		got.LastRestartInstance != "old-inst" || got.CurrentServiceInstance != "new-inst" {
...
func TestHeartbeatSupervisorEventDedupedAcrossHeartbeats(t *testing.T) {
	...
	payload := supervisorHeartbeatPayload(supervisorRestartBeacon, "new-inst")
