...
apply_changes() {
  ...
  if ! verify_behavioral_disclosure "$expected"; then
    rollback_and_exit "gateway behavioral-safety disclosure verification failed" "$config_backup" "$config_sha"
  fi
