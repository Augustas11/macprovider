#!/usr/bin/env python3
"""Build a promotable crash-recovery journey-result payload from redacted evidence."""
...
from check_spec_governance import (
    BUYER_CRASH_RECOVERY_EXECUTION_MODE,
    BUYER_CRASH_RECOVERY_JOURNEY_ID,
    BUYER_CRASH_RECOVERY_PROMOTABLE_REQUIREMENT_IDS,
...
EVIDENCE_SCHEMA = "macprovider.buyer-crash-recovery-evidence.v1"
JOURNEY_ID = "JOURNEY-BUYER-CRASH-RECOVERY"
REPOSITORY = "Augustas11/macprovider"
ARTIFACT_ID = "redacted-buyer-crash-recovery"
...
def die(message: str) -> None:
    print(f"build-buyer-crash-recovery-journey-result: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def require_evidence_source(root: Path, source: str) -> tuple[str, Path]:
    ...
    if not normalized.startswith("journeys/evidence/buyer-crash-recovery-") or not name.endswith(".redacted.json"):
        die("redacted evidence source must be journeys/evidence/buyer-crash-recovery-*.redacted.json")
...
def parse_requirement_ids(raw: str | None, evidence: dict[str, Any]) -> list[str]:
    ...
    allowed = set(BUYER_CRASH_RECOVERY_PROMOTABLE_REQUIREMENT_IDS)
    forbidden = [item for item in input_ids if item not in allowed]
    if forbidden:
        die(f"crash-recovery journey-result cannot promote {', '.join(forbidden)}")
    return input_ids


def load_mapped_crash_recovery_requirements(root: Path) -> set[str]:
    ...
    for row in requirements:
        ...
        if isinstance(journeys, list) and BUYER_CRASH_RECOVERY_JOURNEY_ID in journeys and row.get("state") == "pending":
...
def require_steps(value: Any) -> list[dict[str, Any]]:
    ...
    missing = [step_id for step_id in BUYER_CRASH_RECOVERY_STEP_ID_ORDER if step_id not in by_id]
    extra = [step_id for step_id in by_id if step_id not in BUYER_CRASH_RECOVERY_STEP_ID_ORDER]
    if missing:
        die(f"missing crash-recovery step(s): {', '.join(missing)}")
    if extra:
        die(f"unknown crash-recovery step(s): {', '.join(extra)}")
    return [by_id[step_id] for step_id in BUYER_CRASH_RECOVERY_STEP_ID_ORDER]
...
def require_observations(value: Any) -> dict[str, Any]:
    ...
    if observations.get("settlement_mode") != "observe":
        die("observations.settlement_mode must equal 'observe'")
    ...
    for field in ("isolated_environment", "identity_fallback_recovered", "orphan_quarantined", "idempotent_rescan"):
    ...
    if observations.get("recovery_source") != "startup_scan":
        die("observations.recovery_source must equal 'startup_scan'")
    if observations.get("enforce_activated") is True:
        die("observations.enforce_activated must not be true")
...
def build_payload(root: Path, source: str, *, source_sha: str, evidence_sha: str, requirement_ids: str | None) -> dict[str, Any]:
    ...
    evidence, evidence_bytes = load_object_bytes(path, "crash-recovery redacted evidence")
    ...
    if JOURNEY_ID != BUYER_CRASH_RECOVERY_JOURNEY_ID:
        die("JOURNEY_ID drifted from BUYER_CRASH_RECOVERY_JOURNEY_ID")
    ...
    mapped = load_mapped_crash_recovery_requirements(root)
    not_mapped = [item for item in selected_requirements if item not in mapped]
    if not_mapped:
        die(f"requirement_ids must be pending and mapped to {BUYER_CRASH_RECOVERY_JOURNEY_ID}: {', '.join(not_mapped)}")
    ...
    if environment.get("class") != BUYER_CRASH_RECOVERY_EXECUTION_MODE:
        die(f"environment.class must equal {BUYER_CRASH_RECOVERY_EXECUTION_MODE!r}")
    ...
    return {
        ...
        "redaction": redaction,
        "run_id": run_id,
        "execution_mode": BUYER_CRASH_RECOVERY_EXECUTION_MODE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("redacted_evidence_source", help="journeys/evidence/buyer-crash-recovery-*.redacted.json")
    ...
    print(f"build-buyer-crash-recovery-journey-result: wrote {output}")
    return 0
