#!/usr/bin/env python3
"""Build a promotable paid-path journey-result payload from redacted evidence."""
...
from check_spec_governance import (
    ...
    BUYER_PAID_PATH_STEP_ID_ORDER,  # includes step-07-failure-refund
...
EVIDENCE_SCHEMA = "macprovider.buyer-paid-path-evidence.v1"
JOURNEY_ID = "JOURNEY-BUYER-PAID-PATH"
REPOSITORY = "Augustas11/macprovider"
ARTIFACT_ID = "redacted-buyer-paid-path"
...
def die(message: str) -> None:
    print(f"build-buyer-paid-path-journey-result: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def require_evidence_source(root: Path, source: str) -> tuple[str, Path]:
    ...
    if not normalized.startswith("journeys/evidence/buyer-paid-path-") or not name.endswith(".redacted.json"):
        die("redacted evidence source must be journeys/evidence/buyer-paid-path-*.redacted.json")
...
def load_mapped_paid_path_requirements(root: Path) -> set[str]:
    ...
    for row in requirements:
        ...
        if isinstance(journeys, list) and BUYER_PAID_PATH_JOURNEY_ID in journeys and row.get("state") == "pending":
...
def require_steps(value: Any) -> list[dict[str, Any]]:
    ...
    missing = [step_id for step_id in BUYER_PAID_PATH_STEP_ID_ORDER if step_id not in by_id]
    extra = [step_id for step_id in by_id if step_id not in BUYER_PAID_PATH_STEP_ID_ORDER]
    if missing:
        die(f"missing paid-path step(s): {', '.join(missing)}")
    if extra:
        die(f"unknown paid-path step(s): {', '.join(extra)}")
    return [by_id[step_id] for step_id in BUYER_PAID_PATH_STEP_ID_ORDER]
...
def require_observations(value: Any) -> dict[str, Any]:
    ...
    for field in ("enforce_activated", "payout_ready_mutated", "production_side_effects"):
        if observations.get(field) is not False:
            die(f"observations.{field} must be false")
    for field in ("isolated_environment", "raw_prompt_output_redacted", "bearer_tokens_redacted"):
        if observations.get(field) is not True:
            die(f"observations.{field} must be true")
    if not isinstance(observations.get("buyer_receipt_retrieval_exposed"), bool):
        die("observations.buyer_receipt_retrieval_exposed must be a boolean")
...
def build_payload(root: Path, source: str, *, source_sha: str, evidence_sha: str, requirement_ids: str | None) -> dict[str, Any]:
    ...
    evidence, evidence_bytes = load_object_bytes(path, "paid-path redacted evidence")
    ...
    if JOURNEY_ID != BUYER_PAID_PATH_JOURNEY_ID:
        die("JOURNEY_ID drifted from BUYER_PAID_PATH_JOURNEY_ID")
    ...
    mapped = load_mapped_paid_path_requirements(root)
    not_mapped = [item for item in selected_requirements if item not in mapped]
    if not_mapped:
        die(f"requirement_ids must be pending and mapped to {BUYER_PAID_PATH_JOURNEY_ID}: {', '.join(not_mapped)}")
    ...
    if environment.get("class") != BUYER_PAID_PATH_EXECUTION_MODE:
        die(f"environment.class must equal {BUYER_PAID_PATH_EXECUTION_MODE!r}")
    ...
    return {
        ...
        "redaction": redaction,
        "run_id": run_id,
        "execution_mode": BUYER_PAID_PATH_EXECUTION_MODE,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("redacted_evidence_source", help="journeys/evidence/buyer-paid-path-*.redacted.json")
    ...
    print(f"build-buyer-paid-path-journey-result: wrote {output}")
    return 0
