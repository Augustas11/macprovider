...
def require_evidence_source(root: Path, source: str) -> tuple[str, Path]:
    ...
    if not normalized.startswith("journeys/evidence/local-consumer-endpoint-") or not name.endswith(".redacted.json"):
...
def require_steps(value: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(value):
        ...
        require_exact_keys(step, {"id", "status", "artifacts"}, {"id", "status", "artifacts"}, f"steps[{index}]")
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if any(fragment in lowered for fragment in FORBIDDEN_KEY_FRAGMENTS):
                ...
                if not is_redaction_flag and not is_expected_fingerprint and not is_expected_observation:
...
def require_candidate_identity(value: Any) -> dict[str, Any]:
    ...
    if identity.get("gateway_kind") not in {"staging", "production"}:
