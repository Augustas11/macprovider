...
workflow="$root/.github/workflows/promote-signed-buyer-crash-recovery-journey.yml"
builder="$root/scripts/build-buyer-crash-recovery-journey-result.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-buyer-crash-recovery-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/buyer-crash-recovery-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-buyer-crash-recovery-journey-result.py",
    '--evidence-sha "$EVIDENCE_SHA"',
    "scripts/preflight-signed-journey-promotion.py",
    "Preflight selector freshness before signing",
    "--journey-id JOURNEY-BUYER-CRASH-RECOVERY",
    ...
    "retention-days: 1",
    "signed-buyer-crash-recovery-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-buyer-crash-recovery-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every buyer-crash-recovery requirement subset")
...
required_builder = [
    ...
    "observations.job_enabled must be false",
    "observations.recovery_source must equal 'startup_scan'",
    "production_pearl",
]
for value in required_builder:
    if value not in builder:
        raise SystemExit(f"builder contract is missing: {value}")
if "require_candidate_identity" in builder or "candidate_identity" in builder:
    raise SystemExit("crash-recovery builder must not require candidate_identity")

print("[test-signed-buyer-crash-recovery-journey-workflow] ok: protected buyer-crash-recovery signer exports a short-lived artifact")
PY
