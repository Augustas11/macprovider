...
workflow="$root/.github/workflows/promote-signed-buyer-enforce-journey.yml"
builder="$root/scripts/build-buyer-enforce-journey-result.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-buyer-enforce-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/buyer-enforce-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-buyer-enforce-journey-result.py",
    '--evidence-sha "$EVIDENCE_SHA"',
    "scripts/preflight-signed-journey-promotion.py",
    "Preflight selector freshness before signing",
    "--journey-id JOURNEY-BUYER-ENFORCE",
    ...
    "retention-days: 1",
    "signed-buyer-enforce-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-buyer-enforce-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every buyer-enforce requirement subset")
...
required_builder = [
    ...
    "observations.enforce_activated must be true",
    "observations.settlement_mode_start must equal 'enforce'",
]
...
if "require_candidate_identity" in builder or "candidate_identity" in builder:
    raise SystemExit("enforce builder must not require candidate_identity")

print("[test-signed-buyer-enforce-journey-workflow] ok: protected buyer-enforce signer exports a short-lived artifact")
PY
