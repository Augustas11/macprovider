...
workflow="$root/.github/workflows/promote-signed-buyer-paid-path-journey.yml"
builder="$root/scripts/build-buyer-paid-path-journey-result.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-buyer-paid-path-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/buyer-paid-path-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-buyer-paid-path-journey-result.py",
    '--evidence-sha "$EVIDENCE_SHA"',
    "scripts/preflight-signed-journey-promotion.py",
    "Preflight selector freshness before signing",
    "--journey-id JOURNEY-BUYER-PAID-PATH",
    ...
    "retention-days: 1",
    "signed-buyer-paid-path-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-buyer-paid-path-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every buyer-paid-path requirement subset")
...
print("[test-signed-buyer-paid-path-journey-workflow] ok: protected buyer-paid-path signer exports a short-lived artifact")
PY
