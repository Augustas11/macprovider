...
workflow="$root/.github/workflows/promote-signed-byom-discovery-journey.yml"
builder="$root/scripts/build-byom-discovery-journey-result.py"
contract="$root/scripts/byom_journey_evidence.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-byom-discovery-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/byom-discovery-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-byom-discovery-journey-result.py",
    ...
    "retention-days: 1",
    "signed-byom-discovery-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-byom-discovery-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every BYOM discovery requirement subset")
...
required_builder = [
    "DISCOVERY_CONTRACT",
    "run_builder_cli",
    "build-byom-discovery-journey-result",
]
...
print("[test-signed-byom-discovery-journey-workflow] ok: protected BYOM discovery signer exports a short-lived artifact")
PY
