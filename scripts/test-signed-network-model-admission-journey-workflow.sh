...
workflow="$root/.github/workflows/promote-signed-network-model-admission-journey.yml"
builder="$root/scripts/build-network-model-admission-journey-result.py"
contract="$root/scripts/byom_journey_evidence.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-network-model-admission-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}

[[ -f "$workflow" && ! -L "$workflow" ]] || fail "workflow is absent or unsafe"
[[ -f "$builder" && ! -L "$builder" ]] || fail "builder is absent or unsafe"
[[ -f "$contract" && ! -L "$contract" ]] || fail "contract module is absent or unsafe"
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/network-model-admission-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-network-model-admission-journey-result.py",
    '--evidence-sha "$EVIDENCE_SHA"',
    "scripts/preflight-signed-journey-promotion.py",
    "Preflight selector freshness before signing",
    "--journey-id JOURNEY-NETWORK-MODEL-ADMISSION",
    ...
    "retention-days: 1",
    "signed-network-model-admission-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-network-model-admission-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every network-model-admission requirement subset")
...
required_builder = [
    "ADMISSION_CONTRACT",
    "run_builder_cli",
    "build-network-model-admission-journey-result",
]
...
for value in required_contract:
    if value not in contract:
        raise SystemExit(f"contract module is missing: {value}")

print("[test-signed-network-model-admission-journey-workflow] ok: protected network-model-admission signer exports a short-lived artifact")
PY
