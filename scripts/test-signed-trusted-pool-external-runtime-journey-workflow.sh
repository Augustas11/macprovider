...
workflow="$root/.github/workflows/promote-signed-trusted-pool-external-runtime-journey.yml"
builder="$root/scripts/build-trusted-pool-external-runtime-journey-result.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"

fail() {
  printf '[test-signed-trusted-pool-external-runtime-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/trusted-pool-external-runtime-journey-result-${requirement_slug}.unsigned.json"',
    "scripts/build-trusted-pool-external-runtime-journey-result.py payload",
    '--evidence-sha "$EVIDENCE_SHA"',
    "scripts/preflight-signed-journey-promotion.py",
    "Preflight selector freshness before signing",
    "--journey-id JOURNEY-TRUSTED-POOL-EXTERNAL-RUNTIME",
    ...
    "retention-days: 1",
    "signed-trusted-pool-external-runtime-journey-promotion-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-trusted-pool-external-runtime-journey-promotion.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every trusted-pool-external-runtime requirement subset")
...
print("[test-signed-trusted-pool-external-runtime-journey-workflow] ok: protected trusted-pool-external-runtime signer exports a short-lived artifact")
PY
