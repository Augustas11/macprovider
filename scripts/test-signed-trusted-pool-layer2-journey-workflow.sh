...
workflow="$root/.github/workflows/build-signed-trusted-pool-layer2-journey.yml"
builder="$root/scripts/build-trusted-pool-layer2-journey-result.py"
preflight="$root/scripts/preflight-signed-journey-promotion.py"
validator="$root/scripts/validate-signed-journey-result.py"

fail() {
  printf '[test-signed-trusted-pool-layer2-journey-workflow] ERROR: %s\n' "$*" >&2
  exit 1
}
...
required_workflow = [
    ...
    'payload="$RUNNER_TEMP/trusted-pool-layer2-journey-result-${requirement_slug}.unsigned.json"',
    ...
    "--journey-id JOURNEY-TRUSTED-POOL-LAYER2-MVP",
    ...
    "retention-days: 1",
    "signed-trusted-pool-layer2-journey-evidence-${{ steps.request.outputs.source_sha }}-${{ steps.request.outputs.requirement_slug }}",
    "macprovider.signed-trusted-pool-layer2-journey-evidence.v1",
...
if 'envelope="${REDACTED_EVIDENCE_INPUT%.redacted.json}.journey-result.signed.json"' in workflow:
    raise SystemExit("workflow must not reuse one signed envelope path for every Trusted Pool requirement subset")
...
if "JOURNEY-BUYER-ENFORCE" in builder or "BUYER_ENFORCE" in builder:
    raise SystemExit("Trusted Pool builder must not reuse buyer-enforce constants")

print("[test-signed-trusted-pool-layer2-journey-workflow] ok: protected Trusted Pool Layer 2 signer exports a short-lived non-promoting artifact")
PY
