...
class BYOMJourneyCaptureTests(unittest.TestCase):
    ...
    def test_rejects_captured_status_promoting_a_network_state_locally(self) -> None:
        def mutator(_manifest, root):
            path = root / "captures" / "admission-status-offer-submitted.json"
        ...
        self.assert_capture_fails(
            "admission",
            mutator,
            "admission_state 'offer_submitted' is not a permitted state for "
            "admission_state_source 'local_default'",
        )

    def test_rejects_captured_status_with_an_illegal_next_state(self) -> None:
        def mutator(_manifest, root):
            path = root / "captures" / "admission-status-offer-submitted.json"
            document = json.loads(path.read_text(encoding="utf-8"))
            # SPEC-047-R001 has no `offer_submitted` -> `settlement_capable` edge.
            document["allowed_next_states"] = ["settlement_capable"]
            path.write_text(json.dumps(document), encoding="utf-8")

        self.assert_capture_fails(
            "admission",
            mutator,
            "'settlement_capable' is not an allowed transition from 'offer_submitted'",
        )
    ...
    def test_rejects_captured_withdrawal_with_an_out_of_enum_reason_code(self) -> None:
        def mutator(_manifest, root):
            path = root / "captures" / "admission-withdraw.json"
            document = json.loads(path.read_text(encoding="utf-8"))
            document["reason_code"] = "operator_changed_mind"
            path.write_text(json.dumps(document), encoding="utf-8")

        self.assert_capture_fails(
            "admission", mutator, "reason_code is not a permitted value: 'operator_changed_mind'"
        )

    def test_rejects_captured_economics_row_with_a_boolean_action(self) -> None:
        def mutator(_manifest, root):
            path = root / "captures" / "catalog-economics-unpriced.json"
            document = json.loads(path.read_text(encoding="utf-8"))
            document["rows"][0]["prepare"] = False
            path.write_text(json.dumps(document), encoding="utf-8")

        self.assert_capture_fails("admission", mutator, "prepare must be a JSON object")
