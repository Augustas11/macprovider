...
class HermeticReleaseTest(unittest.TestCase):
    ...
    def test_market_peg_bind_requires_named_market_inputs(self):
        ...
        with self.harness() as harness:
            ...
            harness.bump("published-2026-09-26-market-v1", "2026-09-26T00:00:00Z")
    ...
    def test_non_market_generate_does_not_require_market_provenance_bind(self):
        ...
        with self.harness() as harness:
            harness.bump("published-2026-09-26-manual-v1", "2026-09-26T00:00:00Z")
    ...
    def activate(self, harness) -> str:
        release_id = "published-2026-09-26-activation-v1"
        harness.measure_sizes()
        harness.bump(release_id, "2026-09-26T00:00:00Z")
    ...
    def test_post_activation_generate_without_a_previous_release_fails_closed(self):
        with self.harness() as harness:
            self.activate(harness)
            harness.bump("published-2026-09-26-next-v1", "2026-09-26T00:00:00Z")
            with self.assertRaises(catalog_release.CatalogError) as caught:
                catalog_release.generate(harness.KEY_ID)
            self.assertIn("--previous-release-dir is required", str(caught.exception))
    ...
    def test_post_activation_release_with_the_previous_release_directory(self):
        with self.harness() as harness:
            ...
            ledger = harness.ledger()
            self.assertEqual(
                set(ledger["releases"]["published-2026-09-26-next-v1"]["feeds"]),
    ...
    def test_a_stale_published_feed_never_activates_implicitly(self):
        with self.harness() as harness:
            harness.measure_sizes()
            harness.bump("published-2026-09-26-stale-v1", "2026-09-26T00:00:00Z")
    ...
    def test_intake_decision_is_required_when_a_release_promotes_a_row(self):
        ...
        with self.harness() as harness:
            ...
            harness.bump("published-2026-09-26-demote-v1", "2026-09-26T00:00:00Z")
            self.set_status(harness, "qwen3-8b", "listed")
            harness.cut(previous_release_dir=previous)
            self.assertIsNone(
                harness.ledger()["releases"]["published-2026-09-26-demote-v1"]["intake_decision_sha256"]
            )
            demoted = harness.stage(harness.root / "demoted")

            # Release N+2 PROMOTES it back: null now fails the release closed.
            harness.bump("published-2026-09-27-promote-v1", "2026-09-27T00:00:00Z")
            ...
            (harness.catalog / "intake-decision.json").write_bytes(
                promotion_manifest_bytes(harness, "qwen3-8b", "published-2026-09-27-promote-v1", "2026-09-27T00:00:00Z")
            )
            harness.cut(previous_release_dir=demoted)
            row = harness.ledger()["releases"]["published-2026-09-27-promote-v1"]
    ...
    def test_verify_re_derives_the_intake_transition_with_the_previous_release(self):
        ...
        with self.harness() as harness:
            ...
            promoted_id = "published-2026-09-27-promote-v1"
            harness.bump(promoted_id, "2026-09-27T00:00:00Z")
            self.set_status(harness, "qwen3-8b", "recommendable")
            (harness.catalog / "intake-decision.json").write_bytes(
                promotion_manifest_bytes(harness, "qwen3-8b", promoted_id, "2026-09-27T00:00:00Z")
            )
            harness.cut(previous_release_dir=demoted)
    ...
    def test_a_pre_activation_static_artifact_leftover_fails_verify(self):
        ...
        for leftover in ("autotune-artifacts.json", "autotune-artifacts.json.sig"):
            with self.subTest(leftover=leftover):
                with self.harness() as harness:
                    harness.bump("published-2026-09-26-leftover-v1", "2026-09-26T00:00:00Z")
                    harness.cut()
                    self.assertFalse((harness.catalog / "autotune-artifacts.json").exists())
...
class PreviousReleaseDirectoryTest(unittest.TestCase):
    ...
    def test_a_conforming_previous_release_loads(self):
        with self.activated() as (harness, previous, releases):
            ...
            self.assertEqual(loaded["release_id"], "published-2026-09-26-activation-v1")
