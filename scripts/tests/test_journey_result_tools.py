...
class JourneyResultToolsTests(unittest.TestCase):
    ...
    def test_promoter_rejects_trusted_pool_layer2_evidence_only_without_rewrite(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_repository(root, base_repository())
            commit = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=root, text=True).strip()
            generate_acceptance_key(root)
            evidence_path = root / "journeys" / "evidence" / "trusted-pool-layer2-alpha.signed.json"
            envelope = signed_journey_envelope(commit)
            envelope["signed"]["journey_id"] = "JOURNEY-TRUSTED-POOL-LAYER2-MVP"
            ...
            with self.assertRaises(SystemExit), contextlib.redirect_stderr(stderr):
                promoter.promote(
                    root,
                    "SPEC-001-R001",
                    "journeys/evidence/trusted-pool-layer2-alpha.signed.json",
                    base_ref="HEAD",
                )
