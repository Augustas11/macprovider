...
- **`stats_rollup_state` table (v10 ARCH r9 H1 fix — pin the §9.7 `partial_history_since` storage cross-step seam):** create a concrete table in Step 1 so Step 2 writes a defined source and Step 3 reads a defined source. Step 1 owns the schema + grants; without this, the cross-step seam leaks ("Step 2 writes equivalent persisted source the handler will read" was the v9 prompt's hand-wave):
  ```sql
  CREATE TABLE stats_rollup_state (
    window                  TEXT PRIMARY KEY
      CHECK (window IN ('24h','7d','30d','all')),
    partial_history_since   TIMESTAMPTZ,   -- NULL when Path B / fully overlapped
    backfill_mode           TEXT NOT NULL DEFAULT 'partial'
      CHECK (backfill_mode IN ('partial','full')),
    generated_at            TIMESTAMPTZ NOT NULL DEFAULT now()
  );
  ```
  The migration pre-seeds the four window rows. Grants:
  - `stats_rollup`: SELECT, INSERT, UPDATE on `stats_rollup_state` (Step 2 writes during backfill + every rollup tick that recomputes the partial-history boundary).
  - `stats_reader`: SELECT on `stats_rollup_state` (Step 3 reads on every `/v1/stats/leaderboard?window=30d|all` request to emit `partial_history_since` per §5.2 / §9.7).
  - All other roles: explicit deny.
  Step 2 and Step 3 references to `rollup_state.partial_history_since` MUST be updated to `stats_rollup_state.partial_history_since` (the table is in the `stats_*` namespace). The `backfill_mode` column makes Path A vs Path B selection persistent and queryable from the handler test fixtures.
...
- `stats_reader` (§7.2.1) — request-path role.
  - SELECT on EXACTLY: `stats_overview_current`, `stats_timeseries_rpm_30m`, `stats_timeseries_tpm_30m`, `stats_leaderboard_24h`, `stats_leaderboard_7d`, `stats_leaderboard_30d`, `stats_leaderboard_all`, `stats_components_health`, `stats_rollup_state` (v10 ARCH r9 H1 — the `partial_history_since` source), `provider_visibility`, `partner_keys`, **PLUS the v0.1.7 `rewards_populated` storage** (e.g. `stats_rewards_populated` if you used the lookup-table shape; if you denormalized into `stats_components_health`, no extra grant needed — the handler still reads via the existing `stats_components_health` grant).
...
- `stats_rollup` (§7.2.2) — rollup job role.
  - SELECT, INSERT, UPDATE, DELETE on EXACTLY: `stats_overview_current`, `stats_timeseries_rpm_30m`, `stats_timeseries_tpm_30m`, `stats_leaderboard_24h`, `stats_leaderboard_7d`, `stats_leaderboard_30d`, `stats_leaderboard_all`, `stats_components_health`, `stats_late_events`, `stats_rollup_state` (v10 ARCH r9 H1 — Step 2 writes `partial_history_since` here on every tick that recomputes the boundary), **PLUS write privileges (SELECT, INSERT, UPDATE, DELETE) on the v0.1.7 `rewards_populated` storage if you chose the lookup-table shape** — the rollup is the writer of this signal.
...
- Integration (both backfill modes, rollup-state only — handler-response assertion is Step 3): set `stats.rollup.backfill_mode = "partial"` and run Path A backfill; assert `SELECT partial_history_since FROM stats_rollup_state WHERE window IN ('30d','all')` returns the start timestamp on both rows. Set `= "full"` and run Path B backfill; assert `SELECT partial_history_since FROM stats_rollup_state WHERE window IN ('30d','all')` returns NULL on both. (`stats_rollup_state` is the concrete Step 1 table per v10 ARCH r9 H1.) Step 3 will own the test that `/v1/stats/leaderboard?window=30d` JSON exposes or omits the field accordingly, reading from the same table via the `stats_reader` grant.
...
- **`partial_history_since` exposure (v0.1.7 H4)** — seed `stats_rollup_state.partial_history_since = <timestamp>` for the `30d` and `all` rows (Path A fixture from Step 2); call `GET /v1/stats/leaderboard?window=30d` AND `?window=all`; assert response includes top-level `partial_history_since` matching the seeded value (both windows). Call `?window=24h` AND `?window=7d`; assert the field is OMITTED on both (per §9.7). Seed `partial_history_since = NULL` (Path B fixture); call `?window=30d`; assert the field is OMITTED. All fixtures share the Step 1 `stats_rollup_state` schema + Step 2 rollup writes.
