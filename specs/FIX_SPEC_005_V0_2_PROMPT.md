...
contract, and AC precision gaps before SPEC-005 can be locked.
...
- Rate-limit posture: operator endpoints share existing `/admin/*`
  protection; provider endpoint SHOULD be bounded by a conservative
  per-provider read limit such as 60/minute, configurable.
...
- [ ] Every MAJOR finding above has an explicit fix.
