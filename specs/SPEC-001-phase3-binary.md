...
**Version:** 1.9.9 (2026-09-10, confirmed primary-artifact bootstrap commands)

**Change log v1.9.9 (2026-09-10):** Reserves capability-negotiated primary-artifact preparation, measured prepared-only recommendation, and transaction status/cancel/result/cleanup commands under SPEC-044. Keeps legacy model and adoption JSON schemas unchanged and preserves coordinator-only paid admission.
...
| `models prepare` | SPEC-001 plus SPEC-044 R002/R003/R007 | Confirmed exact signed primary-artifact preparation; `--json` emits only `model_catalog_transaction_event.v1` JSONL, kind `prepare_model`, under `model_catalog_transactions_v1`. It never activates a model. |
| `models recommend-prepared` | SPEC-001 plus SPEC-044 R003/R007 | Confirmed real measured single-target recommendation from verified durable weights, kind `evaluate_model`, under `model_catalog_local_activation_v1`; no download or config application. Progress uses the same transaction event stream and the result is retrieved separately. |
| `models transaction status` / `models transaction cancel` | SPEC-001 plus SPEC-044 R007/R011 | CLI-owned journal readback/cancellation for exact owner, UUID and target, under `model_catalog_transactions_v1`; `--json` uses the unchanged transaction event schema. Cancellation never requires app-side process termination. |
| `models transaction result` | SPEC-001 plus SPEC-044 R003/R007 | Read-only retrieval of the complete `autotune_recommend.v1` document for a succeeded `evaluate_model` transaction with matching owner and target; other kinds, incomplete/failed outcomes and mismatches fail closed. No event/document schema mixing. |
| `models cleanup-staging` | SPEC-001 plus SPEC-044 R007 | Confirmed transaction-owned staging cleanup, kind `cleanup_staging`; `--json` uses the unchanged transaction event stream under `model_catalog_transactions_v1`. No caller-supplied artifact paths. |
...
| `models admission retry` | SPEC-047 R002/R003 | `models admission retry TARGET --yes --json` submits fresh current-key signed evidence for the same exact pending offer to the bounded coordinator retry path, then emits existing `model_admission_status.v1` readback. Never locally promotes admission or changes the legacy offer/status schemas. |
...
The bootstrap command grammar is:

```text
models catalog-economics --local-activation --json
models prepare TARGET --transaction-id UUID --confirm --json
models recommend-prepared TARGET --transaction-id UUID --confirm --json
models transaction status UUID --model TARGET --json
models transaction cancel UUID --model TARGET --json
models transaction result UUID --model TARGET --json
models cleanup-staging UUID --model TARGET --confirm --json
```

`TARGET` is the exact signed primary `action_model_id`, never a path or an
arbitrary authority override. The CLI MUST resolve and authenticate the
catalog/feed itself, bind the UUID to that target and local transaction owner,
and reject missing confirmation, mismatched IDs/targets, and conflicting work.
The cleanup UUID identifies the original transaction whose staging is owned;
cleanup MUST preserve its original terminal result while tracking cleanup
progress separately as kind `cleanup_staging`. A client orders events by the
UUID and kind, and MUST NOT interpret cleanup success as preparation success.
`--confirm` records confirmation of the exact projected target, disclosed size
(or unavailable-size warning), and trust source; the CLI MUST revalidate that
identity when dispatched. Status/cancel/result require no caller path or
provider-supplied trust assertion. A result retrieval never synthesizes a
recommendation; the app MUST NOT render raw model paths or internal evidence in
UI/logs, and the document MUST contain no secrets.
The app passes the retrieved document through the existing adoption command's
stdin mechanism; `model_adoption_event.v1`, `model_adoption_command.v1`, and
`models adopt-recommendation.v1` remain unchanged. Existing recommendation age,
feed identity, configuration, fit and rollback checks are still mandatory.
SPEC-044 R002/R003 defines the explicit projection protocol opt-in; clients
that do not negotiate it retain legacy conservative actions.
...
### 6.14a transaction context and generation amendment (v1.9.9)

New prepare/recommend-prepared/cleanup starts and transaction status/cancel/result MUST bind canonical transaction UUID, exact target, expected kind and canonical lowercase operation-generation UUID. Controls use `--expected-kind KIND --operation-generation GENERATION`. Initial prepare/evaluation generations remain stable; each cleanup attempt receives a distinct generation before projection and repeated reservation/start of that attempt is idempotent. Validate the whole selector under the same journal lock BEFORE reconciliation, hash/cleanup, cancellation or result read. Original terminal readback cannot mutate newer cleanup; stale cleanup cannot fall forward. Heartbeat/cancel/terminal writes MUST validate their generation. Legacy missing-generation records MUST NOT become new authorization.

App-bound dispatch adds private `--transaction-context-fd`, with short controls additionally requiring `--control-lock-fd`, `--control-parent-pid`, `--control-lifetime-fd` together. Reject partial or inappropriate controls before store work. The short helper starts independent10-second parent/EOF/deadline protection and validates inherited private lock identity before config/journal access. No control command spawns descendants. Standalone explicitly invoked commands may retain their non-app route; app context failure MUST NOT fall back to it.

The context expectation is closed JSON (at most64KiB, pipe EOF required) with schema model_transaction_context_expectation.v1, transaction_context_sha256, config_path, config_device, config_inode, config_size, config_sha256, uid, home_directory. Integer widths must be checked; config size is1..1048576. Private metadata stays solely in inherited IPC/pending storage, never projection/log/UI. The fixed config MUST be opened once safely with no-follow and owner/private/regular/single-link/ACL/ancestor validation, bounded UTF-8 read, before/after identity checks and expected path/device/inode/size/hash. All ConfigLoader callbacks and nested consumers MUST use the same captured bytes/resolved AppConfig. Later replacement may use captured A or fail, never decode B.

Resolve artifact root once with frozen config and validated sanitized_app_v1 environment/kernel home. This namespace permits only existing innocuous PATH/HOME/USER/TMPDIR/LANG/NSUnbufferedIO/LC_ values; HOME must match kernel home, and configuration/HF overrides are forbidden. Domain-separated sorted-key compact JSON model_transaction_context.v1 binds config path/identity/byteSHA, uid/kernelhome, durable and transaction paths plus their directory device/inode identities and namespace. Compare its SHA256 to expectation BEFORE store construction/locking/journal reads or mutation. Missing/replaced/unsafe roots fail closed for bound controls; no setup fallback. No subsequent resolver may reread ambient environment or default roots.

Clean first projection uses explicit two-phase initialization: prepare captures/decodes config and derives root without store access or digest; authorized store setup creates/validates roots and returns an opaque store-owned identity receipt; reservations use that same store/config; read-only finalization verifies actual root metadata against that receipt and captured ancestors and produces the immutable bound context/digest without rereading config. Setup/finalization failure emits no executable bound action. Roots replaced between reservation and finalization cannot rebind old actions to a new store. Exact bound invocation uses existing-root-only loading and never this setup path.

Transaction storage may retire validated terminal records from its bounded active index while retaining original UUID records/result bytes for direct lookup. Protected active, unresolved or unsafe records MUST NOT be silently evicted. Completion publication MUST index immutable evaluation success before later fresh reservations can hide it. Adoption pointers bind actual original generation, authority/result digest and original success evidence, excluding mutable cleanup bookkeeping. Legitimate cleanup preserves that commitment; mismatches fail closed. Healthy legacy missing-generation evidence may retire only through validated historical result policy without minting actions. Retention is not authority and does not waive current adoption freshness.

Short transaction controls MUST NOT perform recursive staging deletion or full artifact hashing under the global journal lock. They use nonblocking/bounded lock acquisition, inspect bounded prior verified publication evidence outside the global lock, then revalidate exact generation and record digest before terminal mutation. Remaining owned staging is marked cleanupRequired for the explicit long cleanup operation. Blocked filesystem metadata must not prevent another owner heartbeat; the independent control deadline remains effective. Long owner verification and cleanup retain cancellable progress and exact publication ownership.

A digest-bound publication seal may recover only historical preparation success under the trusted operator-UID boundary. It binds prior full verified bytes and immutable publication identity; it MUST NOT assert present integrity or readiness. Metadata captured outside the lock cannot detect every later in-place descendant write through a root-inode check. The evidence linearization and terminal meaning MUST state that limitation. Fresh discovery readiness, evaluation and adoption MUST still fully verify current artifact integrity. Pre/during-capture substitutions fail closed; post-capture mutation can preserve historical success but MUST fail fresh verification before use. No signature, economic or admission authority is inferred from this seal.

Retention, reservation, cleanup enumeration, completion indexing and archived recommendation lookup MUST perform primary/result/seal bulk reads, hashing, parsing and directory enumeration outside the global journal lock. Short final mutations may decode the bounded format/active index, recheck pinned file placement and metadata plus exact captured-byte commitments/index generation, and perform one durable state transition. Retention receipts MUST distinguish generated active operations, non-creating allocating-intent recovery, supported generation-less legacy retirement, and archived read-only evidence. Never create an owner inode to recover an empty allocating intent, fabricate a legacy generation, or require active membership to read healthy archived evidence. Preserve allocating intent→primary→active serialization and pointer-before-retirement ordering.

Maintenance uses bounded monotonic work budgets, one owner-protected record at a time, nonblocking final locks and fair cursor progression. An incomplete required projection recovery pass MUST report unavailable rather than presenting an older pointer as its complete result. Initialized stores MUST NOT enumerate historical files. Initial bounded migration enumeration uses a separate bootstrap guard outside the journal lock, then rechecks unchanged root/inventory and absent initialized format before publication. Root replacement, captured evidence changes, incompatible schema, unsafe sidecars and stale index generation MUST cause no stale mutation.

Prepare, evaluation and explicit-cleanup owners target a durably fsynced heartbeat every five seconds; busy retries do not reset the monotonic last-success time. Concurrent retention bulk work MUST NOT monopolize heartbeat access. If durable progress is unavailable for eight seconds, fence further artifact/result/config publication and request cancellation; an independent watchdog MUST stop that owning command by ten seconds since last durable progress if graceful failure has not completed. Preserve already committed truth and staging for exact-selector recovery; no fabricated restoration or completion. Candidate/lifecycle child ownership and kernel-parent restoration protections remain in force. Storage-fixture timing is not physical inference qualification.

### 6.14b immutable transaction origin and retirement amendment (v1.9.9)

Generated UUIDs MUST retain an exclusive immutable `model_catalog_transaction_origin.v1` record before active allocation. Its allocated variant binds the actual original generation, kind, creation time, target/model/revision/artifact/candidate/feed/signer tuple and exact initial queued-primary SHA. Allocation intent pins the prospective origin SHA before primary→origin→active durability. Recovery may publish a missing origin only from an exact still-unexposed intent and matching untouched queued bytes; an active missing origin is corruption. No recovery creates an owner inode merely to establish absence. All origin/binding/retirement sidecars participate in required-absence and exact receipt checks.

Evaluation success MUST publish an exclusive `model_catalog_evaluation_success_binding.v2` before the expected binding SHA in its active entry, then the successful primary. The binding pins UUID/generation/origin SHA, context and immutable success commitment, result SHA, exact preterminal-primary SHA, exact successful event, closed cleanupRequired terminal delta and canonical prospective-terminal-primary SHA. Pending replay applies only that delta and event to those exact preterminal bytes, verifies the prospective SHA, and never recomputes timestamps or cleanup outcome. Once the binding is durable, writers MUST NOT append a different failure/cancellation/heartbeat; reconcile the exact pending transition first. A crash before the index reference may pin only this exact pending binding. A success lacking its required reference or a referenced missing binding fails closed. Later authorized cleanup before retirement preserves the original success commitment.

Provenance dispatch MUST precede mutable primary state/generation/context classification. Existing unbound generated development records MUST NOT acquire a new binding from a context pointer or guessed historical bytes. Supported resolved generation-less legacy records may obtain an immutable legacy snapshot without a generation/action; ambiguous, unresolved or generated old records remain protected. Migration uses a retained frozen membership source (at most256KiB,1,024 UUIDs) and authoritative progress (at most1MiB). Under a separate bootstrap guard, publish/fsync an initialized source/progress directory before any origins. Before each origin, durably record the exact classification, source evidence and pending origin bytes; after origin durability, acknowledge its SHA and advance a persisted prefix. Restart resumes that prefix without reclassifying completed primary bodies. Only a pending decision may replay its exact missing origin; an acknowledged missing origin is never regenerated. Finalizing progress pins the exact proposed new-index SHA, generation, membership/phases/origin references and migration/source lineage before index publication and complete progress. Missing/corrupt progress cannot restart baselining; incompatible old-index replay fails closed. Ordinary operations require completed migration. Initial no-index inventory retains the existing8,192-name bound and persists the actual frozen UUID set; initialized history is never enumerated.

Migration source/progress capture, body hashing, closed decoding and prefix validation MUST occur outside the global journal lock. A completion receipt may be reused only within the original bounded operation, retaining exact file descriptors and lineage; every locked use, including final mutation, MUST recheck placement/metadata and index lineage. Under existing bootstrap custody, a bounded migration call may retain its fully validated decoded source/progress across its own writes only after recapturing each durably written progress file and verifying exact equality with the preencoded next bytes. Each new call or restart fully validates durable progress again. Missing, replaced or changed evidence ends that use; no cross-operation cache, per-UUID provenance substitution, budget reset, generic partial-write retry or acknowledged-prefix reclassification is permitted.

Maintenance MAY retain one operation-local active-index receipt containing fully closed-validated format/index bytes, decoded membership and completed migration lineage, captured outside the global journal lock under the original work budget. Every final use MUST validate exact bound-directory and pinned format/index/source/progress placement and metadata before using decoded membership, generation or origin/provenance/binding references. The same receipt MUST cover completed initialization/recovery and nested pointer/retirement validation; retaining decoded data MUST NOT introduce a cross-call cache or fresh budget. After its own authorized durable index write, maintenance may advance that receipt only after outside-lock recapture equals the exact preencoded next bytes. Changed evidence or failed post-publication recapture ends the pass; catch-and-continue cannot reuse stale authority. Cursor-only writes retain the same checks. All per-UUID owner, primary/result/seal/origin/binding/certificate, pointer-before-retirement and immutable certificate recovery requirements remain in force. This optimization does not authorize partial reservation search or assert a storage-throughput guarantee.

Retirement MUST publish/fsync an exclusive `model_catalog_transaction_retirement.v1` certificate before removing active membership. Its closed variants are evaluation_success, prepare_success_snapshot, prepare_non_success_snapshot, evaluation_non_success_snapshot and legacy_terminal_snapshot. All bind UUID and origin SHA. Evaluation success additionally requires actual generation, binding SHA, context/success/result commitments and resolved cleanup evidence, after required pointer publication. Other variants freeze exact retirement-time primary bytes plus closed present/absent result, seal and current-cleanup evidence digests, only after cleanupRequired is false and staging absent. Prepare success requires committed true and valid seal, with no result; non-success must retain its actual failed/cancelled/timed_out label and validate every applicable committed result/seal without promoting it. Legacy retains exact baseline bytes and supported legacy evidence, never fabricated generated requirements. Unknown/orphan/mismatched evidence protects capacity. All variants forbid further primary/current-cleanup mutation once certified, including before membership removal. Per-UUID control records are closed canonical JSON, maximum16KiB, with existing private descriptor/no-follow/single-link/ACL checks.

Archived generated eligibility MUST validate retained origin/certificate and, for evaluation success, binding/result/original immutable commitment without recreating membership or owner files. Missing or substituted evidence cannot be repaired by reclassification, pointer relocation or history scanning. Generic retained historical diagnostics are weaker read-only evidence and MUST NOT mint actions or eligible status. The1,024 unresolved cap still includes corrupt/protected/migrating entries; healthy mixed terminal histories must continue beyond2,052 operations without a lifetime cap. Bulk validation/encoding remains outside the journal lock; final exact receipt/index CAS and durable transitions retain the eight-second work budget and independent owner fence. None of these local persistence proofs grants artifact freshness, model identity, pricing, admission, physical computation or reward authority.

### 6.14c owned read-only catalog protocol (v1.9.9)

SPEC-044's owned catalog-read amendment governs the explicit app-read option groups, exact-target quick/verify/result behavior and closed progress output. The CLI MUST install the validated independent read lock/lifetime/deadline guard before config, resource or artifact work; capture configuration once, reject incompatible overrides/partial read selectors, and preserve existing-root-only expected-context checks. One request-local exact inspection is shared by discovery and local-action readiness, with no metadata/past-seal promotion or duplicate hash. Its original remaining budget propagates through the complete nested recovery, original-binding, result/seal, recommendation-index, reservation, validation and publication chain without renewal. Quick projections perform no weight-content reads; every active member needed for complete recovery MUST be decidable, and final recovery/action witnesses MUST remain valid after action-side reconciliation. Incomplete authority, recovery, pointer publication or fully framed output capacity fails explicitly without false absence, readiness or completed pending recovery. Verify binds all four selected signed feed inputs and preflights the actual completed event framing required by SPEC-044. Standalone protocol1 remains compatible; read authorization never grants mutation-owner, coordinator admission, pricing or settlement authority. The status lifecycle capability is advertised only with executable implementation, not inferred from version alone. Exact schemas, bounds, child ownership and compatibility requirements are normative in SPEC-044.
