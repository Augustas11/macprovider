...
**FR-C9.4. TOFU (trust-on-first-use): the partial unique index is the canonical enforcement; bearer-less duplicate sessions are admitted but non-routable.**
...
This index is the authoritative "at most one valid bearer per `provider_id`" enforcement. The reference store in `phase4-coordinator/internal/auth/tokens.go` installs it in `migrate()`; existing databases get a one-time migration that revokes pre-existing duplicate active rows so the index can be installed without aborting. `IssueToken` MUST surface a unique-constraint failure as a distinct sentinel error (`ErrActiveTokenAlreadyExists` in the reference implementation).

**(b) Settling-window wire contract** (when `RequireProviderTokens=false`). When a tokenless connect arrives for a `provider_id` that already has an unrevoked row, the coordinator MUST:

1. Attempt the mint (the partial unique index will reject the INSERT).
2. Catch `ErrActiveTokenAlreadyExists`.
3. Mark the admission with `AuthBearerlessDuplicate` (a new auth-state enum value; reference implementation: `pool.AuthBearerlessDuplicate`).
4. Admit the connection WITHOUT including `assigned_provider_token` in the ack frame (`hello_ack` or `auth_response`).
5. Log an info-level event with `provider_id` so operators surface duplicate admits in their pre-flag-flip audit.

The coordinator MUST NOT close the connection with `CloseInvalidToken` in step 4 — that hard-reject behavior bricked old-binary cohorts on Pearl during the 2026-06-12 deploy attempt and was reverted in v0.8.3.

**Routing + billing + registry consequences (normative; v0.8.3 fix-pass-3, PR #69 codex security MAJOR-1).** An admission marked `AuthBearerlessDuplicate`:
...
Pure "row existence" is NOT operational evidence under the v0.8.3 contract. The runbook MUST gate the flag flip on `last_used_at` freshness — not on `list-tokens` enumeration alone — to prevent flipping with an attacker-owned bearer in the active set.
