...
- § 17.7 quota-refund matrix gains a `500 route_snapshot_failed` row: a genuine first-attempt, no-prior-dispatch snapshot-write failure (no provider reached, no finality header, no prior-dispatch signal) is a **no-charge refund** (was: settled on the prompt estimate — SPEC-022 "Known limitations (A)", now resolved). Documents the two **prior-dispatch exceptions** that instead settle on the estimate so real provider work credited in `observe` mode is not erased: (1) coordinator-internal failover, signalled request-wide by the coordinator via the `X-MacProvider-Settlement-Prior-Dispatch` header (`attemptN > 0`); (2) gateway retry past a provider-dispatched `provider_*` 502. § 17.1 notes the coordinator `500 route_snapshot_failed` is passed through verbatim. Coordinator + gateway behavior change; SPEC-022 item (A) reconciled to resolved.
...
| 500 `route_snapshot_failed` (pre-dispatch, no prior provider dispatch) | 0 | **none** | Coordinator failed to durably persist the SPEC-022 route snapshot before dispatching any provider; no provider was reached. Gateway refunds the reservation, writes a zero-token audit row, and passes the coordinator body through verbatim (item 18). Applies only to a **genuinely first-attempt, no-prior-dispatch** failure. **Prior-dispatch exceptions** (both settle on the prompt estimate per the 502 rows instead, because prior provider work may have been credited in `observe` settlement mode and refunding would erase it): (1) *coordinator-internal failover* — the coordinator stamps `X-MacProvider-Settlement-Prior-Dispatch` on a `route_snapshot_failed` emitted after an earlier provider attempt within the same request (`attemptN > 0`); (2) *gateway retry* — the gateway reached this terminal 500 only after retrying an earlier provider-dispatched `provider_error`/`provider_failed`/`provider_disconnected` 502. A settlement-finality header on the response also disqualifies the refund. |
