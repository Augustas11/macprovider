...
- **R3-audit reconciliation (2026-07-12, spec-only).** Two residual occurrences fixed: § 5.4.1's tag-trim rule still said "ASCII whitespace" (now Unicode `strings.TrimSpace`, matching § 1.3); and the § 1.3 "buyer-triggered deletion" lifecycle precondition now reconciles with the not-directly-purgeable provider KV cache (satisfied indirectly: `DELETE /v1/sticky` severs sticky routing + provider TTL ages the entry out).
...
- The cache has explicit lifecycle: creation, eviction, and buyer-triggered deletion. **(v0.9.8 — provider-local prefix cache:)** the provider-local KV cache is not *directly* buyer-addressable, so "buyer-triggered deletion" is satisfied **indirectly**: `DELETE /v1/sticky` removes the coordinator sticky entry, after which subsequent turns no longer route stickily to that provider (halting further extension of its cache), and the provider-local entry then ages out under its own TTL/LRU (SPEC-024 v0.2 §11 FR-CI4). The buyer thus retains a lifecycle lever (sever routing + bounded TTL), even though the provider KV is not a directly deletable object — a lifecycle *split* (coordinator map = directly deletable; provider KV = TTL-bounded + indirectly severable) recorded as the SPEC-024 v0.2 §13 disclosure-completeness item.
