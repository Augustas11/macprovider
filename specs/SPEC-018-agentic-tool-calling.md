...
**Version:** 0.1.3 (2026-06-27, Claude blind-spot absorption — drops §3.6 mixed-sentinel rule (DoS), reworks AC-23 (was tautology), drops Claude Code + Cursor IDE from OpenAI-wire framework list (factually wrong), and 12 other security/precision fixes)
...
AC-23. **Forward compatibility invariant (PD r2 Q-1, §10c) — reworked v0.1.3 to fix Critic H-1.** A v0.2-or-later regression test captures non-streaming tool-call response fixtures **from the candidate vN.M release** (with any new fields, deltas, or finish reasons enabled) and verifies that a **v0.1.2-targeted** client parser (e.g. OpenAI Python SDK pinned to the version current at v0.1.2 lock time — the SPEC repo's `tools/version-pins/openai-python-spec-018-v0_1_2-baseline.txt` records the exact semver) successfully parses each response without raising on unknown fields and without rejecting due to schema validation. The v0.1.2-fixture-vs-v0.1.2-parser tautology that v0.1.2's AC-23 specified is explicitly NOT sufficient; the test MUST exercise the new-emission-shape-against-old-parser direction. Verified as a release gate for any SPEC-018 vN.M version that follows v0.1.2.
