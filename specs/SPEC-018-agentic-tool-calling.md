...
**Version:** 0.2.9 (2026-09-21, serial tool-turn stop when `parallel_tool_calls` is omitted)
...
**v0.2.9 buyer-visible deltas (read this if you're skimming):**
- v0.2.9 accepts OpenAI `parallel_tool_calls`. Omitted or `false` is a serial turn: after the first complete valid tool, the provider MUST stop generation, emit `finish_reason:"tool_calls"`, then `[DONE]`, and MUST NOT keep decoding leftover markup. `true` keeps §2.2 multi-call collection.
- This is a §10c.1 lock-amendment (Amendment 4) of §2.2 for the default omitted-field case. Pi and typical agent loops omit the field and execute one tool per turn; stopping at the first complete tool is required so leftover model chatter cannot hang or kill the stream.
- Incomplete JSON, unclosed function-XML, undeclared names, and byte-cap failures MUST NOT be upgraded into a successful serial stop.
- No coordinator/gateway leftover policy change. Registers `SPEC-018-R004`.

- **v0.2.9 (2026-09-21, serial tool-turn stop):** **Load-bearing amendment:** amends §2.2. Historical v0.1–v0.2.8 text required preserving every recognized tool call in the generated text. That rule silently assumed generation would end on its own. Live Qwen agent turns emit a valid tool then leftover tags or prose; hiding the leftover without stopping decode leaves the Mac talking until timeout. Replacement: omitted/`false` `parallel_tool_calls` stops after the first complete valid tool. Residual risk: a client that wants two tools in one assistant message MUST send `parallel_tool_calls: true`. Mitigation: explicit boolean on the request; default matches Pi.
...
**AMENDED v0.2.9:** The N-call preservation rule applies when `parallel_tool_calls` is `true`. When the field is omitted or `false`, the turn is serial: after the first complete valid declared tool (complete JSON object or closed function-XML, within byte/depth caps), the provider MUST stop generation, emit `finish_reason:"tool_calls"` and the transport terminal marker, drop leftover markup, and MUST NOT synthesize further `tool_calls[]` from later text in that turn. A non-boolean `parallel_tool_calls` is HTTP 400 `invalid_request`. Incomplete or undeclared markup MUST NOT trigger serial stop.

Source: `phase3-binary/Sources/macprovider-cli/ToolCallParser.swift:29-50`; locked by `phase3-binary/Tests/macprovider-cliTests/ToolCallParserTests.swift:21-30`; serial stop `phase3-binary/Sources/macprovider-cli/ModelRuntime.swift` and `phase3-binary/Sources/MacProviderCore/ChatCompletionRequest.swift`. Registers `SPEC-018-R004`.
...
AC-65. **v0.2.9 serial stop after first complete tool.** For a Qwen modelID with enabled tools, `parallel_tool_calls` omitted or `false`, and model output a complete declared JSON tool object followed by leftover markup or a second tool, the provider stops generation after the first complete valid call, the stream terminal is `finish_reason:"tool_calls"`, `tool_calls[]` contains only that first call, and leftover is not buyer-visible content. Fail condition: generation continues until a request timeout, the stream dies with `malformed_tool_call`, leftover tags remain in `content`, or a second tool is emitted.

AC-66. **v0.2.9 explicit parallel keeps multi-call order.** For the same two-call model output as AC-65 with `parallel_tool_calls: true`, `tool_calls[]` preserves both calls in textual order. Fail condition: the second call is dropped.
...
- Amendment 4 (v0.2.9): §2.2 v0.1-locked “preserve all N recognized tool calls” → scoped to `parallel_tool_calls: true`. Omitted/`false` is serial: stop after the first complete valid tool. Rationale: agent loops (Pi) omit the field and execute one tool per turn; leftover decode after a valid tool hangs or kills the stream. Residual risk: a client wanting two tools in one assistant message must send `parallel_tool_calls: true`. Mitigation: explicit request boolean; default matches Pi.
