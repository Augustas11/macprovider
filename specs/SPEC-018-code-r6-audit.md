...
CONFIRMED. AC-23 now requires a `v0.1.3-baseline` client parser pinned by `tools/version-pins/openai-python-spec-018-v0_1_3-baseline.txt`, and §10c now matches that same `v0.1.3-baseline parser` wording plus pin-file reference.

Sanity grep found remaining `v0.1.2` references only in metadata/change-log or historical explanatory contexts: dropped mixed-sentinel history, AC-22 placeholder history, and AC-23’s explicit statement that the old v0.1.2 tautology is insufficient. No remaining normative MUST / AC / IMPL obligation points to a v0.1.2 baseline parser.

Checked `eb5bdde..HEAD` for the scoped spec file: no later diff in `specs/SPEC-018-agentic-tool-calling.md`.
