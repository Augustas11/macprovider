...
| AC-022-38 | Partial | `phase4-coordinator/internal/billing/settlement_receipts_test.go` `TestIngestSettlementReceiptPersistsVerifiedStateAndRedactedAudit`; `phase4-coordinator/internal/billing/endpoints_test.go` `TestSettlementReceiptDiagnosticsQueryShapeIsBounded`; add aggregate counter query tests by policy version, model id, entrypoint, and reason code. |
