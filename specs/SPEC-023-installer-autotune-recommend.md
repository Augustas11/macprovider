...
- **v0.16.0 (2026-09-24)** — GGUF `huggingface_revision` source and a
  ...
  MLX-snapshot leg for external runtimes stays deferred (SPEC-010 1.10
  R007(h)).
...
- **v0.14.3 (2026-09-23)** — Identity is not settlement for loopback
  ...
  `network_visible_unpriced`) but cannot reach `settlement_capable`. Every
  `gguf` source is a loopback runtime, and SPEC-047-R003(iv) v0.1.10 bars
...
- **v0.14.0 (2026-09-19)** — Market-peg liquidity basis re-defined after OpenRouter removed per-endpoint 30-day volume (implemented on branch `feat/openrouter-catalog-pipeline`; NOT docs-only). OpenRouter's `GET /api/v1/models/{id}/endpoints` response no longer carries per-endpoint `completion_tokens_last_30d` (removed API-wide and confirmed absent from the documented `PublicEndpoint` schema, verified 2026-09-18). The v0.12.0 §3.3 rule 4 basis — **volume-weighted median gated by a 30-day completion-token floor** — is therefore uncomputable, and its own stated remedy ("extend the snapshot schema until [30-day tokens] are present") is impossible because the upstream source no longer publishes the field. v0.13.0 already shipped as the concurrency-calibration revision (SPEC-023-R009), so this market-peg change is v0.14.0.
  ...
  2. **This replaces the per-endpoint 30-day-token gate, NOT the demand signal.** OpenRouter still publishes per-**model** 30-day volume via `rankings-daily.total_tokens`; §3.4 / rule 5 / `or_completion_tokens_30d` demand is **unchanged**. Only the per-**endpoint** liquidity gate/weight moved to the 30-minute request-count basis. Malibu served-token counts remain forbidden as scoring or a market-pegged sign.
  3. **Snapshot provenance + policy binding.** The snapshot `liquidity_filter` records `liquidity_signal = "openrouter_request_count_last_30m"`, `price_median_method = "unweighted_over_active_endpoints"`, `minimum_request_count_last_30m`, `min_distinct_providers`, `distinct_provider_count`, and one `eligible_endpoint_liquidity` entry per distinct provider. `compute` MUST bind **both** the request-count floor and the distinct-provider quorum to the active policy: a snapshot priced under looser thresholds MUST NOT be repriced under a stricter policy. The snapshot also records `skipped_free_ranked_models` (each dropped `:free` ranked model with its original rank) so an observed-cohort shortfall is auditable: `observed_model_count + len(skipped_free_ranked_models) == requested_top_n`, and the market-peg gate asserts on that requested cohort.
  4. **Policy.** `scripts/openrouter_pricing_policy.json` gains `min_endpoint_request_count_30m` and `min_distinct_providers`; `policy_version` is `openrouter-market-feeds-v0.13.0`.
  5. **`SPEC-023-R008` / AC-MKT-3 amended** to the request-count basis; the "pretending a 30m throughput flag is a 30-day volume filter is non-compliant" prohibition of the prior rule 4 is **repealed for the per-endpoint liquidity gate specifically**, because the 30-day field it protected no longer exists upstream. AC-MKT-4/5 (credits, demand fields) are unchanged.
  6. **Non-earning proposer tool.** `scripts/openrouter_pricing_engine.py propose` emits an `openrouter-catalog-proposal` (`status: "proposal_only_never_applied"`) that ranks yield-first over MLX-servable candidates by capacity tier. It NEVER applies, signs, mints a `rate_class`, or feeds a release input; every recommendable promotion still requires an explicit §16 operator decision. Vision-language models served for text are flagged `manual_serving_verification_required`. It is a review input to §16 intake, not a pricing authority.
  7. **Residual (documented).** Pegging to a third-party oracle's `provider_name` strings is not fully Sybil-proof — one entity may appear under multiple provider names, and OpenRouter exposes no stable provider-entity id. Provider-collapse + the distinct-provider quorum **bound** this (the price cannot be steered by adding endpoints under one name, and a thin sub-quorum market prices nothing), but do not eliminate it; it is backstopped by trusting OpenRouter provider onboarding as the price oracle and by proposals never auto-applying.

- **v0.13.0 (2026-09-18)** — Guarded empirical concurrency calibration replaces the blind concurrency constant (#1589). Provider `max_concurrency_override` (= served `--max-batch` = advertised `slots_total`) was emitted by `autotune --recommend` from a fixed chip/RAM tier constant (`AutotuneRecommendHardware.recommendedMaxBatch`: M-Ultra≥128 GB → 4 flat regardless of 128/256/512 GB, model size, KV footprint, or the box's real GPU-compute/latency ceiling). That constant is never measured; SPEC-023 never governed the value. §9.2 adds an explicit, opt-in `--calibrate-concurrency` post-selection probe that measures the selected, already-verified signed artifact's **aggregate** decode throughput and per-stream tail latency across a bounded batch sweep on the specific box, and emits the empirically-best `max_concurrency_override` under a memory-fit hard upper bound and a tail-latency ceiling. New requirement `SPEC-023-R009`, output-contract field `concurrency_calibration`, acceptance criteria AC-43 and AC-44.
  1. **The constant is the default and the fallback, never removed.** Without `--calibrate-concurrency`, behavior and the §6 byte shape are unchanged: the RAM/chip tier constant still emits and applies exactly as before, and it is also the conservative fallback when the memory-fit bound cannot be computed (rule 1 below). It is NOT a floor on the calibrated value: an opt-in run MAY emit a value below the tier constant (down to 1) when the box cannot sustain the constant within the latency ceiling — that is the correct, safer outcome, not a regression. The opt-in probe only ever emits a value in the closed range `[1, min(memory_fit_cap, max_concurrency_override_limit = 8)]`, so it can neither exceed the served hard cap (`ProviderStatus.maxConcurrencyOverrideLimit`) nor advertise slots a box cannot host.
  2. **Aggregate, not single-stream.** Unlike the Stage-2 sweep — which measures serialized single-stream replicates and therefore cannot observe concurrency's aggregate effect — R009 drives `B` genuinely concurrent uncached probe streams at each candidate batch depth `B` and selects the depth that maximizes aggregate tokens/sec, tie-broken toward the LOWER depth (memory-risk posture, matching SPEC-029 FR-5). A depth is feasible only when every stream succeeds, no stop-token leaks, and per-stream p95 TTFT satisfies the buyer-facing ceiling AND does not regress the single-stream (`B=1`) p95 beyond a bounded factor. The probe measures whatever serve mode production will run: on today's independent single-stream path (SPEC-038 §1) the aggregate optimum is often low and honest; once continuous batching (SPEC-038/039) is enabled the same probe measures the shared-forward aggregate gain. The calibration is engine-agnostic by construction.
  3. **Scope boundary vs SPEC-029 and SPEC-028.** SPEC-029 owns the offline, class-stratified research sweep and the (still-unpublished, class-routed, deferred) `workload_profiles` catalog data; R009 owns a local, per-box, post-selection installer-path calibration analogous to the §9.1 `SPEC-023-R003` context calibration — the two are complementary, not the same artifact, and R009 publishes nothing to the fleet. When a draft model is configured, SPEC-028 FR-4 forces `effective_max_batch = 1`; R009 MUST pin the calibrated value to 1 in that case and MUST NOT sweep, exactly as serve preflight would downshift. This revision changes no catalog JSON, no rate card, no signed feed, and no coordinator/Go code; it is a provider-CLI recommendation-path change plus this governance.

- **v0.12.0 (2026-09-13)** — One OpenRouter snapshot authors rate-card and demand-rank (#1484). Docs-and-governance; no catalog JSON, generator, or CLI/Go code in this revision.
  1. **Four belts, not one lock.** FeedSchema-A/B, MoneyTable-A/B, and Score-A/B are named in §3.3.2. This revision does **not** add keys to **published** `rate-card.json` / `demand-rank.json`. v0.10.3 `AutotuneStrictJSON.exactKeys` and `catalog-release.py` `exact_keys` **reject** unknown keys; a published `source_snapshot` or `or_*` field is a fleet paid-recommend outage. Digest bind is unpublished `market-peg-bind.json` (`macprovider.market-peg-bind.v1`); it MUST NOT extend published `release.json` or v3 ledger exact-key sets.
  2. **Single market snapshot.** `scripts/openrouter_pricing_engine.py fetch` is the only market source. Policy MUST map every `recommendable` key, including Llama 3.2 3B / Llama 3.1 8B / Qwen3-8B. Compute emits proposals only (no apply). Retain-on-absence of the current rate-card row is **repealed** for a market-pegged compute. `catalog-release` MUST take the rate-card proposal as a named input and fail closed unless every recommendable published row's three credit fields equal the proposal.
  3. **Credits include × 1_000_000.** `credits_per_mtok = floor(or_liquid_usd_per_mtok × (1 - undercut) × 1_000_000 / usd_per_million_credits)`. `$0.08/M × 0.80 × 1e6 / 1.0 = 64000`. A 0-credit recommendable row fails closed. Cache-hit = `floor(prompt_rate_per_mtok × cache_hit_fraction)` with policy default `0.25`. One undercut for every mapping; `coding_specialist` does not select a second formula on a v0.12 cut.
  4. **Liquidity is numeric.** Default aggregator is volume-weighted median over paid `status==0` endpoints whose window completion tokens are `>= max(min_endpoint_completion_tokens, floor_fraction × model_tokens_30d)` with defaults `min_endpoint_completion_tokens = 1_000_000` and `floor_fraction = 0.05`. Cheapest-print is forbidden as the published rate. `:free` / zero-price dropped. A 1-token $0.33 quote MUST NOT become `or_liquid_*`. An illiquid recommendable key fails the **whole compute** (no proposals). Demotion from `recommendable` is a prior intake cut, not this compute.
  5. **Demand.** Proposals carry `or_completion_tokens_30d`, sourced from the OpenRouter `rankings-daily` `total_tokens` field — the only per-model 30-day volume OpenRouter publishes — as the demand proxy. `or_requests_30d` is informational only: it MUST be `0` when the ranking source does not expose per-model request counts (it does not today) and is never a scoring input. Published feeds at FeedSchema-A carry neither field. Score-A (shipped CLI) still multiplies by `demand_weight`; rewriting that field is **not** a 16 GB guarantee because `max(weight, cold_start_floor=0.15)` re-flattens small global shares. Score-B (FeedSchema-B CLI, later belt) uses `demand_share` among §4 `eligible_rows`. #1483 remains the 16 GB floor until Score-B. Malibu served-token counts MUST NOT enter scoring or a market-pegged sign.
  6. **SPEC-005 v0.6.7 lockstep.** MoneyTable-A: signed feed is authoring SoT; yaml is a required byte-parity copy; `RateFor` still reads yaml. MoneyTable-B: `RateFor` reads verified signed-feed bytes; yaml `default` + globals only. Class expansion MUST NOT fill a market-pegged recommendable row. Rule 8 byte-identical class-expansion carve-out does not apply to a market-pegged **repricing** cut. `SPEC-005-R010` stays the D1a wholesale unit.
  7. **`SPEC-023-R008`** with AC-MKT-1..AC-MKT-13. This revision ships FeedSchema-A + Score-A + MoneyTable-A. Score-B MUST NOT run against FeedSchema-A. Implementation is #1484. #1483 stays a separate 16 GB RAM-class rail.

- **v0.11.0 (2026-09-11)** — RAM-class recommendation rule (#1483). New §4.1 constrains the §4 `raw_score` argmax on 16 GB-class hardware: because the small-dense onboarding lane is priced at a single `$0.027/M` completion parity (Entry 116 Llama 3.2 3B, RESEARCH_227 Llama 3.1 8B, P2-02 Qwen3-8B all `completion_rate_per_mtok = 27000`) and throughput is a first-order term, the ~2× faster 3 GB onboarding SKU would otherwise out-score the eligible dense 8B rows on a 16 GB Mac even though 8B carries the higher demand weight (`0.45` vs `0.42`). §4.1: when `mac.ram_gb >= 16` and at least one eligible RAM-class-matched row (fixed floor `min_ram_gb >= 12`, i.e. the 16 GB-class dense floor `16 − safety_margin_gb`, a constant applied identically on 16/24/32/64 GB Macs — not a per-Mac `ram_gb − safety_margin_gb`) exists, an eligible RAM-class onboarding SKU (`min_ram_gb <= 4`) MUST NOT be `recommended_model` and is ranked below every eligible RAM-class-matched row; among those rows the §4 argmax and tiebreakers are unchanged (Llama 3.1 8B over Qwen3-8B). Hard §5 gates decide eligibility first, so the rule fires only while a RAM-class row is genuinely eligible — when every RAM-class row fails a hard gate (swap/thermal/buyer-TTFT/headroom) the onboarding SKU may again be the pick; advisory `tps_below_gate` / `ttft_above_gate` / `swap_observed_under_load` never send a 16 GB Mac back to 3B. The 8 GB path is unchanged (RAM-class rows are ineligible on 8 GB, so Llama 3.2 3B stays the SPEC-003 / Entry 116 onboarding default). The selected row keeps the stable `selected_best_expected_earning_potential` slug; the policy is surfaced on the demoted onboarding SKU via the new `lost_reason` value `deprioritized_ram_class_onboarding_sku`. Selector-policy only: reprices nothing, adds no catalog model, needs no signed-feed republish. New requirement `SPEC-023-R007`, acceptance criteria AC-41 and AC-42.
- **v0.10.4 (2026-09-11)** — Intake signals sourced (BYOM v0.2 slice 5, #1453). SPEC-017 v0.2.1 §5.2b adopts the §16.2(a) `unmatched_model_request_count` field contract by amendment — the §16.7 precondition is met from that version and the signal MAY satisfy `INTAKE_BUYER_REQUEST_FLOOR`. Each coordinator-derived signal is bound to ONE authenticated source whose exact response bytes the operator retains outside the repository in the private intake audit store (new §16.8 rule 9) and whose SHA-256 the `*_source_sha256` digests cover: §16.2(a) from `GET /v1/stats/intake` (`macprovider.stats-intake.v1`, the LATEST complete aggregator window), §16.2(b) from SPEC-047 v0.1.6 `GET /admin/model-admission/intake` (`model_admission_intake_offer_counts.v1`), §16.2(c) from the same `/v1/stats/intake` response's `fleet_ram` histogram, evaluated conservatively by class floor (`fleet_fit_fraction_ppm = floor(floor(fraction × 1 000 000) / 50 000) × 50 000`, the exact ratio floored to a 5% grid because the manifest is public). **Normative changes, stated plainly:** (i) §16.2(a) item 9 — k-anonymity for a named bucket is a DISTINCT-PRINCIPAL count (at least `INTAKE_K_ANONYMITY_MIN` principal tokens contributed), not a request count, and a suppressed bucket is omitted from the wire rather than reported by key (AC-CAT-13 amended); (ii) §16.8 `signals` gains per-signal window fields (`unmatched_model_request_window_id`/`_start`/`_end`, `distinct_provider_offer_window_start`/`_end`, `fleet_fit_window_start`/`_end`) — `schema_version` stays `macprovider.intake-decision.v1` because no manifest has ever been committed (the ledger is v2 with no artifact-bound release); (iii) rule 2 — a source digest is non-null whenever the source was read, suppressed or empty results included, and null only when the source did not exist or was unavailable; the absent-reason enums gain `source_unavailable` (all three coordinator signals) and `incomplete_window` (buyer demand); (iv) rule 3 — staleness is checked per signal window; (v) rule 4 — for `unmatched_model_request_count` suppression is not key-attributable on the wire, so `unmatched_model_request_suppressed` is always `false` and an absent key records `no_observations`; (vi) new rule 9 — the exact source responses are retained in the operator's PRIVATE intake audit store (never in the public repository, which would republish a private endpoint) and the generator re-derives every recorded value from them (AC-CAT-21 amended); (vii) §16.4 — `INTAKE_K_ANONYMITY_MIN` is FIXED at 3 in this revision, not release-tunable, because both sources suppress against it and omit what a consumer would need to re-suppress; (viii) rule 3 — `observation_window_*` may be `null` when no windowed signal participates, and "one cadence period" is defined as 31 days; (ix) §16.2(b) — "active route, trust, payout, or registration sanction" is defined by the SPEC-047 v0.1.6 `provider_intake_sanctioned` predicate, cited by name. The generator validates `intake-decision.json` against §16.8 and re-evaluates §16.3 for every entry; `docs/runbooks/catalog-monthly-intake-release.md` is the §16.5 cadence runbook.
- **v0.10.3 (2026-09-10)** — Primary-only settlement lifted by SPEC-010 v1.7 R007 (BYOM v0.2 slice 3; resolves §13 Q14).
  1. `macprovider.gguf-file.v1` is now a canonical SPEC-010-R002 wire pair; §3.7.4 cites SPEC-010 rather than reserving the name.
  2. §3.7, §3.7.4, §3.7.7 R004, AC-CAT-7, and AC-CAT-17 no longer restrict `catalog_priced`, `settlement_capable`, the SPEC-010 wire pair, and SPEC-022 route-time binding to the primary artifact: any `verified` artifact of a `listed`/`recommendable` key that satisfies SPEC-010-R007 (exact pair match against the release-bound feed, six-value route-time evidence for a non-primary member, every tier and admission gate unchanged) may be a settlement identity. Pricing stays model-key scoped; no SPEC-005 change.
  3. §13 Q14 is resolved by reference to SPEC-010 v1.7.
  4. AC-CAT-7(i) scopes the hello leg to a serving runtime path (SPEC-010-R007(e)); the v0.10.3 CLI reports the GGUF pair in the offer only. The v0.10.0 change-log points 8–9 are annotated as historical.
...
The published rate-card JSON schema (FeedSchema-A, also the v0.10.3 closed set) is:
...
v0.10.3 `AutotuneStrictJSON.exactKeys` and `catalog-release.py` allow **exactly** those top-level keys and the five per-row credit/share/multiplier fields. A published feed carrying `source_snapshot` or any other extra key is `rate_card_integrity_failure` and **blocks paid recommendation**. FeedSchema-B MAY add `source_snapshot` to the published bytes only after a bridge CLI, a coordinator parser that allowlists the same keys, and previous-stable at or after that bridge have shipped — the artifact-feed Stage A/B pattern in §3.7.8. Until then the digest bind is an authoring-ledger field (§3.3.2 rule 1), not a served-feed field. `version` hashing in the four steps below is unchanged.
...
#### 3.3.2 OpenRouter market snapshot (v0.12.0)

**SPEC-023-R008:** Rate-card and demand-rank **authoring** for `recommendable` keys MUST follow this rule. Activation is four belts (do not overload artifact-feed Stage A/B):

| Belt | When | Published feed keys | `RateFor` source | Scoring demand term |
|---|---|---|---|---|
| FeedSchema-A | this CLI generation | v0.10.3 closed set only | (see MoneyTable) | Score-A |
| FeedSchema-B | after bridge CLI + coordinator parser + previous-stable | MUST add `or_completion_tokens_30d` on every recommendable row before Score-B may enable; MAY add `source_snapshot` / `or_requests_30d` | (see MoneyTable) | Score-B |
| MoneyTable-A | until coordinator PR | signed feed authors; yaml required parity copy | yaml | — |
| MoneyTable-B | separate coordinator PR | signed feed | verified signed-feed bytes; yaml `default`+globals only | — |
| Score-A | shipped CLI | — | — | `demand_weight` (existing formula) |
| Score-B | after FeedSchema-B | — | — | `demand_share` among §4 `eligible_rows` |

1. **One fetch, two proposals, no apply.** `fetch` writes a snapshot. `compute` reads that snapshot plus `scripts/openrouter_pricing_policy.json` and writes a rate-card proposal and a demand-rank proposal. The engine MUST NOT write published catalog files, MUST NOT write coordinator yaml, and MUST NOT sign. Current-engine **retain-on-absence** of the existing rate-card row is repealed for market-pegged compute: an unmapped or illiquid recommendable key emits **no proposals**.
2. **Named ingest.** `catalog-release` MUST take the rate-card proposal and demand proposal as named release inputs. It MUST recompute both from `(snapshot bytes, policy bytes, engine bytes)` and fail closed unless the three credit fields and Score-A `demand_weight` bytes-equal the recomputation. The bind is an **unpublished** sidecar `market-peg-bind.json` with `"schema_version": "macprovider.market-peg-bind.v1"` and the exact closed set `{schema_version, content_digest, policy_digest, engine_sha256, ranking_window_end_date}`. It is committed with release inputs, never served at `/v1/*`, never baked into a CLI payload, and MUST NOT be added to published `release.json` exact keys or to a v3 ledger row (those closed sets stay `{schema_version, release_id, generated_at, policy_version, feeds}` and `{generated_at, policy_version, feeds, artifact_bindings, intake_decision_sha256}`). A later ledger v4 MAY hash this sidecar; this revision MUST NOT extend v3. `catalog-release` MUST fail closed on an unknown sidecar key, a missing required key, or `engine_sha256` that does not match the engine bytes it executed.
3. **Policy maps every recommendable key.** Including `meta-llama/llama-3.2-3b-instruct`, `meta-llama/llama-3.1-8b-instruct`, and `qwen3-8b`. The four-SKU policy is non-compliant. Rank `top_n` MUST NOT drop a mapped recommendable key.
4. **Liquidity-filtered paid price.** [v0.14.0 — OpenRouter removed per-endpoint 30-day completion tokens; this rule supersedes the v0.12.0 volume-weighted basis, which is uncomputable.] Eligible endpoints: `status == 0`, strictly positive paid completion price, not `:free`, and a per-endpoint 30-minute text activity `perf_last_30m_by_workload.text_generation.request_count >= min_endpoint_request_count_30m` (policy default `30`). Collapse the eligible endpoints to **one representative quote per distinct provider** (the provider's lowest by completion then prompt). If fewer than `min_distinct_providers` (policy default `2`) distinct providers remain, the key is **illiquid**. **Compute `or_liquid_*` (proposal/snapshot only, not a published feed key) is the UNWEIGHTED median** (deterministic lower median) over the collapsed distinct-provider quotes' completion (and separately prompt) prices. `request_count` gates eligibility only and MUST NOT be used as a median weight (an API-controlled weight is manipulable; an unweighted median over collapsed distinct providers is not). Cheapest-print is forbidden as the published rate. A 1-token $0.33 quote MUST NOT become `or_liquid_*` (it fails the request-count / distinct-provider gate). `compute` MUST bind both the request-count floor and the distinct-provider quorum recorded in each row's `liquidity_filter` to the active policy. A mapped recommendable key with no priced result (below the request floor, below the provider quorum, or `:free`/zero-price only) fails the **whole compute** (no proposals). Demotion from `recommendable` is a prior intake cut, not this compute. Partial rate cards are forbidden.
5. **Credit mint (SPEC-005 D6).** For prompt and completion:
   `credits_per_mtok = floor(or_liquid_usd_per_mtok × (1 - undercut) × 1_000_000 / usd_per_million_credits)`.
   Worked example: `$0.08/M × 0.80 × 1e6 / 1.0 = 64000`. A 0-credit recommendable row fails closed. `undercut` is `broad_fleet_undercut_fraction` (default `"0.20"`) for **every** mapping on a v0.12.0 cut — `coding_specialist` does not select a second formula. `prompt_cache_hit_rate_per_mtok = floor(prompt_rate_per_mtok × cache_hit_fraction)` with policy default `0.25`; fail closed if omitted, if `> prompt_rate`, or if `0` unless `cache_hit_fraction` is explicitly `0`. Do not fold `global_multiplier_ppm` into the mint; billing still applies it.
6. **48-hour stale.** A snapshot is stale iff digest-bound `ranking_window_end_date` is older than 48 hours. Mutating unbound `fetched_at` MUST NOT revive it. `content_digest` MUST include the ranking window dates.
7. **No installer live-fetch.** CLI and installer MUST NOT call OpenRouter.
8. **`default` row.** A market-pegged cut MUST set `default` prompt, cache-hit, and completion credits equal to the mapped recommendable row with the **minimum** `completion_rate_per_mtok`. Recommendable keys still MUST NOT resolve only to `default` (rule 7).

### 3.4 Demand signal

The recommendation engine fetches `https://coordinator.malibu.tech/v1/demand-rank` and falls back to a baked snapshot when the signed feed fetch fails, times out, fails Ed25519 detached-signature verification, or fails schema validation. The demand signal is an **OpenRouter 30-day volume snapshot**, signed and served as static JSON. It is not a coordinator demand endpoint and MUST NOT be substituted with Malibu `/v1` served-token counts (those measure current supply, RESEARCH_229 FM-2 / FM-5).

The **published** demand-rank JSON schema at FeedSchema-A is the v0.10.3 closed set:
...
- `source` MUST remain `openrouter_completion_token_rank_operator_curated` on a market-pegged **published** feed so v0.10.3 `validated()` does not reject it. A market-pegged `catalog-release` MUST reject any other source, including `macprovider_buyer_supply_deficit_v1`.
- `cold_start_floor` must equal `0.15`.
- `diversification_band` must equal `0.85`.
- **Proposals** (not published FeedSchema-A bytes) MUST carry `or_completion_tokens_30d` and `or_requests_30d` as required non-negative integers (`0` means no history; do not omit). `or_requests_30d` is diagnostic and is **not** a `raw_score` input. If the rankings series has no request count, store `0`. FeedSchema-B MAY copy those fields onto published rows after the bridge CLI allowlists them.
- `rows.<model_key>.demand_weight` remains a finite number in `[0.0, 1.0]`. On a market-pegged cut it MUST equal `tokens_i / max(tokens_j for recommendable keys in this feed with tokens > 0)`, or `0` when `tokens_i == 0` (do **not** write `cold_start_floor` into `demand_weight`; Score-A already applies `max(weight, 0.15)`). A feed whose positive-token recommendable set is empty fails authoring closed (no divide-by-zero). Hand-typed 0.42/0.45 weights are non-compliant. Score-A `max(weight, 0.15)` **re-flattens** any SKU whose global max-norm is `< 0.15`; that is **not** a 16 GB guarantee. #1483 and Score-B are.
- `rows.<model_key>.rank` is either a positive integer OpenRouter completion-token rank or `null`.
- `rows.<model_key>.recommendable` is the operator's deployability switch.
- `rows.<model_key>.min_provider_target` is the desired buyer-ready provider floor.
- `rows.<model_key>.ready_provider_count` is optional, non-negative. When present and no explicit multiplier is supplied, the effective multiplier is `clamp(min_provider_target / max(ready_provider_count, 1), 0.5, 2.0)`.
- `rows.<model_key>.supply_deficit_multiplier` is optional and, when present, is authoritative in `[0.5, 2.0]`.
- `rows.<model_key>.min_dwell_hours` is optional in `[0, 720]`.
- When neither supply field is present, the effective supply-deficit multiplier is `1.0`.
...
**Pricing is model-key scoped; settlement identity is artifact-scoped.** Two artifacts of one model key are priced identically, because price attaches to the model key (§3.3, §3.3.1). Settlement does not follow pricing. A settlement binding names one specific artifact `hash` together with its `hash_algorithm`. In v0.10.0–v0.10.2 the only artifact that could be named was the model key's **primary** artifact — the `mlx_safetensors` entry named by `primary_artifact_id`, `verified`, hashed under `macprovider.snapshot-manifest.v1`, whose `hash` is byte-identical to the candidate row's `model_sha256` — because SPEC-010 recognized only that identity. From v0.10.3, SPEC-010 v1.7 **R007** recognizes every `verified` artifact of a `listed`/`recommendable` key in the release-bound feed as a member of that key's identity set, so any such artifact may be named by a settlement binding when the R007 conditions hold: the provider's named `(hash_algorithm, hash)` pair equals that member exactly, the pair resolves to exactly one `(model_key, artifact_id)`, and — for every feed-derived binding, the primary included — the SPEC-047-R003 six values ride in the immutable route-time record (only a primary identity bound directly through the signed candidate row is exempt). `verified` remains necessary and never sufficient; every tier gate here and every SPEC-047 admission gate is unchanged.

**Every non-primary artifact — a `gguf` artifact and a secondary `mlx_safetensors` quantization alike — is a matchable identity, and from v0.10.3 an identity member under SPEC-010-R007.** When it is `verified` it may satisfy SPEC-047 `catalog_matched` and support `sandbox_probe_only` and `network_visible_unpriced`. A secondary `mlx_safetensors` artifact served by `mlx_cache` may advance further exactly as the primary does, because SPEC-010 (v1.7) recognizes a release-bound `verified` artifact-feed entry as an expected identity. **(v0.14.3)** Identity is not settlement for loopback runtimes: a `gguf` artifact, whose sources are all loopback, reaches at most `catalog_priced` while the SPEC-047-R003(iv) v0.1.10 loopback settlement bar holds (§3.7.4). Read every later "settlement binds the artifact hash" sentence in this SPEC as naming which fields a binding MUST record. Which artifacts are identity members is SPEC-010-R007's decision; which runtime sources may settle is SPEC-047-R003(iv)'s. Both are cited and never restated here.
...
demand_share(row | mac) =
  if positive is empty:
    fail closed                 # no rankable OpenRouter volume on this Mac
  else if tokens_30d(row) == 0:
    0                           # do not boost no-history above a 1-token peer
  else:
    tokens_30d(row) / sum(tokens_30d(j) for j in positive)
...
recommended_model =
  eligible row with highest raw_score, breaking ties by:
    1. demand_share DESC
    2. measured_sustained_tps DESC
    3. model key ASC
```

Score-B share is among §4 `eligible_rows` on this Mac, not vs gpt-oss-120b. `cold_start_floor` is **not** applied as a Score-B share. Score-B retires `demand_weight` from `raw_score` in that CLI generation; the field remains on the feed for Score-A.

Constants locked in v0.6:

| Constant | Value | Rule |
|---|---:|---|
| `cold_start_floor` | `0.15` | From demand-rank JSON; schema validation fails if the fetched value differs. Score-A only: `max(demand_weight, floor)`. Not a Score-B share. |
...
**Definitions.** The two RAM-class thresholds are **fixed constants**, each defined once from the class that names it — they are NOT recomputed per Mac from `ram_gb`. This makes the rule "on a Mac with headroom for a dense 8B row, never hand back the 8 GB onboarding SKU" for every Mac at or above the 16 GB class, rather than a floor that slides up with memory and would (wrongly) stop preferring the dense 8B row on a 24/32/64 GB Mac whose only other eligible row is the 3B SKU.

- A candidate is **RAM-class-matched** when `model.min_ram_gb >= RAM_CLASS_MATCHED_FLOOR_GB`, where `RAM_CLASS_MATCHED_FLOOR_GB = 16 - safety_margin_gb = 12` (the 16 GB-class dense floor; `safety_margin_gb = 4` per §5). This is a fixed `12`, applied identically on 16, 24, 32, and 64 GB Macs. Llama 3.1 8B and Qwen3-8B (`min_ram_gb = 12`) are RAM-class-matched.
- A candidate is a **RAM-class onboarding SKU** when `model.min_ram_gb <= RAM_CLASS_ONBOARDING_CEILING_GB`, where `RAM_CLASS_ONBOARDING_CEILING_GB = 8 - safety_margin_gb = 4` (the 8 GB-class floor). Llama 3.2 3B (`min_ram_gb = 4`) is the onboarding SKU.

**Rule.** When `mac.ram_gb >= 16` and at least one **eligible** RAM-class-matched row exists, an eligible RAM-class onboarding SKU MUST NOT be selected as `recommended_model`; it is ranked below every eligible RAM-class-matched row regardless of `raw_score`. Among the eligible RAM-class-matched rows the §4 `raw_score` argmax and tiebreakers are unchanged, so Llama 3.1 8B is preferred over Qwen3-8B on M3-class 16 GB hardware (higher demand weight and higher measured TPS), and Qwen3-8B remains eligible as an alternate. The onboarding SKU remains eligible and always carries `lost_reason = "deprioritized_ram_class_onboarding_sku"` in `all_candidates`; it also appears in the displayed `candidates[]` whenever it ranks within that set (§4 caps the display at the top 5 eligible rows), which is always the case on a 16 GB Mac, where it is one of at most three eligible rows. The selected row keeps the ordinary `lost_reason = "selected_best_expected_earning_potential"` (a stable slug that downstream consumers, including the bundled Malibu app, validate the pick against); the RAM-class policy is surfaced on the demoted onboarding SKU's `lost_reason`/summary, not by a distinct winner slug.

**Hard-gate carve-out.** Eligibility is evaluated by §5 first. A RAM-class-matched row that fails a **hard** §5 gate (`swap_detected`, `thermal_throttle_detected`, the buyer-TTFT ceiling, insufficient headroom, or any integrity/trust block) is not eligible and therefore does not count as "an eligible RAM-class-matched row". When **every** RAM-class-matched row fails a hard gate, the rule does not fire and the onboarding SKU may again be the pick, so a 16 GB Mac that genuinely cannot sustain a dense 8B row still receives a paid recommendation. **Advisory** signals never veto eligibility (§5: `tps_below_gate`, `ttft_above_gate`, Warning-majority memory pressure, `swap_observed_under_load` advisory) and therefore MUST NOT send a 16 GB Mac back to the onboarding SKU.

**8 GB path unchanged.** The rule is inert below 16 GB. On an 8 GB Mac the RAM-class-matched rows (`min_ram_gb = 12`) fail `hardware_fits` (`12 > 8 - 4`) and are ineligible, so Llama 3.2 3B remains the 8 GB default (Entry 116 / SPEC-003). No catalog republish or rate-card edit is required for either path.
...
`autotune --recommend --json` MUST emit deterministic field order exactly as shown below. Unknown optional data uses `null`; fields are not reordered, renamed, or omitted, except for the opt-in `context_calibration` extension defined by SPEC-023-R003 and the opt-in `concurrency_calibration` extension defined by SPEC-023-R009. `context_calibration` appears immediately after `serve_config` only when `--calibrate-context` completes successfully; `concurrency_calibration` appears immediately after `context_calibration` (or, when `--calibrate-context` was not requested, immediately after `serve_config`) only when `--calibrate-concurrency` completes successfully. Both are absent otherwise, preserving the pre-v0.9.5 byte shape for ordinary runs. The explanation fields are additive `autotune_recommend.v1` extensions. Older v1 documents without them remain schema-decodable by Malibu for compatibility, but they are not valid recommendation display/adoption evidence for a non-null `recommended_model`; Malibu MUST reject recommended documents that lack selected/candidate explanation parity rather than rendering legacy free-text rationale as trusted recommendation evidence. Malibu treats this document family as advisory display evidence unless a later schema adds verifiable adoption authority; explanation parity alone is not one-click adoption authority.
...
AC-CAT-10 (`SPEC-023-R005`, no silent class-expansion repricing and no orphan `recommendable`): The first **authoring-refactor** class-expansion release publishes a `rate-card.json` whose `rows` map is byte-identical to the preceding release's `rows` map. A **market-pegged** cut is exempt and MUST change credits per §3.3.2. A release in which any `recommendable` candidate row resolves to no rate-card row by exact key or `NormalizeModelKey` fails closed at generation. SPEC-005 §5.5 lookup **order** is unchanged; the money-table owner is SPEC-005-R011.
...
AC-MKT-1 (`SPEC-023-R008`, one snapshot): `compute` whose policy maps every `recommendable` key emits a rate-card proposal and a demand-rank proposal, or emits neither. Unmapped key, illiquid recommendable key, ranking window older than 48 hours, or 0-credit recommendable row: no proposals.

AC-MKT-2 (`SPEC-023-R008`, no apply): the engine writes only under `--output-dir`. It does not modify published catalog files or `coordinator.yaml`.

AC-MKT-3 (`SPEC-023-R008`, liquidity) [v0.14.0 request-count basis]: a `:free` or zero-price endpoint never becomes compute `or_liquid_*`. An endpoint whose 30-minute `text_generation.request_count` is below `min_endpoint_request_count_30m` never becomes `or_liquid_*` (a 1-token $0.33 endpoint fails the gate). A key served by fewer than `min_distinct_providers` distinct providers after per-provider collapse is illiquid and emits no proposals. The **unweighted lower median** over the collapsed distinct-provider quotes passing the request-count gate is the **proposal** liquid price used to mint the three published credit integers; `request_count` is eligibility only and is never a median weight; cheapest-print is not the rate. `compute` rejects a snapshot whose recorded `minimum_request_count_last_30m` or `min_distinct_providers` differs from the active policy. Those `or_liquid_*` names MUST NOT appear on published feed JSON at FeedSchema-A.

AC-MKT-4 (`SPEC-023-R008`, credits): for `$0.08/M` liquid completion, undercut 0.20, `usd_per_million_credits=1.0`, proposal `completion_rate_per_mtok = 64000`. Prompt uses the same mint. Cache-hit = `floor(prompt × 0.25)` unless policy names another `cache_hit_fraction`. No `$0.027` overlay. `coding_specialist` does not change the undercut on this cut.

AC-MKT-5 (`SPEC-023-R008`, proposal demand fields): every recommendable demand **proposal** row carries `or_completion_tokens_30d` (sourced from the OpenRouter `rankings-daily` `total_tokens` field, the sole per-model 30-day volume OpenRouter publishes) and `or_requests_30d` as integers (`0` allowed). `or_requests_30d` is informational and MUST be `0` when the ranking source exposes no per-model request count (it does not today); it is never a scoring input. Published FeedSchema-A demand-rank bytes do **not** contain those keys or `source_snapshot`.

AC-MKT-6 (`SPEC-023-R008`, recompute bind): `catalog-release` fails closed unless it recomputes both proposals from snapshot+policy+engine **bytes** and the three credit fields plus Score-A `demand_weight` match. The unpublished `market-peg-bind.json` (`macprovider.market-peg-bind.v1`) is present, closed, and `engine_sha256` matches the executed engine. A published `release.json` or v3 ledger row that gains bind fields fails generation.

AC-MKT-7 (`SPEC-023-R008`, Score-B 16 GB, FeedSchema-B only): this AC applies only after FeedSchema-B publishes `or_completion_tokens_30d`. Fixture tokens 67000000 / 6100000000 / 1300000000, completion credits 264000 / 64000 / 64000, TPS 38 / 17 / 8 for 3B / Llama 8B / Qwen3-8B, all three eligible: Score-B `recommended_model` is Llama 3.1 8B. 8 GB fixture with only 3B eligible: 3B. Score-B MUST NOT run against FeedSchema-A bytes. This AC does **not** claim Score-A picks 8B.

AC-MKT-8 (`SPEC-023-R008`, no live OR, no Malibu circular demand): CLI/installer never call OpenRouter. A market-pegged `catalog-release` rejects `source=macprovider_buyer_supply_deficit_v1` and rejects demand rows whose token fields were copied from coordinator served-token stats.

AC-MKT-9 (`SPEC-023-R008`, FeedSchema-A): a signed published `rate-card.json` or `demand-rank.json` carrying `source_snapshot` or `or_*` before the bridge CLI+coordinator parser+previous-stable gate fails generation. A v0.10.3 `decodeDemandRank` / `decodeRateCard` of the first market-pegged **published** bytes succeeds.

AC-MKT-10 (`SPEC-023-R008`, no class fallback): a market-pegged cut that materialises any recommendable row from `classes` instead of the proposal fails generation.

AC-MKT-11 (`SPEC-023-R008`, MoneyTable-A parity): extra or missing yaml per-model keys vs the published feed, or a three-credit disagreement, fails generation.

AC-MKT-12 (`SPEC-023-R008`, retain-on-absence repealed): compute that would have kept the previous `$0.027` row because the key was absent from the top-50 cohort emits no proposals.
...
Q14 **[new v0.10.0, broadened; RESOLVED v0.10.3 by SPEC-010 v1.7 R007 (epic #1453 slice 3): both gaps below are closed — `macprovider.gguf-file.v1` is a canonical R002 pair and a release-bound `verified` artifact-feed entry is an expected identity under R001/R004/R007; the v0.10.0 text is kept as the record of why the restriction existed]**: **SPEC-010 multi-artifact identity.** SPEC-010 is the sole authority for model identity, and it currently recognizes exactly one identity per catalog model key: SPEC-010-R001 defines `macprovider.snapshot-manifest.v1` as the `model_sha256` **of the exact signed candidate-catalog row**, and SPEC-010-R004 keeps that same row as session authority for later heartbeats and settlement snapshots. §3.7 gives a model key a SET of artifacts, so two distinct gaps open at once, and v0.10.0 closes both by restriction rather than by amending SPEC-010:
...
Q17 **[new v0.10.4]**: Coordinator-signed intake source envelopes. §16.8 rule 9 retains the exact `/v1/stats/intake` and `/admin/model-admission/intake` response bytes in the operator's private intake audit store, which lets an auditor with access re-derive every recorded value, but bearer authentication proves the caller to the coordinator and not the response's origin to a later reader: the retained bytes are what the operator says the coordinator served. A coordinator-signed envelope over each intake response (a detached signature by an operator-held coordinator signing key, verified by the generator before the digests are recorded) would close that gap; it needs a key-management decision and belongs to a later revision.
...
| Thin-book / manipulated OpenRouter print **[v0.14.0]** | A low-volume paid endpoint lists Llama 3.2 3B at $0.33/M while Llama 8B clears at $0.08/M (cheapest-print 1:1 makes 3B look more valuable); OR a party inflates one endpoint's activity, or spins up many endpoints under one provider, to drag the pegged median | §3.3 rule 4 (v0.14.0): eligibility gated by a per-endpoint 30-minute `text_generation.request_count` floor (`min_endpoint_request_count_30m`, default 30) — 1-token / dead endpoints excluded; `request_count` is NEVER a median weight (so inflating one endpoint's activity does nothing); endpoints are collapsed to ONE quote per distinct provider (so adding endpoints under one provider is one vote); a distinct-provider quorum (`min_distinct_providers`, default 2) makes a thin sub-quorum market price nothing; the price is the UNWEIGHTED lower median over collapsed distinct providers; cheapest-print forbidden; `compute` binds both thresholds to the active policy; Score-B `demand_share` among §4 `eligible_rows`; Score-A is **not** the 16 GB guarantee (#1483) | Residual: not fully Sybil-proof against one entity under multiple `provider_name` strings (no stable provider-entity id upstream); bounded by the quorum + provider-collapse, backstopped by trusting OpenRouter provider onboarding and proposals never auto-applying (§16 operator decision) |
...
- `unmatched_model_request_count` — **a new signal, specified here and to be implemented under SPEC-017 authority.** It is the count of buyer requests whose requested model string resolved to no admitted catalog key, aggregated per normalized requested model key over the SPEC-017 rollup window. Until SPEC-017 implements it, this signal is simply absent and the intake rule falls through to its other terms; its absence MUST NOT block intake. **[adopted v0.10.4]** SPEC-017 v0.2.1 §5.2b is that implementation and that amendment: the signal is read from `GET /v1/stats/intake` (`macprovider.stats-intake.v1`), which serves complete 30-day windows only, from exactly ONE `unmatched_models.windows` entry selected DETERMINISTICALLY — the window with the latest `window_start` whose `window_end` is within one §16.5 cadence period (31 days) before `as_of` — as that entry's bucket `lower_bound` for the normalized key; the operator never chooses among windows, an open or incomplete window is never evidence (`unmatched_model_request_absent_reason: "incomplete_window"` when no complete window qualifies), and a key with no emitted bucket records `"no_observations"` (SPEC-017 omits sub-k buckets from the wire, so suppression is not key-attributable and `unmatched_model_request_suppressed` is `false`). The selected window's `window_id`, `window_start`, and `window_end` are recorded in the manifest (§16.8 rule 3), and its `parameters` MUST equal the manifest `thresholds` they correspond to. Windows are SPEC-017 §3.2a aggregator epochs and are never summed or merged across a close.
...
9. **Suppression, and what may never clear a floor.** A bucket whose post-cap lower-bound count is below the **effective k-anonymity floor** — `INTAKE_K_ANONYMITY_MIN` (§16.4, default `3`), or a higher value SPEC-017 selects — is reported as suppressed rather than as a small number, exactly as `distinct_provider_offer_count` is in §16.2(b). **[v0.10.4]** The floor is a DISTINCT-PRINCIPAL floor for this signal: a retained bucket is suppressed unless at least `INTAKE_K_ANONYMITY_MIN` distinct item-7 principal tokens have contributed to it in the window, whatever its request count, because a request-count floor would let one principal make a buyer-chosen string visible with three requests. The distinct-principal cardinality is internal and never emitted. Because the bucket key is buyer-supplied data, a suppressed bucket is OMITTED from the wire entirely — the emitted record carries only the number of omitted buckets — rather than reported by key with a null count. `INTAKE_BUYER_REQUEST_FLOOR` may be satisfied ONLY by a retained bucket's post-cap, post-suppression **lower-bound** count. `other_suppressed` MUST NOT satisfy it, a suppressed bucket MUST NOT satisfy it, an upper-bound `count` MUST NOT satisfy it, discarded over-cap contributions MUST NOT be added back for the comparison, and overflow-principal contributions MUST NOT be added back either. A key that would clear the floor only because one principal exceeded its cap, or only on its upper-bound count, has not cleared it.
...
- `distinct_provider_offer_count` — the number of DISTINCT providers whose SPEC-047 admission events record an offer naming this catalog key or a served model reference that matches one of its verified artifacts, over the trailing 30 days. Derived from coordinator admission events, aggregated only: no provider id, pseudonym, hardware fingerprint, or per-provider row may appear in the intake record. Offers from providers under an active route, trust, payout, or registration sanction MUST be excluded (SPEC-047-R009, the aggregate authority; SPEC-047-R007 for the offer path). **[v0.10.4]** "Under an active route, trust, payout, or registration sanction" is DEFINED, for this signal, by the SPEC-047 v0.1.6 closed current-state predicate `provider_intake_sanctioned(provider_id, at)` — route: admission rejection or canary sanction; trust: no active hardware-trust root; payout: vacuous until SPEC-016 defines a per-provider payout sanction; registration: a revoked provider token with no active token — evaluated at the aggregate's build time, so a provider sanctioned after offering is excluded with every offer it recorded. The count MUST be reported as **suppressed** below the effective k-anonymity floor — `INTAKE_K_ANONYMITY_MIN` (§16.4, default `3`) — so a two-provider interest cannot be read back as an identification of those providers. `INTAKE_K_ANONYMITY_MIN` is **owned by SPEC-023**, not borrowed from SPEC-017: it is the minimum that applies to BOTH this signal and the §16.2(a) `unmatched_model_request_count` signal. SPEC-017 MAY select a HIGHER floor for the signal it owns and MUST NOT select a lower one (§16.7); it never lowers the floor applied here.
...
- **This signal is available NOW.** `distinct_provider_offer_count` is derived from coordinator-owned SPEC-047 admission events, which are coordinator admission data and not a SPEC-017 stats field. It is therefore NOT gated on the SPEC-017 amendment that §16.7 requires before `unmatched_model_request_count` may satisfy any floor: provider-supply intake is usable in the first release that implements §16, with the SPEC-023-owned `INTAKE_K_ANONYMITY_MIN` as its suppression floor. **[sourced v0.10.4]** The signal is read from SPEC-047 v0.1.6 `GET /admin/model-admission/intake` (`model_admission_intake_offer_counts.v1`, trailing 30 days, sanction-excluded, suppressed below `k_anonymity_min`), whose counting rule — the offer-time `intake_model_key` resolution of the offer's signed artifact hashes against every candidate-catalog row and verified artifact names the key (so a key the catalog has not yet admitted is countable); a provider counts only while eligible (unsanctioned and, where hardware trust is operated, holding an active trust root); later withdrawal or revocation does not un-count — is the one authority for "an offer naming this catalog key or a served model reference that matches one of its verified artifacts".
...
**SPEC-023-R008:** A market-pegged catalog release in **this** revision MUST ship FeedSchema-A + Score-A + MoneyTable-A: author per-model prices and demand from one OpenRouter snapshot under §3.3.2 / §3.4 / §4, no apply, published feed keys unchanged, yaml byte-parity, `RateFor` still yaml. Score-B and FeedSchema-B are later belts and MUST NOT be enabled against FeedSchema-A bytes. Malibu served-token counts MUST NOT enter scoring or a market-pegged sign. Issue #1483 remains a separate 16 GB RAM-class safety rail.
...
| `INTAKE_K_ANONYMITY_MIN` | `3` **(fixed in v0.10.4)** | The SPEC-023-owned k-anonymity floor, applying to BOTH `distinct_provider_offer_count` (§16.2(b)) and the SPEC-017-owned `unmatched_model_request_count` (§16.2(a) item 9). **[v0.10.4] Exception to the release-policy rule below: this knob is NOT operator-tunable in this revision.** Both sources suppress against it and omit the cardinality a consumer would need to re-suppress at a higher floor, so a release that records any other value fails closed at generation, and raising it requires a lockstep amendment of this SPEC, SPEC-017, and SPEC-047. An aggregate below it is reported as suppressed rather than as a small number, and a suppressed aggregate satisfies no floor. Three is the smallest value at which an aggregate cannot be read back as "this one provider" or "these two providers", and it is the floor the intake rule's own `INTAKE_OFFER_FLOOR` default already assumes. SPEC-017 MAY require a higher floor for the signal it owns and MUST NOT require a lower one (§16.7); no SPEC-017 amendment is needed for this default to apply to provider-offer intake. |
...
| `INTAKE_UNKNOWN_KEY_PRINCIPAL_CAP_PCT` | `10%` | Maximum share of `INTAKE_BUYER_REQUEST_FLOOR` that one principal — buyer account, partner key, or source class — may contribute to one bucket in one window (§16.2(a) item 7, applied at step S4 of the §16.2(a) processing order); 25 requests at the default floor. Clearing the floor therefore requires at least ten independent principals, which makes a single integration's traffic — accidental or deliberate — structurally unable to admit a key. **[v0.10.4] Bounded at 10:** the value is in `[1, 10]` and a release or coordinator configuration above it fails closed — the "ten independent principals" property is the privacy argument, not a default. |
...
2. `unmatched_model_request_count` counts authenticated requests whose normalized key is **not a `listed` or `recommendable` row** of the admitted catalog release (SPEC-017 §5.2b.2 — a catalog property, not a pool one: an admitted key no provider serves is capacity, a served non-catalog key is demand). It is the inverse of a served-traffic metric: it goes UP precisely for the models the cold-start loop would otherwise silence.
  ...
  "decisions": [
    {
      ...
      "signals": {
        ...
        "distinct_provider_offer_source_sha256": "<64-hex digest of the privately retained model_admission_intake_offer_counts.v1 response bytes>",
        "distinct_provider_offer_window_start": "RFC3339 timestamp",
        "distinct_provider_offer_window_end": "RFC3339 timestamp",
        "unmatched_model_request_count": null,
        "unmatched_model_request_suppressed": false,
        "unmatched_model_request_absent_reason": "incomplete_window",
        "unmatched_model_request_source_sha256": "<64-hex digest of the privately retained macprovider.stats-intake.v1 response bytes>",
        "unmatched_model_request_window_id": null,
        "unmatched_model_request_window_start": null,
        "unmatched_model_request_window_end": null,
        "fleet_fit_fraction_ppm": 300000,
        "fleet_fit_absent_reason": null,
        "fleet_fit_source_sha256": "<64-hex digest of the same privately retained macprovider.stats-intake.v1 response bytes>",
        "fleet_fit_window_start": "RFC3339 timestamp",
        "fleet_fit_window_end": "RFC3339 timestamp"
      },
...
2. **Evaluated values or authenticated digests, never both absent.** Each signal is recorded as the VALUE the decision was taken against, together with the digest of the authenticated snapshot that value was read from. **[v0.10.4]** Each `*_source_sha256` is the lowercase 64-hex SHA-256 of the EXACT bytes of that source: `demand_rank_source_sha256` over the signed `demand-rank.json` bytes bound in the release, `unmatched_model_request_source_sha256` and `fleet_fit_source_sha256` both over the one `GET /v1/stats/intake` response body the decision read (the two digests of one entry are therefore equal when both signals are present), and `distinct_provider_offer_source_sha256` over the `GET /admin/model-admission/intake` response body. A digest is **non-null whenever its source was read** — a suppressed, empty, or incomplete result is still a result of an authenticated response and keeps that response's digest — and `null` only when the source did not exist or could not be read (`spec017_amendment_not_landed`, `source_unavailable`); `unmatched_model_request_source_sha256` and `fleet_fit_source_sha256` are therefore equal whenever both are non-null. Every signal has its own `*_absent_reason` field, present in every `admit_listed` entry: `demand_rank_absent_reason` (closed enum `unranked`), `distinct_provider_offer_absent_reason` (`suppressed` | `no_observations` | `source_unavailable`), `unmatched_model_request_absent_reason` (`spec017_amendment_not_landed` | `suppressed` | `no_observations` | `incomplete_window` | `source_unavailable`), and `fleet_fit_absent_reason` (`no_observations` | `source_unavailable`). `source_unavailable` records that the source endpoint was disabled, stale (503), unauthorized, or unreachable at `as_of`; `incomplete_window` that the `/v1/stats/intake` response held no complete window qualifying under §16.2(a); `suppressed` on the unmatched signal is unreachable from a SPEC-017 v0.2.1 source (which omits sub-k keys) and is retained for a source that reports suppression per key. A signal that was not available is recorded as `null` with its `*_absent_reason` set to one of its enum values; the `*_absent_reason` field is `null` exactly when the signal value is non-null and non-null exactly when the value is `null`, so an absent signal can be neither silently omitted nor recorded without its reason. Rules 2, 4, and 6 govern the `signals` object, `admission_clause`, `fit_clause`, and `coldstart_slot_used` of `admit_listed` entries only; for `promote_recommendable` entries see rule 6a.
3. **Windows and as-of are explicit, per signal.** Each coordinator-derived signal carries its OWN window in `signals`: `distinct_provider_offer_window_start`/`_end` (the SPEC-047 response's `window_start`/`window_end`), `fleet_fit_window_start`/`_end` (the `/v1/stats/intake` `fleet_ram` bounds), and `unmatched_model_request_window_id`/`_start`/`_end` (the selected SPEC-017 complete window's `window_id` and bounds; `window_end` is that window's recorded close, never `null` for a non-null signal); the offer and fleet pairs are `null` exactly when their signal's `*_source_sha256` is `null`; the unmatched triple is non-null exactly when a complete SPEC-017 window was SELECTED under §16.2(a) — so it is non-null for a `no_observations` result read from a selected window, and `null` (with a non-null digest) for `incomplete_window`, and `null` (with a null digest) for `spec017_amendment_not_landed` and `source_unavailable`; a non-null unmatched signal value requires all three non-null. The top-level `observation_window_start` is the earliest and `observation_window_end` the latest of the non-null signal window bounds — the envelope, recorded for readers, never a substitute for the per-signal bounds — and both are `null` exactly when the entry has no non-null windowed signal (a `demand_rank` + `tier_target` admission, or a `promote_recommendable` entry); `as_of` is when the operator evaluated the signals; all are RFC3339 UTC with second precision in the one form `YYYY-MM-DDTHH:MM:SSZ` (no offset, no fraction), in the manifest and in every retained source, so a window recorded here equals its source's window by byte equality; a served SPEC-017 window is exactly 30 days and a response holds at most three, and the generator rejects any other form. **One cadence period is 31 days** (31 × 86 400 seconds, the longest calendar month): a decision in which ANY non-null signal's window ends more than 31 days before `as_of` fails closed: stale evidence is not evidence.
4. **Suppression state is recorded, and a suppressed signal decides nothing.** `distinct_provider_offer_suppressed` and `unmatched_model_request_suppressed` record whether the aggregate was below the effective k-anonymity floor (§16.2(b), §16.2(a) item 9). A suppressed signal MUST record `null` as its value — never the small number it suppressed — and MUST NOT be named as `admission_clause`. A manifest naming a suppressed signal as the selected clause fails closed. **[v0.10.4]** The manifest is public while both coordinator sources are private, so it never distinguishes "one or two providers" from "none" for a named key: `distinct_provider_offer_suppressed` and `unmatched_model_request_suppressed` are always `false`, a suppressed SPEC-047 row and an absent row both record `null` with `distinct_provider_offer_absent_reason: "no_observations"`, and a key with no SPEC-017 bucket records `no_observations` likewise. The `suppressed` absent-reason values and the two boolean fields stay in the closed schema for a source that may report key-attributable suppression to a private reader; against the v0.10.4 sources they are unreachable, like `offer_rejected` in SPEC-047.
