...
Q17 **[new v0.10.4]**: Coordinator-signed intake source envelopes. §16.8 rule 8 commits the exact `/v1/stats/intake` and `/admin/model-admission/intake` response bytes beside the manifest, which lets an auditor re-derive every recorded value, but bearer authentication proves the caller to the coordinator and not the response's origin to a later reader: the committed bytes are what the operator says the coordinator served. A coordinator-signed envelope over each intake response (a detached signature by an operator-held coordinator signing key, verified by the generator before the digests are recorded) would close that gap; it needs a key-management decision and belongs to a later revision.
...
3. **Windows and as-of are explicit, per signal.** Each coordinator-derived signal carries its OWN window in `signals`: `distinct_provider_offer_window_start`/`_end` (the SPEC-047 response's `window_start`/`window_end`), `fleet_fit_window_start`/`_end` (the `/v1/stats/intake` `fleet_ram` bounds), and `unmatched_model_request_window_id`/`_start`/`_end` (the selected SPEC-017 complete window's `window_id` and bounds; `window_end` is that window's recorded close, never `null` for a non-null signal); each triple is `null` exactly when its signal's `*_source_sha256` is `null`, and a non-null unmatched signal requires all three non-null. The top-level `observation_window_start` is the earliest and `observation_window_end` the latest of the non-null signal window bounds — the envelope, recorded for readers, never a substitute for the per-signal bounds — and `as_of` is when the operator evaluated the signals; all are RFC3339 UTC. A decision in which ANY non-null signal's window ends more than one cadence period (§16.5) before `as_of` fails closed: stale evidence is not evidence.
...
8. **[v0.10.4] Source responses are committed and re-derived.** A digest alone proves which bytes were read, not what they said, so every coordinator source the manifest cites is committed beside it, byte-exact, under `intake-sources/<release_id>/`: `stats-intake.json` (the one `macprovider.stats-intake.v1` response) and `model-admission-intake.json` (the one `model_admission_intake_offer_counts.v1` response); the demand-rank source is the release-bound signed `demand-rank.json` itself. Like the manifest these files are authoring and audit evidence — never served, never baked, never bound in `release.json`. The generator MUST verify that each recorded `*_source_sha256` equals the SHA-256 of the committed file, MUST parse each file under its closed schema (rejecting an unknown key), and MUST re-derive every recorded signal value, suppression flag, absent reason, and window from those bytes — the §16.2(a) window selection, the bucket lower bound, the §16.2(b) row, the §16.2(c) class-floor fraction — failing the release closed on any disagreement. A digest that names no committed file, or a committed file the manifest does not cite, fails closed. Bearer authentication proves the caller to the coordinator, not the response's origin to a later auditor; the committed bytes plus the ledger-bound manifest digest are what an auditor re-evaluates, and a coordinator-signed response envelope is a later revision's concern, recorded in §13.
