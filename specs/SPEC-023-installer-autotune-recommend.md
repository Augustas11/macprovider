...
**Signer identity equality (normative).** "The same static-feed key" is a CHECKED equality, not an assumption. During a key-rotation bridge more than one key may be concurrently trusted by the release-embedded keyring, so unknown-`key_id` rejection alone does not establish that the artifact feed and the candidate catalog of one release were signed by the same operator key. At generation AND at consumption the authenticated artifact sidecar's `key_id` MUST equal `release.json.feeds["autotune-artifacts.json"].signer_key_id`, and MUST equal the authenticated candidate-feed signer `key_id` for that same release. Any mismatch — including a cryptographically valid signature made by a different but concurrently trusted key — is `catalog_artifact_feed_integrity_failure` (§3.7.6 class 2), never `catalog_artifact_feed_update_required`, the release fails closed before signing, and no artifact of that feed may be matched, displayed as catalog-verified, downloaded, prepared, probed, priced, or settled (AC-CAT-1).
...
2. **Evaluated values or authenticated digests, never both absent.** Each signal is recorded as the VALUE the decision was taken against, together with the digest of the authenticated snapshot that value was read from. A signal that was not available is recorded as `null` with its `*_absent_reason` set to a closed enum value (`unranked`, `spec017_amendment_not_landed`, `no_observations`, `suppressed`); an absent signal MUST NOT be silently omitted.
...
6. **Selected clause and cold-start use are explicit.** `admission_clause` is the closed enum `demand_rank` | `provider_offer` | `buyer_request` | `coldstart_slot` and names the ONE demand-or-supply term the operator relied on; `fit_clause` is `fleet_fit` | `tier_target`. `coldstart_slot_used` is `true` exactly when `admission_clause` is `coldstart_slot`, and at most `INTAKE_COLDSTART_SLOTS` entries of one release may set it. `listed_since` is the RFC3339 time the key entered `listed`, REQUIRED and non-null for `promote_recommendable` (so `INTAKE_MIN_LISTED_DAYS` is checkable) and `null` for `admit_listed`.
