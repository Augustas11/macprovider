...
AC-CAT-7 (`SPEC-023-R004`, GGUF settlement identity under SPEC-010-R007 — v0.10.3): An artifact with `hash_algorithm == "macprovider.gguf-file.v1"` AND `verification_status == "verified"`, whose `hash` exactly equals the digest the provider computed over its local GGUF bytes, belonging to a candidate row whose `runtime_status` is `listed` or `recommendable`, reaches `catalog_matched`, `listed` intake, `sandbox_probe_only`, and `network_visible_unpriced`; for a `recommendable` row it may further reach `catalog_priced` and `settlement_capable`, be reported as the SPEC-010 provider `model_hash`/`model_hash_algorithm` wire pair, and bind a SPEC-022 route-time settlement snapshot — only when the route-time record carries the SPEC-047-R003 six values and settlement re-verifies them. Change any precondition and the match itself fails: a `declared` or `blocked` artifact, an inexact digest, a digest adopted from a runtime report rather than recomputed, a `candidate` or `blocked` row, or a feed that is stale, integrity-failed, or of another release reaches no admission tier at all (AC-CAT-6, §3.2, §3.7.6 rule 5); a `listed` row stops at `network_visible_unpriced`.
...
AC-CAT-17 (`SPEC-023-R004`, artifact-set settlement identity — v0.10.3): A **secondary `mlx_safetensors` artifact** — `verified`, correctly declaring `macprovider.snapshot-manifest.v1`, differing from the primary only in quantization and therefore in `hash` — is a member of its key's identity set under SPEC-010-R007 exactly as the primary is: with an exact pair match and the six-value route-time evidence it reaches `catalog_priced`, `settlement_capable`, the SPEC-010 wire pair, and the SPEC-022 binding on a `recommendable` row. The primary member needs no six-value record (its identity is the row's own `model_sha256`, SPEC-010-R001). A provider pair that matches no member — a hash of neither the primary nor any `verified` secondary — is unverified, and a provider-asserted `catalog_model_key` that disagrees with the key the pair resolves to fails closed (SPEC-010-R007(c)).
