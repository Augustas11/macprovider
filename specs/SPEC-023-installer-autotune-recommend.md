...
AC-CAT-13 (`SPEC-023-R006`, bounded unknown-model signal): `unmatched_model_request_count` satisfies the §16.2(a) field contract. Requests that are unauthenticated, rejected before model resolution, or flagged test/synthetic/internal contribute to no bucket. Keys are bucketed only after SPEC-005 `NormalizeModelKey`, and only when the normalized key matches `[a-z0-9._/-]{1,128}` in full; an over-long or out-of-grammar key creates no named bucket. Distinct-key fanout beyond `INTAKE_UNKNOWN_KEY_BUCKETS` (default 64) creates no additional buckets and no additional per-window key-tracking work, and collapses into a single `other_suppressed` bucket carrying a request count and a distinct-key count only. One principal's contribution to one bucket in one window is capped at `INTAKE_UNKNOWN_KEY_PRINCIPAL_CAP_PCT` of `INTAKE_BUYER_REQUEST_FLOOR`, with the excess discarded and no per-principal breakdown emitted or retained. A bucket below the k-anonymity floor is reported as suppressed. `INTAKE_BUYER_REQUEST_FLOOR` is satisfied only by a named bucket's post-cap, post-suppression count: a single principal generating any volume of requests for one key never clears it, `other_suppressed` never clears it, and a suppressed bucket never clears it. No buyer account, API key, IP address, prompt, completion, or per-request row exists at any stage. Until SPEC-017 adopts this contract by amendment, a field of this name satisfies no intake floor.
...
3. **Closed key grammar and byte limit.** A normalized key is eligible for its own bucket only when it matches the closed grammar `[a-z0-9._/-]{1,128}` in full. A request whose normalized key exceeds 128 bytes, or contains any byte outside that set, MUST NOT create or increment a named bucket; it is counted only in `other_suppressed` (item 5).
4. **Bounded bucket count.** At most `INTAKE_UNKNOWN_KEY_BUCKETS` named buckets — default `64`, operator-configured — are retained per rollup window, selected by count (highest first, ties broken by key ascending). The bound is enforced at ingest, not only at emission: the implementation MUST NOT materialise unbounded distinct keys and then trim, and its per-window key-tracking work MUST be bounded by that same constant.
5. **Single overflow bucket.** Every request whose key is ineligible under item 3, or whose key falls outside the retained set under item 4, is collapsed into one `other_suppressed` bucket per window. `other_suppressed` carries a request count and a distinct-key count only; it never carries key material.
6. **Per-principal contribution cap.** Within one bucket and one rollup window, a single principal — buyer account, partner key, or source class — contributes at most `INTAKE_UNKNOWN_KEY_PRINCIPAL_CAP_PCT` of `INTAKE_BUYER_REQUEST_FLOOR` (default `10%`, i.e. 25 requests at the default floor of 250). Contribution beyond the cap is discarded for intake purposes. The cap is applied at aggregation time and leaves no per-principal row behind: the emitted bucket carries the capped total only, never a per-principal breakdown.
7. **Below-k-anonymity suppression.** A bucket whose capped count is below the SPEC-017 k-anonymity floor is reported as suppressed rather than as a small number, exactly as `distinct_provider_offer_count` is in §16.2(b).
8. **Suppressed, overflow, and capped contributions never satisfy the floor.** `INTAKE_BUYER_REQUEST_FLOOR` may be satisfied only by a named bucket's post-cap, post-suppression count. `other_suppressed` MUST NOT satisfy it under any circumstances, a suppressed bucket MUST NOT satisfy it, and discarded over-cap contributions MUST NOT be added back for the comparison. A key that clears the floor only because one principal exceeded the cap has not cleared it.
9. **No identity, no rows.** Aggregated counts only: no buyer account, API key, IP address, prompt, or completion content, and no per-request rows, at any stage — ingest, storage, rollup, or emission. The per-principal cap of item 6 is a counting rule, not a retention licence: any principal-scoped counter it needs is window-scoped, is never emitted, and is never joined to an intake record.
...
| `INTAKE_UNKNOWN_KEY_BUCKETS` | `64` | Named unknown-model-key buckets retained per rollup window (§16.2(a) item 4). The requested model string is buyer-controlled, so the bucket count must be a constant rather than a function of traffic. 64 is far more than the number of models a monthly intake review can consider, and small enough that per-window key tracking is trivially bounded. Everything else collapses into `other_suppressed`. |
| `INTAKE_UNKNOWN_KEY_PRINCIPAL_CAP_PCT` | `10%` | Maximum share of `INTAKE_BUYER_REQUEST_FLOOR` that one principal — buyer account, partner key, or source class — may contribute to one bucket in one window (§16.2(a) item 6); 25 requests at the default floor. Clearing the floor therefore requires at least ten independent principals, which makes a single integration's traffic — accidental or deliberate — structurally unable to admit a key. |
