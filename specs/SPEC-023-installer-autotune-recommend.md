...
**Every non-primary artifact — a `gguf` artifact and a secondary `mlx_safetensors` quantization alike — is a matchable identity, and from v0.10.3 a settlement identity under SPEC-010-R007.** When it is `verified` it may satisfy SPEC-047 `catalog_matched` and support `sandbox_probe_only` and `network_visible_unpriced`; it may advance further exactly as the primary does, because SPEC-010 (v1.7) now recognizes a release-bound `verified` artifact-feed entry as an expected identity. Read every later "settlement binds the artifact hash" sentence in this SPEC as naming which fields a binding MUST record; which artifacts may be bound is SPEC-010-R007's decision, cited and never restated here.
...
- **Which artifacts may be a settlement identity is SPEC-010's decision.** In v0.10.0–v0.10.2 SPEC-010 v1.6 recognized exactly one identity per model key (the row's `model_sha256`), so only the artifact named by `primary_artifact_id` could reach `catalog_priced` or `settlement_capable`, be reported as the SPEC-010 wire pair, or bind a SPEC-022 route-time snapshot. SPEC-010 v1.7 **R007** recognizes every `verified` artifact of the release-bound feed as a member of the key's identity set — `macprovider.gguf-file.v1` is a canonical pair from v1.7 — so from v0.10.3 a non-primary artifact of either format may do all three when the R007 conditions hold (exact pair match, single resolution, six-value route-time evidence for every feed-derived binding). Every gate in this SPEC — `recommendable` for paid defaults, `listed` as unpriced intake, §5, §12 — is unchanged; the widening is one of identity, not of tier.
