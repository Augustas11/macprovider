...
- **v0.10.4 (2026-09-11)** — Intake signals sourced (BYOM v0.2 slice 5, #1453). SPEC-017 v0.2.1 §5.2b adopts the §16.2(a) `unmatched_model_request_count` field contract by amendment — the §16.7 precondition is met from that version and the signal MAY satisfy `INTAKE_BUYER_REQUEST_FLOOR`; `spec017_amendment_not_landed` stays in the §16.8 `unmatched_model_request_absent_reason` enum for a decision evaluated against a coordinator that does not yet serve the endpoint. The three coordinator-derived signals are each bound to ONE authenticated source whose exact response bytes the §16.8 `*_source_sha256` digests cover: §16.2(a) from `GET /v1/stats/intake` (`macprovider.stats-intake.v1`, one aggregator window), §16.2(b) from SPEC-047 v0.1.6 `GET /admin/model-admission/intake` (`model_admission_intake_offer_counts.v1`), and §16.2(c) from the same `/v1/stats/intake` response's `fleet_ram` histogram, evaluated conservatively by class floor (`fleet_fit_fraction_ppm = floor(fraction × 1 000 000)`). The generator validates `intake-decision.json` against §16.8 and re-evaluates §16.3 for every entry (AC-CAT-21); `docs/runbooks/catalog-monthly-intake-release.md` is the §16.5 cadence runbook. No normative rule of §16 changes; §16.2 and §16.7 gain source and adoption statements only.
