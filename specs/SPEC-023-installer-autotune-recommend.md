...
4. **Liquidity-filtered paid price.** Eligible endpoints: `status == 0`, strictly positive paid completion price, not `:free`. Volume gate: snapshot-window completion tokens `>= max(min_endpoint_completion_tokens, floor_fraction × model_tokens_30d)` with defaults `min_endpoint_completion_tokens = 1000000` and `floor_fraction = 0.05`. **Compute `or_liquid_*` (proposal/snapshot only, not a published feed key) is the volume-weighted median** of those endpoints' completion (and separately prompt) prices. Cheapest-print is forbidden as the published rate. If per-endpoint 30-day tokens are not in the snapshot, the implementation MUST extend the snapshot schema until they are; pretending a 30m throughput flag is a 30-day volume filter is non-compliant. A 1-token $0.33 quote MUST NOT become `or_liquid_*`. A mapped recommendable key with no endpoint passing the gate fails the **whole compute** (no proposals). Demotion from `recommendable` is a prior intake cut, not this compute. Partial rate cards are forbidden.
...
AC-MKT-3 (`SPEC-023-R008`, liquidity): a `:free` or zero-price endpoint never becomes compute `or_liquid_*`. A 1-token $0.33 3B endpoint beside a ≥5% $0.06 endpoint never becomes `or_liquid_*`. Volume-weighted median of endpoints passing the numeric floor is the **proposal** liquid price used to mint the three published credit integers. Cheapest-print is not. Those `or_liquid_*` names MUST NOT appear on published feed JSON at FeedSchema-A.
...
| Thin-book OpenRouter print **[v0.12.0]** | A low-volume paid endpoint lists Llama 3.2 3B at $0.33/M while Llama 8B clears at $0.08/M; copying cheapest-print 1:1 makes 3B look more valuable | Numeric volume floor + volume-weighted median; cheapest-print forbidden; 1-token quotes dropped; Score-B `demand_share` among §4 `eligible_rows`; Score-A is **not** the 16 GB guarantee (#1483) | None; volume-weighted median is the v0.12.0 default |
