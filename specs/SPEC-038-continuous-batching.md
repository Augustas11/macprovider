...
- Any positive `cached_prompt_tokens` MUST stay out of the scheduler until
  AC-26 packaged sticky/cross-turn proof, even when retained paged KV exists.
  Canary serial-routes with `sticky_cache_handoff_unavailable`. Strict `.on`
  MUST fail closed with that named reason (FR-CB8), not silently serial-route.
