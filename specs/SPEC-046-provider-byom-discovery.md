...
| `not_offered` | Coordinator offer state is unavailable or has not been queried. | Run status readback, offer dry-run, or submit a refreshed offer if local checks still pass. | May become `offerable` when local checks pass without coordinator readback, or may enter SPEC-047 `offer_submitted` only through a provider-signed offer. |

The table above defines `local_default:not_offered`. The same `not_offered`
value may appear with either `admission_state_source`; callers MUST read the
source field before deciding whether the state is local advisory inventory or
coordinator readback. The exact English source meaning for
`local_default:not_offered` is **Coordinator offer state is unavailable or has
not been queried.** It MUST NOT assert that an offer never existed. The exact
English source meaning for `coordinator:not_offered` is **Coordinator reports
no active network offer for this model.** and is valid only for authoritative
SPEC-047 readback. The exact English source meaning for `local_only` is
**Retained as local inventory only; this admission state does not claim the
model is prepared, installed, ready, reachable, or usable.** Network routing
and earning remain governed independently by `earning_path_class` and
SPEC-047; readiness and usability remain governed by the same validated
candidate's independent readiness/runtime evidence.
...
**SPEC-046-R009 - Loopback serving selectors (v0.2.0, #1690; amended v0.3.0, #1690 M8).** `macprovider-cli serve` MUST recognize a loopback serving selector only by an explicit served-model prefix. v0.2.0 defines `llamacpp:<ref>` through `llamacpp_loopback` and `ollama:<ref>` through `ollama_loopback`; v0.3.0 adds `mlxlm:<ref>` through `mlxlm_loopback` (`LoopbackServeSelection.select` in `phase3-binary/Sources/macprovider-cli/OpenAICompatibleLoopbackRuntime.swift`; wired where `serve` constructs the loopback runtime, the `LoopbackServeSelection.select(loopbackServedRef)` switch in `phase3-binary/Sources/macprovider-cli/MacProviderCLI.swift`). The selected adapter's `runtime_source` is the value the hello reports. The upstream origin comes from the operator's `loopback_origin` config key or its environment override, else the adapter's well-known loopback default, and is loopback-validated exactly as R002 requires. For `llamacpp_loopback`, the GGUF file whose SPEC-010-R007(a) digest the CLI reports is the file the operator names (`MACPROVIDER_LLAMACPP_MODEL_ROOT` / `_PATH`), never one the runtime reports. For `mlxlm_loopback` (v0.3.0), the snapshot whose SPEC-010-R009 `macprovider.snapshot-manifest.v1` pair the CLI reports is the directory the operator names (`MACPROVIDER_MLXLM_MODEL_PATH`); the runtime's `GET /v1/models` must list that directory's resolved path at startup and before every request, and chat requests name `default_model`. `llamacpp_loopback` is the first external runtime for buyer serving and the reference runtime for SPEC-042 Trusted Pool allowlists; `ollama_loopback` is the second, and `mlxlm_loopback` the third. oMLX has no selector: it serves MLX snapshots but has no proof yet that its process serves the declared snapshot. No serving selector exists for `lmstudio_loopback` or `openai_compatible_loopback`, and `lmstudio:` or `openai:` MUST NOT be added until that runtime has a serving-time identity leg. `openai_compatible_loopback` has no GGUF leg at all (`BYOMDiscoveryEnvironment.artifactHashingRuntimes` in `phase3-binary/Sources/macprovider-cli/BYOMDiscovery.swift` excludes it), and `lmstudio_loopback` has a discovery-time leg but no serving-time binding to the file its server loads. A selector grants no money-path authority. A loopback serving session is sandboxed for global traffic (SPEC-032 FR-HG8) and earns only inside a SPEC-042 Trusted Pool whose signed policy allowlists its `runtime_source` (SPEC-042-R001/R013, SPEC-047-R003(iv)).
...
- v0.3.0 - Adds `mlxlm_loopback` (`mlx_lm.server`) to the R002 adapter
  enum and the `mlxlm:` serving selector to R009 (#1690 M8). The adapter is
  attempted only with an operator-named origin and snapshot directory; the
  identity is the SPEC-010-R009 snapshot-manifest pair of that directory,
  bound to the process by its `/v1/models` listing. LM Studio (`lmstudio:`)
  and oMLX stay without a selector until each has a serving-time identity
  leg. A selector still grants no money-path authority.
