...
**[amended v0.1.5]** The authorized actor `coordinator admission policy` acts, for the `network_visible_unpriced`, `network_admitted_unsettled`, `catalog_priced`, `settlement_capable`, and `revoked` edges, through exactly one production caller: an **operator decision request** `POST /admin/model-admission/decisions`, authenticated with the coordinator operator bearer (the same credential as the other `/admin` operator surfaces), bounded by payload size and rate like every SPEC-047-R007 mutation, with the closed schema `model_admission_decision_request.v1` whose fields are exactly `schema`, `provider_id`, `candidate_id`, `next_state` (one of those five states), `reason_code` (closed grammar `^[a-z][a-z0-9_]{2,63}$`), `expected_coordinator_event_id` (the `coordinator_event_id` of the latest event the operator read for that candidate; when the latest event differs the coordinator MUST fail closed as a conflict and append nothing), and `idempotency_key` (a repeated key with identical fields MUST answer with the already-appended event and append nothing; a repeated key with different fields MUST fail closed). Unknown fields MUST be rejected before any state transition. The coordinator MUST validate the edge against this table from the candidate's latest state, MUST append the event with `actor` = `operator`, the reason code, the previous and next state, and the binding fields SPEC-047-R003 requires for the target state, and MUST answer with the stored event in the `model_admission_status.v1` shape extended by `reason_code` and `previous_admission_state`. Synthetic-probe and drift/revocation transitions keep their coordinator actors and are never issued through this request. An operator decides on recorded state, never on a provider's claim: `GET /admin/model-admission/offers?provider_id=<id>` (operator bearer) MUST list, for that provider, the latest event of every candidate together with its recorded catalog-match evidence (below), and a decision request for a candidate with no offer MUST fail closed.
...
**[amended v0.1.5]** The coordinator performs the catalog match at OFFER time, on the signed `artifact_hashes` of the offer package, against the artifact identity set of the CURRENT admitted catalog release under SPEC-010-R007(b) — the release-bound, fresh artifact feed's `verified` artifacts of `listed`/`recommendable` keys, of which each candidate row's own primary pair is the primary member — resolving every offered `(hash_algorithm, hash)` pair exactly. Exactly one resolved `(model_key, artifact_id)` across all offered pairs is a catalog match: the offer event records `catalog_match_state` = `catalog_matched`, `catalog_model_key` = the RESOLVED row key, `expected_catalog_model_hash` / `expected_catalog_model_hash_algorithm` = the matched member's pair, and the six SPEC-047-R003 artifact values (`artifact_feed_sha256`, `artifact_id`, `artifact_hash`, `artifact_hash_algorithm`, `artifact_feed_signer_key_id`, `artifact_candidate_catalog_sha256`) as recorded match evidence. A provider-offered `catalog_model_key` that is non-empty and differs from the resolved key yields NO match (reason `catalog_model_key_disagrees`). No resolvable pair, offered pairs resolving to different members, or no usable identity set (a rate-card-bound release, a missing or stale feed) yields `catalog_match_state` = `unmatched` with the reason recorded (`no_artifact_match`, `catalog_artifact_feed_integrity_failure`, `catalog_artifact_feed_unavailable`): the offer still stands as an unmatched offer, and its provider-offered key MUST be stored as null so nothing later reads it as a catalog identity. A recorded match is never recomputed from a later feed for an old offer: a decision re-resolves the RECORDED pair (SPEC-047-R003 below), and a reload under which it no longer resolves is drift (SPEC-047-R006). Provider status readback (`model_admission_status.v1`) MUST expose `catalog_match_state` and, when matched, `artifact_id` and `artifact_hash_algorithm`, so a provider can see which identity the coordinator matched without the coordinator ever adopting a runtime-reported one.
...
**[amended v0.1.5]** An operator decision (SPEC-047-R001) to `catalog_priced` or `settlement_capable` MUST fail closed — appending nothing, answering with the reason — unless, evaluated at decision time: (i) the candidate's latest offer event carries `catalog_match_state` = `catalog_matched` and its RECORDED pair still resolves, against the artifact identity set of the CURRENT admitted release, to the same `(model_key, artifact_id)` it resolved to at offer time (`catalog_match_stale` otherwise; a match is never repaired from the current feed); (ii) that key's `runtime_status` is `recommendable` (a `listed` key stops at `network_visible_unpriced`, SPEC-023 AC-CAT-7(iii)); (iii) trusted Tier-2 catalog material exists for that key's row, and the decision event binds `catalog_id`, `catalog_body_digest`, `catalog_signature_key_id`, and `catalog_signature_pubkey_fingerprint` from it, `catalog_model_key` = the resolved key, `expected_catalog_model_hash` / `_algorithm` = the matched member's pair, and the six artifact values copied from the offer's recorded match — decision evidence for audit; the IMMUTABLE route-time record stays the settlement authority; (iv) for `settlement_capable` only, a live provider session of that provider serving that row's `model_id` whose SPEC-010 identity verdict is `hash_verified` for that same member (SPEC-010-R007(b)/(c) session authority) and whose SPEC-022 receipt key is present, so the SPEC-022 preconditions are satisfiable now, as SPEC-047-R001 requires. Buyer routing and settlement bind a session to its candidate through the session's admitted candidate binding (candidate id, resolved key, coordinator event id — carried on the session from hello and refreshed on every decision; §4 step 9), never through served-model-name or asserted-key heuristics; a session with no such binding has no admission and stays on the v0.1 non-BYOM path.
...
**[amended v0.1.5]** Two drift sources MUST be live in production, both as coordinator-actor events visible in status readback with their reason code: (a) a heartbeat or refresh under which the session of a `catalog_priced` or `settlement_capable` candidate is no longer `hash_verified` for its bound member — another member, the primary pair for a member-bound session, a hash-less or unnamed-algorithm report, an unverifiable pair — MUST append `revoked` with `runtime_identity_drift` (the existing served-model-id drift path keeps its reason code); (b) after every artifact-feed reload, the RECORDED pair of every `catalog_priced` / `settlement_capable` candidate MUST be re-resolved against the new identity set, and a pair that no longer resolves to the same `(model_key, artifact_id)` MUST append `revoked` with `catalog_artifact_feed_changed`. Revocation is append-only; re-entry is a fresh signed offer through the R001 table.
...
9. **(v0.1.5)** Add the production decision path: offer-time catalog match on `artifact_hashes` recorded on the offer event; the operator decision request and offer-listing endpoints; the session-to-candidate binding carried on the provider session from hello and refreshed on decisions; drift revocation on identity verdict and on feed reload; and the operator runbook `docs/runbooks/byom-admission-decisions.md`.
...
- v0.1.5 - Production decision path (#1453 slice 4). R001: the `coordinator
  ...
  `model_admission_decision_request.v1` with optimistic concurrency on the
  latest `coordinator_event_id` and idempotency) and the operator offer
  listing (`GET /admin/model-admission/offers`). The catalog match is
  performed at OFFER time on the signed `artifact_hashes` against the
  SPEC-010-R007 identity set of the current release and recorded on the
  offer event (`catalog_match_state`, resolved key, member pair, six
  artifact values; unmatched offers store a null key). R003: decisions to
  `catalog_priced` / `settlement_capable` fail closed unless the recorded
  pair still resolves to the same member, the key is `recommendable`,
  Tier-2 material binds the row, and (for `settlement_capable`) a live
  session is `hash_verified` for that member with a receipt key; sessions
  bind to candidates through the admitted candidate binding, not name
  heuristics. R006: identity-verdict drift and feed-reload drift are live
  revocations with reason codes. §4 step 9.
