...
- v0.1.3 - Minimal cross-spec amendment for the SPEC-023 v0.10.0 catalog
  ...
  candidate-catalog body digest — while holding any artifact whose hash
  algorithm SPEC-010-R002 does not name out of `catalog_priced`, out of the
  SPEC-010 wire pair, and out of SPEC-022 settlement, so `gguf` artifacts reach
  identity and unpriced visibility but neither `catalog_priced` nor
  `settlement_capable`. Adds a citation-only note naming the later, stricter
