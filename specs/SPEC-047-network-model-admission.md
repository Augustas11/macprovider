...
- v0.1.3 - Minimal cross-spec amendment for the SPEC-023 v0.10.0 catalog
  ...
  algorithm SPEC-010-R002 does not name out of the SPEC-010 wire pair and out
  of SPEC-022 settlement, so `gguf` artifacts reach identity and unpriced
  visibility but not `settlement_capable`. No admission state, transition,
  schema, signing boundary, disclosure class, or settlement precondition
  changes; no SPEC-047 requirement changes state.
