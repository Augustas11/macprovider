...
|---|---|---|---|
| D1 | Billing model | A) pre-paid Stripe bundles / B) post-paid credit card / C) API key + running balance / D) **donation-only, no per-provider tip jar (recommended — keeps SPEC-006 free-tier lock; ledger records provider credit, not buyer revenue)** | **D** — donation-only for public buyers; no tip jar; SPEC-005 ledger tracks provider credits, not public buyer revenue; no Stripe, no checkout, no credit card collection. **D1a (SCOPE reopen 2026-09-11):** named wholesale partner class MAY receive an operator-issued monthly postpaid USD statement from metered paid-SKU usage; free SKUs $0 to the partner; still no Stripe; live partner inference MUST NOT return HTTP 402. |
