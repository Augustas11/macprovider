...
| AC-022-33 | Covered | `phase4-coordinator/internal/billing/spec022_money_gate_test.go` `TestSPEC022StreamingVerifiedReceiptCreatesBuyerFinalityAndProviderPayout` proves the coordinator side of the same streaming request emits verified buyer-debit finality and receipt-bound provider payout eligibility while buyer receipt retrieval remains absent; `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingReconcileConsumesCoordinatorVerifiedFinality` proves the gateway consumes that internal finality contract and finalizes the buyer debit from coordinator-observed usage; `phase5-gateway/cmd/gateway/main_test.go` `TestRunSettlementReconcilerRunsImmediately` proves automatic worker startup. |
