...
streaming terminal-output capture, streaming verified-receipt money-path
contract coverage, late-receipt quarantine, gateway quota reservation
primitives, Tier 2 model-hash disclosure, SPEC-022 buyer disclosure surfaces,
and provider-facing late-receipt deadline docs. It is not yet ready for
enforce-mode launch because the enforce-mode configuration gate, race
harnesses, and full e2e/live-network gates are not fully proven in this
branch. D9-D14 moved several money-path rows from blocked to partial coverage;
the item 1 closure work adds deterministic coordinator/gateway contract
coverage for the streaming signed-receipt money path.
...
| AC-022-12 | Covered | `phase3-binary/Tests/macprovider-cliTests/HTTPServerReceiptTests.swift` `testHTTPStreamingHandlerWritesV04SettlementReceiptTrailerWithWarmSwapDisabled`; `phase4-coordinator/internal/buyer/route_snapshot_test.go` `TestStreamingSettlementOutputPersistsOpenAICompatibleSSE`; `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingSettlementTrailersControlBuyerDebit` and `TestStreamingReceiptHeaderStripped` prove internal streaming settlement trailers carry coordinator finality while buyer-visible SSE remains OpenAI-compatible. |
| AC-022-13 | Covered | `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingSettlementTrailersControlBuyerDebit` and `TestSPEC022GatewayStreamingNonOKFinalityBoundsHold` prove covered streaming reservations are debited only after verified coordinator finality and remain held/refundable for missing or non-OK finality; `phase4-coordinator/internal/billing/spec022_money_gate_test.go` `TestSPEC022StreamingVerifiedReceiptCreatesBuyerFinalityAndProviderPayout` proves the same streaming signed-receipt finality creates no positive provider settlement before verification and creates receipt-bound provider payout eligibility after verification. |
...
| AC-022-15 | Covered | `phase4-coordinator/internal/billing/spec022_money_gate_test.go` `TestSPEC022StreamingPartialTerminalReceiptsCreateReceiptBoundSettlement` proves provider error, buyer cancel, gateway timeout, and upstream disconnect streaming receipts with non-empty output prefixes open buyer finality and provider settlement only from signed, coordinator-observed, receipt-bound usage; `TestSPEC022StreamingVerifiedReceiptCreatesBuyerFinalityAndProviderPayout` proves the same money path for a normal streaming tool-call prefix; `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingSettlementTrailersControlBuyerDebit`, `TestSPEC022GatewayStreamingNonOKFinalityBoundsHold`, and `TestStreamingProviderReportedUsageCannotUnderstateObservedOutput` prove gateway debit movement follows coordinator finality and bounded observed output usage. |
...
| AC-022-24 | Covered | `phase5-gateway/internal/router/receipts_test.go` `TestBuyerReceiptRetrievalAuthAndRedaction` proves owner 200 metadata, stranger 403, missing 404, unauthenticated 401, operator 200, demo 403, and no raw prompt/output material; `phase4-coordinator/internal/billing/settlement_receipt_retrieval_test.go` `TestLookupBuyerReceiptOwnershipAndMissing` proves coordinator ownership lookup; `test/integration/buyer_paid_path_journey_test.go` `TestJourneyBuyerPaidPathIsolatedCandidate` step 09 exercises the exposed GET on the isolated paid-path harness. |
...
| AC-022-33 | Covered | `phase4-coordinator/internal/billing/spec022_money_gate_test.go` `TestSPEC022StreamingVerifiedReceiptCreatesBuyerFinalityAndProviderPayout` proves the coordinator side of the same streaming request emits verified buyer-debit finality and receipt-bound provider payout eligibility; `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingReconcileConsumesCoordinatorVerifiedFinality` proves the gateway consumes that internal finality contract and finalizes the buyer debit from coordinator-observed usage; `phase5-gateway/cmd/gateway/main_test.go` `TestRunSettlementReconcilerRunsImmediately` proves automatic worker startup. Buyer receipt retrieval is a separate metadata GET covered by AC-022-24/35. |
...
| AC-022-35 | Covered | Same surface as AC-022-24: `phase5-gateway/internal/router/receipts_test.go` `TestBuyerReceiptRetrievalAuthAndRedaction` proves stranger-account 403, operator access, pending metadata visibility, and redaction of raw material; `phase4-coordinator/internal/billing/settlement_receipt_retrieval_test.go` `TestLookupBuyerReceiptOwnershipAndMissing` proves coordinator 403/404/operator 200. |
...
| AC-022-63 | Covered | `phase4-coordinator/internal/billing/spec022_money_gate_test.go` `TestSPEC022StreamingVerifiedReceiptCreatesBuyerFinalityAndProviderPayout` proves verified streaming buyer finality and provider payout eligibility derive from the same coordinator-observed request/output evidence and receipt-bound usage, not provider-signed usage alone; `phase5-gateway/internal/router/server_test.go` `TestSPEC022GatewayStreamingReconcileConsumesCoordinatorVerifiedFinality` proves the gateway debits from that coordinator-observed finality contract. |
...
| AC-022-65 | Covered | `phase4-coordinator/internal/buyer/catalog_material_gate_test.go` `TestCatalogMaterialGate_EnforceExcludesSessionWithoutMaterial` proves an enforce-mode non-BYOM session without Tier-2 route-snapshot material is dropped from candidate, pinned (503), and slot-queue routing; `TestCatalogMaterialGate_ObserveModeUnchanged` keeps observe mode unchanged; `TestPoolCheckCatalogMaterialHold` proves `/v1/pool/check?details=readiness` names `catalog_material_missing` only to a `catalog_material_hold_v1` session and keeps the legacy verdict otherwise; `phase4-coordinator/internal/buyer/route_snapshot_test.go` `TestRouteSnapshotEnforceExcludesSessionWithoutCatalogMaterial` proves the buyer gets a retryable 503 with no route snapshot or ledger credit written. |
| AC-022-66 (v0.2.0) | Partial | `phase4-coordinator/internal/billing/spec022_r012_test.go` `TestSPEC022R012PoolOperatorAttestedSettlesOnlyWhenR012Holds` settles a normal and a partial pool_operator_attested completion through a verified receipt with an exact usage match and reports finality `token_source: pool_operator_attested`; `TestSPEC022R012FailClosedSet` proves a global route, a disputed label, missing settlement-time labels, durable-record rejection, and a byte_estimated attempt never verify; `TestSPEC022R012FinalityTokenSourceWeakerProvenanceGoverns` proves a mixed native and pool-attested request reports `pool_operator_attested` and an all-native one `coordinator_observed`; `phase4-coordinator/internal/buyer/spec042_external_runtime_test.go` `TestSPEC042ExternalRuntimeRecordedByteEstimatedWhenDurableRecordsReject` and `TestSPEC042ExternalRuntimeFailClosedSet` prove the zero-billable recording and no selection on a global route or for a non-creator member. Partial until the #1690 M5 CLI signs pool-authorized receipts and a signed enforce-mode pool journey (#1690 M6) shows a live verified receipt and ledger credit. |
