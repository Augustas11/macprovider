...
| 3 | **G0** — the demand-volume query | Decides whether the observed-performance briefs (B3/B4/B8) are executable |

The G0-consuming briefs (B3/B4/B8) are **conditional on G0's answer**; the rest (B2/B5/B6/B7/B9/B10) gate on their own listed dependencies, not on G0. All of Plane C is analysis, not an approved plan. The other Plane-A pieces (A2/A3/A4/A6/A7/A8) are all independently shippable and add to this minimum in any order.
...
The #744 audit [E-issue], corroborated by the shipped backfill table `AutotuneRecommend.swift:799-840` [E-code]: 4/9 rows measured, all on one M5 32 GB; 3 rows needing >32 GB unmeasurable on the only executor; 2 re-measured rows diverging 1.7–1.8× in *opposite* directions. #745 explains it: until PR #751, `serve --model` overrode identity but not `model_artifact_path`, so **every candidate benchmark loaded the incumbent and recorded it under the candidate's name**, silently. Fix verified end-to-end [E-code: `Config.swift:489-526`; chain `AutotuneRecommend.swift:3040-3063` → `CandidateProviderRunner.swift:289-295` → `MalibuCLI.swift:1024-1026` → `ModelRuntime.swift:601`]. **No gate value predating #751 is trustworthy, and no trusted post-#745 matrix exists** — Entry 196 correctly rejected wholesale re-derivation.
...
- **Do now, unconditional** (Plane A): the eight A-pieces, all supply-neutral or overclaim fixes, none of which slows onboarding. The two that are *unacceptable to leave undone* (not merely cheap) are **A1** (the SPEC-006:343 overclaim violations) and **A5** (ceiling-drift detection — a silent heartbeat switch to a catalogued larger model drifts the capacity ceiling unseen; the live hash chain stays intact for uncatalogued switches, so what A5 restores is *visibility of capacity drift*, not a broken integrity chain). **G0** runs alongside.
...
**#745 fix chain**: `Config.swift:489-526, 592-616`; `CandidateProviderRunner.swift:269-305`; `MalibuCLI.swift:373-378, 559-736, 945, 1024-1026, 2012-2017`; `ModelRuntime.swift:601, 610, 822-829`; `AutotuneHardwareEvidence.swift:210-213, 340-365`; `hardware_evidence.go:64-66`.
